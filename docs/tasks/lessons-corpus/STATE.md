...
Осталось (2/10): F14 — S7.9 memory-guidelines, S7.10 stackalloc-refstruct.
ВАЖНО для восстановления: seed order 20(S7.1)…27(S7.8); следующий S7.9=order 28. Все уроки в CS_S7 (registry.ts), icon "gc". Verbatim-скрипт: извлечь « »-англ.цитаты, нормализовать `\"`→`"`+`\'`→`'`+регистр(.lower)+strip ТОЛЬКО named-html-теги (span/code/b/i, НЕ <T>/<int>)+`&lt;`→`<`, сверить сегменты по … против fetched-текста (WebFetch) — 0 MISS перед коммитом. Апострофы в single-quoted полях эскейпить (авто-regex mid-word: re.sub(r"(?<=\w)'(?=\w)", r"\\'", t)). F14: версионные оговорки обязательны (ref struct ослаблен в C# 13: async/итераторы/интерфейсы/тип-аргумент; stackalloc→указатель вне unsafe — preview C# 15). VOLATILE: конкретный порог stackalloc (1024) — иллюстрация, не гарантия. S7.9=memory-t-usage-guidelines, S7.10=operators/stackalloc + ref-struct.
Грабли прогона: запускать viz-fit и verify:all ПОСЛЕДОВАТЕЛЬНО, не параллельно — конкуренция за preview даёт ложный exit-1/flaky-overlap. Дожидаться терминальной строки лога перед следующим шагом (проверять `pgrep -f verify/all.mjs`, т.к. notification о launcher ≠ конец детача). new-lessons.mjs медленный (35 уроков) → в фоне.
