...
- [ ] 4 BUILD (builder; счётчик майлстоунов: 2/6 — M1 R-05, M2 R-06 ПРИНЯТ)
...
## Следующий шаг
M2 ПРИНЯТ (R-06, оркестратор-прогон: verify:all ALL GREEN, dotnet 67/67, sim-14d 14 дней, fresh due=10, prefetch 3/22, vtable-источники BOTR/ECMA, F4-нав осмотрена). F2–F5 → verified. Task-workspace закоммичен (376b22c) — recovery-риск закрыт. Планировочные докстроки brief/ADR-0001..0004/research/reviews теперь в git.
→ M3: builder №2 продолжается через SendMessage. Шаг 0: фикс home.ts:371 (stale `value-vs-reference` → CS.S1.value-types-copy), grep старом id=0 буквально, коммит. Затем F7 (S1.4–S1.6: структуры-ловушки, records, интерфейсы/DIM) и F8 (S1.7–S1.10: enum/флаги, generics-базис, Nullable+боксинг, приведения is/as) — 7 уроков по DoD, accuracy по research/GT-M3-s1.md. Инкрементальный verify:all --lessons. По остановке M3 → fresh evaluator (app поднимает ОРКЕСТРАТОР) + G7 accuracy-аудит выборки ≥20% по GT-M3-s1.md + БЮДЖЕТНЫЙ ЧЕК (скорость vs 6–10/майлстоун) → отчёт пользователю (его следующий тачпойнт по выбору «автономно, чек после M3»).
