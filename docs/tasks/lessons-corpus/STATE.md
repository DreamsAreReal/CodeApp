...
Осталось (3/10): F14 — S7.8 span-memory, S7.9 memory-guidelines, S7.10 stackalloc-refstruct.
ВАЖНО для восстановления: seed order 20(S7.1)…26(S7.7); следующий S7.8=order 27. Все уроки в CS_S7 (registry.ts), icon "gc". Verbatim-скрипт: извлечь « »-англ.цитаты, нормализовать регистр(.lower)/апострофы/пробелы/теги, сверить сегменты по … против fetched-текста (WebFetch) — 0 MISS перед коммитом. Апострофы в single-quoted полях эскейпить (авто-regex mid-word: re.sub(r"(?<=\w)'(?=\w)", r"\\'", t)). F14 темы Span/Memory/stackalloc: версионные оговорки обязательны (ref struct ослаблен в C# 13: async/итераторы/интерфейсы/тип-аргумент; stackalloc→указатель вне unsafe — preview C# 15). VOLATILE: конкретный порог stackalloc (1024) — иллюстрация, не гарантия.
