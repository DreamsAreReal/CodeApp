...
- [ ] 4 BUILD (builder; счётчик майлстоунов: 1/6 — M1 ПРИНЯТ evaluator-ом R-05)
...
## Следующий шаг
M2 (F2–F5 + долги 0a/0b) ЗАВЕРШЁН builder-ом (2026-07-18), все self-pass с доказательствами и коммитами на lessons-corpus/wave1: 0a vtable-провенанс (48a6ffe), 0b ADR-0005 section-on-demand (3f5e42d), F2 миграция удаления 6 старых (62bfdf7), F3 лимит новых карт+sim-14d (0634552), F4 навигация трек→раздел+турнир (00b86ba), F5 verify:all+density+lint (5d219fe). Смоук+все прогоны зелёные: `npm run verify:all` FULL 7/7 ALL GREEN, `dotnet test` 67/67, миграция grep=0/сироты 0/PY нетронут, fresh due 10 (было 66). Backend/preview остановлены.
→ Оркестратору: запустить verify-workflow M2 (evaluator + монитор; app поднимает ОРКЕСТРАТОР). НЕ идти к M3/S1-остатку до вердикта. Автономно до M3-бюджет-чека.
...
| F2 | Миграция удаления 6 старых | M2 | — | self-pass (62bfdf7) |
| F3 | Лимит новых карт + sim-14d | M2 | — | self-pass (0634552) |
| F4 | Навигация трек→раздел (турнир) | M2 | да | self-pass (00b86ba) |
| F5 | verify:all + lint + density | M2 | — | self-pass (5d219fe) |
