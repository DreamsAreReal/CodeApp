...
→ Оркестратору: fresh evaluator M3 (app поднимает ОРКЕСТРАТОР) + G7 accuracy-аудит выборки ≥20% (≥2 из 7 уроков) по research/GT-M3-s1.md (fetch источника, сверка ≥3 утверждений, 0 красных флагов) + БЮДЖЕТНЫЙ ЧЕК (7 уроков/майлстоун vs оценка 6–10) → отчёт пользователю (его тачпойнт по выбору «автономно, чек после M3»). НЕ идти к M4 (async S2) до вердикта.
