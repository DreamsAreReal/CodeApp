...
Осталось (5/10): F13 — S7.6 finalizers-dispose, S7.7 weak-references; F14 — S7.8 span-memory, S7.9 memory-guidelines, S7.10 stackalloc-refstruct.
ВАЖНО для восстановления: seed order 20(S7.1)…24(S7.5); следующий S7.6=order 25. Все уроки в CS_S7 (registry.ts), icon "gc". Verbatim-скрипт: извлечь « »-англ.цитаты, нормализовать регистр(.lower)/апострофы/пробелы/теги, сверить сегменты по … против fetched-текста — 0 MISS перед коммитом. Апострофы в single-quoted полях эскейпить (авто-regex mid-word: re.sub(r"(?<=\w)'(?=\w)", r"\\'", t)).
