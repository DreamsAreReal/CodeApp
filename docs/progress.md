...
## Волна «viz-fit зелёный на маке, красный на Linux — НАСТОЯЩИЙ Linux-репро» (Docker)

### Постановка
- CI (ubuntu) единственный провал: `FIT: found 4` (WIDTH-ON-LADDER=0, OVERLAP=0). На маке зелено.
- Гипотеза оркестратора: self-host шрифты НЕ грузятся на Linux (404/неверный путь/fonts.ready резолвится
  до загрузки) → текст рендерится широким FALLBACK-ом → FIT. ПРОВЕРЕНА В LINUX-КОНТЕЙНЕРЕ и ОПРОВЕРГНУТА.

### Как воспроизвёл на Linux (авторитетно)
- Образ: `mcr.microsoft.com/dotnet/sdk:10.0` (Debian) + node 22 (NodeSource) + `npx playwright install
  --with-deps chromium`. Внутри: `npm run build`, поднял backend :5080 (Development) + `vite preview` :4173,
  прогнал `node verify/viz-fit.mjs`. Dockerfile/скрипты — scratchpad/repro (вне репо).
- Артефакт контейнера (НЕ продукт): в этом контейнере `localhost` резолвится в IPv6 ::1, а сервисы бинд
  IPv4 → Node 22 fetch (undici) не достаёт preview. Фикс репро: сервисы на 0.0.0.0 + `NODE_OPTIONS=
  --dns-result-order=ipv4first`, всё на `localhost` (совпадает с зашитым в бандл VITE_API_BASE). На реальном
  CI (ubuntu) localhost dual-stack — артефакта нет.
- РЕПРО ДО фикса: `✗ zero FIT violations … (found 6)` — воспроизвёл (CI даёт 4; разница 4 vs 6 = патч-версия
  freetype/Chromium, класс бага тот же).

### ДИАГНОСТИКА В LINUX-БРАУЗЕРЕ (verify/linux-font-diag.mjs) — ИСТИННАЯ ПРИЧИНА
- document.fonts: **38 loaded / 44** — {Rubik:10, Onest:16, JetBrains Mono:12}. woff2-запросов **33, ВСЕ 200,
  404 = НЕТ**. computed font-family 4-6 провальных `<text>` = `"JetBrains Mono", …` (НЕ fallback);
  `document.fonts.check` → Rubik/Onest/JetBrainsMono = **true**. → ШРИФТЫ ГРУЗЯТСЯ И ПРИМЕНЯЮТСЯ. Гипотеза о
  незагрузке — ЛОЖНА.
- Истинная причина — **метрик-вариация one-и-того-же woff2 между macOS и Linux**: Linux (freetype)
  рендерит `getComputedTextLength` шире, чем canvas `domMeasure` мерил при сайзинге бокса. Overflow-ы:
  два ~1px (11.08>10, 47.22>46 — прямо на границе FIT_TOL=1), один ~12.79px (async-await, где fitLabels
  упёрся в старый пол 0.74 и не дожал). Боксы правильной ширины по ладдеру — но текст на Linux чуть шире.

### ЧТО ПОЧИНИЛ — FIT-ГАРАНТИЯ (engine/vizPlayer.ts fitLabels), НЕ шрифты
- Цель ужима теперь `avail - SAFETY (0.6px)`, не ровно `avail`: под-пиксельный зазор поглощает
  кросс-платформенный джиттер, чтобы влезшая на маке метка не вылезла на Linux в пределах FIT_TOL.
- Двухступенчато: (1) шрифт-сайз ужим (пол 0.72, mono остаётся читаемым) — покрывает частый мелкий overflow;
  (2) ПОСЛЕ ужима РЕ-ЗАМЕР, и если всё ещё > target — ЖЁСТКИЙ клэмп `textLength=target` +
  `lengthAdjust="spacingAndGlyphs"`: геометрически гарантирует ≤ avail. Для ~1px незаметно; для редкой
  длинной метки — грациозный последний рубеж. Дизайн не ломается: боксы не раздуваются (ладдер цел).

### CI-диагностика (deploy.yml, логика харнесов НЕ тронута)
- Шаги viz-fit / (run+loop+shell) / new-lessons обёрнуты: `node … 2>&1 | tee /tmp/x.log; c=${PIPESTATUS[0]};
  [ $c -ne 0 ] && grep -aE '✗|FAIL|violation' /tmp/x.log | sed 's/^/::error::/'; exit $c`. Провалы теперь
  видны как ::error::-аннотации без доступа к логам. YAML валиден (python yaml.safe_load — jobs целы).

### Верификация (доказано прогоном)
- **Linux-контейнер, ДО фикса**: viz-fit `found 6`, exit 1. **ПОСЛЕ фикса**: `zero FIT … (found 0)` —
  **ALL GREEN, exit 0** (96 сцен + 189 mid-reads, все прочие проверки 0). Диагностика после: 0 overflow.
- **macOS**: `npm run build` чисто (JS gz 59.84КБ). viz-fit/run/shell/loop/new-lessons/polish-flow — все прогнаны.

### Решения / грабли
- Гипотеза «шрифты не грузятся» — ОПРОВЕРГНУТА в Linux (404=0, family корректна). Причина — метрика, не загрузка.
- Нельзя растить бокс (WIDTH-ON-LADDER=0 обязателен) → фикс ТОЛЬКО в fitLabels: гарантировать текст ≤ avail.
- textLength+spacingAndGlyphs — жёсткая геом-гарантия; ставится ТОЛЬКО когда шрифт-ужим (пол 0.72) не дожал.
- Репро-инфра (Dockerfile, run-repro.sh, boot-diag) — в scratchpad, вне репо; в продукт добавлен только
  verify/linux-font-diag.mjs (read-only проба, вердикт viz-fit не меняет).
