...
- 2026-07-11 — АВТОЛЕЙАУТ движка v2 (bug-proof authoring, шаг 1/2). Убраны ручные x/y узлов.
  * ДВИЖОК: `app/src/engine/layout.ts` — чистая детерминированная `layoutScene(scene,zones,measure,viewBox?)`:
    проставляет x/y/w/h (snap GRID=2) из `at`. Зоны→ряды (центр по X, ОДИН center-Y на ряд, блок
    центрируется по Y), вложение `at:{in}` (родитель авто-растёт: header20/PAD8 + Σдети; дети
    заполняют parent−2·PAD, snapped down на лестницу), overflow → шаг вниз по лестнице в innerW−2
    (WIDTH-ON-LADDER цел, PAD≥8) или THROW. Escape hatch (явные x/y) → snap+clamp в РЕАЛЬНЫЙ viewBox.
  * ТИПЫ: `DiagramNode.x?/y?` опциональны, `+at:NodePlacement`, `Zone.id?`. Реэкспорт из engine/index.ts.
  * ИНТЕГРАЦИЯ: `VizPlayer` зовёт `layoutScene` ДО StepPlayer (FLIP анимирует переезд по at). Чисто,
    детерминированно; un-migrated 5 уроков на x/y работают без изменений (обратная совместимость).
  * МИГРАЦИЯ: `closures.ts` — все 5 сегментов на `at` (zones id stack/heap + for/foreach; nf→at:{in:dc};
    ручные h:60 и x/y убраны). Текст/цитаты/источники не тронуты. Визуально ровнее (см. скрины).
  * ХАРНЕС: `viz-fit.mjs` +AUTHORING-PROOF (pure `layoutScene`: in-zone PAD≥8, nested-contained,
    center-Y на ряд, no overlap, snap, ladder) + строка `autolayout: 1/6 lessons fully on at [closures]`.
    ROW-BASELINE ослаблен под v2: разные высоты в ряду при общем center-Y — НЕ баг (центр-выравнивание);
    вложенные узлы исключены из геом. row-пар. `closures-shots.mjs` — 5 скринов финалов сегментов.
  * ДОКИ: AUTHORING-AI.md §3a (только at, до/после, вложение) + плоская схема; viz-design-spec.md
    (impl-notes layoutScene).
  * ДОКАЗАТЕЛЬСТВА (исполнено): `npm run build` чисто. ВСЕ харнесы ALL GREEN: viz-fit (autolayout 1/6),
    run.mjs (F1-F6), new-lessons.mjs (gc/closures/async/hashtable), shell.mjs. Pure layout-proof GREEN
    (5 сегм × все сцены). Escape-hatch: 81 un-migrated сцена, 0 throws, 0 сдвигов>3u. Скрины:
    docs/evidence/autolayout/closures/ (s1..s5). НЕ коммичено (ждёт визуального ревью closures + миграции
    остальных 5 уроков следующим билдером).

## Грабли (автолейаут)
- viewBox: Scene не несёт viewBox (он на Segment). Изначально клампил по union зон → ложно двигал
  escape-hatch узлы hashtable (y190→180). Фикс: прокинул реальный `cfg.viewBox` в layoutScene.
- Вложенный slot шириной 56 (ладдер-min) рендерился криво (38u name-gutter не влезал, обрезка).
  Фикс: nested дети заполняют parent−2·PAD (→72) — слоту хватает места. Видно на скрине s2.
- gate в узкой зоне 'for' (inner134) хотел 144 (ладдер) → шаг вниз до 120, влезает с PAD. On-ladder цел.
