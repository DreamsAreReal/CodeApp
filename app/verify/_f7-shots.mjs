/**
 * F7 evidence screenshots (390×844): every segment of PY.M7.context-managers
 * (autoplay frame stopping at predict gates, then forced final frame) + the typed
 * card before/after grading. Writes PNGs to docs/tasks/python-track/evidence/F7.
 */
import { chromium } from "playwright";

const APP = "http://localhost:4173";
const EV = "/Users/admin/Desktop/CodeApp/docs/tasks/python-track/evidence/F7";
...
await page.evaluate(() => window.__app.openLesson("PY.M7.context-managers"));
await page.waitForFunction(() => window.__viz && window.__viz.ready && window.__lesson.id === "PY.M7.context-managers", { timeout: 15000 });
await sleep(300);
await page.screenshot({ path: `${EV}/390-lesson-head-hook.png` });

const segIds = ["s1", "s2", "s3", "s4", "s5"];
