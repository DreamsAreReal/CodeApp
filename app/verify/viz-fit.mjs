 ...
 *       region's available width (+5px tolerance for cross-platform font variance). Region widths mirror
...
// 5px, not 1-2: getComputedTextLength differs by several px between local (macOS
// Chromium) and the Linux CI runner for the SAME label (different font hinting/
// rasterization), which repeatedly flipped borderline labels across the corpus one at
// a time. Geometry bounds the risk: `avail` is box-10 (5px padding per side), so a
// label only CLIPS the box when it exceeds avail by >10px; at +5px it still sits well
// inside the box with ~2.5px padding per side — legible, not clipping. So 5px absorbs
// the cross-platform variance without ever masking a real box-clip, while genuinely
// tight labels (>5px over, <2.5px padding) are still flagged for a widen/`w`-pin.
const FIT_TOL = 5; // px tolerance on getComputedTextLength vs available
...
function measureInPage(lessonId) {
  ...
  const FIT_TOL = 5; // see module-level FIT_TOL: absorbs macOS<->Linux-CI font variance
  const CLIP_TOL = 0.75;
