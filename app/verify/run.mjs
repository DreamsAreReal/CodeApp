...
async function main() {
  ...
  assert(ids.includes("CS.S1.value-types-copy"), "value-types-copy present in path");
  ...
  log("\n== F2: value-types-copy renders 6 segments, animation plays ==");
  ...
  await page.screenshot({ path: `${EV}/F2/390-value-types-copy-autoplay.png`, fullPage: false });
  ...
  assert(allFinal, "all 6 segments reach their final frame (data-join/StepPlayer end)");
  await page.screenshot({ path: `${EV}/F2/375-value-types-copy-full.png`, fullPage: true });

  // ===================== F3 — the loop: TYPED answer -> objective grade -> review =====
  // The card is typed generation: TYPE value-types-copy/c1's real output -> objective ✓ -> Good
  // is PRE-SELECTED -> confirm -> POST /api/review posts Good -> the schedule moves (card leaves due).
  ...
  const vtcC1Before = dueBefore.items.some((i) => i.itemId === "CS.S1.value-types-copy/c1");
  assert(vtcC1Before, "value-types-copy/c1 is due before review");
  ...
  await page.fill("#qTyped", VTC_C1_EXPECT); // the REAL expected output (verify.expect)
  ...
  assert(review.itemId === "CS.S1.value-types-copy/c1", "review posted for value-types-copy/c1");
  ...
  assert(!dueAfter.items.some((i) => i.itemId === "CS.S1.value-types-copy/c1"), "value-types-copy/c1 left the due queue (schedule moved)");
  ...
  log("\n== F4: type-system-map renders 5 segments, plays ==");
  await page.evaluate(() => window.__app.openLesson("CS.S1.type-system-map"));
  await page.waitForFunction(() => window.__viz && window.__viz.ready && window.__lesson.id === "CS.S1.type-system-map", { timeout: 15000 });
  const segCount2 = await page.evaluate(() => Object.keys(window.__viz.vizByKey).length);
  assert(segCount2 === 5, "type-system-map built 5 animated segments (got " + segCount2 + ")");
  await page.evaluate(() => window.__viz.forcePlayAll());
  await sleep(300);
  const allFinal2 = await page.evaluate(() => Object.values(window.__viz.segments).every((s) => s.index === s.total - 1));
  assert(allFinal2, "all type-system-map segments reach final frame");
  await page.screenshot({ path: `${EV}/F4/375-type-system-map-full.png`, fullPage: true });
  await page.setViewportSize(VIEWPORTS[390]);
  await page.evaluate(() => window.__viz.vizByKey["s2"].showFinal());
  await sleep(200);
  await page.screenshot({ path: `${EV}/F4/390-type-system-map-seg.png`, fullPage: false });
  ...
  await rmPage.evaluate(() => window.__app.openLesson("CS.S1.value-types-copy"));
  ...
  await rmPage.screenshot({ path: `${EV}/F5/390-reduced-value-types-copy.png`, fullPage: false });
  for (const vp of [375, 768, 1440]) {
  ...
  assert(dueBeforeWrong.items.some((i) => i.itemId === "CS.S1.type-system-map/c1"), "type-system-map/c1 is due before the wrong review");
  await page.locator("#qTyped").scrollIntoViewIfNeeded();
  await page.fill("#qTyped", "nope"); // WRONG — the real expect is "object overload\nstring overload"
  ...
  await page.waitForFunction(() => window.__lastReview && window.__lastReview.itemId === "CS.S1.type-system-map/c1", { timeout: 8000 });
  ...
  for (let waited = 0; waited < 80 && !requeued; waited += 3) {
    ...
    requeued = d.items.some((i) => i.itemId === "CS.S1.type-system-map/c1");
    if (!requeued) await sleep(3000);
  }
  assert(requeued, "type-system-map/c1 RE-APPEARS in /api/due after its learning step (same-session relearning re-queue)");
