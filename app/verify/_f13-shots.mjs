/**
 * F13 evidence screenshots (390×844): every segment of PY.M13.stdlib-idioms
 * (autoplay frame stopping at predict gates, then forced final frame) + the typed
 * card before/after grading. Writes PNGs to docs/tasks/python-track/evidence/F13.
 */
import { chromium } from "playwright";

const APP = process.env.APP_BASE || "http://localhost:4173";
const EV = "/Users/admin/Desktop/CodeApp/docs/tasks/python-track/evidence/F13";
const RUN_USER = 960000 + Math.floor(Math.random() * 9000);
...
await page.evaluate(() => window.__app.openLesson("PY.M13.stdlib-idioms"));
await page.waitForFunction(() => window.__viz && window.__viz.ready && window.__lesson.id === "PY.M13.stdlib-idioms", { timeout: 15000 });
await sleep(300);
await page.screenshot({ path: `${EV}/390-lesson-head-hook.png` });

const segIds = ["s1", "s2", "s3"];
