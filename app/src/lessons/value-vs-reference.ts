  ...
  segments: [
    {
      ...
      explain: 'C# делит все типы на две категории. <b>Value</b>-тип: «A variable of a value type <span class="hl">contains an instance</span> of the type». <b>Reference</b>-тип: «…which contains a <span class="hl">reference</span> to an instance of the type». Для класса дословно: «enough memory is allocated <b>on the managed heap</b> for that specific object, and the variable holds only a reference to the location of said object»; при этом «This reference refers to the new object but doesn\'t contain the object data itself». Локальное значение живёт в стеке потока, а объект класса — в общей управляемой куче: «There\'s a managed heap for each managed process».',
    ...
    },
    {
      ...
      explain: 'Присваивание value-типа копирует данные целиком: «on assignment… variable values are copied. In the case of value-type variables, the <span class="hl">corresponding type instances are copied</span>». У struct семантика значения: «Structure types have <b>value semantics</b>». Поэтому «each variable has its own copy of the data, and it\'s not possible for operations on one variable to affect the other» — <code>b.X = 9</code> не меняет <code>a</code>.',
    ...
    },
    {
      ...
      explain: 'Класс — ссылочный тип: присваивание одной переменной другой <span class="hl">копирует ссылку</span>, и обе смотрят на один объект. Дословно: «This code creates two object references that both refer to the same object. Therefore, any changes to the object made through <code>object3</code> are reflected in subsequent uses of <code>object4</code>». То же в справке по ссылочным типам: «two variables can reference the same object… operations on one variable can <b>affect the object referenced by the other variable</b>». <code>b.X = 9</code> меняет общий объект — <code>a.X</code> тоже становится 9.',
    ...
    },
    {
      ...
      explain: '«A structure type… is a <b>value type</b>», а «A type that is defined as a class is a <b>reference type</b>» — отсюда разное поведение при копировании. И размещение разное: «reference types are allocated on the heap… whereas value types are allocated either on the <span class="hl">stack</span> or <span class="hl">inline</span> in containing types». Один и тот же <code>var y = x; y.X = 9;</code> даёт <code>x.X == 1</code> для struct и <code>x.X == 9</code> для class.',
