...
export const delegateVariance: LessonData = {
  ...
  spec: [
    { text: "«Covariance preserves assignment compatibility and contravariance reverses it.» <span class=\"ru-tr\">«Ковариантность сохраняет совместимость присваивания, а контравариантность её обращает.»</span>", source: "ms-variance" },
  ],
  edgeCases: [
    { text: "Ковариантность — про <b>возврат</b>: «Covariance permits a method to have return type that is <b>more derived</b> than that defined in the delegate». <span class=\"ru-tr\">«Ковариантность разрешает методу иметь тип возврата, который <b>более производный</b>, чем объявленный в делегате».</span>", source: "ms-variance-del" },
    { text: "Контравариантность — про <b>параметр</b>: «Contravariance permits a method that has parameter types that are <b>less derived</b> than those in the delegate type». <span class=\"ru-tr\">«Контравариантность разрешает методу иметь типы параметров, которые <b>менее производные</b>, чем те, что в типе делегата».</span>", source: "ms-variance-del" },
    { text: "Вариантность массивов <b>не</b> типобезопасна: «Covariance for arrays… <b>But this operation is not type safe</b>» <span class=\"ru-tr\">«Ковариантность для массивов… <b>Но эта операция не типобезопасна</b>»</span> — реальный прогон даёт <code>ArrayTypeMismatchException</code> в рантайме.", source: "ms-variance" },
  ],

  misconceptions: [
    {
      wrong: "метод должен точно совпадать с сигнатурой делегата — байт в байт",
      hook: 'Точное совпадение <b>не</b> требуется: делегат допускает вариантность. «This enables you to assign to delegates <span class="hl">not only methods that have matching signatures, but also methods that return more derived types (covariance) or that accept parameters that have less derived types (contravariance)</span> than that specified by the delegate type». <span class="ru-tr">«Это позволяет присваивать делегатам не только методы с совпадающими сигнатурами, но и методы, которые возвращают более производные типы (ковариантность) или принимают параметры менее производных типов (контравариантность), чем указано типом делегата».</span> Направления зеркальны: «<b>Covariance</b> preserves assignment compatibility and <b>contravariance</b> reverses it». <span class="ru-tr">«<b>Ковариантность</b> сохраняет совместимость присваивания, а <b>контравариантность</b> её обращает».</span> Конкретно: «Covariance permits a method to have <b>return type that is more derived</b>… Contravariance permits a method that has <b>parameter types that are less derived</b>». <span class="ru-tr">«Ковариантность разрешает методу иметь <b>тип возврата более производный</b>… Контравариантность разрешает методу иметь <b>типы параметров менее производные</b>».</span> Дальше <b>шесть разборов</b>: две оси, ковариантность возврата (метод вернул <code>Dog</code> вместо <code>Mammal</code>), контравариантность параметра (метод взял <code>Animal</code> вместо <code>Cat</code>), обобщённые <code>out</code>/<code>in</code>, <b>машинная панель</b> — конверсия <code>Func&lt;Dog&gt;</code> → <code>Func&lt;Animal&gt;</code> (реальный прогон), и контраст с небезопасной вариантностью массивов.',
      source: "ms-variance",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Вариантность даёт делегату гибкость сверх точной сигнатуры. Базовое определение: «In C#, covariance and contravariance enable <b>implicit reference conversion</b> for array types, delegate types, and generic type arguments. <span class="hl">Covariance preserves assignment compatibility and contravariance reverses it</span>». <span class="ru-tr">«В C# ковариантность и контравариантность включают <b>неявное преобразование ссылок</b> для типов массивов, типов делегатов и аргументов обобщённых типов. Ковариантность сохраняет совместимость присваивания, а контравариантность её обращает».</span> Применительно к делегатам: «When you assign a method to a delegate, <b>covariance</b> and <b>contravariance</b> provide flexibility for matching a delegate type with a method signature». <span class="ru-tr">«Когда вы присваиваете метод делегату, <b>ковариантность</b> и <b>контравариантность</b> дают гибкость при сопоставлении типа делегата с сигнатурой метода».</span> Две оси и их направления: «Covariance permits a method to have <b>return type that is more derived</b> than that defined in the delegate. Contravariance permits a method that has <b>parameter types that are less derived</b> than those in the delegate type». <span class="ru-tr">«Ковариантность разрешает методу иметь <b>тип возврата более производный</b>, чем объявленный в делегате. Контравариантность разрешает методу иметь <b>типы параметров менее производные</b>, чем те, что в типе делегата».</span> Запомнить просто: возврат может «сузиться» (ко-), параметр может «расшириться» (контра-).',
    ...
    },
    {
      ...
      explain: 'Контравариантность — гибкость по <b>параметру</b>: «Contravariance permits a method that has <b>parameter types that are less derived</b> than those in the delegate type». <span class="ru-tr">«Контравариантность разрешает методу иметь <b>типы параметров менее производные</b>, чем те, что в типе делегата».</span> В доках: делегат объявляет параметр как <code>string</code>, но ему можно присвоить метод, берущий <code>object</code>. Здесь <code>Action&lt;Cat&gt;</code> принимает метод <code>HandleAny(Animal)</code>, потому что метод, готовый к <b>любому</b> Animal, тем более обработает Cat. Практический смысл прямой: «With contravariance, you can use one event handler instead of separate handlers» <span class="ru-tr">«Благодаря контравариантности можно использовать один обработчик событий вместо отдельных обработчиков»</span> — один обработчик с параметром <code>EventArgs</code> подходит и для <code>KeyEventArgs</code>, и для <code>MouseEventArgs</code>. Реальный прогон: <code>onCat(new Cat())</code> → <code>handled: animal</code> (метод видит Cat через ссылку Animal).',
      sources: ["ms-variance-del", "ms-variance"],
    },
    {
  ...
  takeaways: [
    { icon: "why", k: "Две оси", v: '<b>Ковариантность</b> — возврат может быть <span class="hl">более производным</span>; <b>контравариантность</b> — параметр может быть <span class="hl">менее производным</span>. «Covariance preserves assignment compatibility and contravariance reverses it». <span class="ru-tr">«Ковариантность сохраняет совместимость присваивания, а контравариантность её обращает».</span> Точное совпадение сигнатуры не обязательно.' },
    ...
    { icon: "avoid", k: "Type safety", v: 'Делегатная вариантность <b>type safe</b> — направление доказывает компилятор. Ковариантность <b>массивов</b> — нет: «But this operation is not type safe» <span class="ru-tr">«Но эта операция не типобезопасна»</span> → реальный прогон даёт <span class="hl">ArrayTypeMismatchException</span> в рантайме. Массивной вариантности избегай.' },
  ],
