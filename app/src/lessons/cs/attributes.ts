...
export const attributes: LessonData = {
  ...
  spec: [
    { text: "«Attributes provide a powerful way to associate metadata, or declarative information, with code (assemblies, types, methods, properties, and so on).»", source: "ms-attr" },
  ],
  edgeCases: [
    { text: "Читаешь атрибут — <b>конструируешь объект</b>: «declare an instance of the attribute… use the <code>Attribute.GetCustomAttribute</code> method to initialize the new attribute».", source: "ms-attr-retrieve" },
    { text: "Нет атрибута → <span class=\"hl\">null</span>: «If the attribute isn't found, the <code>GetCustomAttribute</code> method initializes… to a null value».", source: "ms-attr-retrieve" },
    { text: "Несколько экземпляров на цель → нужен «<code>AllowMultiple</code> property set to <b>true</b> in the <code>AttributeUsageAttribute</code>» + <code>GetCustomAttributes</code> (массив).", source: "ms-attr-retrieve" },
  ],

  misconceptions: [
    {
      wrong: "атрибут — это «активная» аннотация, которая сама что-то делает при загрузке класса",
      hook: 'Атрибут <b>ничего не делает сам</b>: это <span class="hl">пассивные метаданные</span>, впаянные в декларацию. «Attributes provide a powerful way to associate <b>metadata, or declarative information</b>, with code». Пока кто-то (фреймворк, ты) не <b>прочитает</b> их через reflection — они молчат: «After you associate an attribute with a program entity, you can <b>query</b> the attribute at run time by using… reflection». И чтение — это <b>конструирование объекта</b>: «declare an instance of the attribute… use <code>Attribute.GetCustomAttribute</code> to initialize the new attribute». Дальше <b>пять разборов</b>: метаданные на декларации, определение своего атрибута, positional vs named параметры, чтение (или null), и <b>машинная панель</b> — оба вида параметров, восстановленные из IL в живой объект (реальный прогон).',
      source: "ms-attr",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Атрибут — это <b>данные</b>, а не поведение: «Attributes provide a powerful way to associate <span class="hl">metadata, or declarative information</span>, with code (assemblies, types, methods, properties, and so on)». Первое свойство доки прямое: «Attributes <b>add metadata</b> to your program. <i>Metadata</i> is information about the types defined in a program». Компилятор записывает атрибут в ту же таблицу метаданных сборки, что и имена типов/членов. Ключевое: он пассивен, пока не прочитан — «After you associate an attribute with a program entity, you can <b>query</b> the attribute at run time by using… reflection». Никакого кода «при загрузке» атрибут не запускает.',
      sources: ["ms-attr"],
    },
    {
      ...
      explain: 'Свой атрибут = класс-наследник <code>System.Attribute</code>. По конвенции: «all attribute names end with the suffix "Attribute"… you don\'t need to specify the attribute suffix when you use attributes in code» — <code>[Info]</code> и <code>[InfoAttribute]</code> эквивалентны. <code>AttributeUsage</code> ограничивает цели («A specific attribute might restrict the types of declarations on which it\'s valid»): <code>AttributeTargets.Class</code>, <code>.Method</code>, <code>.Property</code> и т.д. А «Attributes can <b>accept arguments</b> in the same way as methods and properties» — через конструктор и свойства. Никакого спец-синтаксиса: обычный класс, чьи экземпляры создаёт компилятор из литерала.',
    ...
    },
    {
      ...
      explain: 'Параметры атрибута — двух родов: «Many attributes have parameters, which can be <b>positional</b>, unnamed, or <b>named</b>». Positional — «<span class="hl">Parameters of the attribute constructor</span>»: обязательны, идут первыми, в фиксированном порядке. Named — «<span class="hl">Properties or fields of the attribute</span>»: опциональны, после позиционных, в любом порядке. Пример доки — три эквивалентных <code>DllImport</code>: <code>[DllImport("user32.dll", SetLastError=false, ExactSpelling=false)]</code>, где имя dll позиционно, а остальное — named-присваивания свойств. Компилятор кодирует и то, и другое в метаданные литерала.',
      sources: ["ms-attr"],
    },
    {
      ...
      explain: 'Чтение атрибута — <b>двухшаговый</b> процесс с конструированием объекта: «Retrieving a custom attribute is a simple process. First, declare an instance of the attribute you want to retrieve. Then, use the <code>Attribute.GetCustomAttribute</code> method to <span class="hl">initialize the new attribute</span> to the value of the attribute you want to retrieve. Once the new attribute is initialized, you can use its <b>properties</b> to get the values». То есть reflection читает метаданные литерала и <b>воссоздаёт</b> экземпляр атрибута с теми значениями. Если атрибута нет: «If the attribute isn\'t found, the <code>GetCustomAttribute</code> method initializes… to a <b>null</b> value» — поэтому проверка на null обязательна. Для нескольких экземпляров — <code>GetCustomAttributes</code> (массив) + <code>AllowMultiple = true</code>.',
  ...
  cards: [
    {
      ...
      noText: '«use <code>GetCustomAttribute</code> to initialize the new attribute». Значения из литерала восстановлены: <code>Author=ada</code>, <code>Version=2</code>.',
    ...
    },
    {
      ...
      noText: 'Positional = «Parameters of the attribute constructor», named = «Properties or fields». <code>Name=api</code> (ctor), <code>Cached=True</code> (свойство).',
    ...
    },
    {
      ...
      okText: 'Атрибута нет → «<code>GetCustomAttribute</code> method initializes… to a <span class="hl">null</span> value» → <code>== null</code> это <b>True</b>. Своих атрибутов на <code>Plain</code> ноль → <code>Length = 0</code>.',
  ...
  takeaways: [
    { icon: "why", k: "Пассивные метаданные", v: 'Атрибут — «metadata, or declarative information», впаянная в декларацию. Сам <b>ничего не делает</b>, пока кто-то не <span class="hl">прочитает</span> его reflection-ом.' },
