...
export const genericsBasics: LessonData = {
  ...
  edgeCases: [
    { text: "<b>Open</b> (generic type definition) — шаблон с плейсхолдерами, экземпляр создать нельзя; <b>closed</b> (constructed) — результат подстановки конкретных типов. «you cannot create instances of a… generic type definition».", source: "ms-generics-std" },
  ...
  segments: [
    ...
    },
    {
      ...
      explain: 'Терминология точная. «A <b>generic type definition</b> is a class, structure, or interface declaration that functions as a <span class="hl">template</span>… Because a generic type definition is only a template, you <b>cannot create instances</b> of a… generic type definition». А «A <b>constructed generic type</b>… is the result of specifying types for the generic type parameters». Прогон подтверждает: <code>typeof(List&lt;&gt;)</code> (open) — <code>IsGenericTypeDefinition</code> True; <code>typeof(List&lt;int&gt;)</code> (closed) — False, зато <code>IsConstructedGenericType</code> True. Инстанцируешь только закрытый тип.',
      sources: ["ms-generics-std"],
    },
    {
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: '<code>First&lt;int?&gt;</code>: <code>int?</code> — <b>nullable</b> value type → <span class="hl">гейт закрыт</span>, CS0453 «must be a non-nullable value type».', nodes: [{ id: "g", kind: "gate", at: { zone: "structc", row: 0 }, state: "ok", label: "T = int", detail: "OK" }, { id: "no", kind: "gate", at: { zone: "structc", row: 1 }, state: "fail", label: "T = int?", detail: "CS0453", accent: true }], edges: [] },
      ],
      explain: 'Опровержение мифа «<code>where T:struct</code> пускает Nullable». Дословно из таблицы constraints: «<code>where T : struct</code> — The type argument must be a <span class="hl">non-nullable value type</span>… the <code>struct</code> constraint implies the <code>new()</code> constraint». Прогон: <code>First(int[])</code> компилируется, а <code>First(int?[])</code> даёт <b>CS0453</b> — «must be a non-nullable value type». Без constraint компилятор «can only assume the members of <code>System.Object</code>»; constraint разблокирует операции над типом. Другие: <code>where T:class</code> (reference type), <code>new()</code> (public parameterless ctor, всегда последний).',
  ...
  cards: [
    ...
    },
    {
      ...
      noText: '«a generic type definition is only a template… you cannot create instances»; constructed = подстановка типов. Реальный вывод: <b>True False True</b>.',
  ...
  takeaways: [
    ...
    { icon: "cost", k: "Без боксинга (замер)", v: '<code>List&lt;int&gt;</code> хранит <code>int</code> inline — <b>~8.4 KB</b> на 1000, а <code>ArrayList</code> боксит каждый — <b>~40.6 KB</b>. «no need to box the value types» = ~5× дешевле.' },
