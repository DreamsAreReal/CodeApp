...
export const asyncIteratorStatemachine: LessonData = {
  ...
  spec: [
    { text: "«Represents a builder for asynchronous iterators.»", source: "ms-builder" },
  ],
  edgeCases: [
    { text: 'Async-двойник любого итератора — заменой типа возврата: «All of these preceding examples would have an asynchronous counterpart. In each case, you\'d replace the return type of <code>IEnumerable&lt;T&gt;</code> with an <code>IAsyncEnumerable&lt;T&gt;</code>».', source: "ms-iterators" },
  ...
  segments: [
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: '<code>Create()</code> — «Creates an instance of the <code>AsyncIteratorMethodBuilder</code> struct»; <code>MoveNext&lt;TStateMachine&gt;</code> гонит автомат.', nodes: [{ id: "b", kind: "obj", at: { zone: "builder", row: 0 }, typeTag: "AsyncIteratorMethodBuilder", value: "struct" }, { id: "cr", kind: "chip", at: { zone: "builder", row: 1, col: 0 }, value: "Create()" }, { id: "mn", kind: "chip", at: { zone: "builder", row: 1, col: 1 }, value: "MoveNext<TSM>", accent: true }], edges: [] },
        { codeLine: 3, caption: 'Плюс <code>AwaitOnCompleted</code>/<code>AwaitUnsafeOnCompleted</code> (расписать resume) и <span class="hl"><code>Complete()</code></span> — «Marks iteration as being completed».', nodes: [{ id: "b", kind: "obj", at: { zone: "builder", row: 0 }, typeTag: "AsyncIteratorMethodBuilder", value: "члены builder-а" }, { id: "aw", kind: "chip", at: { zone: "builder", row: 1, col: 0 }, value: "AwaitOnCompleted" }, { id: "cm", kind: "chip", at: { zone: "builder", row: 1, col: 1 }, value: "Complete()", accent: true }], edges: [] },
      ],
      explain: 'Прогресс async-итератора ведёт специальный builder — <code>AsyncIteratorMethodBuilder</code>: «Represents a builder for asynchronous iterators». Это <code>struct</code> (по API: «public struct AsyncIteratorMethodBuilder»), поле которого хранится внутри сгенерированной машины. Его члены (дословно из API): <code>Create()</code> — «Creates an instance of the <code>AsyncIteratorMethodBuilder</code> struct»; <code>MoveNext&lt;TStateMachine&gt;(TStateMachine)</code> — «Invokes <code>MoveNext()</code> on the state machine while guarding the <code>ExecutionContext</code>»; <code>AwaitOnCompleted</code>/<code>AwaitUnsafeOnCompleted</code> — «Schedules the state machine to proceed to the next action when the specified awaiter completes»; <code>Complete()</code> — «Marks iteration as being completed, whether successfully or otherwise». Это тот же method-builder-паттерн, что у обычного async-метода (M4), но специализированный под итератор — потому «asynchronous iterator».',
      sources: ["ms-builder"],
    },
    {
      ...
      explain: 'Механика прогона — тот же цикл «MoveNext → await → callback → MoveNext», что у async-метода, но обёрнутый в итератор. <code>MoveNextAsync()</code> (потребитель дёргает его в <code>await foreach</code>) внутри вызывает <code>builder.MoveNext(ref stateMachine)</code>: «Invokes <code>MoveNext()</code> on the state machine while guarding the <code>ExecutionContext</code>». Если тело упирается в <b>незавершённый</b> <code>await</code>, builder через <code>AwaitOnCompleted</code>/<code>AwaitUnsafeOnCompleted</code> — «Schedules the state machine to proceed to the next action when the specified awaiter completes» — расписывает продолжение и <code>MoveNext</code> выходит, освобождая поток (никакого блокирования). Когда awaiter завершается, callback снова зовёт <code>MoveNext</code>, тело возобновляется до следующего <code>yield return</code> (элемент готов) либо до конца, где builder делает <code>Complete()</code> — «Marks iteration as being completed». Так поле состояния (<code>&lt;&gt;1__state</code>) кодирует и точки <code>yield</code>, и точки <code>await</code>.',
      sources: ["ms-builder"],
    },
    {
  ...
  cards: [
    ...
    },
    {
      ...
      okText: '<code>AsyncIteratorMethodBuilder</code> — <code>struct</code> («public struct…») с членами <code>Create</code>/<code>MoveNext</code>/<code>Complete</code>: builder «for asynchronous iterators». Печать: <b>isStruct=True Create=True MoveNext=True Complete=True</b>.',
  ...
  takeaways: [
    ...
    { icon: "cost", k: "AsyncIteratorMethodBuilder", v: '«Represents a builder for asynchronous iterators» — <code>struct</code> с <code>Create</code>/<code>MoveNext&lt;TSM&gt;</code> (guard ExecutionContext)/<code>AwaitOnCompleted</code>/<code>Complete</code>. Гонит машину по циклу MoveNext → await → callback → MoveNext.' },
