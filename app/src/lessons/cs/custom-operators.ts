...
export const customOperators: LessonData = {
  ...
  spec: [
    { text: "«Extension methods are static methods, but they're called as if they were instance methods on the extended type… Both forms of extension methods are compiled to the same IL (Intermediate Language).» <span class=\"ru-tr\">«Extension-методы — это статические методы, но их вызывают так, будто это методы экземпляра расширяемого типа… Обе формы extension-методов компилируются в один и тот же IL (промежуточный язык, Intermediate Language).»</span>", source: "ms-ext" },
  ],
  edgeCases: [
    { text: "LINQ-операторы — это и есть extension-методы: «The most common extension members are the <span class=\"hl\">LINQ standard query operators</span> that add query functionality to the existing <code>System.Collections.IEnumerable</code> and <code>System.Collections.Generic.IEnumerable&lt;T&gt;</code> types». <span class=\"ru-tr\">«Самые распространённые extension-члены — это стандартные операторы запросов LINQ, которые добавляют функциональность запросов к существующим типам <code>System.Collections.IEnumerable</code> и <code>System.Collections.Generic.IEnumerable&lt;T&gt;</code>».</span>", source: "ms-ext" },
    { text: "Кастомный оператор обязан быть отложенным: «Custom implementations of the standard query operators should use <span class=\"hl\">deferred execution</span> to return the results». <span class=\"ru-tr\">«Собственные реализации стандартных операторов запросов должны использовать отложенное выполнение для возврата результатов».</span>", source: "ms-linq" },
    { text: "Приоритет instance: «At compile time, extension members always have <span class=\"hl\">lower priority than instance</span> (or static) members defined in the type itself… if a type has a method named <code>Process(int i)</code>, and you have an extension method with the same signature, the compiler always binds to the member method». <span class=\"ru-tr\">«На этапе компиляции extension-члены всегда имеют более низкий приоритет, чем члены экземпляра (или статические), определённые в самом типе… если у типа есть метод с именем <code>Process(int i)</code>, а у вас есть extension-метод с той же сигнатурой, компилятор всегда привязывается к методу-члену».</span>", source: "ms-ext" },
  ],

  misconceptions: [
    {
      wrong: "чтобы добавить свой LINQ-оператор, надо менять исходники .NET или наследовать коллекцию",
      hook: 'Ложная преграда: <span class="wrong">свой оператор LINQ = патчить .NET или наследовать <code>List</code></span>. На деле — обычный <b>extension-метод</b>: «Extension members enable you to <span class="hl">"add" methods to existing types without creating a new derived type, recompiling, or otherwise modifying the original type</span>». <span class="ru-tr">«Extension-члены позволяют вам «добавлять» методы к существующим типам, не создавая новый производный тип, не перекомпилируя и не изменяя иным образом исходный тип».</span> Сами <code>Where/Select</code> — тоже extension: «The most common extension members are the <b>LINQ standard query operators</b>». <span class="ru-tr">«Самые распространённые extension-члены — это стандартные операторы запросов <b>LINQ standard query operators</b>».</span> Ниже <b>пять разборов</b>: LINQ как extension, анатомия оператора, обязательная <b>отложенность</b> (yield), приоритет instance-метода, и <b>машинная панель</b> — реальный IL: <code>seq.EveryOther()</code> компилируется в <span class="hl">обычный статический вызов</span> <code>MyOperators::EveryOther</code>, а <code>widget.Tag()</code> связывается с instance-методом, не с extension.',
      source: "ms-ext",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'LINQ построен на extension-методах — и ты расширяешь его тем же приёмом. Дословно: «Extension members enable you to <span class="hl">"add" methods to existing types without creating a new derived type, recompiling, or otherwise modifying the original type</span>». <span class="ru-tr">«Extension-члены позволяют вам «добавлять» методы к существующим типам, не создавая новый производный тип, не перекомпилируя и не изменяя иным образом исходный тип».</span> И: «The most common extension members are the <b>LINQ standard query operators</b> that add query functionality to the existing <code>System.Collections.IEnumerable</code> and <code>System.Collections.Generic.IEnumerable&lt;T&gt;</code> types. To use the standard query operators, first bring them into scope with a <code>using System.Linq</code> directive». <span class="ru-tr">«Самые распространённые extension-члены — это стандартные операторы запросов <b>LINQ standard query operators</b>, которые добавляют функциональность запросов к существующим типам <code>System.Collections.IEnumerable</code> и <code>System.Collections.Generic.IEnumerable&lt;T&gt;</code>. Чтобы использовать стандартные операторы запросов, сначала введите их в область видимости директивой <code>using System.Linq</code>».</span> Значит <code>Where/Select</code> «висят» на <code>IEnumerable&lt;T&gt;</code> ровно так же, как повиснет твой <code>EveryOther</code> — и в цепочке они неразличимы.',
      sources: ["ms-ext"],
    },
    {
      ...
      explain: 'Анатомия extension-метода — дословно: «Extension methods are defined as static methods but are called by using instance method syntax. Their <b>first parameter specifies which type the method operates on</b>. The parameter follows the <span class="hl"><code>this</code> modifier</span>. Extension methods are only in scope when you explicitly import the namespace into your source code with a <code>using</code> directive». <span class="ru-tr">«Extension-методы определяются как статические методы, но вызываются с помощью синтаксиса методов экземпляра. Их <b>первый параметр указывает, к какому типу применяется метод</b>. Этот параметр следует за модификатором <code>this</code>. Extension-методы находятся в области видимости, только когда вы явно импортируете пространство имён в свой исходный код директивой <code>using</code>».</span> Плюс структурное требование: «Both forms of extensions must be defined inside a <b>non-nested, nongeneric static class</b>». <span class="ru-tr">«Обе формы расширений должны быть определены внутри <b>невложенного, неуниверсального статического класса</b>».</span> То есть: top-level <code>static class</code> → <code>static</code> метод → первый параметр <code>this IEnumerable&lt;T&gt; src</code> → добавляешь <code>using</code> — и <code>seq.EveryOther()</code> работает как родной оператор.',
      sources: ["ms-ext"],
    },
    {
      ...
      explain: 'Главное правило кастомного оператора — <b>отложенность</b>. Дословно (LINQ): «Custom implementations of the standard query operators should use <span class="hl">deferred execution</span> to return the results». <span class="ru-tr">«Собственные реализации стандартных операторов запросов должны использовать отложенное выполнение для возврата результатов».</span> Реализация через <code>yield return</code> даёт это бесплатно: метод становится компиляторным итератором (стейт-машина из раздела S18), тело исполняется лениво при перечислении. Если вместо этого собрать всё в <code>List</code> и вернуть его — оператор станет <b>immediate</b>: потеряет ленивость, перестанет работать с бесконечными/дорогими источниками и будет читать вход целиком даже под <code>.First()</code>. Машинная панель докажет отложенность счётчиком (before=0).',
      sources: ["ms-linq"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: 'Extension <code>Tag(this object)</code> <b>не вызывается</b>: «extension members always have <span class="hl">lower priority</span> than instance (or static) members». <span class="ru-tr">«extension-члены всегда имеют более низкий приоритет, чем члены экземпляра (или статические)».</span>', nodes: [{ id: "i", kind: "gate", at: { zone: "inst", row: 0 }, state: "ok", label: "instance", detail: "выбран" }, { id: "e", kind: "gate", at: { zone: "extm", row: 0 }, state: "fail", label: "Ext.Tag(this object)", detail: "проигнорирован", accent: true }], edges: [] },
      ...
      explain: 'Связывание extension-методов — <b>compile-time</b>, с чёткой приоритезацией. Дословно: «You can use extension members to extend a class or interface, but <b>not to override behavior</b> defined in a class. An extension member with the same name and signature as an interface or class members are never called. At compile time, extension members always have <span class="hl">lower priority than instance (or static) members</span> defined in the type itself… When the compiler encounters a member invocation, it first looks for a match in the type\'s members. If no match is found, it searches for any extension members». <span class="ru-tr">«Вы можете использовать extension-члены, чтобы расширить класс или интерфейс, но <b>не для того, чтобы переопределить поведение</b>, определённое в классе. Extension-член с тем же именем и сигнатурой, что и члены интерфейса или класса, никогда не вызывается. На этапе компиляции extension-члены всегда имеют более низкий приоритет, чем члены экземпляра (или статические), определённые в самом типе… Когда компилятор встречает вызов члена, он сначала ищет соответствие среди членов типа. Если соответствие не найдено, он ищет любые extension-члены».</span> Практический риск: если в тип позже добавят instance-метод с той же сигнатурой, вызовы «молча» переключатся на него — твой extension перестанет вызываться без единой ошибки компиляции.',
      sources: ["ms-ext"],
    },
    {
      ...
      explain: 'Машинная панель — реальный Release-IL (ilspycmd), доказывающий два факта. (1) <b>Instance-синтаксис — иллюзия</b>: <code>Enumerable.Range(1,10).EveryOther()</code> компилируется в <code>call …MyOperators::EveryOther&lt;int32&gt;(IEnumerable&lt;int&gt;)</code> — обычный статический вызов, где приёмник <code>seq</code> стал <b>первым аргументом</b>. Дословно: «Extension methods are static methods, but they\'re called as if they were instance methods… Both forms of extension methods are <span class="hl">compiled to the same IL</span>». <span class="ru-tr">«Extension-методы — это статические методы, но их вызывают так, будто это методы экземпляра… Обе формы extension-методов компилируются в один и тот же IL».</span> (2) <b>Приоритет instance</b>: <code>new Widget().Tag()</code> компилируется в <code>call instance string Widget::Tag()</code> — extension <code>MyOperators::Tag</code> в IL <b>не вызывается</b>, что подтверждает «lower priority than instance (or static) members». <span class="ru-tr">«более низкий приоритет, чем у членов экземпляра (или статических)».</span> (Реальный оператор снят компиляцией; exec-карты урока гоняют тело оператора локальным итератором, т.к. scripting-хост не хостит top-level static class.)',
  ...
  cards: [
    ...
    },
    {
      ...
      okText: 'Кастомный оператор на <code>yield return</code> — <b>deferred</b>: до <code>ToList</code> счётчик 0 (before=0); перечисление вызывает <code>onEach</code> 5 раз (after=5). Именно поэтому «Custom implementations … should use deferred execution». <span class="ru-tr">«Собственные реализации … должны использовать отложенное выполнение».</span>',
  ...
  takeaways: [
    { icon: "why", k: "свой оператор = extension", v: 'LINQ-операторы сами extension-методы; свой пишется так же: top-level <code>static class</code>, <code>static</code> метод, первый параметр <code>this IEnumerable&lt;T&gt;</code>, <code>using</code>. «"add" methods to existing types without … modifying the original type». <span class="ru-tr">««добавлять» методы к существующим типам, не … изменяя исходный тип».</span>' },
    { icon: "cost", k: "обязательно deferred", v: '«Custom implementations … should use deferred execution» <span class="ru-tr">«Собственные реализации … должны использовать отложенное выполнение»</span> — реализуй через <code>yield return</code> (замер: before=0 after=5). Так оператор ленив, streaming, композируется с <code>Where/Select</code> и работает с бесконечными источниками.' },
    { icon: "avoid", k: "instance бьёт extension", v: 'Связывание compile-time: «extension members always have lower priority than instance (or static) members» <span class="ru-tr">«extension-члены всегда имеют более низкий приоритет, чем члены экземпляра (или статические)»</span> (замер IL: <code>widget.Tag()</code> → instance-вызов). Добавили instance-метод той же сигнатуры — extension молча перестал вызываться.' },
  ],
