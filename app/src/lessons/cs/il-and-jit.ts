...
export const ilAndJit: LessonData = {
  ...
  edgeCases: [
    ...
    { text: "IL хранит и <b>верификацию</b>: перед компиляцией «CLR выполняется процедура, называемая верификацией — анализ высокоуровневого кода IL и проверка безопасности всех операций». Вся нужная для неё информация «хранится в метаданных управляемого модуля».", source: "clr-ch1" },
  ],
