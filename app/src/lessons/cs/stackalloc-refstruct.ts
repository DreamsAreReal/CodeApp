...
export const stackallocRefstruct: LessonData = {
  ...
  spec: [
    { text: "«A stackalloc expression allocates a block of memory on the stack. A stack-allocated memory block created during the method execution is automatically discarded when that method returns.» <span class=\"ru-tr\">«Выражение stackalloc выделяет блок памяти на стеке. Блок памяти, размещённый на стеке во время выполнения метода, автоматически отбрасывается, когда этот метод возвращает управление.»</span>", source: "ms-stackalloc" },
  ],
  edgeCases: [
    { text: "Не GC, не fixed (verbatim): «You can\'t explicitly free the memory allocated by <code>stackalloc</code>. A stack-allocated memory block <span class=\"hl\">isn\'t subject to garbage collection and doesn\'t need to be pinned by a <code>fixed</code> statement</span>». <span class=\"ru-tr\">«Нельзя явно освободить память, выделенную <code>stackalloc</code>. Блок памяти на стеке не подлежит сборке мусора и не нуждается в закреплении оператором <code>fixed</code>».</span>", source: "ms-stackalloc" },
    { text: "Стек ограничен (verbatim): «The amount of memory available on the stack is limited. If you allocate too much memory on the stack, a <span class=\"hl\">StackOverflowException is thrown</span>». <span class=\"ru-tr\">«Объём памяти, доступной на стеке, ограничен. Если выделить слишком много памяти на стеке, выбрасывается StackOverflowException».</span> Правила: лимит размера (условный <code>stackalloc</code> vs <code>new[]</code>) и «<b>Avoid using stackalloc inside loops</b>». <span class=\"ru-tr\">«<b>Избегайте использования stackalloc внутри циклов</b>».</span>", source: "ms-stackalloc" },
    { text: "Содержимое не инициализировано (verbatim): «The content of the newly allocated memory is <span class=\"hl\">undefined</span>… <b>Not initializing memory allocated by stackalloc is an important difference from the <code>new</code> operator</b>. Memory allocated by using the <code>new</code> operator is initialized to the 0 bit pattern». <span class=\"ru-tr\">«Содержимое только что выделенной памяти не определено… <b>Отсутствие инициализации памяти, выделенной stackalloc, — важное отличие от оператора <code>new</code></b>. Память, выделенная оператором <code>new</code>, инициализируется нулевым битовым шаблоном».</span> <code>T</code> должен быть unmanaged-типом.", source: "ms-stackalloc" },
  ],

  misconceptions: [
    {
      wrong: "stackalloc безопасен без ограничений; результат stackalloc всегда требует unsafe; память обнулена как у new",
      hook: 'Три мифа про stackalloc. «<span class="wrong">безопасен без ограничений</span>» — нет: «<span class="hl">The amount of memory available on the stack is limited</span>… a <code>StackOverflowException</code> is thrown» <span class="ru-tr">«Объём памяти, доступной на стеке, ограничен… выбрасывается <code>StackOverflowException</code>»</span> → лимит размера, не в циклах. «<span class="wrong">результат всегда требует unsafe</span>» — нет: «<span class="hl">You don\'t need to use an <code>unsafe</code> context</span> when you assign a stack-allocated memory block to a <code>Span&lt;T&gt;</code>» <span class="ru-tr">«Не нужен контекст <code>unsafe</code>, когда вы присваиваете блок памяти со стека переменной <code>Span&lt;T&gt;</code>»</span>; unsafe нужен только для указателя. «<span class="wrong">память обнулена</span>» — нет: «The content… is <span class="hl">undefined</span>» <span class="ru-tr">«Содержимое… не определено»</span>, в отличие от <code>new</code>. Ниже <b>пять разборов</b>: stackalloc на стеке, → Span без unsafe, лимит стека + правила, undefined + unmanaged + ref struct disposable, и <b>машинная панель</b> — реальный замер.',
  ...
  segments: [
    {
      ...
      explain: 'Что такое stackalloc (verbatim): «A <code>stackalloc</code> expression <span class="hl">allocates a block of memory on the stack</span>. A stack-allocated memory block created during the method execution <span class="hl">is automatically discarded when that method returns</span>. You can\'t explicitly free the memory allocated by <code>stackalloc</code>. A stack-allocated memory block <b>isn\'t subject to garbage collection and doesn\'t need to be pinned by a <code>fixed</code> statement</b>». <span class="ru-tr">«Выражение <code>stackalloc</code> выделяет блок памяти на стеке. Блок памяти на стеке, созданный во время выполнения метода, автоматически отбрасывается, когда этот метод возвращает управление. Нельзя явно освободить память, выделенную <code>stackalloc</code>. Блок памяти на стеке <b>не подлежит сборке мусора и не нуждается в закреплении оператором <code>fixed</code></b>».</span> То есть stackalloc — способ взять временный буфер <b>без давления на GC</b>: он живёт на стеке кадра метода, исчезает при выходе, GC его не касается. Отсюда и ограничения (стек мал, содержимое не инициализировано — разборы 03–04), и связь со Span (разбор 02): Span поверх stackalloc — типобезопасный, allocation-free буфер.',
    ...
    },
    {
      ...
      explain: 'stackalloc и Span (verbatim): «You can assign the result of a <code>stackalloc</code> expression to a variable of one of the following types: <code>System.Span&lt;T&gt;</code> or <code>System.ReadOnlySpan&lt;T&gt;</code>… <span class="hl">You don\'t need to use an <code>unsafe</code> context when you assign a stack-allocated memory block to a <code>Span&lt;T&gt;</code> or <code>ReadOnlySpan&lt;T&gt;</code> variable</span>». <span class="ru-tr">«Результат выражения <code>stackalloc</code> можно присвоить переменной одного из следующих типов: <code>System.Span&lt;T&gt;</code> или <code>System.ReadOnlySpan&lt;T&gt;</code>… Не нужен контекст <code>unsafe</code>, когда вы присваиваете блок памяти со стека переменной <code>Span&lt;T&gt;</code> или <code>ReadOnlySpan&lt;T&gt;</code>».</span> А для указателя: «You must use an <code>unsafe</code> context when you work with pointer types» <span class="ru-tr">«Нужно использовать контекст <code>unsafe</code>, когда вы работаете с указательными типами»</span> (<code>int* p = stackalloc int[…]</code>). Рекомендация доки прямо: «Use <code>Span&lt;T&gt;</code> or <code>ReadOnlySpan&lt;T&gt;</code> types to work with stack-allocated memory whenever possible». <span class="ru-tr">«По возможности используйте типы <code>Span&lt;T&gt;</code> или <code>ReadOnlySpan&lt;T&gt;</code> для работы с памятью на стеке».</span> Замер: <code>Span&lt;int&gt; s = stackalloc int[100]</code> добавляет <b>0</b> байт managed heap — буфер на стеке, Span лишь вью. Это идеальная связка: временный буфер без аллокации кучи и без <code>unsafe</code>. (Оговорка: конвертация <code>stackalloc</code>→указатель <b>вне</b> <code>unsafe</code> — preview-фича C# 15.)',
      sources: ["ms-stackalloc"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: 'Правило 2: <span class="hl">НЕ в циклах</span> — «Allocate the memory block outside a loop and reuse it inside the loop». <span class="ru-tr">«Выделяйте блок памяти вне цикла и переиспользуйте его внутри цикла».</span>', nodes: [{ id: "r", kind: "gate", at: { zone: "limit", row: 0 }, state: "ok", label: "условный лимит", detail: "OK" }, { id: "n", kind: "gate", at: { zone: "rules", row: 0 }, state: "fail", label: "stackalloc в цикле", detail: "нельзя", accent: true }], edges: [] },
      ],
      explain: 'Ограничение стека (verbatim, анти-миф MM20): «<span class="hl">The amount of memory available on the stack is limited. If you allocate too much memory on the stack, a <code>StackOverflowException</code> is thrown</span>. To avoid that exception, follow these rules: Limit the amount of memory you allocate by using <code>stackalloc</code>… <span class="hl">Avoid using <code>stackalloc</code> inside loops</span>. Allocate the memory block outside a loop and reuse it inside the loop». <span class="ru-tr">«Объём памяти, доступной на стеке, ограничен. Если выделить слишком много памяти на стеке, выбрасывается <code>StackOverflowException</code>. Чтобы избежать этого исключения, соблюдайте следующие правила: ограничивайте объём памяти, выделяемой через <code>stackalloc</code>… Избегайте использования <code>stackalloc</code> внутри циклов. Выделяйте блок памяти вне цикла и переиспользуйте его внутри цикла».</span> Каноничный безопасный паттерн — <b>условный</b> stackalloc: <code>Span&lt;byte&gt; buffer = inputLength &lt;= MaxStackLimit ? stackalloc byte[inputLength] : new byte[inputLength]</code>. Важно (VOLATILE): порог <code>1024</code>/<code>MaxStackLimit</code> — <b>иллюстрация</b>, не гарантия: «the amount of memory available on the stack depends on the environment… be conservative when you define the actual limit value». <span class="ru-tr">«объём памяти, доступной на стеке, зависит от окружения… будьте консервативны, когда задаёте фактическое значение лимита».</span> Замером в панели увидим оба пути: малое — на стеке (0 байт кучи), большое — на <code>new[]</code> (heap).',
      sources: ["ms-stackalloc"],
    },
    {
      ...
      explain: 'Инициализация и тип (verbatim): «The content of the newly allocated memory is <span class="hl">undefined</span>. You should initialize it before it\'s used». <span class="ru-tr">«Содержимое только что выделенной памяти не определено. Вы должны инициализировать её перед использованием».</span> И критично: «<b>Not initializing memory allocated by <code>stackalloc</code> is an important difference from the <code>new</code> operator</b>. Memory allocated by using the <code>new</code> operator is initialized to the 0 bit pattern». <span class="ru-tr">«<b>Отсутствие инициализации памяти, выделенной <code>stackalloc</code>, — важное отличие от оператора <code>new</code></b>. Память, выделенная оператором <code>new</code>, инициализируется нулевым битовым шаблоном».</span> Тип: «In expression <code>stackalloc T[E]</code>, <code>T</code> must be an <span class="hl">unmanaged type</span> and <code>E</code> must evaluate to a non-negative <code>int</code> value» <span class="ru-tr">«В выражении <code>stackalloc T[E]</code> тип <code>T</code> должен быть неуправляемым типом, а <code>E</code> должно вычисляться в неотрицательное значение <code>int</code>»</span> (замер: <code>stackalloc string[3]</code> — <code>CS0208</code>). Безопасность: «Using <code>stackalloc</code> automatically turns on <span class="hl">buffer overrun detection</span> features in the common language runtime (CLR)». <span class="ru-tr">«Использование <code>stackalloc</code> автоматически включает функции обнаружения переполнения буфера в среде CLR (common language runtime)».</span> А <code>ref struct</code> (verbatim, ref-struct): можно сделать disposable (паттерн <code>Dispose</code> + <code>using</code>), и «<span class="hl">Beginning with C# 13, you can also implement the <code>IDisposable</code></span> on <code>ref struct</code> types» <span class="ru-tr">«Начиная с C# 13, вы также можете реализовать <code>IDisposable</code> для типов <code>ref struct</code>»</span> (замер: <code>refStructDisposed=True</code>). Всё это — как безопасно пользоваться стековой памятью.',
  ...
  cards: [
    {
      ...
      noText: '«You don\'t need to use an unsafe context… assign… to a Span<T>» <span class="ru-tr">«Не нужен контекст unsafe… присваивая… переменной Span<T>»</span>; на стеке → 0 байт кучи. Реальный вывод: <b>stackallocNoHeapAlloc=True lastElem=99</b>.',
    ...
    },
    {
      ...
      noText: '«Beginning with C# 13, you can also implement the IDisposable on ref struct types» <span class="ru-tr">«Начиная с C# 13, вы также можете реализовать IDisposable для типов ref struct»</span>. Реальный вывод: <b>refStructDisposed=True</b>.',
      verify: { kind: "exec", run: "dotnet run", expect: "refStructDisposed=True" }, sourceRefs: ["ms-refstruct"],
    },
  ],

  takeaways: [
    { icon: "why", k: "stackalloc = стек", v: '«<span class="hl">allocates a block of memory on the stack</span>… automatically discarded when that method returns» <span class="ru-tr">«выделяет блок памяти на стеке… автоматически отбрасывается, когда этот метод возвращает управление»</span>; не GC, не нужен <code>fixed</code>. Результат → <code>Span&lt;T&gt;</code> <b>без unsafe</b> (замер: 0 байт кучи); unsafe — только для указателя.' },
    { icon: "cost", k: "стек ограничен", v: '«<span class="hl">a StackOverflowException is thrown</span>» <span class="ru-tr">«выбрасывается StackOverflowException»</span> на слишком большой блок. Правила: условный <code>n&lt;=лимит ? stackalloc : new[]</code> (замер: small=стек, large=heap), не в циклах. Порог (1024) — иллюстрация, env-dependent.' },
