...
export const propertyPositional: LessonData = {
  ...
  spec: [
    { text: "«Use a property pattern to match an expression's properties or fields against nested patterns, as the following example shows:»", source: "ms-patterns" },
    { text: "«The order of members in a positional pattern must match the order of parameters in the Deconstruct method. The code generated for the positional pattern calls the Deconstruct method.»", source: "ms-patterns" },
  ],
  edgeCases: [
    { text: "Пустой property pattern <code>is { }</code> — «matches everything <b>non-null</b>», и его можно использовать вместо <code>is not null</code> для создания переменной: <code>x is { } notNull</code>.", source: "ms-patterns" },
    { text: "Extended property pattern — вложенный путь: <code>{ Start.Y: 0 }</code> эквивалентно <code>{ Start: { Y: 0 } }</code>.", source: "ms-patterns" },
    { text: "Records с позиционными параметрами <b>неявно</b> дают <code>Deconstruct</code>: «the compiler creates a <code>Deconstruct</code> method with an <code>out</code> parameter for each positional parameter».", source: "ms-deconstruct" },
  ],

  misconceptions: [
    {
      wrong: "property pattern { X: 0 } и positional (0, 0) — просто два синтаксиса одного и того же",
      hook: 'Разница <span class="hl">механическая</span>, а не косметическая. Property pattern читает свойства/поля <b>по имени</b>: «Use a property pattern to match an expression\'s properties or fields against <b>nested patterns</b>». Positional pattern — <b>деконструирует</b> значение и матчит <b>по позиции</b>, и это реальный вызов метода: «The code generated for the positional pattern <b>calls the <code>Deconstruct</code> method</b>». Ниже абстракции — жёсткое правило: «The order of members in a positional pattern <span class="hl">must match the order of parameters in the <code>Deconstruct</code> method</span>». Дальше <b>пять разборов</b> — property, пустой <code>{ }</code>, extended-путь, tuple-positional и <b>машинная панель</b> вызова Deconstruct.',
      source: "ms-patterns",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Property pattern читает члены по имени и сверяет с вложенными паттернами: «Use a property pattern to match an expression\'s <b>properties or fields</b> against nested patterns». Условие матча: «A property pattern matches an expression when the expression result is <span class="hl">non-null</span> and <b>every nested pattern matches</b> the corresponding property or field». Он рекурсивен — «You can use <b>any pattern</b> as a nested pattern». Здесь <code>{ Items: &gt; 10, Cost: &gt; 1000.00m }</code> вкладывает два relational-паттерна; <code>Order(12, 1200m)</code> проходит оба → <code>0.10</code>.',
      sources: ["ms-patterns"],
    },
    {
  ...
  cards: [
    ...
    },
    {
      ...
      noText: '«The code generated for the positional pattern calls the Deconstruct method». Порядок членов = порядок out-параметров. Итог: <code>Origin / positive X basis end / Just a point</code>.',
