...
export const returnTypes: LessonData = {
  ...
  spec: [
    { text: "«An async method that has a void return type can't be awaited, and the caller of a void-returning method can't catch any exceptions that the method throws.»", source: "ms-tap" },
  ],
  edgeCases: [
  ...
  misconceptions: [
    {
      ...
      hook: 'Опасная путаница: «<span class="wrong">async void — то же, что async Task</span>». Нет: <code>async void</code> <b>нельзя await</b>, и его исключения «<span class="hl">can\'t be caught</span> by the caller» — они уходят на sync-context и <b>обрушивают приложение</b>. <code>async void</code> оправдан ТОЛЬКО как обработчик события. И второй миф: «<span class="wrong">Result — норм способ взять значение</span>» — нет, <code>.Result</code> блокирует поток. Ниже <b>пять разборов</b>: карта return-типов, <code>Task</code> vs <code>Task&lt;T&gt;</code>, <code>async void</code>-ловушка, исключения (Task ловит / void escape), и <b>машинная панель</b> — <code>Task&lt;T&gt;</code> reference vs <code>ValueTask&lt;T&gt;</code> value type.',
      source: "ms-tap",
    },
  ],
  ...
  segments: [
    ...
    },
    {
      ...
      code: ["button.Click += async (s,e) => { ... };   // ОК: event handler", "async void DoWork(){ ... }                  // ловушка: нельзя await", "// исключение из async void → на SynchronizationContext → crash"],
      scenes: [
        ...
        { codeLine: 2, caption: 'Исключение из <code>async void</code> <span class="hl">уходит на SynchronizationContext</span> и обрушивает приложение — вызывающий его не поймает.', nodes: [{ id: "no", kind: "gate", at: { zone: "void", row: 0 }, state: "fail", label: "исключение", detail: "escape · crash", accent: true }], edges: [] },
      ],
      explain: 'Дословно про <code>void</code>: «An async method can also have a <code>void</code> return type. This return type is used <b>primarily to define event handlers</b>, where a <code>void</code> return type is required. An async method that has a <code>void</code> return type <span class="hl">can\'t be awaited</span>, and the caller of a void-returning method <span class="hl">can\'t catch any exceptions</span> that the method throws». Практика (проверено — такой сниппет реально роняет процесс, потому его нет в exec-карточках урока): исключение из <code>async void</code> эскалируется на <code>SynchronizationContext</code> и обрушивает приложение. Правило: для методов без значения возвращай <code>Task</code>, а не <code>void</code>; <code>void</code> — только для событий.',
      sources: ["ms-tap"],
    },
    {
      ...
      explain: 'Разница async Task и async void в исключениях — ядро выбора типа. У <code>async Task</code>: «its exceptions are stored in the returned Task» — исключение живёт в задаче и перебрасывается на <code>await</code>, поэтому <code>try/catch</code> вокруг <code>await Safe()</code> его ловит (прогон: <code>Task caught: boom</code>). А у <code>async void</code> ловить нечего: «the caller… <span class="hl">can\'t catch any exceptions</span>» — они уходят на <code>SynchronizationContext</code>. Вот почему для «fire-and-forget» с обработкой ошибок нужен <code>Task</code> (можно <code>await</code> и обернуть <code>try/catch</code>), а не <code>void</code>.',
      sources: ["ms-tap"],
    },
    {
      ...
      explain: 'Это машинная панель урока — тип-категория return-типов, снятая через рефлексию. Дословно: «Because <code>Task</code> is a <b>reference type</b>, a <code>Task</code> object is <span class="hl">allocated from the heap</span>. If a method declared with the <code>async</code> modifier returns a cached result or completes synchronously, the extra allocations can accrue significant time costs». Прогон: <code>ValueTask&lt;int&gt;.IsValueType</code> → <b>True</b> (struct), <code>Task&lt;int&gt;.IsValueType</code> → <b>False</b> (reference, куча). Отсюда весь смысл <code>ValueTask</code>: когда результат часто готов синхронно и метод в горячем цикле, value-обёртка избегает аллокации <code>Task</code> из кучи. Но это не «всегда быстрее» — трейдофы и ограничения в S2.5.',
      sources: ["ms-tap", "ms-return"],
    },
  ],
  ...
  cards: [
    {
      ...
      noText: '«Task<TResult> if… return… operand of type TResult; Task if… no return operand». Реальный вывод: <b>done 42</b>.',
    ...
    },
    {
      ...
      noText: '«Task is a reference type, a Task object is allocated from the heap»; ValueTask — struct. Реальный вывод: <b>True False</b>.',
