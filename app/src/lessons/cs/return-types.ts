...
export const returnTypes: LessonData = {
  ...
  segments: [
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: 'Исключение из <code>async void</code> вызывающий <span class="hl">не может поймать</span> — оно остаётся необработанным и, «likely to cause your application to fail», обрушивает приложение.', nodes: [{ id: "no", kind: "gate", at: { zone: "void", row: 0 }, state: "fail", label: "исключение", detail: "unhandled · crash", accent: true }], edges: [] },
      ],
      explain: 'Дословно про <code>void</code> (async-return-types): «You use the <code>void</code> return type in asynchronous event handlers, which require a <code>void</code> return type. For methods other than event handlers that don\'t return a value, you should return a <code>Task</code> instead, because <span class="hl">an async method that returns <code>void</code> can\'t be awaited</span>». И про исключения — дословно: «The caller of a void-returning async method <span class="hl">can\'t catch exceptions thrown from the method</span>. <span class="hl">Such unhandled exceptions are likely to cause your application to fail</span>» (проверено — такой сниппет реально роняет процесс, потому его нет в exec-карточках урока). Правило (тоже дословно): «Make sure that any async method that can produce an exception has a return type of <code>Task</code> or <code>Task&lt;TResult&gt;</code> and that calls to the method are awaited»; <code>void</code> — только для событий.',
      sources: ["ms-return"],
    },
    {
      ...
      explain: 'Разница async Task и async void в исключениях — ядро выбора типа. Дословно (async-return-types): «If a method that returns a <code>Task</code> or <code>Task&lt;TResult&gt;</code> throws an exception, <span class="hl">the exception is stored in the returned task. The exception is rethrown when the task is awaited</span>». Поэтому <code>try/catch</code> вокруг <code>await Safe()</code> ловит исключение (прогон: <code>Task caught: boom</code>). А у <code>async void</code> ловить нечего: «The caller of a void-returning async method <span class="hl">can\'t catch exceptions thrown from the method</span>. Such unhandled exceptions are likely to cause your application to fail». Вот почему для «fire-and-forget» с обработкой ошибок нужен <code>Task</code> (можно <code>await</code> и обернуть <code>try/catch</code>), а не <code>void</code>.',
      sources: ["ms-return"],
    },
    {
  ...
  cards: [
    ...
    },
    {
      ...
      noText: '«Because <code>Task</code> and <code>Task&lt;TResult&gt;</code> are reference types»; <code>ValueTask&lt;T&gt;</code> — value type (struct). Реальный вывод: <b>True False</b>.',
