...
export const enumFlags: LessonData = {
  ...
  spec: [
    { text: "«An enumeration type (or enum type) is a value type defined by a set of named constants of the underlying integral numeric type.» <span class=\"ru-tr\">«Перечислимый тип (enum-тип) — это value type, заданный набором именованных констант базового целочисленного (integral) типа.»</span>", source: "ms-enum" },
  ],
  edgeCases: [
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: 'Значения по умолчанию: <span class="hl">старт с 0</span>, +1 по порядку. Spring=0, Summer=1, …', nodes: [{ id: "sp", kind: "chip", at: { zone: "names", row: 0 }, value: "Spring" }, { id: "su", kind: "chip", at: { zone: "names", row: 1 }, value: "Summer" }, { id: "v0", kind: "chip", at: { zone: "ints", row: 0 }, value: "int = 0" }, { id: "v1", kind: "chip", at: { zone: "ints", row: 1 }, value: "int = 1", accent: true }], edges: [] },
      ],
      explain: 'Enum — это value type над числами: «An enumeration type… is a <b>value type</b> defined by a set of named constants of the underlying <span class="hl">integral numeric</span> type». <span class="ru-tr">«Перечислимый тип… — это <b>value type</b>, заданный набором именованных констант базового целочисленного типа».</span> Значения по умолчанию: «By default, the associated constant values of enum members are of type <code>int</code>. They <b>start with zero and increase by one</b> following the definition text order». <span class="ru-tr">«По умолчанию связанные с членами enum константные значения имеют тип <code>int</code>. Они <b>начинаются с нуля и увеличиваются на единицу</b> в порядке следования в тексте определения».</span> Имена — удобная обёртка над целыми: <code>Season.Autumn</code> и есть <code>2</code>. Метод внутри enum объявить нельзя — только extension-члены.',
      sources: ["ms-enum"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Опровержение мифа «enum всегда int». Дословно: «You can <b>explicitly specify any other integral numeric type</b> as an underlying type of an enumeration type». <span class="ru-tr">«Вы можете <b>явно указать любой другой целочисленный (integral) тип</b> как базовый тип перечислимого типа».</span> Прогон: <code>enum Size : byte</code> даёт underlying <code>Byte</code>, <code>(int)Size.L</code> → <b>4</b>. Каст между enum и его базовым типом — <span class="hl">явный</span>: «explicit conversions exist between the enumeration type and its underlying integral type». <span class="ru-tr">«между перечислимым типом и его базовым целочисленным типом существуют явные преобразования».</span> Практика: выбирай узкий базовый тип для компактных enum (например, в структурах данных), но помни — по умолчанию это int.',
      sources: ["ms-enum"],
    },
    {
      ...
      explain: 'Тонкая ловушка. Дословно: «The default value of an enumeration type <code>E</code> is the value produced by expression <code>(E)0</code>, <span class="hl">even if zero doesn\'t have the corresponding enum member</span>». <span class="ru-tr">«Значение по умолчанию перечислимого типа <code>E</code> — это значение, получаемое выражением <code>(E)0</code>, даже если нулю не соответствует ни один член enum».</span> Прогон: <code>default(Color)</code> = 0, но у <code>Color</code> нет члена 0, поэтому <code>ToString</code> печатает <code>0</code>, а <code>Enum.IsDefined</code> → <b>False</b>. Почему так: «the 0-bit pattern is the default for all struct types, including all enum types». <span class="ru-tr">«нулевой битовый шаблон является значением по умолчанию для всех struct-типов, включая все enum-типы».</span> Рекомендация доки: «Almost always define a member with value <code>0</code> in your enums» <span class="ru-tr">«Почти всегда определяйте в своих enum член со значением <code>0</code>»</span> и валидируй <code>Enum.IsDefined</code> при конвертации из чисел.',
      sources: ["ms-enum"],
    },
    {
  ...
  cards: [
    ...
    },
    {
      ...
      noText: '«You can explicitly specify any other integral numeric type as an underlying type». <span class="ru-tr">«Вы можете явно указать любой другой целочисленный (integral) тип как базовый тип».</span> Underlying — <b>Byte</b>, значение L — <b>4</b>. Реальный вывод: <b>4 Byte</b>.',
    ...
    },
    {
      ...
      noText: '«The default value… is (E)0, even if zero doesn\'t have the corresponding enum member». <span class="ru-tr">«Значение по умолчанию… — это (E)0, даже если нулю не соответствует ни один член enum».</span> Реальный вывод: <b>0 0 False</b>. Всегда определяй член со значением 0.',
