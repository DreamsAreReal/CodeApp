...
export const genericMethods: LessonData = {
  ...
  spec: [
    { text: "«The compiler can infer the type parameters based on the method arguments you pass in; it cannot infer the type parameters only from a constraint or return value.» <span class=\"ru-tr\">«Компилятор может вывести параметры типа на основе аргументов метода, которые вы передаёте; он не может вывести параметры типа только из ограничения или возвращаемого значения.»</span>", source: "ms-gen-methods" },
  ],
  edgeCases: [
    { text: "Метод обобщён <b>только своим</b> списком <code>&lt;T&gt;</code>: «A generic method is a method that is <span class=\"hl\">declared with type parameters</span>» <span class=\"ru-tr\">«Обобщённый метод — это метод, объявленный с параметрами типа»</span> — не потому что лежит в generic-типе.", source: "ms-gen-methods" },
    { text: "Вывод <b>не работает без параметров</b>: «type inference does not work with methods that have <span class=\"hl\">no parameters</span>» <span class=\"ru-tr\">«вывод типов не работает с методами, у которых нет параметров»</span> — тогда пиши <code>Make&lt;int&gt;()</code> явно (иначе CS0411).", source: "ms-gen-methods" },
    { text: "Вывод идёт <b>до</b> резолва перегрузок: «Type inference occurs at compile time <span class=\"hl\">before the compiler tries to resolve overloaded method signatures</span>» <span class=\"ru-tr\">«Вывод типов происходит во время компиляции, прежде чем компилятор пытается разрешить перегруженные сигнатуры методов»</span>.", source: "ms-gen-methods" },
  ],

  misconceptions: [
    {
      wrong: "раз метод внутри generic-класса, он уже generic; и <T> всегда надо писать явно",
      hook: 'Оба тезиса неверны. Во-первых, метод обобщён <b>только своим</b> списком параметров типа: «A generic method is a method that is <span class="hl">declared with type parameters</span>» <span class="ru-tr">«Обобщённый метод — это метод, объявленный с параметрами типа»</span> — не потому, что лежит в <code>Class&lt;T&gt;</code>. Во-вторых, <code>&lt;T&gt;</code> почти всегда писать <b>не надо</b>: «You can also omit the type argument and the <span class="hl">compiler will infer it</span>» <span class="ru-tr">«Вы также можете опустить аргумент типа, и компилятор выведет его»</span>. Компилятор выводит <code>T</code> из аргументов: «The compiler can infer the type parameters based on the <b>method arguments</b> you pass in; it cannot infer the type parameters only from a constraint or return value» <span class="ru-tr">«Компилятор может вывести параметры типа на основе <b>аргументов метода</b>, которые вы передаёте; он не может вывести параметры типа только из ограничения или возвращаемого значения»</span>. Отсюда границы: без параметров вывода нет, а идёт он <b>до</b> резолва перегрузок. Дальше <b>пять разборов</b>: что делает метод обобщённым, вывод из аргументов, вывод нескольких <code>&lt;K,V&gt;</code>, границы вывода (нет параметров / до overload), и <b>машинная панель</b> — один <code>Echo&lt;T&gt;</code>, три вывода по вызову (реальный прогон: <code>42:Int32 hi:String 3.5:Double</code>).',
  ...
  segments: [
    {
      ...
      explain: 'Точное определение: «A generic method is a method that is <span class="hl">declared with type parameters</span>» <span class="ru-tr">«Обобщённый метод — это метод, объявленный с параметрами типа»</span>. Ключевой нюанс — обобщённость не наследуется от содержащего типа: метод обобщён только тем, что имеет <b>свой</b> список <code>&lt;T&gt;</code>. Внутри generic-класса «non-generic methods can access the class-level type parameters» <span class="ru-tr">«необобщённые методы могут обращаться к параметрам типа уровня класса»</span> — то есть <code>M(T x)</code> может использовать <code>T</code> класса, оставаясь необобщённым методом. А если объявить у метода параметр с тем же именем, что у класса, будет предупреждение: «the compiler generates warning <span class="hl">CS0693</span> because within the method scope, the argument supplied for the inner <code>T</code> hides the argument supplied for the outer <code>T</code>» <span class="ru-tr">«компилятор выдаёт предупреждение CS0693, потому что в области видимости метода аргумент, переданный для внутреннего <code>T</code>, скрывает аргумент, переданный для внешнего <code>T</code>»</span>. Практика: дай методу своё имя параметра (<code>&lt;U&gt;</code>), если он должен быть обобщён отдельно от класса.',
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: '<code>Echo(42)</code> без <code>&lt;int&gt;</code>: «the <span class="hl">compiler will infer it</span>» <span class="ru-tr">«компилятор выведет его»</span>. Из аргумента <code>42</code> компилятор выводит <code>T=int</code>.', nodes: [{ id: "c", kind: "obj", at: { zone: "call", row: 0 }, typeTag: "Echo(42)", value: "без <T>" }, { id: "i", kind: "gate", at: { zone: "infer", row: 0 }, state: "ok", label: "вывод", detail: "T = int", accent: true }], edges: [{ id: "e", from: "c", to: "i", accent: true }] },
        { codeLine: 2, out: "", caption: 'Вывод — только из <b>аргументов</b>: «based on the <span class="hl">method arguments you pass in</span>; it cannot infer… only from a constraint or return value» <span class="ru-tr">«на основе аргументов метода, которые вы передаёте; он не может вывести… только из ограничения или возвращаемого значения»</span>. Тип аргумента → тип параметра.', nodes: [{ id: "i", kind: "gate", at: { zone: "infer", row: 0 }, state: "ok", label: "42 → int", detail: "из аргумента", accent: true }], edges: [] },
      ],
      explain: 'Type inference убирает шум явных <code>&lt;T&gt;</code>: «You can also omit the type argument and the <span class="hl">compiler will infer it</span>. The following call to <code>Swap</code> is equivalent to the previous call:» <span class="ru-tr">«Вы также можете опустить аргумент типа, и компилятор выведет его. Следующий вызов <code>Swap</code> эквивалентен предыдущему вызову:»</span> — то есть <code>Swap(ref a, ref b)</code> без явного <code>&lt;T&gt;</code>. Источник вывода строго один — аргументы: «The compiler can infer the type parameters based on the <b>method arguments</b> you pass in; it cannot infer the type parameters only from a constraint or return value» <span class="ru-tr">«Компилятор может вывести параметры типа на основе <b>аргументов метода</b>, которые вы передаёте; он не может вывести параметры типа только из ограничения или возвращаемого значения»</span>. Поэтому <code>Echo(42)</code> даёт <code>T=int</code>, <code>Echo("hi")</code> — <code>T=string</code>: тип берётся из фактического аргумента, а <code>typeof(T)</code> в теле это подтверждает (реификация из первого урока секции). Правило одинаково для статических и инстансных методов: «The same rules for type inference apply to static methods and instance methods» <span class="ru-tr">«Те же правила вывода типов применяются к статическим методам и методам экземпляра»</span>.',
      sources: ["ms-gen-methods"],
    },
    {
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: '<code>Make&lt;T&gt;()</code> без параметров: выводить <code>T</code> <b>не из чего</b>. «type inference does not work with methods that have <span class="hl">no parameters</span>» <span class="ru-tr">«вывод типов не работает с методами, у которых нет параметров»</span> → CS0411.', nodes: [{ id: "n", kind: "gate", at: { zone: "noinf", row: 0 }, state: "fail", label: "Make()", detail: "CS0411", accent: true }], edges: [] },
        ...
        { codeLine: 3, out: "", caption: 'Порядок фаз: «Type inference occurs at compile time <span class="hl">before the compiler tries to resolve overloaded method signatures</span>» <span class="ru-tr">«Вывод типов происходит во время компиляции, прежде чем компилятор пытается разрешить перегруженные сигнатуры методов»</span>. Сначала вывод, потом выбор перегрузки.', nodes: [{ id: "e", kind: "gate", at: { zone: "noinf", row: 0 }, state: "ok", label: "Make<int>()", detail: "явно" }, { id: "b", kind: "gate", at: { zone: "before", row: 0 }, state: "ok", label: "вывод → overload", detail: "два шага", accent: true }], edges: [] },
      ],
      explain: 'Две границы, которые ловят на собеседованиях. Первая — <b>нет вывода без параметров</b>: «it cannot infer the type parameters only from a constraint or return value. Therefore <span class="hl">type inference does not work with methods that have no parameters</span>» <span class="ru-tr">«он не может вывести параметры типа только из ограничения или возвращаемого значения. Поэтому вывод типов не работает с методами, у которых нет параметров»</span>. <code>Make&lt;T&gt;()</code> нельзя вызвать как <code>Make()</code> — компилятор бросает CS0411, надо писать <code>Make&lt;int&gt;()</code>. Вторая — <b>порядок фаз</b>: «Type inference occurs at compile time before the compiler tries to resolve overloaded method signatures. The compiler applies type inference logic to <span class="hl">all generic methods that share the same name</span>. In the overload resolution step, the compiler includes only those generic methods on which type inference succeeded» <span class="ru-tr">«Вывод типов происходит во время компиляции, прежде чем компилятор пытается разрешить перегруженные сигнатуры методов. Компилятор применяет логику вывода типов ко всем обобщённым методам с одинаковым именем. На шаге разрешения перегрузки компилятор включает только те обобщённые методы, для которых вывод типов удался»</span>. То есть сначала для каждой одноимённой обобщённой перегрузки проверяется, выводится ли <code>T</code>; те, где вывод провалился, из резолва выбрасываются.',
      sources: ["ms-gen-methods"],
    },
    {
      ...
      explain: 'Это машинная панель урока — вывод <code>T</code> по каждому вызову, снятый прогоном. Один исходник <code>Echo&lt;T&gt;(T x)</code> при трёх вызовах даёт три разных <code>T</code>: <code>Echo(42)</code>→<code>int</code>, <code>Echo("hi")</code>→<code>string</code>, <code>Echo(3.5)</code>→<code>double</code>. Реальный вывод — <code>42:Int32 hi:String 3.5:Double</code>: значение плюс <b>реальное</b> имя выведенного типа (<code>typeof(T)</code> реифицирован, как в первом уроке секции). Это и есть суть вывода: «The compiler can infer the type parameters based on the method arguments you pass in» <span class="ru-tr">«Компилятор может вывести параметры типа на основе аргументов метода, которые вы передаёте»</span> — тип аргумента становится типом параметра, автоматически и на компиляции. Явные <code>&lt;int&gt;</code>/<code>&lt;string&gt;</code>/<code>&lt;double&gt;</code> не нужны, но были бы эквивалентны.',
      sources: ["ms-gen-methods"],
    },
  ],
  ...
  cards: [
    ...
    },
    {
      ...
      noText: 'Вывод «based on the method arguments you pass in» <span class="ru-tr">«на основе аргументов метода, которые вы передаёте»</span> работает позиционно для каждого параметра. Реальный вывод: <b>Int32->String | Boolean->Double</b>.',
  ...
  takeaways: [
    { icon: "why", k: "Свой <T> = generic", v: '«A generic method is a method that is <span class="hl">declared with type parameters</span>» <span class="ru-tr">«Обобщённый метод — это метод, объявленный с параметрами типа»</span> — обобщённость от собственного списка <code>&lt;T&gt;</code>, не от класса. Одинаковое имя с параметром класса → <b>CS0693</b>.' },
    { icon: "cost", k: "Вывод из аргументов", v: '«compiler will infer it» <span class="ru-tr">«компилятор выведет его»</span> из аргументов: <code>Echo(42)</code>→int, <code>Pair(1,"x")</code>→(int,string). Явный <code>&lt;T&gt;</code> обычно не нужен. Замер: <code>42:Int32 hi:String 3.5:Double</code>.' },
    { icon: "avoid", k: "Границы вывода", v: 'Вывод только из аргументов — «cannot infer the type parameters only from a constraint or return value» <span class="ru-tr">«не может вывести параметры типа только из ограничения или возвращаемого значения»</span>, значит без параметров <b>CS0411</b> (нужен явный <code>&lt;T&gt;</code>). Идёт <b>до</b> резолва перегрузок; точная перегрузка бьёт generic (замер: <code>int generic</code>).' },
  ],
