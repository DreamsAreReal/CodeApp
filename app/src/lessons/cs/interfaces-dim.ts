...
export const interfacesDim: LessonData = {
  ...
  spec: [
    { text: "«An explicit interface implementation is a class member that is only called through the specified interface… neither are available directly on the class.» <span class=\"ru-tr\">«Явная реализация интерфейса — это член класса, который вызывается только через указанный интерфейс… ни один из них недоступен напрямую на классе.»</span>", source: "ms-explicit" },
  ],
  edgeCases: [
  ...
  misconceptions: [
    {
      ...
      hook: 'Две ложные интуиции. Первая — «<span class="wrong">интерфейс не может иметь тело метода</span>»: с <b>C# 8.0</b> может — это default interface method (DIM). Вторая — «<span class="wrong">default-метод виден на экземпляре класса</span>»: нет, и explicit, и DIM живут <b>только за ссылкой интерфейса</b> — «that method is only accessible through a reference of the interface type». <span class="ru-tr">«этот метод доступен только через ссылку типа интерфейса».</span> Один объект через <code>класс</code>-переменную и через <code>интерфейс</code>-переменную зовёт <span class="hl">разный код</span>. Ниже <b>пять разборов</b>: контракт, <b>машинная панель</b> гейта интерфейс-ссылки (реально снятое <code>class-Hi Hi</code> + CS1061), DIM как версионирование, дизамбигуация двух интерфейсов и границы.',
      source: "ms-explicit",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Интерфейс — контракт, а не наследование: «An interface defines a <b>contract</b>: a group of related methods, properties, events, and indexers that a class or struct must implement. Interfaces let a single type <span class="hl">implement multiple contracts</span>, which is important because C# doesn\'t support multiple inheritance of classes». <span class="ru-tr">«Интерфейс определяет <b>контракт</b>: набор связанных методов, свойств, событий и индексаторов, которые класс или структура обязаны реализовать. Интерфейсы позволяют одному типу реализовать несколько контрактов, что важно, поскольку C# не поддерживает множественное наследование классов».</span> Любой класс/структура, реализующий контракт, обязан дать все члены; инстанцировать интерфейс нельзя. Ключевая асимметрия: «A class can inherit from only one base class but can <b>implement multiple interfaces</b>». <span class="ru-tr">«Класс может наследоваться только от одного базового класса, но может <b>реализовать несколько интерфейсов</b>».</span> Члены интерфейса <code>public</code> по умолчанию, instance-полей в нём нет.',
      sources: ["ms-iface"],
    },
    {
      ...
      explain: 'Это машинная панель урока — гейт интерфейс-ссылки, снятый числом. Explicit-реализация живёт <b>только</b> за ссылкой интерфейса: «An explicit interface implementation is a class member that is <span class="hl">only called through the specified interface</span>… neither are available directly on the class». <span class="ru-tr">«Явная реализация интерфейса — это член класса, который вызывается только через указанный интерфейс… ни один из них недоступен напрямую на классе».</span> Прогон: <code>e.Hello()</code> (класс-переменная) → <code>class-Hi</code>, <code>i.Hello()</code> (интерфейс-переменная) → <code>Hi</code>. Тот же принцип у DIM: default-метод «is <b>only accessible through a reference of the interface type</b>. The inherited member doesn\'t appear as part of the public interface» <span class="ru-tr">«доступен <b>только через ссылку типа интерфейса</b>. Унаследованный член не отображается как часть публичного интерфейса».</span> — вызов <code>s.Info(...)</code> прямо на экземпляре даёт <b>CS1061</b> (проверено). Диспетчер контракта решает по типу переменной, а не по объекту.',
      sources: ["ms-explicit"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Explicit нужен, когда «two interfaces declare members with the same name». <span class="ru-tr">«два интерфейса объявляют члены с одним и тем же именем».</span> Дословно повод: «If a class implements two interfaces that contain a member with the same signature, then implementing that member on the class will cause <b>both interfaces to use that member</b>» <span class="ru-tr">«Если класс реализует два интерфейса, содержащих член с одинаковой сигнатурой, то реализация этого члена в классе приведёт к тому, что <b>оба интерфейса будут использовать этот член</b>».</span> — а если нужны РАЗНЫЕ реализации, помечаешь их именем интерфейса. Прогон: <code>(IA)c.Id()</code> → <b>A</b>, <code>(IB)c.Id()</code> → <b>B</b>. «The class member <code>IControl.Paint</code> is only available through the <code>IControl</code> interface, and <code>ISurface.Paint</code> is only available through <code>ISurface</code>. Both method implementations are <span class="hl">separate</span>». <span class="ru-tr">«Член класса <code>IControl.Paint</code> доступен только через интерфейс <code>IControl</code>, а <code>ISurface.Paint</code> — только через <code>ISurface</code>. Обе реализации методов раздельны».</span> Одна общая (implicit) реализация годится, только если устраивают оба контракта.',
      sources: ["ms-explicit"],
    },
    {
      ...
      explain: 'Границы контракта. Explicit-реализация <b>без модификатора доступа</b>: «An explicit interface implementation doesn\'t have an access modifier since it isn\'t accessible as a member of the type… If you specify an access modifier… you get compiler error <span class="hl">CS0106</span>». <span class="ru-tr">«Явная реализация интерфейса не имеет модификатора доступа, поскольку она недоступна как член типа… Если указать модификатор доступа… получишь ошибку компилятора CS0106».</span> Интерфейс не может иметь instance-полей/конструкторов/финализаторов — поэтому DIM <b>не</b> даёт множественного наследования состояния (наследуется поведение, не поля). И DIM — не только про компилятор: она «requires runtime support» <span class="ru-tr">«требует поддержки со стороны рантайма»</span> — CLR-доработки в <b>.NET Core 3.0+</b>. Всё это делает интерфейс мощным контрактом без побочных эффектов настоящего множественного наследования.',
  ...
  cards: [
    {
      ...
      noText: '«explicit interface implementation… is only called through the specified interface». <span class="ru-tr">«явная реализация интерфейса… вызывается только через указанный интерфейс».</span> Через класс — public (<b>class-Hi</b>), через интерфейс — explicit (<b>Hi</b>).',
    ...
    },
    {
      ...
      noText: '«Both method implementations are separate». <span class="ru-tr">«Обе реализации методов раздельны».</span> Маршрут решает тип ссылки: через <code>IA</code> — <b>A</b>, через <code>IB</code> — <b>B</b>. Реальный вывод: <b>A B</b>.',
