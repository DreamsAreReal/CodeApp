...
export const reflectionOverview: LessonData = {
  ...
  spec: [
    { text: "«The classes in the System.Reflection namespace, together with System.Type, enable you to obtain information about loaded assemblies and the types defined within them.»", source: "ms-reflection" },
  ],
  edgeCases: [
    { text: "Иерархия вложенности буквальна: «Assemblies contain modules, modules contain types, and types contain members» — reflection даёт объект на каждый уровень.", source: "ms-reflection" },
    { text: "<code>CustomAttributeData</code> читает атрибуты <b>без</b> их инстанцирования — «allows you to examine attributes without creating instances of them» (нужно в MetadataLoadContext).", source: "ms-reflection" },
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: 'API читает эту таблицу метаданных из <span class="hl">загруженной сборки</span> — отсюда и «loaded assemblies».', nodes: [{ id: "api", kind: "chip", at: { zone: "api", row: 0 }, value: "System.Reflection" }, { id: "type", kind: "chip", at: { zone: "api", row: 1 }, value: "System.Type" }, { id: "md", kind: "obj", at: { zone: "meta", row: 0 }, typeTag: "Metadata", value: "типы, члены" }, { id: "asm", kind: "obj", at: { zone: "meta", row: 1 }, typeTag: "Assembly", value: "loaded", accent: true }], edges: [{ id: "e1", from: "type", to: "md", accent: true }] },
      ],
      explain: 'Reflection — это API самоописания рантайма: «The classes in the <b>System.Reflection</b> namespace, together with <b>System.Type</b>, enable you to obtain information about <span class="hl">loaded assemblies</span> and the types defined within them, such as classes, interfaces, and value types». Ключевое слово — <b>loaded</b>: ты читаешь метаданные уже загруженной в процесс сборки, которые компилятор записал при билде. И не только читаешь: «You can also use reflection to <b>create type instances at runtime</b>, and to invoke and access them». То есть reflection — не «медленная магия», а типизированный доступ к таблице метаданных.',
  ...
  cards: [
    {
      ...
      okText: 'Ты объявил <code>Type</code>, но <code>Type</code> <b>абстрактный</b> — в куче лежит его CLR-подкласс <span class="hl">RuntimeType</span>. «the object is really a RuntimeType. RuntimeType derives from Type».',
    ...
    },
    {
      ...
      okText: 'Граф <b>assembly → module</b>: <code>Uri</code> живёт в сборке <span class="hl">System.Private.Uri</span>, её модуль — одноимённый .dll. «Assemblies contain modules, modules contain types».',
    ...
    },
    {
      ...
      noText: '<code>Activator.CreateInstance</code> строит настоящий объект, <code>MethodInfo.Invoke</code> реально его меняет: <code>sb</code> печатает <b>hi</b>, а <code>GetType().Name</code> — <b>StringBuilder</b>. «create an instance of a type… then invoke the type\'s methods».',
