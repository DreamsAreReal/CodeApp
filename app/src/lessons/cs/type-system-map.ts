...
export const typeSystemMap: LessonData = {
  ...
  spec: [
    { text: "«The compile-time type controls overload resolution and available conversions. The run-time type controls virtual method dispatch, is expressions, and switch expressions.» <span class=\"ru-tr\">«Compile-time тип управляет разрешением перегрузки и доступными преобразованиями. Run-time тип управляет виртуальной диспетчеризацией методов, выражениями is и выражениями switch.»</span>", source: "ms-types" },
  ],
  edgeCases: [
  ...
  misconceptions: [
    {
      ...
      hook: 'У переменной <span class="wrong">не один тип, а два</span>: <b>compile-time</b> (что написано в коде) и <b>run-time</b> (чем объект оказался в куче). <code>object o = "hi"</code> — статически это <code>object</code>, а <code>o.GetType()</code> вернёт <code>String</code>. И это не педантизм: «The <b>compile-time type</b> controls overload resolution… The <b>run-time type</b> controls virtual method dispatch, <code>is</code> expressions, and <code>switch</code> expressions». <span class="ru-tr">«<b>Compile-time тип</b> управляет разрешением перегрузки… <b>Run-time тип</b> управляет виртуальной диспетчеризацией методов, выражениями <code>is</code> и выражениями <code>switch</code>».</span> Дальше <b>пять разборов</b>: иерархия CTS, два типа одной переменной, и <b>машинная панель</b> — как один и тот же объект уходит в <span class="hl">разные перегрузки</span> по compile-time типу (реальный прогон).',
      source: "ms-types",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'CTS — единая иерархия: «All types ultimately derive from <code>System.Object</code>. Value types derive from <code>System.ValueType</code>, which derives from <code>object</code>. This unified hierarchy is called the <b>Common Type System</b> (CTS)». <span class="ru-tr">«Все типы в конечном счёте наследуют <code>System.Object</code>. Value-типы наследуют <code>System.ValueType</code>, который наследует <code>object</code>. Эта единая иерархия называется <b>общей системой типов</b> (CTS)».</span> Практический смысл: любое значение можно трактовать как <code>object</code> (отсюда boxing value-типов), а компилятор строго типизирован — «C# is a strongly typed language. Every variable, constant, and expression has a type… The compiler enforces <span class="hl">type safety</span>». <span class="ru-tr">«C# — строго типизированный язык. Каждая переменная, константа и выражение имеет тип… Компилятор обеспечивает безопасность типов».</span> Тип — не подсказка, а контракт, встроенный в метаданные сборки.',
    ...
    },
    {
      ...
      explain: 'У переменной два типа: «The <b>compile-time type</b> is the declared or inferred type in source code. The <b>run-time type</b> is the actual type of the instance the variable refers to». <span class="ru-tr">«<b>Compile-time тип</b> — это объявленный или выведенный тип в исходном коде. <b>Run-time тип</b> — это фактический тип экземпляра, на который ссылается переменная».</span> Здесь compile-time — <code>object</code>, run-time — <code>String</code>: «boxed has a compile-time type of <code>object</code> but a run-time type of <code>string</code>. The assignment works because <code>string</code> derives from <code>object</code>». <span class="ru-tr">«boxed имеет compile-time тип <code>object</code>, но run-time тип <code>string</code>. Присваивание работает, потому что <code>string</code> наследует <code>object</code>».</span> Легально любое присваивание, где есть implicit conversion от run-time к compile-time типу (identity, reference, boxing). Реальный вывод <code>GetType().FullName</code> — <code>System.String</code>.',
      sources: ["ms-types"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Зеркальный случай: тот же дуализм, но другой механизм. Виртуальный вызов идёт по <b>run-time</b> типу: «The <b>run-time type</b> controls <span class="hl">virtual method dispatch</span>, <code>is</code> expressions, and <code>switch</code> expressions». <span class="ru-tr">«<b>Run-time тип</b> управляет виртуальной диспетчеризацией методов, выражениями <code>is</code> и выражениями <code>switch</code>».</span> Поэтому <code>a.ToString()</code> при <code>Animal a = new Dog()</code> печатает <code>Dog</code> — рантайм смотрит в <b>метод-таблицу (vtable)</b> объекта, а не на объявленный тип. Механику vtable Learn не называет по имени; она задокументирована в рантайме дословно: «The slot is stored in <b>MethodTable</b> for methods that require efficient lookup via slot index, e.g. <span class="hl">virtual methods</span>» <span class="ru-tr">«Слот хранится в <b>MethodTable</b> для методов, которым нужен эффективный поиск по индексу слота, например виртуальных методов».</span> (полный разбор — урок S1.3). Собери оба факта: <b>compile-time</b> тип решает перегрузку и доступные конверсии (s3), <b>run-time</b> тип — виртуальную диспетчеризацию, <code>is</code>, <code>switch</code>. Это и есть карта: какой тип что контролирует.',
    ...
    },
    {
      ...
      explain: 'Категория типа (value/reference) — это про то, как переменная <b>хранит данные</b>: «Value types hold their data directly. When you assign a value type to a new variable, the runtime <span class="hl">copies the data</span>… Reference types hold a reference to an object on the managed heap. When you assign a reference type… both variables point to the <b>same object</b>». <span class="ru-tr">«Value-типы хранят свои данные напрямую. Когда вы присваиваете value-тип новой переменной, среда выполнения копирует данные… Reference-типы хранят ссылку на объект в управляемой куче. Когда вы присваиваете reference-тип… обе переменные указывают на <b>один и тот же объект</b>».</span> Поэтому struct-копия независима, а <code>list2.Add(4)</code> виден через <code>list1</code> (<code>Count == 4</code>). Эта развилка — фундамент раздела: следующий урок (S1.2) разбирает копирование value-типов на машинном уровне.',
      sources: ["ms-types"],
    },
  ],
  ...
  cards: [
    {
      ...
      noText: '«The compile-time type controls overload resolution». <span class="ru-tr">«Compile-time тип управляет разрешением перегрузки».</span> <code>o</code> статически <code>object</code>, поэтому <code>Describe(o)</code> → <b>object overload</b>, а <code>Describe("hi")</code> → <b>string overload</b>. Run-time тип (String) тут ни при чём.',
    ...
    },
    {
      ...
      noText: '«The run-time type controls virtual method dispatch». <span class="ru-tr">«Run-time тип управляет виртуальной диспетчеризацией методов».</span> Объект — <code>Dog</code>, поэтому <code>a.ToString()</code> = <b>Dog</b> и <code>a.GetType().Name</code> = <b>Dog</b>, хоть переменная объявлена <code>Animal</code>.',
