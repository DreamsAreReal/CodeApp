...
export const stringImmutability: LessonData = {
  ...
  spec: [
    { text: "«A String object is called immutable (read-only), because its value cannot be modified after it has been created. Methods that appear to modify a String object actually return a new String object that contains the modification.» <span class=\"ru-tr\">«Объект String называют иммутабельным (только для чтения), потому что его значение нельзя изменить после создания. Методы, которые как будто изменяют объект String, на самом деле возвращают новый объект String, содержащий модификацию.»</span>", source: "ms-string" },
  ],
  edgeCases: [
    { text: "«Изменение» — это <b>новый объект</b>: «Methods that appear to modify a <span class=\"hl\">String</span> object actually return a new <span class=\"hl\">String</span> object that contains the modification» <span class=\"ru-tr\">«Методы, которые как будто изменяют объект String, на самом деле возвращают новый объект String, содержащий модификацию»</span>. Собственный прогон: <code>s.ToUpper()</code> ≠ <code>s</code> по ссылке (<code>ReferenceEquals</code> = <code>False</code>).", source: "ms-string" },
    { text: "String — <b>ссылочный</b> тип, но значение read-only: «its value cannot be modified after it has been created» <span class=\"ru-tr\">«его значение нельзя изменить после создания»</span>. Присваивание <code>s += \"c\"</code> не мутирует объект, а <b>переставляет ссылку</b> <code>s</code> на новый; алиас <code>t</code> держит старый.", source: "ms-string" },
    { text: "Конкатенация в цикле бьёт по производительности: «string manipulation routines that perform repeated additions or deletions to what appears to be a single string can exact a <span class=\"hl\">significant performance penalty</span>» <span class=\"ru-tr\">«процедуры обработки строк, выполняющие повторяющиеся добавления или удаления к тому, что выглядит как одна строка, могут повлечь значимый штраф по производительности»</span> — каждый шаг создаёт новый <code>String</code>.", source: "ms-string" },
  ],

  misconceptions: [
    {
      wrong: "s += \"c\" дописывает символ В ТОТ ЖЕ строковый объект — string мутабельна, просто это медленно",
      hook: 'Нет — <code>String</code> <b>иммутабельна</b>, и «дописать» в неё нельзя в принципе: «A <b>String</b> object is called <span class="hl">immutable (read-only)</span>, because its value cannot be modified after it has been created» <span class="ru-tr">«Объект <b>String</b> называют иммутабельным (только для чтения), потому что его значение нельзя изменить после создания»</span>. Любой метод, который «меняет» строку, на деле <b>возвращает новый объект</b>: «Methods that appear to modify a <b>String</b> object actually return a <span class="hl">new String object</span> that contains the modification» <span class="ru-tr">«Методы, которые как будто изменяют объект <b>String</b>, на самом деле возвращают новый объект String, содержащий модификацию»</span>. То есть <code>s += "c"</code> не трогает старый объект, а создаёт новый и <b>переставляет ссылку</b> <code>s</code> — а алиас, указывавший на старый, продолжает видеть исходное значение. Отсюда и цена: «repeated additions or deletions… can exact a <b>significant performance penalty</b>» <span class="ru-tr">(значимый штраф по производительности)</span>. Это <b>строковый</b> угол на аллокацию из секции памяти (S7): не «GC вообще», а конкретно — сколько мусорных <code>String</code> рождает наивный <code>+=</code>. Дальше <b>пять разборов</b>: что такое String, метод возвращает новый объект, алиасинг и переустановка ссылки, цена += в цикле, и <b>машинная панель</b> — <code>ReferenceEquals</code> до/после <code>+=</code> (реальный прогон: <code>False</code>, старое значение цело).',
      source: "ms-string",
    },
  ],
  ...
  cards: [
    {
      ...
      noText: 'String иммутабельна: «actually return a new String object» <span class="ru-tr">«на самом деле возвращают новый объект String»</span>. <code>s</code> ушёл на новый объект, <code>t</code> остался на <code>"ab"</code>. Реальный вывод: <b>False</b>, затем <b>ab</b>.',
    ...
    },
    {
      ...
      okText: '<code>Replace</code> «appear to modify… actually return a new String object» <span class="ru-tr">«как будто изменяют… на самом деле возвращают новый объект String»</span>. <code>a</code> цел (<b>hello</b>), <code>b</code> — новый (<b>heLLo</b>), это <span class="hl">разные</span> объекты (<b>False</b>).',
  ...
  takeaways: [
    ...
    { icon: "cost", k: "Метод → новый объект", v: '«Methods that appear to modify a String object actually return a <b>new String object</b>» <span class="ru-tr">«Методы, которые как будто изменяют объект String, на самом деле возвращают <b>новый объект String</b>»</span>. <code>Replace</code>/<code>ToUpper</code>/<code>Trim</code> не мутируют получателя (замер: <code>ReferenceEquals</code> = False). Не присвоил результат — изменение потеряно.' },
