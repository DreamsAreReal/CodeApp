...
export const threadAsyncLocal: LessonData = {
  ...
  spec: [
    { text: "ThreadLocal<T> — per-thread хранилище: «Provides thread-local storage of data». Значение <code>Value</code> «is specific for the thread on which the property is accessed».", source: "ms-threadlocal" },
    { text: "AsyncLocal<T> — амбиентное по async-потоку: «Represents ambient data that is local to a given asynchronous control flow, such as an asynchronous method».", source: "ms-asynclocal" },
  ],
  edgeCases: [
    { text: "Ленивая инициализация фабрикой: конструктор «Initializes the ThreadLocal<T> instance with the specified valueFactory function»; свойство IsValueCreated «Gets whether Value is initialized on the current thread».", source: "ms-threadlocal" },
    { text: "AsyncLocal тянется через потоки: «Because the task-based asynchronous programming model tends to abstract the use of threads, AsyncLocal<T> instances can be used to persist data across threads».", source: "ms-asynclocal" },
    { text: "AsyncLocal умеет уведомлять об изменении: «provides optional notifications when the value associated with the current thread changes… or implicitly changed when the thread encountered an await or other context transition».", source: "ms-asynclocal" },
  ],

  misconceptions: [
    {
      wrong: "ThreadLocal и AsyncLocal — одно и то же «локальное хранилище», просто разные имена",
      hook: 'Это <b>разные оси</b> локальности. <code>ThreadLocal&lt;T&gt;</code> — <span class="hl">per-thread</span>: «Provides thread-local storage of data» — у каждого потока своя копия, и <code>Value</code> «is <span class="hl">specific for the thread</span> on which the property is accessed». <code>AsyncLocal&lt;T&gt;</code> — <span class="hl">per-async-flow</span>: «Represents <span class="hl">ambient data that is local to a given asynchronous control flow</span>, such as an asynchronous method» — оно <b>переживает</b> <code>await</code> и смену потока: «Because the task-based asynchronous programming model tends to abstract the use of threads, <code>AsyncLocal&lt;T&gt;</code> instances can be used to <span class="hl">persist data across threads</span>». В async-коде поток под тобой меняется — <code>ThreadLocal</code> сбросится, а <code>AsyncLocal</code> дотечёт. Дальше <b>пять разборов</b>: ThreadLocal-изоляция, фабрика + IsValueCreated, AsyncLocal-поток управления, почему в async нужен AsyncLocal, и <b>машинная панель</b> — AsyncLocal переживает await реальным прогоном (before=ctx after=ctx).',
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>ThreadLocal&lt;T&gt;</code> — «<span class="hl">Provides thread-local storage of data</span>»: у каждого потока отдельная ячейка.', nodes: [{ id: "a", kind: "obj", at: { zone: "t1", row: 0 }, typeTag: "Thread A", value: "tl = 100", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Поток B читает <code>tl.Value</code> — получает <b>свою</b> копию (100 из фабрики), не A. «specific for the thread on which the property is accessed».', nodes: [{ id: "a", kind: "obj", at: { zone: "t1", row: 0 }, typeTag: "Thread A", value: "tl = 100" }, { id: "b", kind: "obj", at: { zone: "t2", row: 0 }, typeTag: "Thread B", value: "tl = 100", accent: true }], edges: [] },
      ...
      explain: '<code>ThreadLocal&lt;T&gt;</code> даёт каждому потоку <b>изолированную</b> ячейку: «<span class="hl">Provides thread-local storage of data</span>». Из раздела Thread Safety: «The value returned for the <code>Value</code> and <code>IsValueCreated</code> properties <span class="hl">is specific for the thread on which the property is accessed</span>». То есть <code>tl.Value</code> — не общее поле, а «моё значение на этом потоке»: запись из одного потока не видна другому. Это удобно для не-thread-safe ресурсов (например, свой <code>Random</code> на поток) без блокировок. Реальный прогон (машинная база урока): main видит 100, worker видит 100 (своя копия из фабрики), и после записи worker\'ом 999 у main всё ещё 100 (main=100 worker=100 mainAfter=100).',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'Конструктор с <b>valueFactory</b>: «Initializes the <code>ThreadLocal&lt;T&gt;</code> instance with the specified <code>valueFactory</code> function».', nodes: [{ id: "f", kind: "gate", at: { zone: "factory", row: 0 }, state: "ok", label: "valueFactory: () => 42", detail: "ещё не вызвана", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'До доступа — <code>IsValueCreated == False</code>. Свойство «<span class="hl">Gets whether Value is initialized on the current thread</span>».', nodes: [{ id: "f", kind: "gate", at: { zone: "factory", row: 0 }, state: "ok", label: "valueFactory", detail: "не вызвана" }, { id: "b", kind: "gate", at: { zone: "factory", row: 1 }, state: "fail", label: "IsValueCreated", detail: "False", accent: true }], edges: [] },
      ...
      explain: 'Инициализация ленива и — что важно — <b>по потоку</b>. Конструктор с фабрикой «<span class="hl">Initializes the ThreadLocal&lt;T&gt; instance with the specified valueFactory function</span>». Свойство контроля <code>IsValueCreated</code> «<span class="hl">Gets whether Value is initialized on the current thread</span>». На каждом новом потоке значение создаётся при <b>первом</b> обращении к <code>Value</code> — фабрика вызывается заново для этого потока. Реальный прогон: <code>IsValueCreated</code> до доступа — <b>False</b>, первое <code>Value</code> — <b>42</b> (фабрика), после — <b>True</b> (печать <b>before=False value=42 after=True</b>). Есть ещё <code>Values</code> — «<span class="hl">Gets a list containing the values stored by all threads</span> that have accessed this instance». И <code>ThreadLocal</code> реализует <code>IDisposable</code>: освобождать через <code>Dispose()</code>.',
      sources: ["ms-threadlocal"],
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: '<code>AsyncLocal&lt;T&gt;</code> — «<span class="hl">Represents ambient data that is local to a given asynchronous control flow</span>». Ставим значение в текущем потоке управления.', nodes: [{ id: "a", kind: "obj", at: { zone: "flow", row: 0 }, typeTag: "AsyncLocal", value: "= ctx", accent: true }], edges: [] },
        ...
        { codeLine: 3, out: "", caption: 'Но <code>AsyncLocal</code> <span class="hl">течёт по async-потоку управления</span>: «can be used to persist data across threads». Значение сохраняется.', nodes: [{ id: "aw", kind: "gate", at: { zone: "flow", row: 0 }, state: "ok", label: "после await", detail: "другой поток" }, { id: "ok", kind: "gate", at: { zone: "flow", row: 1 }, state: "ok", label: "al.Value", detail: "всё ещё ctx", accent: true }], edges: [] },
      ],
      explain: '<code>AsyncLocal&lt;T&gt;</code> привязано не к потоку, а к <b>логическому потоку управления</b>: «<span class="hl">Represents ambient data that is local to a given asynchronous control flow, such as an asynchronous method</span>». Ключевое свойство — оно течёт вместе с async-контекстом: «<span class="hl">Because the task-based asynchronous programming model tends to abstract the use of threads, AsyncLocal&lt;T&gt; instances can be used to persist data across threads</span>». То есть значение, установленное до <code>await</code>, видно и после — даже если продолжение подхватил другой поток пула. Это делает <code>AsyncLocal</code> правильным носителем «амбиентного контекста» запроса (correlation id, культура, пользователь) в async-коде. Плюс есть уведомления: «provides optional notifications when the value… changes… or implicitly changed when the thread encountered an <code>await</code> or other context transition».',
  ...
  cards: [
    {
      ...
      noText: '<code>ThreadLocal</code> «is specific for the thread on which the property is accessed» — worker\'ова запись не видна main. Реальный вывод: <b>main=100 worker=100 mainAfter=100</b>.',
    ...
    },
    {
      ...
      okText: 'Значение <span class="hl">лениво</span>: до первого <code>Value</code> — <code>IsValueCreated == False</code>; первый доступ зовёт фабрику → 42; после — True. «Gets whether Value is initialized on the current thread».',
    ...
    },
    {
      ...
      okText: '<code>AsyncLocal</code> — «ambient data that is local to a given asynchronous control flow»: значение <span class="hl">переживает await</span> и смену потока. И до, и после — <b>ctx</b>.',
      noText: '«<code>AsyncLocal&lt;T&gt;</code> instances can be used to persist data across threads» — значение дотекает через await. Реальный вывод: <b>before=ctx after=ctx</b>.',
      verify: { kind: "exec", run: "dotnet run", expect: "before=ctx after=ctx" }, sourceRefs: ["ms-asynclocal"],
    },
  ],

  takeaways: [
    { icon: "why", k: "ThreadLocal", v: '«<span class="hl">Provides thread-local storage of data</span>» — у каждого потока своя копия (<code>Value</code> «is specific for the thread»). Лениво через <code>valueFactory</code>; <code>IsValueCreated</code> = «whether Value is initialized on the current thread» (main=100 worker=100 mainAfter=100).' },
    { icon: "cost", k: "AsyncLocal", v: '«<span class="hl">Represents ambient data that is local to a given asynchronous control flow</span>» — переживает <code>await</code>/смену потока: «persist data across threads» (before=ctx after=ctx). Правильный носитель амбиентного контекста запроса.' },
