...
export const exceptionsOverview: LessonData = {
  ...
  spec: [
    { text: "«In .NET, an exception is an object that inherits from the System.Exception class. An exception is thrown from an area of code where a problem has occurred. The exception is passed up the stack until the application handles it or the program terminates.» <span class=\"ru-tr\">«В .NET исключение — это объект, унаследованный от класса System.Exception. Исключение бросается из участка кода, где возникла проблема. Исключение передаётся вверх по стеку, пока приложение не обработает его или программа не завершится.»</span>", source: "ms-stdexc" },
    { text: "«When an exception is thrown, the CLR will unwind the stack, looking for a method with a catch block for the specific exception type, and it will execute the first such catch block that it finds.» <span class=\"ru-tr\">«Когда бросается исключение, CLR раскручивает стек в поисках метода с блоком catch для конкретного типа исключения и выполняет первый же найденный такой блок catch.»</span>", source: "ms-csexc" },
  ],
  edgeCases: [
    { text: "Исключение может бросить не тот метод, что ты вызвал напрямую: «an exception may be thrown not by a method that your code has called directly, but by another method further down in the call stack». <span class=\"ru-tr\">«исключение может быть брошено не тем методом, который твой код вызвал напрямую, а другим методом глубже вниз по стеку вызовов».</span>", source: "ms-csexc" },
    { text: "Все исключения наследуют один корень: «Exceptions are types that all ultimately derive from <code>System.Exception</code>». <span class=\"ru-tr\">«Исключения — это типы, которые в конечном счёте все наследуют от <code>System.Exception</code>».</span> Поэтому <code>catch (Exception)</code> ловит любое.", source: "ms-csexc" },
    { text: "Объект несёт диагностику: «Exception objects contain detailed information about the error, such as the <b>state of the call stack</b> and a text description of the error». <span class=\"ru-tr\">«Объекты-исключения содержат подробную информацию об ошибке, такую как <b>состояние стека вызовов</b> и текстовое описание ошибки».</span> Не находится ни один обработчик — «it will terminate the process». <span class=\"ru-tr\">«оно завершит процесс».</span>", source: "ms-csexc" },
  ],

  misconceptions: [
    {
      wrong: "исключение — это просто «код ошибки» / goto, который прыгает в ближайший catch того же метода",
      hook: 'Исключение — не флаг возврата, а <b>объект</b>: «<span class="hl">In .NET, an exception is an object that inherits from the System.Exception class</span>. An exception is thrown from an area of code where a problem has occurred. <span class="hl">The exception is passed up the stack until the application handles it or the program terminates</span>». <span class="ru-tr">«В .NET исключение — это объект, унаследованный от класса System.Exception. Исключение бросается из участка кода, где возникла проблема. Исключение передаётся вверх по стеку, пока приложение не обработает его или программа не завершится».</span> И ловит его не обязательно текущий метод: «an exception may be thrown not by a method that your code has called directly, but by <span class="hl">another method further down in the call stack</span>». <span class="ru-tr">«исключение может быть брошено не тем методом, который твой код вызвал напрямую, а другим методом глубже вниз по стеку вызовов».</span> Механика — <b>раскрутка стека</b>: «When an exception is thrown, the CLR will <span class="hl">unwind the stack, looking for a method with a catch block for the specific exception type</span>, and it will execute the <span class="hl">first such catch block that it finds</span>». <span class="ru-tr">«Когда бросается исключение, CLR раскручивает стек в поисках метода с блоком catch для конкретного типа исключения и выполняет первый же найденный такой блок catch».</span> Дальше <b>пять разборов</b>: объект <code>: System.Exception</code>, раскрутка снизу вверх, ловля по типу (база ловит наследника), состояние объекта (Message/стек/Inner), и <b>машинная панель</b> — first-chance срабатывает <b>в момент throw</b>, до поиска обработчика (реальный прогон).',
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'В точке проблемы код делает <code>throw new …Exception(…)</code> — <span class="hl">создаёт объект</span>. «Exceptions are created by using the <code>throw</code> keyword». <span class="ru-tr">«Исключения создаются с помощью ключевого слова <code>throw</code>».</span>', nodes: [{ id: "t", kind: "gate", at: { zone: "err", row: 0 }, state: "fail", label: "throw new", detail: "проблема тут", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Это <b>объект</b>, а не флаг: «<span class="hl">an exception is an object that inherits from the System.Exception class</span>». <span class="ru-tr">«исключение — это объект, унаследованный от класса System.Exception».</span> У него есть тип, <code>Message</code>, стек.', nodes: [{ id: "t", kind: "gate", at: { zone: "err", row: 0 }, state: "fail", label: "throw new", detail: "проблема тут" }, { id: "o", kind: "obj", at: { zone: "obj", row: 0 }, typeTag: "InvalidOperationException", value: ": System.Exception", accent: true }], edges: [{ id: "e1", from: "t", to: "o", accent: true }] },
        { codeLine: 2, out: "", caption: 'Кто может бросить: «Exceptions can be generated by the <span class="hl">common language runtime (CLR)</span>, by .NET or third-party libraries, or by application code» <span class="ru-tr">«Исключения могут порождаться общеязыковой средой выполнения (CLR), самим .NET или сторонними библиотеками, либо кодом приложения»</span> — единая модель для всех источников.', nodes: [{ id: "t", kind: "gate", at: { zone: "err", row: 0 }, state: "fail", label: "throw", detail: "проблема" }, { id: "o", kind: "obj", at: { zone: "obj", row: 0 }, typeTag: "…Exception", value: ": System.Exception" }, { id: "src", kind: "chip", at: { zone: "err", row: 1 }, value: "CLR · библиотека · код", accent: true }], edges: [] },
      ],
      explain: 'Модель .NET: ошибка сигнализируется не возвратом кода, а <b>исключением-объектом</b>. «.NET provides a model for notifying applications of errors in a uniform way: <span class="hl">.NET operations indicate failure by throwing exceptions</span>». <span class="ru-tr">«.NET предоставляет модель для единообразного уведомления приложений об ошибках: операции .NET сигнализируют о сбое, бросая исключения».</span> Конкретно: «<span class="hl">In .NET, an exception is an object that inherits from the System.Exception class</span>. An exception is thrown from an area of code where a problem has occurred». <span class="ru-tr">«В .NET исключение — это объект, унаследованный от класса System.Exception. Исключение бросается из участка кода, где возникла проблема».</span> Преимущество перед кодами возврата: «Failures don\'t go unnoticed because if an exception is thrown and you don\'t handle it, the runtime <span class="hl">terminates your application</span>» <span class="ru-tr">«Сбои не остаются незамеченными, потому что если исключение брошено и ты его не обрабатываешь, среда выполнения завершает твоё приложение»</span> — забыть проверить нельзя, в отличие от игнорируемого return-кода. Источник может быть любой: «Exceptions can be generated by the common language runtime (CLR), by .NET or third-party libraries, or by application code». <span class="ru-tr">«Исключения могут порождаться общеязыковой средой выполнения (CLR), самим .NET или сторонними библиотеками, либо кодом приложения».</span>',
    ...
    },
    {
      ...
      explain: 'Ключевая механика: исключение не обязано ловиться там, где брошено. «In many cases, an exception may be thrown <span class="hl">not by a method that your code has called directly, but by another method further down in the call stack</span>». <span class="ru-tr">«Во многих случаях исключение может быть брошено не тем методом, который твой код вызвал напрямую, а другим методом глубже вниз по стеку вызовов».</span> Что делает рантайм: «When an exception is thrown, the CLR will <span class="hl">unwind the stack, looking for a method with a catch block for the specific exception type</span>, and it will execute the <span class="hl">first such catch block that it finds</span>». <span class="ru-tr">«Когда бросается исключение, CLR раскручивает стек в поисках метода с блоком catch для конкретного типа исключения и выполняет первый же найденный такой блок catch».</span> Пошагово из reference: «the CLR looks at the method that called the current method, and so on up the call stack. If there\'s no compatible <code>catch</code> block, the CLR terminates the executing thread». <span class="ru-tr">«CLR смотрит на метод, вызвавший текущий метод, и так далее вверх по стеку вызовов. Если совместимого блока <code>catch</code> нет, CLR завершает выполняемый поток».</span> Отсюда: бросок из <code>C</code> ловится в <code>A</code> через промежуточный <code>B</code> без единого <code>catch</code> в <code>B</code>. «If it finds no appropriate catch block anywhere in the call stack, it will <span class="hl">terminate the process</span>». <span class="ru-tr">«Если он не находит подходящего блока catch нигде в стеке вызовов, он завершит процесс».</span>',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: 'Индексация <code>a[5]</code> вне диапазона — рантайм бросает конкретный <code>IndexOutOfRangeException</code>. «Thrown by the runtime only when an array is indexed improperly». <span class="ru-tr">«Бросается средой выполнения только тогда, когда массив индексируется неправильно».</span>', nodes: [{ id: "t", kind: "obj", at: { zone: "thrown", row: 0 }, typeTag: "brошено", value: "IndexOutOfRangeException", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: '<code>catch (SystemException)</code> — база в иерархии. Все исключения «<span class="hl">ultimately derive from System.Exception</span>» <span class="ru-tr">«в конечном счёте наследуют от System.Exception»</span>, значит наследник <b>подходит</b> под базовый catch.', nodes: [{ id: "t", kind: "obj", at: { zone: "thrown", row: 0 }, typeTag: "brошено", value: "IndexOutOfRange" }, { id: "h", kind: "gate", at: { zone: "hand", row: 0 }, state: "ok", label: "catch (SystemException)", detail: "матч по базе", accent: true }], edges: [{ id: "e1", from: "t", to: "h", accent: true }] },
      ...
      explain: 'Матч <code>catch</code> — по <b>совместимости типа</b>, а не по точному равенству. «Use a <i>catch clause</i> to specify the <span class="hl">base type of exceptions you want to handle</span> in the corresponding <code>catch</code> block». <span class="ru-tr">«Используй <i>предложение catch</i>, чтобы указать базовый тип исключений, которые ты хочешь обрабатывать, в соответствующем блоке <code>catch</code>».</span> Так как «Exceptions are types that all ultimately <span class="hl">derive from System.Exception</span>», <span class="ru-tr">«Исключения — это типы, которые в конечном счёте все наследуют от System.Exception»,</span> <code>catch (SystemException)</code> — и тем более <code>catch (Exception)</code> — ловит любой производный тип. Но пойманный объект остаётся собой: его <code>GetType()</code> возвращает <b>реальный</b> брошенный тип (<code>IndexOutOfRangeException</code>), а не тип из <code>catch</code>. Практика: лови <b>конкретный</b> тип, который умеешь обработать; широкий <code>catch (Exception)</code> — только если реально можешь оставить приложение в известном состоянии («Don\'t catch an exception unless you can handle it and leave the application in a known state» <span class="ru-tr">«Не лови исключение, если не можешь его обработать и оставить приложение в известном состоянии»</span>).',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'Брошено <code>InvalidOperationException("inner")</code>. Объект несёт <b>диагностику</b>: «Exception objects contain <span class="hl">detailed information about the error</span>». <span class="ru-tr">«Объекты-исключения содержат подробную информацию об ошибке».</span>', nodes: [{ id: "i", kind: "obj", at: { zone: "state", row: 0 }, typeTag: "inner: InvalidOperationException", value: "Message=\"inner\"", accent: true }], edges: [] },
        ...
        { codeLine: 2, out: "outer|inner", caption: 'Печать <code>Message|InnerException.Message</code> = <span class="hl">outer|inner</span> (реальный прогон). Состояние ошибки — «the <b>state of the call stack</b> and a text description of the error» <span class="ru-tr">«<b>состояние стека вызовов</b> и текстовое описание ошибки»</span> — живёт в объекте.', nodes: [{ id: "o", kind: "obj", at: { zone: "state", row: 0 }, typeTag: "outer", value: "Message=\"outer\"" }, { id: "i", kind: "obj", at: { zone: "state", row: 1 }, typeTag: "inner (Inner)", value: "Message=\"inner\"", accent: true }], edges: [{ id: "e1", from: "o", to: "i" }] },
      ],
      explain: 'Исключение — носитель <b>состояния об ошибке</b>, а не просто сигнал. «Exception objects contain <span class="hl">detailed information about the error, such as the state of the call stack and a text description of the error</span>». <span class="ru-tr">«Объекты-исключения содержат подробную информацию об ошибке, такую как состояние стека вызовов и текстовое описание ошибки».</span> Текстовое описание — свойство <code>Message</code>; снимок стека на момент броска — <code>StackTrace</code>; а <code>InnerException</code> хранит <b>первопричину</b>, если ты обернул низкоуровневую ошибку в доменную (передав её вторым аргументом конструктора). Реальный прогон печатает <code>outer|inner</code>: внешнее исключение помнит своё сообщение и держит ссылку на внутреннее. Это позволяет ловить, добавлять контекст и перебрасывать, не теряя исходную причину — «If a <code>catch</code> block defines an exception variable, you can use it to <span class="hl">obtain more information about the type of exception</span> that occurred». <span class="ru-tr">«Если блок <code>catch</code> определяет переменную исключения, ты можешь использовать её, чтобы получить больше информации о типе возникшего исключения».</span>',
      sources: ["ms-csexc"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: 'Только <b>после</b> уведомления CLR начинает раскрутку и ищет <code>catch</code> — «unwind the stack, looking for a method with a <code>catch</code> block». <span class="ru-tr">«раскручивает стек в поисках метода с блоком <code>catch</code>».</span> Обработка идёт <span class="hl">вторым</span> шагом.', nodes: [{ id: "th", kind: "gate", at: { zone: "throw", row: 0 }, state: "fail", label: "throw", detail: "1 · firstchance" }, { id: "s", kind: "gate", at: { zone: "search", row: 0 }, state: "ok", label: "поиск catch", detail: "2 · раскрутка", accent: true }], edges: [{ id: "e1", from: "th", to: "s", accent: true }] },
      ...
      explain: 'Машинная панель — временной порядок событий исключения, снятый событием <code>AppDomain.FirstChanceException</code>. Оно поднимается <b>в момент броска</b>, ещё до того как CLR начнёт искать обработчик: сначала «первый шанс» (диагностика/логи), только потом «When an exception is thrown, the CLR will <span class="hl">unwind the stack, looking for a method with a catch block for the specific exception type</span>». <span class="ru-tr">«Когда бросается исключение, CLR раскручивает стек в поисках метода с блоком catch для конкретного типа исключения».</span> Реальный прогон печатает <code>firstchance:InvalidOperationException</code> раньше, чем <code>caught:InvalidOperationException</code> — детерминированный порядок: уведомление о броске → раскрутка/поиск → тело <code>catch</code>. Практический смысл: first-chance перехватывает <b>все</b> исключения (даже те, что будут обработаны) в точке возникновения, где стек ещё цел, — база для APM/трейсинга. Не находится ни один <code>catch</code> → «it will terminate the process». <span class="ru-tr">«оно завершит процесс».</span>',
      sources: ["ms-csexc"],
    },
  ],
  ...
  cards: [
    {
      ...
      okText: 'Бросок в <code>C</code> (3 кадра вниз) раскручивает стек через <code>B</code> без <code>catch</code> и ловится в <code>A</code>: «unwind the stack… execute the <span class="hl">first such catch block that it finds</span>». <span class="ru-tr">«раскручивает стек… выполняет первый же найденный такой блок catch».</span> <code>e.Message</code> = <b>boom</b>.',
      noText: 'Исключение «passed up the stack until the application handles it»: <span class="ru-tr">«передаётся вверх по стеку, пока приложение не обработает его»:</span> брошено в <code>C</code>, поймано в <code>A</code>. <code>Message</code> сохраняется. Реальный вывод: <b>boom</b>.',
    ...
    },
    {
      ...
      okText: 'Внутреннее (<code>"inner"</code>) обёрнуто во внешнее (<code>"outer"</code>) через второй аргумент — он кладёт причину в <code>InnerException</code>. Объект несёт «detailed information about the error». <span class="ru-tr">«подробную информацию об ошибке».</span> Печать: <b>outer|inner</b>.',
    ...
    },
    {
      ...
      okText: 'Рантайм бросает конкретный <code>IndexOutOfRangeException</code>; базовый <code>catch (SystemException)</code> его ловит (все «<span class="hl">ultimately derive from System.Exception</span>» <span class="ru-tr">«в конечном счёте наследуют от System.Exception»</span>), но <code>GetType()</code> отдаёт <b>реальный</b> тип. Печать: <b>IndexOutOfRangeException</b>.',
  ...
  takeaways: [
    { icon: "why", k: "Объект, не флаг", v: '«<span class="hl">an exception is an object that inherits from the System.Exception class</span>… passed up the stack until the application handles it or the program terminates». <span class="ru-tr">«исключение — это объект, унаследованный от класса System.Exception… передаётся вверх по стеку, пока приложение не обработает его или программа не завершится».</span> Несёт <code>Message</code>, снимок стека, <code>InnerException</code> (замер: outer|inner).' },
    { icon: "cost", k: "Раскрутка", v: 'Брошено глубоко — поймано выше: CLR «<span class="hl">unwind the stack, looking for a method with a catch block</span>… the first such catch block that it finds». <span class="ru-tr">«раскручивает стек в поисках метода с блоком catch… первый же найденный такой блок catch».</span> Нет обработчика нигде → «terminate the process» <span class="ru-tr">«завершить процесс»</span> (замер: boom из C, поймано в A).' },
