...
export const dynamicLoading: LessonData = {
  ...
  spec: [
    { text: "«Reflection provides infrastructure used by language compilers to implement implicit late binding.» <span class=\"ru-tr\">«Reflection предоставляет инфраструктуру, которую компиляторы языков используют для реализации неявного позднего связывания.»</span>", source: "ms-dynload" },
  ],
  edgeCases: [
    { text: "Late binding — про <b>момент</b>: «When this process occurs at runtime rather than at compile time, it's called <b>late binding</b>». <span class=\"ru-tr\">«Когда этот процесс происходит во время выполнения, а не во время компиляции, он называется <b>поздним связыванием</b>».</span>", source: "ms-dynload" },
    { text: "<code>CreateInstance</code> — частный случай <code>InvokeMember</code>: «specialized forms of <code>InvokeMember</code> that create new instances of the specified type». <span class=\"ru-tr\">«специализированные формы <code>InvokeMember</code>, которые создают новые экземпляры указанного типа».</span>", source: "ms-dynload" },
  ...
  misconceptions: [
    {
      ...
      hook: 'Late binding — это про <b>момент связывания</b>, а не про скорость. «Binding is the process of <span class="hl">locating the declaration</span>… that corresponds to a uniquely specified type. When this process occurs at <b>runtime</b> rather than at <b>compile time</b>, it\'s called late binding». <span class="ru-tr">«Связывание — это процесс поиска объявления (то есть реализации), соответствующего однозначно заданному типу. Когда этот процесс происходит во время <b>выполнения</b>, а не во время <b>компиляции</b>, он называется поздним связыванием».</span> И это не хаос: reflection даёт точный протокол — «you can <b>load an assembly at runtime</b>, obtain information about types in that assembly, specify the type that you want, and then <b>invoke methods</b>… on that type». <span class="ru-tr">«вы можете <b>загрузить сборку во время выполнения</b>, получить информацию о типах в этой сборке, указать нужный тип, а затем <b>вызвать методы</b>… у этого типа».</span> Дальше <b>пять разборов</b>: early vs late, загрузка типа по строке, диспетч <code>InvokeMember</code>, роль <code>Binder</code>, и <b>машинная панель</b> — как <code>ChangeType</code> <span class="hl">расширяет</span> аргумент <code>Int32→Int64</code>, но пасует на <code>String→Double</code> (реальный прогон).',
      source: "ms-dynload",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Разница не в скорости, а в <b>моменте</b>: «Binding is the process of <span class="hl">locating the declaration (that is, the implementation)</span> that corresponds to a uniquely specified type. When this process occurs at <b>runtime</b> rather than at compile time, it\'s called <b>late binding</b>». <span class="ru-tr">«Связывание — это процесс поиска объявления (то есть реализации), соответствующего однозначно заданному типу. Когда этот процесс происходит во время <b>выполнения</b>, а не во время компиляции, он называется <b>поздним связыванием</b>».</span> Reflection — это инфраструктура late binding: «Reflection provides infrastructure used by language compilers to implement implicit late binding». <span class="ru-tr">«Reflection предоставляет инфраструктуру, которую компиляторы языков используют для реализации неявного позднего связывания».</span> В early-случае компилятор вшивает вызов по метаданным на этапе билда; в late — имя метода это строка, а нужную реализацию ищет рантайм. Так работают плагины и DI: «useful if you don\'t know an object\'s type at compile time». <span class="ru-tr">«полезно, если вы не знаете тип объекта во время компиляции».</span>',
      sources: ["ms-dynload"],
    },
    {
      ...
      explain: 'Протокол динамики буквально описан: «you can <b>load an assembly at runtime</b>, obtain information about types in that assembly, <span class="hl">specify the type that you want</span>, and then invoke methods or access fields or properties on that type». <span class="ru-tr">«вы можете <b>загрузить сборку во время выполнения</b>, получить информацию о типах в этой сборке, указать нужный тип, а затем вызывать методы либо обращаться к полям или свойствам этого типа».</span> Тип задаётся <b>строкой</b> (<code>asm.GetType(name)</code> или <code>Type.GetType(name)</code>) — в коде класса нет. Дальше — <code>Activator.CreateInstance(t)</code>: «The <code>CreateInstance</code> methods… are <b>specialized forms of InvokeMember</b> that create new instances of the specified type». <span class="ru-tr">«Методы <code>CreateInstance</code>… являются <b>специализированными формами InvokeMember</b>, которые создают новые экземпляры указанного типа».</span> Так грузятся плагины: имя типа приходит из конфига/ввода, а не из <code>using</code>.',
      sources: ["ms-dynload"],
    },
    {
      ...
      explain: '<code>Type.InvokeMember</code> — единый вход для late-вызова: «Use <b>Type.InvokeMember</b> to invoke a member of a type». <span class="ru-tr">«Используйте <b>Type.InvokeMember</b>, чтобы вызвать член типа».</span> Когда одноимённых членов несколько, включается разрешение перегрузок: «Overload resolution is needed when more than one member with the same name is available. The <b>Binder.BindToMethod</b>… methods are used to resolve binding to a single member». <span class="ru-tr">«Разрешение перегрузок требуется, когда доступно более одного члена с одинаковым именем. Методы <b>Binder.BindToMethod</b>… используются, чтобы свести связывание к единственному члену».</span> Тип фактического аргумента (<code>42L</code> → <code>long</code>) определяет выбор — как в доке Case 2 с двумя <code>PrintValue</code>. Реальный прогон: <code>42L</code> → <code>long:42</code>. То есть late binding не отменяет типовую строгость — просто разрешение отложено в рантайм.',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: '<b>Работа 1 — выбор члена</b>: <code>BindToMethod</code> «returns the MethodBase to invoke» <span class="ru-tr">«возвращает MethodBase для вызова»</span> из списка кандидатов.', nodes: [{ id: "sel", kind: "gate", at: { zone: "select", row: 0 }, state: "ok", label: "BindToMethod", detail: "→ MethodBase", accent: true }], edges: [] },
        ...
        { codeLine: 3, out: "", caption: 'Важно: «<code>ChangeType</code> is called for <b>every argument</b> even if the types match exactly» <span class="ru-tr">«<code>ChangeType</code> вызывается для <b>каждого аргумента</b>, даже если типы совпадают точно»</span> — коэрция всегда в цепочке.', nodes: [{ id: "sel", kind: "gate", at: { zone: "select", row: 0 }, state: "ok", label: "BindToMethod", detail: "→ MethodBase" }, { id: "ch", kind: "gate", at: { zone: "coerce", row: 0 }, state: "ok", label: "ChangeType", detail: "каждый аргумент", accent: true }], edges: [{ id: "e", from: "sel", to: "ch" }] },
      ],
      explain: 'Late-вызов делает не «магия», а объект <code>Binder</code> — «The <code>Binder</code> class provides custom control of member selection and invocation». <span class="ru-tr">«Класс <code>Binder</code> обеспечивает пользовательский контроль над выбором и вызовом члена».</span> У него две задачи. Первая — <b>выбор члена</b>: «<code>BindToMethod</code> returns the <span class="hl">MethodBase</span> to invoke, or a null reference… if no such invocation is possible». <span class="ru-tr">«<code>BindToMethod</code> возвращает MethodBase для вызова либо пустую ссылку… если такой вызов невозможен».</span> Вторая — <b>коэрция аргументов</b>: «<code>ChangeType</code> performs argument coercion (type conversion), which converts the actual arguments to the type of the formal arguments of the selected method», <span class="ru-tr">«<code>ChangeType</code> выполняет коэрцию аргументов (преобразование типов), которая приводит фактические аргументы к типу формальных аргументов выбранного метода»,</span> причём «<code>ChangeType</code> is called for <b>every argument</b> even if the types match exactly». <span class="ru-tr">«<code>ChangeType</code> вызывается для <b>каждого аргумента</b>, даже если типы совпадают точно».</span> Дефолтный binder делает это по правилам CTS; кастомный (<code>: Binder</code>) может переопределить всё.',
  ...
  cards: [
    ...
    },
    {
      ...
      okText: 'Binder выбирает перегрузку по <span class="hl">типу аргумента</span>: <code>42L</code> (long) → <code>PrintValue(long)</code> → <b>long:42</b>; <code>"hi"</code> (string) → <b>str:hi</b>. «The appropriate method is selected by the call to <code>BindToMethod</code>». <span class="ru-tr">«Подходящий метод выбирается вызовом <code>BindToMethod</code>».</span>',
  ...
  takeaways: [
    { icon: "why", k: "Момент, не скорость", v: 'Late binding — связывание в <b>рантайме</b>, а не на компиляции. Reflection — «infrastructure… to implement implicit late binding». <span class="ru-tr">«инфраструктура… для реализации неявного позднего связывания».</span> Тип может прийти из конфига/ввода.' },
    { icon: "cost", k: "Протокол", v: 'Загрузить сборку → <code>asm.GetType("имя")</code> → <code>Activator.CreateInstance</code> → <code>InvokeMember("метод", args)</code>. <code>CreateInstance</code> — одна из «specialized forms of <code>InvokeMember</code>». <span class="ru-tr">«специализированных форм <code>InvokeMember</code>».</span>' },
