...
export const collectionsOverview: LessonData = {
  ...
  spec: [
    { text: "«There are two main types of collections; generic collections and non-generic collections. Generic collections are type-safe at compile time.» <span class=\"ru-tr\">«Есть два основных типа коллекций: generic-коллекции и non-generic-коллекции. Generic-коллекции типобезопасны на этапе компиляции.»</span>", source: "ms-collections" },
    { text: "«In general, you should use generic collections.» <span class=\"ru-tr\">«В общем случае следует использовать generic-коллекции.»</span>", source: "ms-collections" },
  ],
  edgeCases: [
    ...
    { text: "Любая коллекция, реализующая <code>IEnumerable&lt;T&gt;</code>, — <i>queryable</i>: работает <code>foreach</code> и LINQ. Энумератор — «movable pointer to any element». <span class=\"ru-tr\">«перемещаемый указатель на любой элемент».</span>", source: "ms-collections" },
  ...
  misconceptions: [
    {
      ...
      hook: 'Non-generic <code>ArrayList</code> и generic <code>List&lt;int&gt;</code> дают одинаковый API, но платят по-разному. Дока проводит границу прямо: «There are two main types of collections; generic collections and non-generic collections. <span class="hl">Generic collections are type-safe at compile time</span>». <span class="ru-tr">«Есть два основных типа коллекций: generic-коллекции и non-generic-коллекции. Generic-коллекции типобезопасны на этапе компиляции».</span> Non-generic хранят элемент как <code>object</code> — каждый <code>int</code> <span class="wrong">упаковывается в кучу</span>. Дальше <b>пять разборов</b>: две дорожки хранения, контракт <code>IEnumerable&lt;T&gt;</code>, карта выбора и <b>машинная панель</b> — реально снятый счётчик: 1000 <code>int</code> в <code>ArrayList</code> = <b>40568 байт</b> кучи против <b>8392</b> у <code>List&lt;int&gt;</code> (тот же путь без presize).',
  ...
  segments: [
    {
      ...
      explain: 'Верхнеуровневая карта коллекций стоит на одной оси: «There are <b>two main types</b> of collections; generic collections and non-generic collections. Generic collections are <span class="hl">type-safe at compile time</span>. Because of this, generic collections typically offer better performance». <span class="ru-tr">«Есть <b>два основных типа</b> коллекций: generic-коллекции и non-generic-коллекции. Generic-коллекции типобезопасны на этапе компиляции. Благодаря этому generic-коллекции обычно дают лучшую производительность».</span> Generic принимают параметр типа и «don\'t require that you cast to and from the <code>Object</code> type». <span class="ru-tr">«не требуют приведения к типу <code>Object</code> и обратно».</span> Non-generic «store items as <code>Object</code>, require casting» <span class="ru-tr">«хранят элементы как <code>Object</code>, требуют приведения»</span> — а для value-типа хранение как <code>object</code> означает boxing на вставке и unboxing на чтении. Это не стиль, а разные машинные затраты.',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'Каждая .NET-коллекция реализует <code>IEnumerable</code> или <code>IEnumerable&lt;T&gt;</code>. Энумератор — «movable pointer to any element in the collection». <span class="ru-tr">«перемещаемый указатель на любой элемент коллекции».</span>', nodes: [{ id: "coll", kind: "obj", at: { zone: "iface", row: 0 }, typeTag: "IEnumerable<T>", value: "GetEnumerator", accent: true }], edges: [] },
        { codeLine: 1, caption: '<code>foreach</code> берёт энумератор через <code>GetEnumerator()</code> и «hide the complexity of manipulating the enumerator» <span class="ru-tr">«скрывает сложность управления энумератором»</span> — двигает указатель за тебя.', nodes: [{ id: "coll", kind: "obj", at: { zone: "iface", row: 0 }, typeTag: "IEnumerable<T>", value: "GetEnumerator" }, { id: "fe", kind: "chip", at: { zone: "consume", row: 0 }, value: "foreach", accent: true }], edges: [{ id: "e1", from: "coll", to: "fe" }] },
        { codeLine: 2, caption: 'Реализующий <code>IEnumerable&lt;T&gt;</code> тип «is considered a <span class="hl">queryable type</span>» <span class="ru-tr">«считается queryable-типом»</span> — к нему применим LINQ: <code>Where</code>, <code>Select</code>, <code>OrderBy</code>.', nodes: [{ id: "coll", kind: "obj", at: { zone: "iface", row: 0 }, typeTag: "IEnumerable<T>", value: "GetEnumerator" }, { id: "fe", kind: "chip", at: { zone: "consume", row: 0 }, value: "foreach" }, { id: "linq", kind: "chip", at: { zone: "consume", row: 1 }, value: "LINQ Where", accent: true }], edges: [{ id: "e1", from: "coll", to: "fe" }, { id: "e2", from: "coll", to: "linq", accent: true }] },
      ],
      explain: 'То, что делает семейство коллекций единым, — интерфейс перечисления: «.NET collections either implement <code>System.Collections.IEnumerable</code> or <code>System.Collections.Generic.IEnumerable&lt;T&gt;</code> to enable the collection to be iterated through. An enumerator can be thought of as a <span class="hl">movable pointer</span> to any element in the collection». <span class="ru-tr">«.NET-коллекции реализуют либо <code>System.Collections.IEnumerable</code>, либо <code>System.Collections.Generic.IEnumerable&lt;T&gt;</code>, чтобы коллекцию можно было перебирать. Энумератор можно представлять как перемещаемый указатель на любой элемент коллекции».</span> <code>foreach</code> «use the enumerator exposed by the <code>GetEnumerator</code> method and hide the complexity of manipulating the enumerator». <span class="ru-tr">«используют энумератор, предоставляемый методом <code>GetEnumerator</code>, и скрывают сложность управления энумератором».</span> А любой тип с <code>IEnumerable&lt;T&gt;</code> «is considered a queryable type and can be queried with LINQ» <span class="ru-tr">«считается queryable-типом и может запрашиваться через LINQ»</span> — поэтому один контракт даёт и цикл, и запросы.',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'Дока предлагает выбирать <span class="hl">от сценария</span>. «Store items as key/value pairs for quick look-up by key» <span class="ru-tr">«Хранить элементы как пары ключ/значение для быстрого поиска по ключу»</span> → <code>Dictionary&lt;TKey,TValue&gt;</code>.', nodes: [{ id: "kv", kind: "chip", at: { zone: "map", row: 0, col: 0 }, value: "key/value" }, { id: "dict", kind: "obj", at: { zone: "map", row: 0, col: 1 }, typeTag: "Dictionary", value: "K,V", accent: true }], edges: [{ id: "e1", from: "kv", to: "dict", accent: true }] },
        { codeLine: 1, caption: '«Access items by index» <span class="ru-tr">«Обращаться к элементам по индексу»</span> → <code>List&lt;T&gt;</code> (non-generic аналог — <code>Array</code>/<code>ArrayList</code>). Массив под капотом, индекс O(1).', nodes: [{ id: "kv", kind: "chip", at: { zone: "map", row: 0, col: 0 }, value: "key/value" }, { id: "dict", kind: "obj", at: { zone: "map", row: 0, col: 1 }, typeTag: "Dictionary", value: "K,V" }, { id: "idx", kind: "chip", at: { zone: "map", row: 1, col: 0 }, value: "по индексу" }, { id: "list", kind: "obj", at: { zone: "map", row: 1, col: 1 }, typeTag: "List<T>", value: "index O(1)", accent: true }], edges: [{ id: "e1", from: "kv", to: "dict" }, { id: "e2", from: "idx", to: "list", accent: true }] },
        { codeLine: 3, caption: '«A set for mathematical functions» <span class="ru-tr">«Множество для математических операций»</span> → <code>HashSet&lt;T&gt;</code>; FIFO → <code>Queue&lt;T&gt;</code>, LIFO → <code>Stack&lt;T&gt;</code>. Общее правило: «you should use <b>generic</b> collections». <span class="ru-tr">«следует использовать <b>generic</b>-коллекции».</span>', nodes: [{ id: "idx", kind: "chip", at: { zone: "map", row: 0, col: 0 }, value: "по индексу" }, { id: "list", kind: "obj", at: { zone: "map", row: 0, col: 1 }, typeTag: "List<T>", value: "index O(1)" }, { id: "set", kind: "chip", at: { zone: "map", row: 1, col: 0 }, value: "множество" }, { id: "hs", kind: "obj", at: { zone: "map", row: 1, col: 1 }, typeTag: "HashSet<T>", value: "без дублей", accent: true }], edges: [{ id: "e3", from: "set", to: "hs", accent: true }] },
      ],
      explain: 'Учебник выбора коллекции идёт от задачи к типу: «you should use generic collections… The following table describes some common collection scenarios». <span class="ru-tr">«следует использовать generic-коллекции… В следующей таблице описаны некоторые типичные сценарии работы с коллекциями».</span> Ключевые строки: «Store items as key/value pairs for quick look-up by key» <span class="ru-tr">«Хранить элементы как пары ключ/значение для быстрого поиска по ключу»</span> → <code>Dictionary&lt;TKey,TValue&gt;</code> (non-generic <code>Hashtable</code>); «Access items by index» <span class="ru-tr">«Обращаться к элементам по индексу»</span> → <code>List&lt;T&gt;</code> (<code>Array</code>/<code>ArrayList</code>); «A set for mathematical functions» <span class="ru-tr">«Множество для математических операций»</span> → <code>HashSet&lt;T&gt;</code>/<code>SortedSet&lt;T&gt;</code>. Та же страница в разделе сложности прямо говорит: «When choosing a collection class, it\'s worth considering potential tradeoffs in performance» <span class="ru-tr">«При выборе класса коллекции стоит учитывать возможные компромиссы в производительности»</span> — ось алгоритмической сложности поверх выбора по задаче, но дефолт всегда generic.',
      sources: ["ms-collections"],
    },
    {
      ...
      explain: 'Машинная панель урока — реально снятое число. <code>GC.GetAllocatedBytesForCurrentThread()</code> даёт «The total number of bytes allocated to the current thread since the beginning of its lifetime» <span class="ru-tr">«Общее число байт, выделенных текущему потоку с начала его жизни»</span>; вокруг двух заливок <b>без presize</b> прирост — <b>ArrayList: 40568 bytes</b> и <b>List&lt;int&gt;: 8392 bytes</b> (собственный прогон, ×4.8). Разница — это boxing: «the elements of <code>Hashtable</code> are of type <code>Object</code>; therefore, boxing and unboxing typically occur when you store or retrieve a value type» <span class="ru-tr">«элементы <code>Hashtable</code> имеют тип <code>Object</code>; поэтому при сохранении или извлечении value-типа обычно происходят boxing и unboxing»</span> — та же природа у <code>ArrayList</code>. Каждый <code>int</code> стал отдельным объектом в куче; в <code>List&lt;int&gt;</code> все 1000 лежат в одном массиве. Вот что стоит за словами «better performance». <span class="ru-tr">«лучшая производительность».</span>',
  ...
  cards: [
    ...
    },
    {
      ...
      okText: '<code>Count</code> = <b>3</b>; и <code>List&lt;int&gt;</code> реализует <code>IEnumerable&lt;int&gt;</code> → <b>True</b>. Именно этот интерфейс делает тип «queryable» <span class="ru-tr">«запрашиваемым»</span> — доступным для <code>foreach</code> и LINQ.',
  ...
  takeaways: [
    { icon: "why", k: "Ось семейств", v: '<b>generic</b> (<code>List&lt;T&gt;</code>, <code>Dictionary&lt;K,V&gt;</code>) — «type-safe at compile time» <span class="ru-tr">«типобезопасны на этапе компиляции»</span>, хранят <code>T</code> без приведений; <b>non-generic</b> (<code>ArrayList</code>, <code>Hashtable</code>) — элемент <code>object</code>, cast + boxing. Дефолт — generic.' },
    { icon: "cost", k: "Цена боксинга", v: '1000 <code>int</code> в <code>ArrayList</code> = <b>40568 байт</b> кучи (1000 box, растущий путь), в <code>List&lt;int&gt;</code> = <b>8392</b> (один <code>int[]</code>) — реальный замер, ×4.8. «Better performance» <span class="ru-tr">«Лучшая производительность»</span> — это про эту разницу.' },
    { icon: "avoid", k: "Контракт", v: 'Единый двигатель — <code>IEnumerable&lt;T&gt;</code>: даёт <code>foreach</code> (энумератор — «movable pointer» <span class="ru-tr">«перемещаемый указатель»</span>) и делает тип queryable для LINQ. <code>List&lt;T&gt;</code> — <b>массив</b>, не связный список.' },
  ],
