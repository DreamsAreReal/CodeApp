...
export const spanMemory: LessonData = {
  ...
  spec: [
    { text: "«A Span<T> instance can be backed by an array of type T, a buffer allocated with stackalloc, or a pointer to unmanaged memory. Because it has to be allocated on the stack, it has a number of restrictions.» <span class=\"ru-tr\">«Экземпляр Span<T> может быть основан на массиве типа T, на буфере, выделенном через stackalloc, или на указателе на неуправляемую память. Поскольку он должен размещаться на стеке, у него есть ряд ограничений.»</span>", source: "ms-span" },
  ],
  edgeCases: [
    { text: "Без копирования/аллокации (verbatim): «These types are designed to allow the creation of algorithms that <span class=\"hl\">avoid copying memory or allocating on the managed heap</span> more than necessary. Creating them … <b>does not involve duplicating the underlying buffers</b>: only the relevant references and offsets, which represent the \"view\" of the wrapped memory, are updated». <span class=\"ru-tr\">«Эти типы предназначены для того, чтобы позволить создавать алгоритмы, которые избегают копирования памяти или выделения в управляемой куче больше необходимого. Их создание … <b>не включает дублирование лежащих в основе буферов</b>: обновляются только соответствующие ссылки и смещения, которые представляют \"вью\" обёрнутой памяти».</span>", source: "ms-span" },
    { text: "Ограничения Span (verbatim): «<span class=\"hl\">a field in a class cannot be of type Span<T>, nor can span be used in asynchronous operations</span>». <span class=\"ru-tr\">«поле в классе не может быть типа Span<T>, а также span нельзя использовать в асинхронных операциях».</span> ReadOnlySpan<T> — «an immutable version… Instances can be also backed by a String». <span class=\"ru-tr\">«иммутабельная версия… Экземпляры также могут быть основаны на String».</span>", source: "ms-span" },
    { text: "ref struct стек-только (verbatim, ref-struct): «You allocate instances of a <code>ref struct</code> type on the stack, and they <span class=\"hl\">can\'t escape to the managed heap</span>». <span class=\"ru-tr\">«Экземпляры типа <code>ref struct</code> размещаются на стеке, и они не могут сбежать в управляемую кучу».</span> Компилятор запрещает: элемент массива, поле class/non-ref struct, боксинг, захват в лямбде. <i>(Часть ограничений — async/итераторы/интерфейсы/тип-аргумент — ослаблена с C# 13.)</i>", source: "ms-refstruct" },
  ],

  misconceptions: [
    {
      wrong: "Span<T> = Memory<T>; Span можно положить в поле класса / использовать в async / боксить; Span аллоцирует на куче",
      hook: 'Пучок мифов про Span. «<span class="wrong">Span аллоцирует на куче</span>» — нет: создание «<span class="hl">does not involve duplicating the underlying buffers</span>» <span class="ru-tr">«не включает дублирование лежащих в основе буферов»</span>, только ссылки/смещения (вью). «<span class="wrong">Span в поле класса / в async</span>» — нет: «<span class="hl">a field in a class cannot be of type Span<T></span>, nor can span be used in asynchronous operations» <span class="ru-tr">«поле в классе не может быть типа Span<T>, а также span нельзя использовать в асинхронных операциях»</span> (это <b>ref struct</b>, стек-только). <span class="wrong">Span = Memory</span> — нет: <code>Memory<T></code> живёт на куче и годна для async («none of the limitations of Span<T>» <span class="ru-tr">«ни одного из ограничений Span<T>»</span>). Ниже <b>пять разборов</b>: Span как вью, без аллокации (замер), ref struct/стек-только, ReadOnlySpan + Memory, и <b>машинная панель</b> — реальный замер 0 байт + IsByRefLike.',
      source: "ms-span",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Что такое span-типы (verbatim): «.NET includes a number of interrelated types that represent a contiguous, strongly typed region of arbitrary memory. These types are designed to allow the creation of algorithms that <span class="hl">avoid copying memory or allocating on the managed heap</span> more than necessary». <span class="ru-tr">«.NET включает ряд взаимосвязанных типов, представляющих непрерывный, строго типизированный участок произвольной памяти. Эти типы предназначены для того, чтобы позволить создавать алгоритмы, которые избегают копирования памяти или выделения в управляемой куче больше необходимого».</span> Про <code>Span&lt;T&gt;</code>: «A <code>Span&lt;T&gt;</code> instance can be backed by an array of type <code>T</code>, a buffer allocated with <code>stackalloc</code>, or a pointer to unmanaged memory». <span class="ru-tr">«Экземпляр <code>Span&lt;T&gt;</code> может быть основан на массиве типа <code>T</code>, на буфере, выделенном через <code>stackalloc</code>, или на указателе на неуправляемую память».</span> Ключ: Span — не контейнер, а <b>вью</b>: он ссылается на чужой буфер и позволяет читать/писать его срез без владения и без копии. Отсюда его назначение — избегать аллокаций (замер в панели: срез Span — 0 байт кучи). Но за это платим ограничениями: Span — ref struct, стек-только (разбор 03).',
      sources: ["ms-span"],
    },
    {
      ...
      explain: 'Почему Span экономит память (verbatim): «Creating them (either via <code>Slice</code>, <code>AsSpan()</code>, a collection expression, or their constructors) <span class="hl">does not involve duplicating the underlying buffers</span>: only the relevant references and offsets, which represent the "view" of the wrapped memory, are updated. In high-performance code, spans are often used to avoid allocating strings unnecessarily». <span class="ru-tr">«Их создание (через <code>Slice</code>, <code>AsSpan()</code>, коллекционное выражение или их конструкторы) не включает дублирование лежащих в основе буферов: обновляются только соответствующие ссылки и смещения, которые представляют "вью" обёрнутой памяти. В высокопроизводительном коде span-ы часто используют, чтобы избежать лишнего выделения строк».</span> Замер через <code>GC.GetAllocatedBytesForCurrentThread()</code>: <code>arr.AsSpan(100,500)</code> прибавляет <b>0</b> байт (вью), а <code>arr[100..600]</code> (копирующий range на массиве) реально аллоцирует новый массив: <b>spanAllocates=False arrayCopyAllocates=True</b>, детерминированно 3/3. Практика: где раньше резали строку/массив копией (мусор для GC), теперь берут <code>Span</code>/<code>ReadOnlySpan</code> — тот же срез без единой аллокации.',
      sources: ["ms-span"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, caption: '<code>Span&lt;T&gt;</code> — <b>ref struct</b>: «allocate… on the stack, and they <span class="hl">can\'t escape to the managed heap</span>». <span class="ru-tr">«размещается… на стеке, и они не могут сбежать в управляемую кучу».</span>', nodes: [{ id: "r", kind: "gate", at: { zone: "refstruct", row: 0 }, state: "ok", label: "ref struct", detail: "стек-только", accent: true }], edges: [] },
      ...
      explain: 'Почему у Span ограничения (verbatim, ref-struct): «You allocate instances of a <code>ref struct</code> type on the stack, and they <span class="hl">can\'t escape to the managed heap</span>. To ensure this property, the compiler limits the usage of <code>ref struct</code> types as follows: You can\'t use a <code>ref struct</code> as the element type of an array. <span class="hl">You can\'t declare a <code>ref struct</code> as the type of a field in a class or a non-<code>ref struct</code></span>. You can\'t box a <code>ref struct</code>… You can\'t capture a <code>ref struct</code> variable in a lambda expression or a local function». <span class="ru-tr">«Экземпляры типа <code>ref struct</code> размещаются на стеке, и они не могут сбежать в управляемую кучу. Чтобы обеспечить это свойство, компилятор ограничивает использование типов <code>ref struct</code> так: нельзя использовать <code>ref struct</code> как тип элемента массива. Нельзя объявить <code>ref struct</code> как тип поля в классе или в non-<code>ref struct</code>. Нельзя боксить <code>ref struct</code>… Нельзя захватывать переменную <code>ref struct</code> в лямбда-выражении или локальной функции».</span> Про async (с версией): «<span class="hl">Before C# 13, you can\'t use <code>ref struct</code> variables in an <code>async</code> method</span>. Beginning with C# 13, a <code>ref struct</code> variable can\'t be used in the same block as the <code>await</code> expression». <span class="ru-tr">«До C# 13 нельзя использовать переменные <code>ref struct</code> в <code>async</code>-методе. Начиная с C# 13 переменную <code>ref struct</code> нельзя использовать в том же блоке, что и выражение <code>await</code>».</span> Замер подтверждает поле-запрет: <code>class C { Span&lt;int&gt; field; }</code> — <b>CS8345</b>. И «In .NET, examples of a <code>ref struct</code> are <code>System.Span&lt;T&gt;</code> and <code>System.ReadOnlySpan&lt;T&gt;</code>». <span class="ru-tr">«В .NET примеры <code>ref struct</code> — это <code>System.Span&lt;T&gt;</code> и <code>System.ReadOnlySpan&lt;T&gt;</code>».</span>',
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: '<code>Memory&lt;T&gt;</code> — <span class="hl">можно хранить на куче</span>: «has none of the limitations of Span<T>» <span class="ru-tr">«не имеет ни одного из ограничений Span<T>»</span> → годна для async и полей класса.', nodes: [{ id: "r", kind: "gate", at: { zone: "ro", row: 0 }, state: "ok", label: "ReadOnlySpan", detail: "стек" }, { id: "m", kind: "gate", at: { zone: "mem", row: 0 }, state: "ok", label: "Memory<T>", detail: "куча · async OK", accent: true }], edges: [] },
      ...
      explain: 'Иммутабельная версия и heap-вариант (verbatim): «<code>System.ReadOnlySpan&lt;T&gt;</code>, an immutable version of the <code>Span&lt;T&gt;</code> structure. <span class="hl">Instances can be also backed by a <code>String</code></span>». <span class="ru-tr">«<code>System.ReadOnlySpan&lt;T&gt;</code> — иммутабельная версия структуры <code>Span&lt;T&gt;</code>. Экземпляры также могут быть основаны на <code>String</code>».</span> И <code>Memory&lt;T&gt;</code>: «A <code>Memory&lt;T&gt;</code> instance can be backed by an array of type <code>T</code> or a memory manager. <span class="hl">As it can be stored on the managed heap, <code>Memory&lt;T&gt;</code> has none of the limitations of <code>Span&lt;T&gt;</code></span>». <span class="ru-tr">«Экземпляр <code>Memory&lt;T&gt;</code> может быть основан на массиве типа <code>T</code> или на менеджере памяти. Поскольку его можно хранить в управляемой куче, <code>Memory&lt;T&gt;</code> не имеет ни одного из ограничений <code>Span&lt;T&gt;</code>».</span> То есть <code>ReadOnlySpan&lt;char&gt;</code> поверх <code>String</code> — способ резать строки без единой аллокации (замер: <code>"hello world".AsSpan(6)</code> → "world", 0 байт). А когда срез должен <b>пережить <code>await</code></b> или лежать в поле класса — берут <code>Memory&lt;T&gt;</code> (у неё нет стек-ограничений Span; ownership и guidelines — в S7.9).',
      sources: ["ms-span"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "spanAllocates=False arrayCopyAllocates=True · IsByRefLike=True · поле=CS8345", caption: 'Замер 3: поле <code>Span&lt;int&gt;</code> в классе — <span class="hl">компилятор отвергает</span>: <b>CS8345</b>. Три числа/факта — реальный рантайм и компилятор.', nodes: [{ id: "n", kind: "gate", at: { zone: "noalloc", row: 0 }, state: "ok", label: "0 байт", detail: "spanAlloc=False" }, { id: "r", kind: "gate", at: { zone: "isref", row: 0 }, state: "ok", label: "ref struct", detail: "поле=CS8345", accent: true }], edges: [] },
      ],
      explain: 'Машинная панель — три проверяемых факта про Span, снятые в рантайме и компиляторе. (1) <b>Без аллокации</b>: срез <code>arr.AsSpan(100,500)</code> добавляет <b>0</b> байт по <code>GC.GetAllocatedBytesForCurrentThread()</code>, а копирующий <code>arr[100..600]</code> — аллоцирует: <code>spanAllocates=False arrayCopyAllocates=True</code> (3/3). (2) <b>ref struct</b>: <code>typeof(Span&lt;int&gt;).IsByRefLike</code> → <b>True</b> (и <code>IsValueType</code> → True) — это тот самый by-ref-like value type, что живёт только на стеке. (3) <b>Стек-только на практике</b>: попытка объявить <code>Span&lt;int&gt;</code> полем класса — <b>ошибка компиляции CS8345</b> (текст компилятора CS8345: «Field or auto-implemented property cannot be of type Span&lt;int&gt;…» <span class="ru-tr">«Поле или автосвойство не может быть типа Span&lt;int&gt;…»</span>). Всё это следствие реализации Span через <code>ref</code>-поле (<code>ref T _reference</code> + <code>int _length</code>): ссылка не должна пережить референт, потому — стек-только. Числа реальны; Span — allocation-free вью, а не ещё один контейнер на куче.',
  ...
  cards: [
    {
      ...
      okText: 'Срез <code>Span</code> — <span class="hl">вью, 0 байт кучи</span> («does not involve duplicating the underlying buffers» <span class="ru-tr">«не включает дублирование лежащих в основе буферов»</span>); <code>arr[100..600]</code> — копия в новый массив, аллоцирует. Печать: <b>spanAllocates=False arrayCopyAllocates=True</b>.',
    ...
    },
    {
      ...
      okText: '<code>Span&lt;T&gt;</code> — <b>ref struct</b> (<code>IsByRefLike=True</code>), это value type (<code>IsValueType=True</code>) → «You <span class="hl">allocate instances of a ref struct type on the stack, and they can\'t escape to the managed heap</span>». <span class="ru-tr">«Вы размещаете экземпляры типа ref struct на стеке, и они не могут сбежать в управляемую кучу».</span> Печать: <b>spanIsValueType=True spanIsRefStruct=True</b>.',
    ...
    },
    {
      ...
      okText: '<code>ReadOnlySpan&lt;char&gt;</code> «Instances can be also backed by a <code>String</code>» <span class="ru-tr">«Экземпляры также могут быть основаны на <code>String</code>»</span>: срез <code>"hello world".AsSpan(6)</code> = "world" (длина <b>5</b>), <b>0</b> аллокаций (вью в строку). Печать: <b>sliceLen=5 allocatedBytes=0</b>.',
  ...
  takeaways: [
    { icon: "why", k: "Span = вью без аллокации", v: 'Span<T> — вью над непрерывной памятью (backed <code>T[]</code>/<code>stackalloc</code>/unmanaged); создание «<span class="hl">does not involve duplicating the underlying buffers</span>» <span class="ru-tr">«не включает дублирование лежащих в основе буферов»</span> → 0 байт кучи (замер: spanAllocates=False vs copy=True). Для срезов без мусора.' },
    { icon: "cost", k: "ref struct · стек-только", v: 'Span — <b>ref struct</b> (<code>IsByRefLike=True</code>): «You allocate instances of a ref struct type on the stack, and they <span class="hl">can\'t escape to the managed heap</span>». <span class="ru-tr">«Вы размещаете экземпляры типа ref struct на стеке, и они не могут сбежать в управляемую кучу».</span> Нельзя: поле class (замер: CS8345), элемент массива, боксинг, лямбда, async (до C# 13; с C# 13 — не в одном блоке с <code>await</code>).' },
    { icon: "avoid", k: "ReadOnlySpan / Memory", v: '<code>ReadOnlySpan&lt;T&gt;</code> — иммутабельна, backed <code>String</code> (срез строки, 0 байт — замер). Нужен async / поле / куча → <code>Memory&lt;T&gt;</code>: «<span class="hl">none of the limitations of Span<T></span>» <span class="ru-tr">«ни одного из ограничений Span<T>»</span> (S7.9). Span ≠ Memory.' },
  ],
