...
export const listPatterns: LessonData = {
  ...
  spec: [
    { text: "«a list pattern matches when each nested pattern matches the corresponding element of an input sequence.» <span class=\"ru-tr\">«списочный паттерн совпадает, когда каждый вложенный паттерн совпадает с соответствующим элементом входной последовательности.»</span>", source: "ms-patterns" },
    { text: "«A slice pattern matches zero or more elements. You can use at most one slice pattern in a list pattern. The slice pattern can only appear in a list pattern.» <span class=\"ru-tr\">«Slice-паттерн совпадает с нулём или более элементов. В списочном паттерне можно использовать не более одного slice-паттерна. Slice-паттерн может встречаться только в списочном паттерне.»</span>", source: "ms-patterns" },
  ],
  edgeCases: [
  ...
  misconceptions: [
    {
      ...
      hook: 'List pattern матчит <b>форму</b>, а не значения: «a list pattern matches when <span class="hl">each nested pattern matches</span> the corresponding element of an input sequence» <span class="ru-tr">«списочный паттерн совпадает, когда каждый вложенный паттерн совпадает с соответствующим элементом входной последовательности»</span> — внутри каждой позиции может стоять любой паттерн (relational, or, var, discard). Отдельная сила — <b>slice</b> <code>..</code>: «A slice pattern matches <span class="hl">zero or more elements</span>. You can use <b>at most one</b> slice pattern in a list pattern» <span class="ru-tr">«Slice-паттерн совпадает с нулём или более элементов. Можно использовать <b>не более одного</b> slice-паттерна в списочном паттерне»</span>. А ниже абстракции — slice умеет <b>захватывать</b>: <code>.. var s</code> связывает середину. Дальше <b>пять разборов</b> — форма списка, discard/var, slice, «один slice» и <b>машинная панель</b> захвата среза.',
      source: "ms-patterns",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'List pattern сопоставляет последовательность с набором вложенных паттернов: «You can match an array or a list against a <b>sequence of patterns</b>… a list pattern matches when <span class="hl">each nested pattern matches the corresponding element</span> of an input sequence» <span class="ru-tr">«Можно сопоставить массив или список с <b>последовательностью паттернов</b>… списочный паттерн совпадает, когда каждый вложенный паттерн совпадает с соответствующим элементом входной последовательности»</span>. Длина учитывается — <code>[1, 2, 3, 4]</code> не совпадёт с трёхэлементным массивом. И внутри позиции — любой паттерн: «You can use <b>any pattern within a list pattern</b>» <span class="ru-tr">«Можно использовать <b>любой паттерн внутри списочного паттерна</b>»</span>, поэтому <code>[0 or 1, &lt;= 2, &gt;= 3]</code> проверяет форму, а не конкретные числа. Прогон: <code>True / False / False / True</code>.',
      sources: ["ms-patterns"],
    },
    {
      ...
      explain: 'Внутри списка работают те же «универсальные» паттерны: «To match any element, use the <b>discard pattern</b> or, if you also want to <b>capture the element</b>, use the <b>var pattern</b>» <span class="ru-tr">«Чтобы сопоставить любой элемент, используйте <b>discard-паттерн</b> или, если также нужно <b>захватить элемент</b>, используйте <b>var-паттерн</b>»</span>. Так <code>[var first, _, _]</code> означает «ровно три элемента; захвати первый, остальные пропусти». <code>_</code> матчит любой элемент, но ничего не связывает; <code>var</code> — матчит и связывает. Число позиций фиксирует длину, а не значения. Отсюда идиома «достать голову списка известной длины».',
      sources: ["ms-patterns"],
    },
    {
      ...
      explain: 'Slice pattern даёт гибкую форму: «To match elements only at the start or end - or both - of an input sequence, use the <b>slice pattern</b> <code>..</code>» <span class="ru-tr">«Чтобы сопоставить элементы только в начале или конце — или в обоих — входной последовательности, используйте <b>slice-паттерн</b> <code>..</code>»</span>. Ключевое свойство: «A slice pattern matches <span class="hl">zero or more elements</span>» <span class="ru-tr">«Slice-паттерн совпадает с нулём или более элементов»</span>. Поэтому <code>[&gt; 0, &gt; 0, ..]</code> фиксирует начало, <code>[.., &gt; 0, &gt; 0]</code> — конец, а <code>[.., 2, 4]</code> на <code>{2,4}</code> истинно, потому что slice может съесть <b>ноль</b> элементов. Прогон подтверждает все три: <code>True / True / True</code>. Slice — не «хотя бы один», а «сколько угодно, включая ноль».',
      sources: ["ms-patterns"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: '<b>Два</b> slice — нельзя: «at most one slice pattern» <span class="ru-tr">«не более одного slice-паттерна»</span>. Иначе позиция среза неоднозначна. И <code>..</code> живёт только в list pattern.', nodes: [{ id: "ok", kind: "gate", at: { zone: "onez", row: 0 }, state: "ok", label: "один ..", detail: "ok" }, { id: "no", kind: "gate", at: { zone: "twoz", row: 0 }, state: "fail", label: "[.., 2, ..]", detail: "два — ошибка", accent: true }], edges: [] },
      ],
      explain: 'У slice — жёсткие ограничения: «A slice pattern matches zero or more elements. You can use <b>at most one slice pattern</b> in a list pattern. The slice pattern <span class="hl">can only appear in a list pattern</span>» <span class="ru-tr">«Slice-паттерн совпадает с нулём или более элементов. Можно использовать <b>не более одного slice-паттерна</b> в списочном паттерне. Slice-паттерн может встречаться только в списочном паттерне»</span>. Два <code>..</code> запрещены — иначе непонятно, куда отнести элементы между ними (позиция среза стала бы неоднозначной). И <code>..</code> не паттерн общего назначения: вне <code>[ ]</code> его использовать нельзя. Один slice + фиксированные края — вот полный контракт формы.',
      sources: ["ms-patterns"],
    },
    {
      ...
      explain: 'Это машинная панель урока — slice с захватом, снятая прогоном. Дока: «You can also <b>nest a subpattern within a slice pattern</b>» <span class="ru-tr">«Можно также <b>вложить субпаттерн внутрь slice-паттерна</b>»</span>, пример — <code>message is [\'a\' or \'A\', .. var s, \'a\' or \'A\']</code>, где <code>MatchMessage("aBBA")</code> даёт «Message aBBA matches; inner part is <b>BB</b>.» <span class="ru-tr">«Сообщение aBBA совпадает; внутренняя часть — <b>BB</b>.»</span>, а <code>MatchMessage("apron")</code> — «Message apron <span class="hl">doesn\'t match</span>.» <span class="ru-tr">«Сообщение apron не совпадает.»</span>. То есть один list pattern одновременно (1) проверяет <b>форму</b> — первый/последний символ буква <code>a/A</code>, (2) через <code>.. var s</code> <b>связывает</b> середину в переменную. Slice — не только пропуск, но и захват произвольно длинного среза. Так S5 замыкается: паттерны сопоставляют не значения, а форму данных, попутно доставая нужное.',
