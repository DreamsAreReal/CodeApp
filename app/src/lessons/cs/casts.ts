...
export const casts: LessonData = {
  ...
  spec: [
    { text: "«If the conversion isn't possible, the `as` operator returns null. Unlike a cast expression, the `as` operator never throws an exception.» <span class=\"ru-tr\">«Если преобразование невозможно, оператор `as` возвращает null. В отличие от выражения приведения, оператор `as` никогда не бросает исключение.»</span>", source: "ms-cast" },
  ],
  edgeCases: [
  ...
  segments: [
    {
      ...
      explain: 'Это машинная панель урока — три оператора, три поведения, снятые на одном объекте. Дословно: «The <code>is</code> operator checks if the run-time type… is <b>compatible</b>» <span class="ru-tr">«Оператор <code>is</code> проверяет, <b>совместим</b> ли тип времени выполнения…»</span> — возвращает <code>bool</code> (<code>o is int</code> → False), не бросает. «Use the <code>as</code> operator to explicitly convert… If the conversion isn\'t possible, the <code>as</code> operator returns <code>null</code>. Unlike a cast expression, the <code>as</code> operator <span class="hl">never throws</span>» <span class="ru-tr">«Используйте оператор <code>as</code>, чтобы явно преобразовать… Если преобразование невозможно, оператор <code>as</code> возвращает <code>null</code>. В отличие от выражения приведения, оператор <code>as</code> никогда не бросает.»</span> (<code>o as string</code> → "hello"). А cast: «At run time, an explicit conversion might not succeed and a cast expression <b>might throw</b> an exception» <span class="ru-tr">«Во время выполнения явное преобразование может не удаться, и выражение приведения <b>может бросить</b> исключение»</span> — <code>(int)o</code> над строкой даёт <code>InvalidCastException</code>. Выбор оператора — это выбор поведения при несовпадении: тихий bool, тихий null или исключение.',
      sources: ["ms-cast"],
    },
    {
      ...
      explain: 'Границы <code>is</code> точные. Дословно: «the <code>is</code> operator takes into account <b>boxing and unboxing conversions</b> but doesn\'t consider <span class="hl">numeric conversions</span>» <span class="ru-tr">«оператор <code>is</code> учитывает <b>преобразования упаковки и распаковки</b>, но не рассматривает числовые преобразования»</span>. Прогон: <code>(object)42 is int</code> → True (unbox к точному типу), <code>… is long</code> → False (int→long это numeric, <code>is</code> её не делает). Плюс: «The <code>is</code> operator <b>doesn\'t consider user-defined conversions</b>» <span class="ru-tr">«Оператор <code>is</code> <b>не рассматривает пользовательские преобразования</b>»</span>. А главный бонус — pattern matching: «The <code>is</code> operator also tests an expression result against a <b>pattern</b>» <span class="ru-tr">«Оператор <code>is</code> также проверяет результат выражения на соответствие <b>шаблону</b>»</span> — <code>o is string s</code> проверяет тип И объявляет <code>s</code>, заменяя связку <code>as</code>+<code>!= null</code>.',
      sources: ["ms-cast"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Разница <code>typeof</code> и <code>is</code> — про наследование. «Use the <code>typeof</code> operator to check if the run-time type… <span class="hl">exactly matches</span> a given type» <span class="ru-tr">«Используйте оператор <code>typeof</code>, чтобы проверить, <span class="hl">точно ли совпадает</span> тип времени выполнения… с заданным типом»</span>. Прогон (пример прямо из доки): <code>d.GetType() == typeof(Dog)</code> → True, <code>== typeof(Animal)</code> → False, а <code>d is Animal</code> → True. Ключевое про аргумент: «The argument to the <code>typeof</code> operator must be the <b>name of a type</b>… An expression can\'t be an argument of the <code>typeof</code> operator. To get the <code>System.Type</code>… for the run-time type of an expression, use <code>Object.GetType</code>» <span class="ru-tr">«Аргументом оператора <code>typeof</code> должно быть <b>имя типа</b>… Выражение не может быть аргументом оператора <code>typeof</code>. Чтобы получить <code>System.Type</code>… для типа времени выполнения выражения, используйте <code>Object.GetType</code>»</span>. Отсюда правило: <code>typeof</code> — для точного типа по имени, <code>GetType()</code> — для типа значения, <code>is</code> — для проверки «is-a» с учётом иерархии.',
      sources: ["ms-cast"],
    },
    {
  ...
  cards: [
    ...
    },
    {
      ...
      noText: '«is takes into account boxing and unboxing conversions but doesn\'t consider numeric conversions» <span class="ru-tr">«is учитывает преобразования упаковки и распаковки, но не рассматривает числовые преобразования»</span>. Реальный вывод: <b>True False</b>.',
    ...
    },
    {
      ...
      noText: '«typeof… exactly matches» <span class="ru-tr">«typeof… точно совпадает»</span>; <code>is</code> учитывает наследование. Dog is-a Animal, но GetType()==Dog. Реальный вывод: <b>True False True</b>.',
