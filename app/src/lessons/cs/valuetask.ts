 ...
 * (the _obj/_result/_token/_flag union), vs Task<int> reference type (False). A real reflection
...
export const valueTask: LessonData = {
  ...
  spec: [
    { text: "«The ValueTask<TResult> instance may be awaited only once.» — экземпляр можно await ровно один раз; для повторного использования — AsTask() или Preserve().", source: "ms-vt" },
  ],
  edgeCases: [
  ...
  misconceptions: [
    {
      ...
      hook: 'Три опасные ошибки разом. «<span class="wrong">ValueTask всегда быстрее</span>» — нет: это struct с <b>несколькими полями</b>, его возврат копирует больше, чем один ссылочный field у <code>Task</code>, и раздувает стейт-машину; выигрыш есть ТОЛЬКО когда результат обычно готов синхронно и метод в горячем цикле. «<span class="wrong">await сколько угодно</span>» — нет: «<span class="hl">may be awaited only once</span>»; повторно — undefined, нужен повтор → <code>AsTask()</code>. «<span class="wrong">.Result как у Task</span>» — нет: до завершения/повторно — undefined. Ниже <b>пять разборов</b>: что это, когда брать, «один раз» + AsTask, <code>default</code>, и <b>машинная панель</b> — value type в 4 поля vs reference.',
      source: "ms-vt",
    },
  ],
  ...
  segments: [
    ...
    },
    {
      ...
      explain: 'Точное условие (verbatim, API remarks): использовать <code>ValueTask&lt;T&gt;</code> когда «results are likely to be available synchronously and where the method is expected to be invoked so frequently that the cost of allocating a new <code>Task&lt;TResult&gt;</code> for each call will be prohibitive». То есть два фактора ОДНОВРЕМЕННО: (1) результат часто готов синхронно, (2) метод в горячем цикле. Если хоть один не выполнен — берите <code>Task</code>. Прямая рекомендация: «The default choice for any asynchronous method should be to return a <code>Task</code> or <code>Task&lt;TResult&gt;</code>»; <code>ValueTask</code> — «only if performance analysis proves it worthwhile».',
      sources: ["ms-vt"],
    },
    {
      ...
      console: true,
      scenes: [
        { codeLine: 0, out: "", caption: 'Экземпляр <code>ValueTask</code> — «<span class="hl">may be awaited only once</span>». Один инстанс = одно потребление.', nodes: [{ id: "o", kind: "gate", at: { zone: "once", row: 0 }, state: "ok", label: "await vt", detail: "первый раз OK", accent: true }], edges: [] },
      ...
      explain: 'Жёсткое правило потребления (verbatim): «The <code>ValueTask&lt;TResult&gt;</code> instance <span class="hl">may be awaited only once</span> and the object may not be awaited concurrently. … If the intent is to be able to consume the results multiple times, or to consume it concurrently, use <code>AsTask()</code>». Список «should never do» иначе поведение undefined: awaiting несколько раз; несколько <code>AsTask</code>; чтение <code>.Result</code>/<code>.GetAwaiter().GetResult()</code> до завершения или повторно; смешивание способов. Практика (замер: <b>5 5</b>): <code>Load().AsTask()</code> даёт <code>Task&lt;int&gt;</code>, который <code>await</code>-ится дважды и оба раза даёт 5. Поэтому: <code>ValueTask</code> — «await один раз и забудь»; нужен повтор — сразу <code>AsTask()</code>.',
      sources: ["ms-vt"],
    },
    {
      ...
      explain: 'Дословно: «A <code>default</code> value of <code>ValueTask&lt;TResult&gt;</code> is equivalent to a synchronously-completed operation whose result is <code>default(TResult)</code>». То есть <code>default(ValueTask&lt;int&gt;)</code> — не «пустой/невалидный», а полноценный <span class="hl">уже-завершённый</span> результат со значением <code>default(int) == 0</code>. Прогон: <code>IsCompletedSuccessfully</code> → <b>True</b>, <code>Result</code> → <b>0</b> (печать «True 0»). Практическая польза: возвращать «синхронно готово, значение по умолчанию» из <code>ValueTask</code>-метода без единой аллокации — просто <code>return default;</code>.',
      sources: ["ms-vt"],
    },
    {
      ...
      code: ["typeof(ValueTask<int>).IsValueType   // True  — value type (struct)", "typeof(Task<int>).IsValueType        // False — reference (куча)", "ValueTask<int> instance fields = 4   // _obj / _result / _token / _flag"],
      ...
      explain: 'Машинная панель урока — струк­турная цена <code>ValueTask</code>, снятая рефлексией. Прогон: <code>typeof(ValueTask&lt;int&gt;).IsValueType</code> → <b>True</b>, <code>typeof(Task&lt;int&gt;).IsValueType</code> → <b>False</b>, число instance-полей <code>ValueTask&lt;int&gt;</code> → <b>4</b> (<code>_obj</code> — Task/источник, <code>_result</code> — значение, <code>_token</code>, <code>_continueOnCapturedContext</code>-флаг). Вот материальная основа трейдофа (verbatim): «<code>ValueTask&lt;TResult&gt;</code> is a structure with two fields, whereas <code>Task&lt;TResult&gt;</code> is a reference type, so using it means copying more data … a method that awaits it will result in a larger state machine». Отсюда правило: <code>ValueTask</code> экономит аллокацию на sync-пути, но платит копированием и размером — выигрыш реален только на подтверждённо горячем, обычно-синхронном коде.',
      sources: ["ms-vt"],
    },
  ],
  ...
  cards: [
    ...
    },
    {
      ...
      okText: '«A <code>default</code> value of <code>ValueTask&lt;TResult&gt;</code> is equivalent to a synchronously-completed operation whose result is <code>default(TResult)</code>»: завершён (<b>True</b>), <code>Result</code> == <code>default(int)</code> == <b>0</b>.',
    ...
    },
    {
      ...
      okText: '<code>ValueTask&lt;int&gt;</code> — <b>value type</b> (True) с <b>4 полями</b> (_obj/_result/_token/_flag), <code>Task&lt;int&gt;</code> — <b>reference</b> (False, 1 ссылка). Struct-ширина — корень трейдофа копирования.',
      noText: '«ValueTask is a structure … copying more data»; 4 instance-поля vs 1 ссылка у Task. Реальный вывод: <b>True False fields=4</b>.',
