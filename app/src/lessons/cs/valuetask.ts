...
export const valueTask: LessonData = {
  ...
  spec: [
    { text: "«A ValueTask<TResult> instance may only be awaited once, and consumers may not read Result until the instance has completed.» <span class=\"ru-tr\">«Экземпляр ValueTask<TResult> можно await только один раз, и потребители не могут читать Result, пока экземпляр не завершится.»</span> — экземпляр можно await ровно один раз; для повторного использования — AsTask() или Preserve().", source: "ms-vt" },
  ],
  edgeCases: [
    { text: "«should never» <span class=\"ru-tr\">«никогда не следует»</span> (иначе поведение UNDEFINED): await один экземпляр <b>несколько раз</b>; вызывать <code>AsTask</code> несколько раз; читать <code>.Result</code>/<code>.GetAwaiter().GetResult()</code> <b>до завершения</b> или повторно; <b>смешивать</b> способы потребления. Один экземпляр — один способ, один раз.", source: "ms-vt" },
    { text: "«The non generic version of ValueTask is not recommended for most scenarios» <span class=\"ru-tr\">«Не-обобщённая версия ValueTask не рекомендуется для большинства сценариев»</span> — брать почти всегда стоит <code>ValueTask&lt;T&gt;</code> либо <code>Task</code>, а не голый <code>ValueTask</code>.", source: "ms-vt" },
  ...
  misconceptions: [
    {
      ...
      hook: 'Три опасные ошибки разом. «<span class="wrong">ValueTask всегда быстрее</span>» — нет: это struct с <b>несколькими полями</b>, его возврат копирует больше, чем один ссылочный field у <code>Task</code>, и раздувает стейт-машину; выигрыш есть ТОЛЬКО когда результат обычно готов синхронно и метод в горячем цикле. «<span class="wrong">await сколько угодно</span>» — нет: «<span class="hl">may only be awaited once</span>» <span class="ru-tr">«можно await только один раз»</span>; повторно — undefined, нужен повтор → <code>AsTask()</code>. «<span class="wrong">.Result как у Task</span>» — нет: до завершения/повторно — undefined. Ниже <b>пять разборов</b>: что это, когда брать, «один раз» + AsTask, <code>default</code>, и <b>машинная панель</b> — value type в 4 поля vs reference.',
      source: "ms-vt",
    },
  ],
  ...
  segments: [
    ...
    },
    {
      ...
      explain: 'Точное условие (verbatim, valuetask-1 Remarks): «A method may return an instance of this value type when <span class="hl">it\'s likely that the result of its operation will be available synchronously</span>, and when it\'s <span class="hl">expected to be invoked so frequently</span> that the cost of allocating a new <code>Task&lt;TResult&gt;</code> for each call will be prohibitive» <span class="ru-tr">«Метод может возвращать экземпляр этого типа-значения, когда вероятно, что результат его операции будет доступен синхронно, и когда ожидается, что он будет вызываться настолько часто, что стоимость выделения нового <code>Task&lt;TResult&gt;</code> на каждый вызов окажется недопустимой»</span>. То есть два фактора ОДНОВРЕМЕННО: (1) результат часто готов синхронно, (2) метод в горячем цикле. Если хоть один не выполнен — берите <code>Task</code>. Прямая рекомендация: «the <span class="hl">default choice for any asynchronous method should be to return a <code>Task</code> or <code>Task&lt;TResult&gt;</code></span>. Only if performance analysis proves it worthwhile should a <code>ValueTask&lt;TResult&gt;</code> be used instead of a <code>Task&lt;TResult&gt;</code>» <span class="ru-tr">«выбором по умолчанию для любого асинхронного метода должно быть возвращение <code>Task</code> или <code>Task&lt;TResult&gt;</code>. Только если анализ производительности докажет, что это оправдано, вместо <code>Task&lt;TResult&gt;</code> следует использовать <code>ValueTask&lt;TResult&gt;</code>»</span>.',
      sources: ["ms-vt"],
    },
    {
      ...
      console: true,
      scenes: [
        { codeLine: 0, out: "", caption: 'Экземпляр <code>ValueTask</code> — «<span class="hl">may only be awaited once</span>» <span class="ru-tr">«можно await только один раз»</span>. Один инстанс = одно потребление.', nodes: [{ id: "o", kind: "gate", at: { zone: "once", row: 0 }, state: "ok", label: "await vt", detail: "первый раз OK", accent: true }], edges: [] },
      ...
      explain: 'Жёсткое правило потребления (verbatim, valuetask-1 Remarks): «A <code>ValueTask&lt;TResult&gt;</code> instance <span class="hl">may only be awaited once</span>, and consumers may not read <code>Result</code> until the instance has completed. If these limitations are unacceptable, convert the <code>ValueTask&lt;TResult&gt;</code> to a <code>Task&lt;TResult&gt;</code> by calling <code>AsTask</code>» <span class="ru-tr">«Экземпляр <code>ValueTask&lt;TResult&gt;</code> можно await только один раз, и потребители не могут читать <code>Result</code>, пока экземпляр не завершится. Если эти ограничения неприемлемы, преобразуйте <code>ValueTask&lt;TResult&gt;</code> в <code>Task&lt;TResult&gt;</code>, вызвав <code>AsTask</code>»</span>. Список «should never be performed» <span class="ru-tr">«никогда не следует выполнять»</span> (иначе «the results are undefined» <span class="ru-tr">«результаты не определены»</span>): «Awaiting the instance multiple times» <span class="ru-tr">«Await экземпляра несколько раз»</span>; «Calling <code>AsTask</code> multiple times» <span class="ru-tr">«Вызов <code>AsTask</code> несколько раз»</span>; «Using <code>.Result</code> or <code>.GetAwaiter().GetResult()</code> when the operation hasn\'t yet completed, or using them multiple times» <span class="ru-tr">«Использование <code>.Result</code> или <code>.GetAwaiter().GetResult()</code>, когда операция ещё не завершилась, либо их использование несколько раз»</span>; «Using more than one of these techniques to consume the instance» <span class="ru-tr">«Использование более одного из этих способов для потребления экземпляра»</span>. Практика (замер: <b>5 5</b>): <code>Load().AsTask()</code> даёт <code>Task&lt;int&gt;</code>, который <code>await</code>-ится дважды и оба раза даёт 5. Поэтому: <code>ValueTask</code> — «await один раз и забудь»; нужен повтор — сразу <code>AsTask()</code>.',
      sources: ["ms-vt"],
    },
    {
      ...
      explain: 'Дословно (valuetask-1 Remarks): «An instance created with the parameterless constructor or by the <code>default(ValueTask&lt;TResult&gt;)</code> syntax (a zero-initialized structure) represents a <span class="hl">synchronously, successfully completed operation with a result of <code>default(TResult)</code></span>» <span class="ru-tr">«Экземпляр, созданный конструктором без параметров или синтаксисом <code>default(ValueTask&lt;TResult&gt;)</code> (структура, инициализированная нулями), представляет синхронно и успешно завершённую операцию с результатом <code>default(TResult)</code>»</span>. То есть <code>default(ValueTask&lt;int&gt;)</code> — не «пустой/невалидный», а полноценный <span class="hl">уже-завершённый</span> результат со значением <code>default(int) == 0</code>. Прогон: <code>IsCompletedSuccessfully</code> → <b>True</b>, <code>Result</code> → <b>0</b> (печать «True 0»). Практическая польза: возвращать «синхронно готово, значение по умолчанию» из <code>ValueTask</code>-метода без единой аллокации — просто <code>return default;</code>.',
      sources: ["ms-vt"],
    },
    {
  ...
  cards: [
    ...
    },
    {
      ...
      okText: '<code>default(ValueTask&lt;TResult&gt;)</code> — «a zero-initialized structure» <span class="ru-tr">«структура, инициализированная нулями»</span> — «represents a synchronously, successfully completed operation with a result of <code>default(TResult)</code>» <span class="ru-tr">«представляет синхронно и успешно завершённую операцию с результатом <code>default(TResult)</code>»</span>: завершён (<b>True</b>), <code>Result</code> == <code>default(int)</code> == <b>0</b>.',
    ...
    },
    {
      ...
      noText: '«it also contains multiple fields, whereas a <code>Task&lt;TResult&gt;</code> as a reference type is a single field» <span class="ru-tr">«он также содержит несколько полей, тогда как <code>Task&lt;TResult&gt;</code> как ссылочный тип — это одно поле»</span>; 4 instance-поля vs 1 ссылка у Task. Реальный вывод: <b>True False fields=4</b>.',
  ...
  takeaways: [
    ...
    { icon: "avoid", k: "await один раз", v: 'Экземпляр — «<span class="hl">may only be awaited once</span>» <span class="ru-tr">«можно await только один раз»</span>; повторно/<code>.Result</code> до завершения — <b>undefined</b>. Нужен повтор — <code>AsTask()</code> (замер: 5 5). Non-generic <code>ValueTask</code> «is not recommended for most scenarios» <span class="ru-tr">«не рекомендуется для большинства сценариев»</span>.' },
