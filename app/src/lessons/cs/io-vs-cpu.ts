...
export const ioVsCpu: LessonData = {
  ...
  spec: [
    { text: "«I/O-bound code starts an operation represented by a `Task` or `Task<T>` object within the `async` method. CPU-bound code starts an operation on a background thread with the `Task.Run` method.»", source: "ms-scenarios" },
  ],
  edgeCases: [
    { text: "<b>Всегда замеряй</b>: «You might discover that your CPU-bound work isn't <span class=\"hl\">costly enough</span> compared with the overhead of context switches when multithreading. Every choice has tradeoffs».", source: "ms-scenarios" },
  ...
  segments: [
    {
      ...
      explain: 'Это машинная панель урока — решение о потоке, снятое по thread-id. Дословно: «I/O-bound code starts an operation represented by a <code>Task</code>… within the async method. CPU-bound code starts an operation <span class="hl">on a background thread with the <code>Task.Run</code></span> method». Прогон CPU-пути: работа в <code>Task.Run</code> идёт на ДРУГОМ потоке (<code>main != poolId</code> → True), и это поток пула (<code>IsThreadPoolThread</code> → True). А I/O-путь (<code>await GetStringAsync</code>) потока на время ожидания не берёт вообще. Отсюда правило: <code>Task.Run</code> — инструмент увести CPU-работу с текущего потока; для «просто подождать» он не нужен и вреден.',
    ...
    },
    {
      ...
      explain: 'CPU-bound — «дорого считаешь». Таблица: «<i>Should the code run an expensive computation?</i> → <b>CPU-bound</b> → Use the <code>async</code> modifier and <code>await</code>, but <span class="hl">spawn off the work on another thread with the <code>Task.Run</code></span> method… If the work is appropriate for concurrency and parallelism, also consider using the Task Parallel Library». Механика: «CPU-bound code starts an operation on a <b>background thread</b> with the <code>Task.Run</code> method». Прогон (разбор 01) это подтвердил: <code>Task.Run</code> уводит счёт на пул-поток, вызывающий поток свободен. Здесь <code>Task.Run</code> уместен — есть настоящая работа, которую надо снять с текущего потока.',
      sources: ["ms-scenarios"],
    },
    {
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: 'Если работа <b>недостаточно дорогая</b> — overhead многопоточности может съесть выигрыш. «Every choice has tradeoffs».', nodes: [{ id: "m", kind: "gate", at: { zone: "measure", row: 0 }, state: "ok", label: "работа мала", detail: "overhead > выигрыш" }, { id: "t", kind: "gate", at: { zone: "measure", row: 1 }, state: "fail", label: "tradeoff", detail: "context switch", accent: true }], edges: [] },
      ...
      explain: 'Выбор I/O vs CPU — не догма, а замер. Дословно: «<span class="hl">Always measure the execution of your code</span>. You might discover that your CPU-bound work isn\'t costly enough compared with the overhead of <b>context switches</b> when multithreading. Every choice has tradeoffs. Pick the correct tradeoff for your situation». То есть даже для формально CPU-bound работы <code>Task.Run</code> окупается не всегда: если вычисление дешёвое, стоимость переключения между потоками пула перевесит выигрыш. И родственный перф-намёк из этой же страницы: для горячих путей с частым синхронным завершением рассмотри <code>ValueTask</code> (<code>Task</code> — reference type, аллоцируется из кучи) — тема S2.5. Инженерное правило: не гадай — измеряй.',
      sources: ["ms-scenarios"],
    },
  ],
  ...
  cards: [
    {
      ...
      noText: '«CPU-bound code starts an operation on a background thread with Task.Run». Другой поток, поток пула → <b>True True</b>.',
    ...
    },
    {
      ...
      noText: '<code>Task.Run</code> «spawn off the work on another thread» — счёт идёт на пуле, не на вызывающем. Реальный вывод: <b>True</b>.',
    ...
    },
    {
      ...
      okText: 'Обёртка I/O в <code>Task.Run</code> НЕ ускоряет — время <span class="hl">то же</span> («same»): «a background thread doesn\'t help with a process that\'s just waiting». Лишний поток потрачен впустую.',
  ...
  takeaways: [
    ...
    { icon: "cost", k: "Task.Run НЕ ускоряет I/O", v: 'Для I/O потока на ожидание нет вообще — оборачивать нечего. Обёртка <code>Task.Run(Io)</code> = <span class="hl">то же время</span> (same) + лишний поток впустую. «a background thread doesn\'t help… just waiting».' },
