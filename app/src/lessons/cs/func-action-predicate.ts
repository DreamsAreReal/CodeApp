...
export const funcActionPredicate: LessonData = {
  ...
  spec: [
    { text: "«You can use this delegate to represent a method that can be passed as a parameter without explicitly declaring a custom delegate.» <span class=\"ru-tr\">«Этот делегат можно использовать, чтобы представить метод, который передаётся как параметр, без явного объявления собственного делегата.»</span>", source: "ms-func" },
  ],
  edgeCases: [
    { text: "<code>Func</code> обязан вернуть значение: «the encapsulated method must have one parameter that is passed to it by value, and that it must <b>return a value</b>». <span class=\"ru-tr\">«инкапсулируемый метод должен иметь один параметр, который передаётся ему по значению, и должен <b>вернуть значение</b>».</span>", source: "ms-func" },
    { text: "<code>Action</code> — только эффект, без результата: «it must <b>not return a value</b>. (In C#, the method must return <code>void</code>…)». <span class=\"ru-tr\">«он не должен <b>возвращать значение</b>. (В C# метод должен возвращать <code>void</code>…)».</span>", source: "ms-action" },
    { text: "<code>Predicate&lt;T&gt;</code> используют коллекции для поиска: «This delegate is used by several methods of the <code>Array</code> and <code>List&lt;T&gt;</code> classes to search for elements in the collection». <span class=\"ru-tr\">«Этот делегат используется несколькими методами классов <code>Array</code> и <code>List&lt;T&gt;</code> для поиска элементов в коллекции».</span>", source: "ms-predicate" },
  ],

  misconceptions: [
    {
      wrong: "под каждый callback надо объявлять свой delegate-тип",
      hook: 'Почти никогда — в BCL уже есть <b>обобщённые</b> семейства делегатов. «You can use this delegate to <span class="hl">represent a method that can be passed as a parameter without explicitly declaring a custom delegate</span>». <span class="ru-tr">«Этот делегат можно использовать, чтобы представить метод, который передаётся как параметр, без явного объявления собственного делегата».</span> Их три ключевых: <code>Func&lt;…,TResult&gt;</code> — «Encapsulates a method that has one parameter and <b>returns a value</b>» <span class="ru-tr">«Инкапсулирует метод, у которого один параметр и который <b>возвращает значение</b>»</span>; <code>Action&lt;T&gt;</code> — «Encapsulates a method that has a single parameter and <b>does not return a value</b>» <span class="ru-tr">«Инкапсулирует метод, у которого один параметр и который <b>не возвращает значения</b>»</span>; <code>Predicate&lt;T&gt;</code> — «Represents the method that defines a set of criteria and <b>determines whether the specified object meets those criteria</b>». <span class="ru-tr">«Представляет метод, который задаёт набор критериев и <b>определяет, отвечает ли указанный объект этим критериям</b>».</span> И лямбда сама компилируется в них: «The <b>underlying type of a lambda expression is one of the generic <code>Func</code> delegates</b>». <span class="ru-tr">«<b>Базовый тип лямбда-выражения — это один из обобщённых делегатов <code>Func</code></b>».</span> Дальше <b>шесть разборов</b>: карта семейств, <code>Func</code>=возврат, <code>Action</code>=void, <code>Predicate</code>=bool-фильтр, <b>машинная панель</b> — последний тип-аргумент <code>Func</code> это возврат (реальный прогон: арность <code>4</code>), и вариантность <code>in</code>/<code>out</code>.',
      source: "ms-func",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'BCL поставляет <b>обобщённые</b> делегаты, чтобы под callback не плодить свои типы. <code>Func&lt;T,TResult&gt;</code> — «Encapsulates a method that has one parameter and <b>returns a value</b> of the type specified by the <code>TResult</code> parameter» <span class="ru-tr">«Инкапсулирует метод, у которого один параметр и который <b>возвращает значение</b> типа, заданного параметром <code>TResult</code>»</span> (перегрузки принимают до 16 параметров). <code>Action&lt;T&gt;</code> — «Encapsulates a method that has a single parameter and <b>does not return a value</b>». <span class="ru-tr">«Инкапсулирует метод, у которого один параметр и который <b>не возвращает значения</b>».</span> <code>Predicate&lt;T&gt;</code> — «Represents the method that defines a set of criteria and <b>determines whether the specified object meets those criteria</b>». <span class="ru-tr">«Представляет метод, который задаёт набор критериев и <b>определяет, отвечает ли указанный объект этим критериям</b>».</span> Официальная развилка: «To reference a method that has one parameter and returns <code>void</code>… use the generic <code>Action&lt;T&gt;</code> delegate instead» <span class="ru-tr">«Чтобы сослаться на метод, у которого один параметр и который возвращает <code>void</code>… используйте вместо этого обобщённый делегат <code>Action&lt;T&gt;</code>»</span> и наоборот «To reference a method that has one parameter and returns a value, use the generic <code>Func&lt;T,TResult&gt;</code> delegate instead». <span class="ru-tr">«Чтобы сослаться на метод, у которого один параметр и который возвращает значение, используйте вместо этого обобщённый делегат <code>Func&lt;T,TResult&gt;</code>».</span>',
    ...
    },
    {
      ...
      explain: '<code>Func</code> — делегат для метода <b>с результатом</b>. Контракт дословно: «the encapsulated method must have one parameter that is passed to it by value, and that <b>it must return a value</b>». <span class="ru-tr">«инкапсулируемый метод должен иметь один параметр, который передаётся ему по значению, и <b>должен вернуть значение</b>».</span> Главная выгода — не плодить свои типы: «When you use the <code>Func&lt;T,TResult&gt;</code> delegate, you do not have to explicitly define a delegate that encapsulates a method with a single parameter». <span class="ru-tr">«Когда вы используете делегат <code>Func&lt;T,TResult&gt;</code>, вам не нужно явно определять делегат, инкапсулирующий метод с одним параметром».</span> И именно на <code>Func</code> опирается LINQ: «because many methods of types in the <code>System.Linq</code> namespace have <code>Func&lt;T,TResult&gt;</code> parameters, you can pass these methods a <b>lambda expression</b> without explicitly instantiating a <code>Func&lt;T,TResult&gt;</code> delegate». <span class="ru-tr">«поскольку многие методы типов из пространства имён <code>System.Linq</code> имеют параметры <code>Func&lt;T,TResult&gt;</code>, вы можете передавать этим методам <b>лямбда-выражение</b> без явного создания экземпляра делегата <code>Func&lt;T,TResult&gt;</code>».</span> Реальный прогон: <code>add(2,3)</code> → <code>5</code>.',
      sources: ["ms-func"],
    },
    {
      ...
      explain: '<code>Action</code> — зеркало <code>Func</code>: метод без результата. «Encapsulates a method that has a single parameter and <b>does not return a value</b>» <span class="ru-tr">«Инкапсулирует метод, у которого один параметр и который <b>не возвращает значения</b>»</span>, и контракт: «the encapsulated method must have one parameter that is passed to it by value, and it must <b>not return a value</b>. (In C#, the method must return <code>void</code>. In Visual Basic, it must be defined by the <code>Sub</code>…<code>End Sub</code> construct. It can also be a method that returns a value that is ignored.) <span class="hl">Typically, such a method is used to perform an operation</span>». <span class="ru-tr">«инкапсулируемый метод должен иметь один параметр, который передаётся ему по значению, и он не должен <b>возвращать значение</b>. (В C# метод должен возвращать <code>void</code>. В Visual Basic он должен быть определён конструкцией <code>Sub</code>…<code>End Sub</code>. Это также может быть метод, возвращающий значение, которое игнорируется.) Обычно такой метод используется для выполнения операции».</span> То есть <code>Action</code> берут для эффекта — логирование, мутация, <code>ForEach</code>: «The <code>ForEach</code>… methods each take an <code>Action&lt;T&gt;</code> delegate as a parameter. The method encapsulated by the delegate allows you to perform an action on each element». <span class="ru-tr">«Методы <code>ForEach</code>… каждый принимают делегат <code>Action&lt;T&gt;</code> в качестве параметра. Метод, инкапсулированный делегатом, позволяет выполнить действие над каждым элементом».</span> Реальный прогон: <code>log("hi")</code> → <code>log: hi</code>.',
      sources: ["ms-action"],
    },
    {
      ...
      explain: '<code>Predicate&lt;T&gt;</code> — специализированный делегат «да/нет»: «Represents the method that defines a set of criteria and <b>determines whether the specified object meets those criteria</b>». <span class="ru-tr">«Представляет метод, который задаёт набор критериев и <b>определяет, отвечает ли указанный объект этим критериям</b>».</span> Возврат — булев: «<code>true</code> if <code>obj</code> meets the criteria defined within the method represented by this delegate; otherwise, <code>false</code>». <span class="ru-tr">«<code>true</code>, если <code>obj</code> отвечает критериям, заданным в методе, который представляет этот делегат; иначе — <code>false</code>».</span> И под него заточены коллекции: «This delegate is used by several methods of the <code>Array</code> and <code>List&lt;T&gt;</code> classes to <span class="hl">search for elements in the collection</span>» <span class="ru-tr">«Этот делегат используется несколькими методами классов <code>Array</code> и <code>List&lt;T&gt;</code> для поиска элементов в коллекции»</span> (<code>Find</code>, <code>FindAll</code>, <code>Exists</code>, <code>RemoveAll</code>). По форме <code>Predicate&lt;T&gt;</code> = <code>Func&lt;T,bool&gt;</code>, но это <b>разные именованные типы</b> — LINQ берёт <code>Func&lt;T,bool&gt;</code>, а <code>List&lt;T&gt;.FindAll</code> — именно <code>Predicate&lt;T&gt;</code> (одну лямбду примут оба, а вот переменные напрямую не взаимозаменяемы). Реальный прогон: <code>2,4,6</code> и <code>2</code>.',
    ...
    },
    {
      ...
      explain: 'У <code>Func</code>/<code>Action</code>/<code>Predicate</code> тип-параметры <b>вариантны</b>, и это видно в их объявлениях (<code>in</code>/<code>out</code>). Параметр — контравариантен: «The type of the parameter of the method that this delegate encapsulates. This type parameter is <b>contravariant</b>. That is, you can use either the type you specified or <span class="hl">any type that is less derived</span>» <span class="ru-tr">«Тип параметра метода, который инкапсулирует этот делегат. Этот тип-параметр <b>контравариантен</b>. То есть можно использовать либо указанный вами тип, либо любой менее производный тип»</span> — потому <code>Action&lt;object&gt;</code> присваивается к <code>Action&lt;string&gt;</code> (метод, принимающий object, справится и со string). Возврат <code>Func</code> — ковариантен: «The type of the return value… This type parameter is <b>covariant</b>. That is, you can use either the type you specified or <span class="hl">any type that is more derived</span>». <span class="ru-tr">«Тип возвращаемого значения… Этот тип-параметр <b>ковариантен</b>. То есть можно использовать либо указанный вами тип, либо любой более производный тип».</span> Реальный прогон подтверждает присваивание и вызов: <code>printStr("hello")</code> → <code>hello</code>. Механика <code>in</code>/<code>out</code> целиком — в соседнем уроке о вариантности делегатов.',
  ...
  cards: [
    ...
    },
    {
      ...
      okText: '<code>Predicate&lt;int&gt;</code> = тест <b>bool</b>. <code>FindAll</code> оставляет все, где <code>true</code> → <span class="hl">2,4,6</span>; <code>Find</code> — первый подходящий → <b>2</b>. «used… to search for elements in the collection». <span class="ru-tr">«используется… для поиска элементов в коллекции».</span>',
  ...
  takeaways: [
    { icon: "why", k: "Три семейства", v: '<code>Func&lt;…,TResult&gt;</code> — метод с результатом, <code>Action&lt;…&gt;</code> — <b>void</b>, <code>Predicate&lt;T&gt;</code> — тест <b>→ bool</b>. «represent a method… <span class="hl">without explicitly declaring a custom delegate</span>». <span class="ru-tr">«представить метод… без явного объявления собственного делегата».</span> Лямбда сама компилируется в <code>Func</code>/<code>Action</code>.' },
    ...
    { icon: "avoid", k: "Вариантность", v: 'Параметр <code>in T</code> контравариантен: <code>Action&lt;object&gt;</code> присваивается к <code>Action&lt;string&gt;</code> (реальный прогон: <code>hello</code>). Возврат <code>Func</code> ковариантен (<code>out TResult</code>) — «any type that is more derived». <span class="ru-tr">«любой более производный тип».</span>' },
  ],
