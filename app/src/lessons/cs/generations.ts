...
export const generations: LessonData = {
  ...
  spec: [
    { text: "«the managed heap is divided into three generations, 0, 1, and 2, so it can handle long-lived and short-lived objects separately. The garbage collector stores new objects in generation 0.»", source: "ms-gc-fund" },
  ],
  edgeCases: [
    { text: "Сбор поколения захватывает и младшие (verbatim): «<span class=\"hl\">Collecting a generation means collecting objects in that generation and all its younger generations</span>. A generation 2 garbage collection is also known as a <b>full garbage collection</b> because it reclaims objects in all generations».", source: "ms-gc-fund" },
    { text: "Продвижение (verbatim): «Objects that survive a generation 0 garbage collection are <span class=\"hl\">promoted to generation 1</span>. … Objects that survive a generation 1 garbage collection are promoted to generation 2. … Objects that survive a generation 2 garbage collection remain in generation 2».", source: "ms-gc-fund" },
    { text: "Большие объекты — на LOH: «if they\'re large objects, they go on the large object heap (LOH), which is <span class=\"hl\">sometimes referred to as <i>generation 3</i></span>. Generation 3 is a physical generation that\'s <b>logically collected as part of generation 2</b>» (детали LOH — S7.5).", source: "ms-gc-fund" },
  ],

  misconceptions: [
    {
      wrong: "любой сбор мусора дорогой и надолго стопает приложение / gen0-сбор такой же тяжёлый, как полный",
      hook: 'Миф: «<span class="wrong">GC всегда дорого стопает приложение</span>» / «gen0-сбор такой же тяжёлый, как полный». Нет — вся идея поколений в обратном: «<span class="hl">It\'s faster to compact the memory for a portion of the managed heap than for the entire managed heap</span>». gen0 — самый частый и <b>дешёвый</b>: «Garbage collection occurs <span class="hl">most frequently in this generation</span>», и «Most objects are reclaimed… in generation 0 and don\'t survive to the next generation». Дорогой — только <b>full</b> gen2. Ниже <b>пять разборов</b>: три поколения, зачем они, продвижение, сбор поколения = оно + младшие (gen2 = full GC), и <b>машинная панель</b> — реальный замер продвижения и full-GC.',
      source: "ms-gc-fund",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Дословно: «To optimize the performance of the garbage collector, <span class="hl">the managed heap is divided into three generations, 0, 1, and 2</span>, so it can handle long-lived and short-lived objects separately. The garbage collector stores new objects in generation 0. Objects created early in the application\'s lifetime that survive collections are promoted and stored in generations 1 and 2». Модель: <b>gen0</b> — самый юный, короткоживущие (напр. временная переменная); <b>gen1</b> — буфер; <b>gen2</b> — долгожители. Это НЕ произвольное деление, а оптимизация: собирать часть кучи дешевле, чем всю (разбор 02). Замером подтверждается: <code>GC.MaxGeneration == 2</code> → ровно 3 поколения.',
      sources: ["ms-gc-fund"],
    },
    {
      ...
      explain: 'Основание модели (verbatim, раздел The GC algorithm is based on several considerations): «<span class="hl">It\'s faster to compact the memory for a portion of the managed heap than for the entire managed heap</span>. … Newer objects have shorter lifetimes, and older objects have longer lifetimes. … Newer objects tend to be related to each other and accessed by the application around the same time». Отсюда про gen0: «Garbage collection occurs <span class="hl">most frequently in this generation</span>» и «<span class="hl">Most objects are reclaimed for garbage collection in generation 0 and don\'t survive to the next generation</span>». Практический вывод (анти-миф MM3): рядовой gen0-сбор — дешёвый и частый; дорогая пауза — это <b>full</b> gen2 (разбор 04). Меньше короткоживущего мусора → меньше даже дешёвых gen0-сборов.',
      sources: ["ms-gc-fund"],
    },
    {
  ...
  cards: [
    {
      ...
      noText: '«Objects that survive a generation 0 garbage collection are promoted to generation 1». Реальный вывод: <b>gen0=0 afterCollect=1</b>.',
    ...
    },
    {
      ...
      okText: 'Heap разделён на <span class="hl">три поколения — 0, 1, 2</span>; старшее — <code>GC.MaxGeneration == 2</code>. Печать: <b>maxGen=2 generations=3</b>. LOH иногда зовут «generation 3», но физически, логически — часть gen2.',
      noText: '«divided into three generations, 0, 1, and 2»; MaxGeneration=2. Реальный вывод: <b>maxGen=2 generations=3</b>.',
    ...
    },
    {
      ...
      okText: '<code>GC.Collect(2)</code> — <b>full GC</b>: «collecting… that generation and <span class="hl">all its younger generations</span>» → счётчики и gen0, и gen2 растут. Печать: <b>gen0Bumped=True gen2Bumped=True</b>.',
  ...
  takeaways: [
    { icon: "why", k: "три поколения", v: 'Heap = <span class="hl">gen0/gen1/gen2</span> (<code>MaxGeneration==2</code>). Новые → gen0; пережил сбор → промоушен в следующее (замер: gen0→gen1). LOH иногда зовут «generation 3» — физически отдельно, логически с gen2.' },
    { icon: "cost", k: "gen0 дёшев, full дорог", v: 'Идея: «It\'s faster to compact the memory for a portion of the managed heap than for the entire managed heap». gen0 — «most frequently», дёшево; дорогая пауза — <b>full</b> gen2. Миф «GC всегда дорого» неверен.' },
