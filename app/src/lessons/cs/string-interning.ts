...
export const stringInterning: LessonData = {
  ...
  spec: [
    { text: "«The common language runtime maintains a table, called the intern pool, that holds a single reference for each unique string value.»", source: "ms-intern" },
  ],
  edgeCases: [
    { text: "Два равных <b>литерала</b> ссылаются на один объект в пуле: «each variable is set to reference the <span class=\"hl\">same constant in the intern pool</span> instead of referencing several different instances of String that have identical values». Собственный прогон: <code>ReferenceEquals</code> двух литералов = <code>True</code>.", source: "ms-isinterned" },
    { text: "<code>String.Intern</code> возвращает ссылку из пула: «The system's reference to <code>str</code>, if it is interned; otherwise, a new reference to a string with the value of <code>str</code>». Собственный прогон: построенная копия ≠ литерал (<code>False</code>), но <code>Intern(copy)</code> == литерал (<code>True</code>).", source: "ms-intern" },
    { text: "<code>String.IsInterned</code> только <b>проверяет</b>, не добавляя: «If <code>str</code> has already been interned, a reference to that instance is returned; otherwise, <code>null</code> is returned». Собственный прогон: значение, собранное в рантайме, не в пуле → <code>null</code>.", source: "ms-isinterned" },
    { text: "Автоинтернирование литералов <b>не гарантировано</b>: «automatic interning of string literals isn't guaranteed—depending on how the assembly was compiled and executed, some literals might not be added to the pool».", source: "ms-intern" },
  ],
  ...
  cards: [
    {
      ...
      noText: 'Пул держит «single reference for each unique string value». Литералы совпадают по ссылке, <code>new string</code> — нет, а <code>Intern</code> возвращает пуловую ссылку. Реальный вывод: <b>True / False / True</b>.',
    ...
    },
    {
      ...
      noText: '«otherwise, null is returned»: <code>IsInterned</code> только проверяет. Литерал в пуле, рантайм-значение нет. Реальный вывод: <b>True / True / 1000-runtime</b>.',
    ...
    },
    {
      ...
      noText: '<code>new string</code> — отдельный объект (False). <code>Intern</code> «search for a string equal to the value of str» и вернул пуловую ссылку == <code>x</code>. Реальный вывод: <b>False / True</b>.',
