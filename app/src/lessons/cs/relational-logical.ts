...
export const relationalLogical: LessonData = {
  ...
  spec: [
    { text: "«Use a relational pattern to compare an expression result with a constant, as the following example shows:» <span class=\"ru-tr\">«Используйте relational-паттерн, чтобы сравнить результат выражения с константой, как показано в следующем примере:»</span>", source: "ms-patterns" },
    { text: "«The pattern combinators check expressions in this order, based on the binding order of expressions:» <span class=\"ru-tr\">«Комбинаторы паттернов проверяют выражения в этом порядке, исходя из порядка связывания выражений:»</span> (not, and, or)", source: "ms-patterns" },
  ],
  edgeCases: [
  ...
  segments: [
    {
      ...
      explain: 'Relational pattern — сравнение с константой на языке паттернов: «Use a relational pattern to <b>compare an expression result with a constant</b>… In a relational pattern, use any of the relational operators <code>&lt;</code>, <code>&gt;</code>, <code>&lt;=</code>, or <code>&gt;=</code>. The right-hand part of a relational pattern must be a <b>constant expression</b>. The constant expression can be of an integer, floating-point, char, or enum type». <span class="ru-tr">«Используйте relational-паттерн, чтобы <b>сравнить результат выражения с константой</b>… В relational-паттерне используйте любой из relational-операторов <code>&lt;</code>, <code>&gt;</code>, <code>&lt;=</code> или <code>&gt;=</code>. Правая часть relational-паттерна должна быть <b>константным выражением</b>. Константное выражение может быть целочисленного типа, типа с плавающей точкой, char или enum».</span> Ограничение на null/конверсию: «If an expression result is <code>null</code> or fails to convert… a relational pattern <span class="hl">doesn\'t match</span>». <span class="ru-tr">«Если результат выражения — <code>null</code> или не удаётся преобразовать… relational-паттерн не совпадает».</span> Отсюда классическая ловушка <code>NaN</code> — ни одно <code>&lt;/&gt;</code> его не поймает.',
      sources: ["ms-patterns"],
    },
    {
      ...
      explain: 'Диапазон в паттернах собирается через conjunctive <code>and</code>: «To check if an expression result is in a certain range, match it against a <b>conjunctive <code>and</code> pattern</b>». <span class="ru-tr">«Чтобы проверить, попадает ли результат выражения в определённый диапазон, сопоставьте его с <b>конъюнктивным паттерном <code>and</code></b>».</span> Дословно про сам комбинатор: «<b>Conjunctive</b> <code>and</code> pattern that matches an expression when <span class="hl">both patterns match</span> the expression». <span class="ru-tr">«<b>Конъюнктивный</b> паттерн <code>and</code>, который совпадает с выражением, когда с ним совпадают оба паттерна».</span> Так <code>&gt;= 10.0 and &lt; 20.0</code> — это полуинтервал [10, 20); <code>Classify(13)</code> в него попадает → <code>High</code>. <code>and</code> здесь — не булев оператор над bool, а комбинатор над <b>паттернами</b>.',
      sources: ["ms-patterns"],
    },
    {
      ...
      explain: 'Два оставшихся комбинатора. Disjunctive: «<b>Disjunctive</b> <code>or</code> pattern that matches an expression when <span class="hl">either pattern matches</span> the expression». <span class="ru-tr">«<b>Дизъюнктивный</b> паттерн <code>or</code>, который совпадает с выражением, когда с ним совпадает любой из паттернов».</span> Negation: «<b>Negation</b> <code>not</code> pattern that matches an expression when the negated pattern <span class="hl">doesn\'t match</span> the expression». <span class="ru-tr">«<b>Отрицающий</b> паттерн <code>not</code>, который совпадает с выражением, когда негируемый паттерн с ним не совпадает».</span> Их можно применять сколько нужно: «you can use the pattern combinators <b>repeatedly</b> in a pattern». <span class="ru-tr">«вы можете использовать комбинаторы паттернов <b>многократно</b> в одном паттерне».</span> Скобки — для группировки: <code>not (float or double)</code> отрицает объединение. Но у комбинаторов есть неочевидный порядок связывания — следующий разбор.',
      sources: ["ms-patterns"],
    },
    {
      ...
      scenes: [
        { codeLine: 1, caption: '<code>not</code> «binds to its operand <span class="hl">first</span>» <span class="ru-tr">«связывается со своим операндом первым»</span> — прежде всех прочих комбинаторов.', nodes: [{ id: "n", kind: "gate", at: { zone: "bindz", row: 0 }, state: "ok", label: "1. not", detail: "к операнду", accent: true }], edges: [] },
      ...
      explain: 'Порядок связывания комбинаторов — фиксированный и не «слева направо»: «The pattern combinators check expressions in this order, based on the binding order of expressions: <b>not</b>, <b>and</b>, <b>or</b>. The <code>not</code> pattern binds to its operand <span class="hl">first</span>. The <code>and</code> pattern binds <b>after</b> any <code>not</code> pattern expression binding. The <code>or</code> pattern binds <b>after all</b> <code>not</code> and <code>and</code> patterns bind to operands». <span class="ru-tr">«Комбинаторы паттернов проверяют выражения в этом порядке, исходя из порядка связывания выражений: <b>not</b>, <b>and</b>, <b>or</b>. Паттерн <code>not</code> связывается со своим операндом <b>первым</b>. Паттерн <code>and</code> связывается <b>после</b> связывания любого выражения с паттерном <code>not</code>. Паттерн <code>or</code> связывается <b>после всех</b> связываний паттернов <code>not</code> и <code>and</code> с операндами».</span> Есть и оговорка про равный порядок: «The order in which the compiler checks patterns that have the same binding order is <b>undefined</b>». <span class="ru-tr">«Порядок, в котором компилятор проверяет паттерны с одинаковым порядком связывания, <b>не определён</b>».</span> Именно из этого правила растёт ловушка следующего разбора.',
      sources: ["ms-patterns"],
    },
    {
      ...
      explain: 'Это машинная панель урока — ловушка приоритета, снятая прогоном. Дока приводит именно этот баг: «The following example tries to match all characters that aren\'t lowercase letters… It has an <b>error</b>, because the <code>not</code> pattern binds before the <code>and</code> pattern» <span class="ru-tr">«Следующий пример пытается сопоставить все символы, которые не являются строчными буквами… В нём есть <b>ошибка</b>, потому что паттерн <code>not</code> связывается раньше паттерна <code>and</code>».</span> — <code>c is not &gt;= \'a\' and &lt;= \'z\'</code> «is parsed as… <code>c is ((not &gt;= \'a\') and &lt;= \'z\')</code>». <span class="ru-tr">«разбирается как… <code>c is ((not &gt;= \'a\') and &lt;= \'z\')</code>».</span> Починка — скобки: «To fix the error, specify that you want the <code>not</code> pattern to bind to the <code>&gt;= \'a\' and &lt;= \'z\'</code> expression» <span class="ru-tr">«Чтобы исправить ошибку, укажите, что вы хотите, чтобы паттерн <code>not</code> связывался с выражением <code>&gt;= \'a\' and &lt;= \'z\'</code>».</span> → <code>c is not (&gt;= \'a\' and &lt;= \'z\')</code>. Прогон на <code>\'{\'</code> (сразу за <code>\'z\'</code>) обнажает разницу: <code>Wrong=False</code>, <code>Right=True</code>. Мораль доки: «use parentheses to <span class="hl">clarify your patterns</span>». <span class="ru-tr">«используйте скобки, чтобы уточнить свои паттерны».</span>',
      sources: ["ms-patterns"],
    },
  ],
