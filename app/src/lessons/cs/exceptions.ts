...
export const exceptions: LessonData = {
  ...
  spec: [
    { text: "«AggregateException cannot be explicitly caught when using await task» <span class=\"ru-tr\">«AggregateException нельзя явно поймать при использовании await task»</span> — await разворачивает и бросает само (первое) внутреннее исключение, а не AggregateException.", source: "ms-tap" },
  ],
  edgeCases: [
  ...
  cards: [
    {
      ...
      noText: '«AggregateException cannot be caught when using await» <span class="ru-tr">«AggregateException нельзя поймать при использовании await»</span>: await разворачивает, .Wait оборачивает. Реальный вывод: <b>await=InvalidOperationException wait=AggregateException</b>.',
