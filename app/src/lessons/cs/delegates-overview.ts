...
export const delegatesOverview: LessonData = {
  ...
  spec: [
    { text: "«A delegate is a type that represents references to methods with a particular parameter list and return type.»", source: "ms-delegates" },
  ],
  edgeCases: [
    { text: "Делегат — не сам метод, а <b>ссылка</b> на него: «When you instantiate a delegate, you can associate the delegate instance with any method that has a compatible signature and return type. You can invoke (or call) the method through the delegate instance».", source: "ms-delegates" },
    { text: "Метод может быть <b>любым</b> совместимым: «You can assign any method from any accessible class or struct that matches the delegate type to the delegate. The method can be either static or an instance method».", source: "ms-delegates" },
    { text: "В контексте делегата сигнатура <b>включает возврат</b>: «in the context of delegates, the signature does include the return value» — метод обязан иметь совместимый тип возврата с делегатом.", source: "ms-delegates" },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Делегат существует ради <b>позднего связывания</b>: «Delegates provide a <b>late binding</b> mechanism in .NET. Late Binding means that you create an algorithm where the caller also supplies at least one method that implements part of the algorithm». Классический пример из доков — сортировка: «the Sort() method does essentially the same thing: arranges the items in the list based on some comparison. The code that compares two stars is different for each of the sort orderings». То есть каркас алгоритма один, а «сменную деталь» — метод сравнения — подставляет вызывающий. Делегат — это первоклассная языковая поддержка этой идеи: «The C# language delegate concept provides first class language support, and type safety around the concept».',
      sources: ["ms-overview"],
    },
    {
      ...
      explain: 'Ключевое: делегат — это <b>тип</b>, а его значение — ссылка на метод. Дословно: «A <b>delegate</b> is a type that represents references to methods with a particular parameter list and return type. When you instantiate a delegate, you can <span class="hl">associate the delegate instance with any method</span> that has a compatible signature and return type. You can <b>invoke (or call) the method through the delegate instance</b>». То есть <code>op = Twice</code> связывает переменную с методом, а <code>op(21)</code> вызывает метод через неё. Метод может быть любым совместимым: «You can assign any method from any accessible class or struct that matches the delegate type to the delegate. The method can be either static or an instance method». Именно это делает делегаты идеальными для callback\'ов: «The ability to refer to a method as a parameter makes delegates ideal for defining callback methods».',
  ...
  cards: [
    {
      ...
      noText: 'Делегат — <b>ссылка</b> на метод, которую можно подменить: «programmatically change method calls». Тот же код вызова, другой результат. Реальный вывод: <code>42</code>, затем <code>121</code>.',
  ...
  takeaways: [
    { icon: "why", k: "Что это", v: 'Делегат — <span class="hl">тип, ссылающийся на метод</span> с заданной сигнатурой: «a type that represents references to methods with a particular parameter list and return type». Вызов метода идёт <b>через</b> делегат; это механизм <i>позднего связывания</i>.' },
    { icon: "cost", k: "Типобезопасность", v: 'В отличие от сырого указателя на функцию, делегат <b>типобезопасен</b>: «The compiler ensures that the types match for arguments and return types». В контексте делегата сигнатура <span class="hl">включает тип возврата</span> — несовместимый метод не присвоится.' },
