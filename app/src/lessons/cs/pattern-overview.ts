...
export const patternOverview: LessonData = {
  ...
  spec: [
    { text: "«Pattern matching is a technique where you test an expression to determine if it has certain characteristics.» <span class=\"ru-tr\">«Сопоставление с образцом — это приём, при котором вы проверяете выражение, чтобы определить, обладает ли оно определёнными характеристиками.»</span>", source: "ms-pm-overview" },
    { text: "«The result of a switch expression is the value of the expression of the first switch expression arm whose pattern matches the input expression and whose case guard, if present, evaluates to true. The switch expression arms are evaluated in text order.» <span class=\"ru-tr\">«Результат switch-выражения — это значение выражения первого рукава switch-выражения, чей образец совпадает с входным выражением и чей охранник case, если он присутствует, вычисляется в true. Рукава switch-выражения вычисляются в порядке текста.»</span>", source: "ms-switch-expr" },
  ],
  edgeCases: [
    ...
    { text: "<code>is not null</code> вместо <code>!= null</code>: «because you're not using the <code>==</code> operator, this pattern works when a type overloads the <code>==</code> operator». <span class=\"ru-tr\">«поскольку вы не используете оператор <code>==</code>, этот образец работает, когда тип перегружает оператор <code>==</code>».</span>", source: "ms-pm-overview" },
    { text: "Пропустишь <code>_</code>-рукав — компилятор предупреждает «that your pattern expression doesn't handle all possible input values» <span class=\"ru-tr\">«что ваше образцовое выражение не обрабатывает все возможные входные значения»</span>; на непокрытом входе рантайм бросает <code>SwitchExpressionException</code>.", source: "ms-pm-overview" },
  ],

  misconceptions: [
    {
      wrong: "pattern matching — это синтаксический сахар над if/switch, ничего нового под ним нет",
      hook: 'Под pattern matching — не «сахар», а <span class="hl">машина проверки по признакам</span> плюс гарантия компилятора. Дословно: «Pattern matching is a technique where you test an expression to determine if it has <b>certain characteristics</b>». <span class="ru-tr">«Сопоставление с образцом — это приём, при котором вы проверяете выражение, чтобы определить, обладает ли оно <b>определёнными характеристиками</b>».</span> <code>is</code>-выражение тестирует И объявляет переменную одним шагом; <code>switch</code>-выражение выбирает <b>первый совпавший</b> паттерн. А ниже абстракции — <span class="hl">exhaustiveness</span>: полный <code>switch</code> компилятор превращает в гарантию, а неполный <b>бросает <code>SwitchExpressionException</code> в рантайме</b> (реальный прогон). Дальше <b>пять разборов</b> — от null-паттернов до <b>машинной панели</b> exhaustiveness.',
  ...
  segments: [
    {
      ...
      explain: 'Pattern matching — это тест по признакам, а не по значению: «Pattern matching is a technique where you test an expression to determine if it has certain characteristics». <span class="ru-tr">«Сопоставление с образцом — это приём, при котором вы проверяете выражение, чтобы определить, обладает ли оно определёнными характеристиками».</span> Две конструкции его несут: «The "<code>is</code> expression" supports pattern matching to test an expression and conditionally declare a new variable to the result of that expression. The "<code>switch</code> expression" enables you to perform actions based on the <b>first matching pattern</b> for an expression». <span class="ru-tr">«<code>is</code>-выражение поддерживает сопоставление с образцом, чтобы проверить выражение и условно объявить новую переменную под результат этого выражения. <code>switch</code>-выражение позволяет выполнять действия на основе <b>первого совпавшего образца</b> для выражения».</span> Обе опираются на один словарь: «These two expressions support a rich vocabulary of patterns». <span class="ru-tr">«Эти два выражения поддерживают богатый словарь образцов».</span> Дальше — как этот словарь ложится на null-проверки, порядок рукавов и exhaustiveness.',
    ...
    },
    {
      ...
      explain: 'Самый частый сценарий pattern matching — гарантировать, что значение не <code>null</code>: «One of the most common scenarios for pattern matching is to ensure values aren\'t <code>null</code>». <span class="ru-tr">«Один из самых частых сценариев сопоставления с образцом — убедиться, что значения не равны <code>null</code>».</span> Здесь работает declaration pattern: «The preceding code is a declaration pattern to test the type of the variable, and assign it to a new variable». <span class="ru-tr">«Приведённый выше код — это declaration-образец, чтобы проверить тип переменной и присвоить её новой переменной».</span> И это <b>безопаснее</b> обычной проверки: «The variable <code>number</code> is only accessible and assigned in the <span class="hl">true portion</span> of the <code>if</code> clause. If you try to access it elsewhere… the compiler issues an error». <span class="ru-tr">«Переменная <code>number</code> доступна и присвоена только в true-части <code>if</code>-конструкции. Если вы попытаетесь обратиться к ней в другом месте… компилятор выдаёт ошибку».</span> Реальный вывод при <code>maybe = 12</code> — <code>has value 12</code>.',
      sources: ["ms-pm-overview"],
    },
    {
      ...
      explain: 'Проверка «не null» через паттерн собирается из двух: «The preceding example used a <b>constant pattern</b> to compare the variable to <code>null</code>. The <code>not</code> is a <b>logical pattern</b> that matches when the negated pattern doesn\'t match». <span class="ru-tr">«В предыдущем примере использовался <b>constant-образец</b>, чтобы сравнить переменную с <code>null</code>. <code>not</code> — это <b>logical-образец</b>, который совпадает, когда отрицаемый образец не совпал».</span> Почему это лучше <code>!= null</code>: «because you\'re not using the <code>==</code> operator, this pattern works when a type <span class="hl">overloads the <code>==</code> operator</span>. That makes it an ideal way to check null reference values, adding the <code>not</code> pattern». <span class="ru-tr">«поскольку вы не используете оператор <code>==</code>, этот образец работает, когда тип перегружает оператор <code>==</code>. Это делает его идеальным способом проверять ссылочные значения на null, добавляя образец <code>not</code>».</span> То есть <code>is not null</code> — не стилистика, а обход возможной перегрузки равенства.',
      sources: ["ms-pm-overview"],
    },
    {
      ...
      explain: 'Механика <code>switch</code>-выражения точная: «The result of a <code>switch</code> expression is the value of the expression of the <b>first switch expression arm</b> whose pattern matches the input expression and whose case guard, if present, evaluates to <code>true</code>. The <code>switch</code> expression arms are evaluated in <span class="hl">text order</span>». <span class="ru-tr">«Результат <code>switch</code>-выражения — это значение выражения <b>первого рукава switch-выражения</b>, чей образец совпадает с входным выражением и чей охранник case, если он присутствует, вычисляется в <code>true</code>. Рукава <code>switch</code>-выражения вычисляются в порядке текста».</span> Тут <code>(> 32) and (< 212)</code> — «the conjunctive <code>and</code> logical pattern to check that both relational patterns match». <span class="ru-tr">«конъюнктивный logical-образец <code>and</code>, чтобы проверить, что оба relational-образца совпадают».</span> <code>WaterState(100)</code> ловит второй рукав → <code>liquid</code>. Порядок рукавов — часть смысла: верхний матч закрывает нижние.',
    ...
    },
    {
      ...
      explain: 'Это машинная панель урока — exhaustiveness, снятая реальным прогоном. Без <code>_</code>-рукава на непокрытом входе: «If none of a <code>switch</code> expression\'s patterns matches an input value, the runtime throws an exception. In .NET Core 3.0 and later versions, the exception is a <b>System.Runtime.CompilerServices.SwitchExpressionException</b>». <span class="ru-tr">«Если ни один из образцов <code>switch</code>-выражения не совпадает со входным значением, рантайм бросает исключение. В .NET Core 3.0 и более поздних версиях это исключение — <b>System.Runtime.CompilerServices.SwitchExpressionException</b>».</span> Прогон подтверждает: <code>Name(3)</code> без <code>_</code> → <code>threw: SwitchExpressionException</code>. Лечит это <b>discard pattern</b>: «To guarantee that a <code>switch</code> expression handles all possible input values, provide a <code>switch</code> expression arm with a <span class="hl">discard pattern</span>» <span class="ru-tr">«Чтобы гарантировать, что <code>switch</code>-выражение обрабатывает все возможные входные значения, добавьте рукав <code>switch</code>-выражения с discard-образцом».</span> — с <code>_ => "other"</code> тот же <code>Name(3)</code> печатает <code>other</code>. Exhaustiveness — это контракт, который компилятор проверяет, а рантайм охраняет.',
