...
export const plinq: LessonData = {
  ...
  spec: [
    { text: "«Parallel LINQ (PLINQ) is a parallel implementation of the Language-Integrated Query (LINQ) pattern. PLINQ implements the full set of LINQ standard query operators as extension methods for the System.Linq namespace and has additional operators for parallel operations.»", source: "ms-plinq" },
  ],
  edgeCases: [
    { text: "Партиционирование: «PLINQ attempts to make full use of all the processors on the system. It does this by <span class=\"hl\">partitioning the data source into segments</span>, and then executing the query on each segment on <b>separate worker threads</b> in parallel».", source: "ms-plinq" },
    { text: "Консервативен: «By default, PLINQ is conservative… If it is <b>not safe to parallelize</b> a query, PLINQ just runs the query sequentially. If PLINQ has a choice between a potentially expensive parallel algorithm or an inexpensive sequential algorithm, it <span class=\"hl\">chooses the sequential algorithm by default</span>».", source: "ms-plinq" },
    { text: "Порядок стоит денег: «An <code>AsOrdered</code> sequence is still processed in parallel, but its results are <span class=\"hl\">buffered and sorted</span>… an <code>AsOrdered</code> sequence might be processed <b>more slowly</b> than the default AsUnordered sequence».", source: "ms-plinq" },
  ],
  ...
  segments: [
    {
      ...
      explain: 'PLINQ — параллельная реализация той же модели запросов. Дословно: «<span class="hl">Parallel LINQ (PLINQ) is a parallel implementation of the Language-Integrated Query (LINQ) pattern</span>. PLINQ implements the full set of LINQ standard query operators as extension methods for the <code>System.Linq</code> namespace and has <b>additional operators for parallel operations</b>». Вход — <code>AsParallel()</code>: «The <code>AsParallel</code> extension method <b>binds the subsequent query operators</b>… to the <code>System.Linq.ParallelEnumerable</code> implementations». И как обычный LINQ, «PLINQ queries, just like sequential LINQ queries… have <b>deferred execution</b>, which means they do not begin executing until the query is enumerated». То есть синтаксис прежний, меняется исполнитель — <code>ParallelEnumerable</code>.',
      sources: ["ms-plinq"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Порядок в параллельном мире — не бесплатен. Дословно: «In some queries, a query operator must produce results that preserve the ordering of the source sequence. PLINQ provides the <code>AsOrdered</code> operator for this purpose. <code>AsOrdered</code> is distinct from <code>AsSequential</code>. An <span class="hl">AsOrdered sequence is still processed in parallel, but its results are buffered and sorted</span>. Because order preservation typically involves <b>extra work</b>, an <code>AsOrdered</code> sequence might be processed <b>more slowly</b> than the default <code>AsUnordered</code> sequence». Важно: <code>AsOrdered</code> ≠ <code>AsSequential</code> — первое сохраняет параллелизм, второе делает участок однопоточным. Машинная панель покажет, что <code>AsOrdered</code> реально даёт исходный порядок (20,40,…,200).',
      sources: ["ms-plinq"],
    },
    {
  ...
  cards: [
    {
      ...
      noText: '<code>AsOrdered</code> «is still processed in parallel, but its results are buffered and sorted» — порядок источника восстановлен. Чётные ×10: <b>20,40,60,80,100,120,140,160,180,200</b>.',
