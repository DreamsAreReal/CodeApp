...
export const memoryGuidelines: LessonData = {
  ...
  spec: [
    { text: "«Because these types can only be stored on the stack, they\'re unsuitable for scenarios such as asynchronous method calls. To address this problem, .NET 2.1 added some additional types, including Memory<T>, ReadOnlyMemory<T>, IMemoryOwner<T>, and MemoryPool<T>.» <span class=\"ru-tr\">«Поскольку эти типы могут храниться только в стеке, они непригодны для сценариев вроде асинхронных вызовов методов. Чтобы решить эту проблему, .NET 2.1 добавил несколько дополнительных типов, включая Memory<T>, ReadOnlyMemory<T>, IMemoryOwner<T> и MemoryPool<T>.»</span>", source: "ms-memguide" },
  ],
  edgeCases: [
    { text: "Три концепта (verbatim): «<b>Ownership</b>. The owner of a buffer instance is responsible for lifetime management… <span class=\"hl\">All buffers have a single owner</span>… Ownership can also be transferred». <span class=\"ru-tr\">«<b>Ownership</b> (владение). Владелец экземпляра буфера отвечает за управление временем жизни… <span class=\"hl\">У всех буферов есть единственный владелец</span>… Владение также можно передать».</span> «<b>Consumption</b>… <span class=\"hl\">Buffers can have one consumer at a time unless some external synchronization mechanism is provided</span>». <span class=\"ru-tr\">«<b>Consumption</b> (потребление)… <span class=\"hl\">У буфера может быть один потребитель за раз, если не предоставлен внешний механизм синхронизации</span>».</span> «<b>Lease</b>. The lease is the length of time that a particular component is allowed to be the consumer of the buffer». <span class=\"ru-tr\">«<b>Lease</b> (аренда). Аренда — это отрезок времени, в течение которого конкретному компоненту разрешено быть потребителем буфера».</span>", source: "ms-memguide" },
    { text: "Аренда void/Task (verbatim, Rule #3/#4): «If your method accepts <code>Memory<T></code> and returns <code>void</code>, you <span class=\"hl\">must not use the Memory<T> instance after your method returns</span>» <span class=\"ru-tr\">«Если ваш метод принимает <code>Memory<T></code> и возвращает <code>void</code>, вы <span class=\"hl\">не должны использовать экземпляр Memory<T> после того, как метод вернул управление</span>»</span> и «…returns a Task, you <span class=\"hl\">must not use the Memory<T> instance after the Task transitions to a terminal state</span>». <span class=\"ru-tr\">«…возвращает Task, вы <span class=\"hl\">не должны использовать экземпляр Memory<T> после того, как Task перешёл в терминальное состояние</span>».</span>", source: "ms-memguide" },
    { text: "IMemoryOwner (verbatim, Rule #7/#8): «If you have an <code>IMemoryOwner<T></code> reference, you must at some point <span class=\"hl\">dispose of it or transfer its ownership (but not both)</span>»; <span class=\"ru-tr\">«Если у вас есть ссылка <code>IMemoryOwner<T></code>, вы в какой-то момент обязаны <span class=\"hl\">освободить её или передать её владение (но не то и другое)</span>»;</span> «If you have an <code>IMemoryOwner<T></code> parameter in your API surface, you are <b>accepting ownership</b> of that instance». <span class=\"ru-tr\">«Если у вас есть параметр <code>IMemoryOwner<T></code> в поверхности вашего API, вы <b>принимаете владение</b> этим экземпляром».</span> MemoryPool<T>.Rent → «The caller becomes the owner… and is responsible for disposing». <span class=\"ru-tr\">«Вызывающий становится владельцем… и отвечает за освобождение».</span>", source: "ms-memguide" },
  ],

  misconceptions: [
    {
      wrong: "Memory<T> — то же, что Span<T>; можно свободно конвертировать туда-обратно и держать без правил владения",
      hook: 'Мифы про Memory. <span class="wrong">Memory = Span</span> — нет: <code>Span&lt;T&gt;</code> «<span class="hl">can only be stored on the stack</span>» <span class="ru-tr">«<span class="hl">может храниться только в стеке</span>»</span> → непригоден для async; <code>Memory&lt;T&gt;</code> — на куче, годен для async. «<span class="wrong">конвертировать туда-обратно</span>» — нет: <code>Memory&lt;T&gt;.Span</code> есть, а «<span class="hl">Span<T>-to-Memory<T> conversion isn\'t possible</span>» <span class="ru-tr">«<span class="hl">конверсия Span<T> в Memory<T> невозможна</span>»</span> (потому Rule #1: в sync-API бери Span). «<span class="wrong">без правил владения</span>» — нет: есть <b>ownership/consumption/lease</b> и аренда (void/Task). Ниже <b>пять разборов</b>: зачем Memory, три концепта, Rule #1, аренда + IMemoryOwner, и <b>машинная панель</b> — реальный замер.',
      source: "ms-memguide",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, caption: '<code>Span&lt;T&gt;</code> «<span class="hl">can only be stored on the stack</span>» <span class="ru-tr">«<span class="hl">может храниться только в стеке</span>»</span> → «unsuitable for… asynchronous method calls». <span class="ru-tr">«непригоден для… асинхронных вызовов методов».</span>', nodes: [{ id: "s", kind: "gate", at: { zone: "stack", row: 0 }, state: "fail", label: "Span", detail: "стек · не async", accent: true }], edges: [] },
        ...
        { codeLine: 2, caption: '<code>Memory&lt;T&gt;</code> «<span class="hl">can be stored on the managed heap</span>» <span class="ru-tr">«<span class="hl">может храниться в управляемой куче</span>»</span> → годен для async, полей класса; но с правилами владения (разборы 02–04).', nodes: [{ id: "m", kind: "gate", at: { zone: "heap", row: 0 }, state: "ok", label: "Memory<T>", detail: "куча · async OK", accent: true }], edges: [] },
      ],
      explain: 'Зачем нужен <code>Memory&lt;T&gt;</code> (verbatim): «<code>Span&lt;T&gt;</code> and <code>ReadOnlySpan&lt;T&gt;</code> are lightweight memory buffers that wrap references to managed or unmanaged memory. <span class="hl">Because these types can only be stored on the stack, they\'re unsuitable for scenarios such as asynchronous method calls</span>. To address this problem, .NET 2.1 added some additional types, including <code>Memory&lt;T&gt;</code>, <code>ReadOnlyMemory&lt;T&gt;</code>, <code>IMemoryOwner&lt;T&gt;</code>, and <code>MemoryPool&lt;T&gt;</code>… <span class="hl">Unlike <code>Span&lt;T&gt;</code>, <code>Memory&lt;T&gt;</code> can be stored on the managed heap</span>». <span class="ru-tr">«<code>Span&lt;T&gt;</code> и <code>ReadOnlySpan&lt;T&gt;</code> — это легковесные буферы памяти, которые оборачивают ссылки на управляемую или неуправляемую память. <span class="hl">Поскольку эти типы могут храниться только в стеке, они непригодны для сценариев вроде асинхронных вызовов методов</span>. Чтобы решить эту проблему, .NET 2.1 добавил несколько дополнительных типов, включая <code>Memory&lt;T&gt;</code>, <code>ReadOnlyMemory&lt;T&gt;</code>, <code>IMemoryOwner&lt;T&gt;</code> и <code>MemoryPool&lt;T&gt;</code>… <span class="hl">В отличие от <code>Span&lt;T&gt;</code>, <code>Memory&lt;T&gt;</code> может храниться в управляемой куче</span>».</span> То есть Memory — «heap-версия» вью: её можно положить в поле класса и пережить <code>await</code> (что Span не может — S7.8, CS8345). Замером в панели подтвердим: <code>Memory</code> как поле класса компилируется. Цена — модель владения (три концепта, разбор 02).',
      sources: ["ms-memguide"],
    },
    {
      ...
      explain: 'Три ядерных концепта (verbatim): «<b>Ownership</b>. The owner of a buffer instance is responsible for lifetime management, including destroying the buffer when it\'s no longer in use. <span class="hl">All buffers have a single owner</span>… Ownership can also be transferred; Component-A can relinquish control… Component-B becomes responsible…». <span class="ru-tr">«<b>Ownership</b> (владение). Владелец экземпляра буфера отвечает за управление временем жизни, включая уничтожение буфера, когда он больше не используется. <span class="hl">У всех буферов есть единственный владелец</span>… Владение также можно передать; Компонент-A может отказаться от контроля… Компонент-B становится ответственным…».</span> «<b>Consumption</b>. The consumer… is allowed to use the buffer… <span class="hl">Buffers can have one consumer at a time unless some external synchronization mechanism is provided</span>. The active consumer of a buffer isn\'t necessarily the buffer\'s owner». <span class="ru-tr">«<b>Consumption</b> (потребление). Потребителю… разрешено использовать буфер… <span class="hl">У буфера может быть один потребитель за раз, если не предоставлен внешний механизм синхронизации</span>. Активный потребитель буфера не обязательно является владельцем буфера».</span> «<b>Lease</b>. The lease is the length of time that a particular component is allowed to be the consumer of the buffer». <span class="ru-tr">«<b>Lease</b> (аренда). Аренда — это отрезок времени, в течение которого конкретному компоненту разрешено быть потребителем буфера».</span> Смысл: буфер разделяется между компонентами пайплайна, поэтому нужны явные правила «кто владеет / кто сейчас читает-пишет / как долго» — иначе гонки и порча данных.',
      sources: ["ms-memguide"],
    },
    {
      ...
      explain: 'Rule #1 и его обоснование (verbatim): «Rule #1: For a synchronous API, use <code>Span&lt;T&gt;</code> instead of <code>Memory&lt;T&gt;</code> as a parameter if possible». <span class="ru-tr">«Правило №1: в синхронном API по возможности используйте параметром <code>Span&lt;T&gt;</code> вместо <code>Memory&lt;T&gt;</code>».</span> Почему: «<code>Span&lt;T&gt;</code> is more versatile than <code>Memory&lt;T&gt;</code>… also offers better performance… you can use the <code>Memory&lt;T&gt;.Span</code> property to convert a <code>Memory&lt;T&gt;</code> instance to a <code>Span&lt;T&gt;</code>, <span class="hl">although Span<T>-to-Memory<T> conversion isn\'t possible</span>. So if your callers happen to have a <code>Memory&lt;T&gt;</code> instance, they\'ll be able to call your methods with <code>Span&lt;T&gt;</code> parameters anyway». <span class="ru-tr">«<code>Span&lt;T&gt;</code> более универсален, чем <code>Memory&lt;T&gt;</code>… а также даёт лучшую производительность… вы можете использовать свойство <code>Memory&lt;T&gt;.Span</code>, чтобы преобразовать экземпляр <code>Memory&lt;T&gt;</code> в <code>Span&lt;T&gt;</code>, <span class="hl">хотя конверсия Span<T> в Memory<T> невозможна</span>. Поэтому, если у ваших вызывающих окажется экземпляр <code>Memory&lt;T&gt;</code>, они всё равно смогут вызвать ваши методы с параметрами <code>Span&lt;T&gt;</code>».</span> Замер (рефлексия): <code>Memory&lt;int&gt;</code> имеет свойство <code>Span</code>, а у <code>Span&lt;int&gt;</code> нет метода «в Memory» — <b>memoryHasSpanProp=True spanHasToMemory=False</b>. Плюс Span-параметр даёт compile-time-проверки аренды. Итог: <code>Memory</code> берут, когда нужен async / хранение (иначе — <code>Span</code>).',
      sources: ["ms-memguide"],
    },
    {
      ...
      explain: 'Правила аренды и владения (verbatim). Аренда (концепт lease): «Rule #3: If your method accepts <code>Memory&lt;T&gt;</code> and returns <code>void</code>, <span class="hl">you must not use the Memory<T> instance after your method returns</span>»; <span class="ru-tr">«Правило №3: если ваш метод принимает <code>Memory&lt;T&gt;</code> и возвращает <code>void</code>, <span class="hl">вы не должны использовать экземпляр Memory<T> после того, как метод вернул управление</span>»;</span> «Rule #4: …returns a Task, <span class="hl">you must not use the Memory<T> instance after the Task transitions to a terminal state</span>» <span class="ru-tr">«Правило №4: …возвращает Task, <span class="hl">вы не должны использовать экземпляр Memory<T> после того, как Task перешёл в терминальное состояние</span>»</span> (terminal = «completed, faulted, or canceled»). <span class="ru-tr">(терминальное = «завершён, дал сбой или отменён»).</span> Владение через <code>IMemoryOwner&lt;T&gt;</code>: «Rule #7: If you have an <code>IMemoryOwner&lt;T&gt;</code> reference, you must at some point <span class="hl">dispose of it or transfer its ownership (but not both)</span>»; <span class="ru-tr">«Правило №7: если у вас есть ссылка <code>IMemoryOwner&lt;T&gt;</code>, вы в какой-то момент обязаны <span class="hl">освободить её или передать её владение (но не то и другое)</span>»;</span> «Rule #8: If you have an <code>IMemoryOwner&lt;T&gt;</code> parameter in your API surface, you are <b>accepting ownership</b> of that instance». <span class="ru-tr">«Правило №8: если у вас есть параметр <code>IMemoryOwner&lt;T&gt;</code> в поверхности вашего API, вы <b>принимаете владение</b> этим экземпляром».</span> И про пул: «This rule also applies to code that calls factory methods like <code>MemoryPool&lt;T&gt;.Rent</code>. The caller becomes the owner… and is responsible for disposing». <span class="ru-tr">«Это правило также относится к коду, который вызывает фабричные методы вроде <code>MemoryPool&lt;T&gt;.Rent</code>. Вызывающий становится владельцем… и отвечает за освобождение».</span> Нарушение аренды (напр. фоновая <code>Task.Run</code> с чужой Memory после return) → порча данных.',
      sources: ["ms-memguide"],
    },
    {
      ...
      explain: 'Машинная панель — три проверяемых факта про Memory, снятые в рантайме. (1) <b>Куча</b>: <code>class Holder { public Memory&lt;int&gt; field; }</code> компилируется и работает (<code>memoryAsClassField=True</code>, 3/3) — ровно «Memory<T> can be stored on the managed heap», <span class="ru-tr">«Memory<T> может храниться в управляемой куче»,</span> в отличие от Span (там было <code>CS8345</code>). (2) <b>Односторонняя конверсия</b>: <code>typeof(Memory&lt;int&gt;).GetProperty("Span")</code> ≠ null, а у <code>Span&lt;int&gt;</code> нет метода «в Memory»: <code>memoryHasSpanProp=True spanHasToMemory=False</code> — исполняемое подтверждение «Span<T>-to-Memory<T> conversion isn\'t possible», <span class="ru-tr">«конверсия Span<T> в Memory<T> невозможна»,</span> обоснование Rule #1. (3) <b>Владение</b>: <code>MemoryPool&lt;int&gt;.Shared.Rent(100)</code> отдаёт <code>IMemoryOwner&lt;int&gt;</code>, который <code>is IDisposable</code> — «The caller becomes the owner… responsible for disposing» <span class="ru-tr">«Вызывающий становится владельцем… отвечает за освобождение»</span> (<code>rentGivesDisposableOwner=True lenAtLeast100=True</code>). Числа реальны; Memory — heap/async-вариант вью с явной моделью владения и аренды.',
      sources: ["ms-memguide"],
    },
  ],
  ...
  cards: [
    {
      ...
      okText: '<code>Memory&lt;T&gt;</code> «<span class="hl">can be stored on the managed heap</span>» <span class="ru-tr">«<span class="hl">может храниться в управляемой куче</span>»</span> → <b>можно полем класса</b> (в отличие от <code>Span&lt;T&gt;</code>, где был CS8345). Печать: <b>memoryAsClassField=True</b>.',
    ...
    },
    {
      ...
      okText: '<code>Memory.Span</code> есть (Memory→Span), но «<span class="hl">Span<T>-to-Memory<T> conversion isn\'t possible</span>» <span class="ru-tr">«<span class="hl">конверсия Span<T> в Memory<T> невозможна</span>»</span> → у Span нет обратного. Печать: <b>memoryHasSpanProp=True spanHasToMemory=False</b>. Отсюда Rule #1 (sync API → Span).',
    ...
    },
    {
      ...
      okText: '<code>MemoryPool&lt;T&gt;.Rent</code> → <code>IMemoryOwner&lt;T&gt;</code> (это <code>IDisposable</code>): «<span class="hl">The caller becomes the owner… responsible for disposing</span>» <span class="ru-tr">«<span class="hl">Вызывающий становится владельцем… отвечает за освобождение</span>»</span> (Rule #7). Печать: <b>rentGivesDisposableOwner=True lenAtLeast100=True</b>.',
  ...
  takeaways: [
    { icon: "why", k: "Memory на куче", v: 'Span «<span class="hl">can only be stored on the stack</span>» <span class="ru-tr">«<span class="hl">может храниться только в стеке</span>»</span> (не async); <b>.NET 2.1</b> добавил <code>Memory&lt;T&gt;</code> (+ReadOnlyMemory/IMemoryOwner/MemoryPool), который «can be stored on the managed heap» <span class="ru-tr">«может храниться в управляемой куче»</span> → поле класса, async (замер: memoryAsClassField=True).' },
    { icon: "cost", k: "Rule #1 · односторонняя конверсия", v: 'sync API → <b>Span<T></b> (универсальнее, быстрее). <code>Memory.Span</code> есть, но «<span class="hl">Span<T>-to-Memory<T> conversion isn\'t possible</span>» <span class="ru-tr">«<span class="hl">конверсия Span<T> в Memory<T> невозможна</span>»</span> (замер: True/False). Memory берут для async/хранения.' },
