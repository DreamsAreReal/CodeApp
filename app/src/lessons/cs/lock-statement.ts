...
export const lockStatement: LessonData = {
  ...
  segments: [
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: 'До входа поток блокировку <b>не</b> держит: <code>IsHeldByCurrentThread == False</code>.', nodes: [{ id: "o", kind: "gate", at: { zone: "state", row: 0 }, state: "fail", label: "до lock", detail: "False", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: 'Внутри тела <code>lock (l)</code> поток <span class="hl">владеет</span>: <code>True</code>. «A thread that enters a lock is said to hold or own the lock until it exits the lock».', nodes: [{ id: "o", kind: "gate", at: { zone: "state", row: 0 }, state: "fail", label: "до", detail: "False" }, { id: "i", kind: "gate", at: { zone: "state", row: 1 }, state: "ok", label: "внутри lock", detail: "True", accent: true }], edges: [] },
