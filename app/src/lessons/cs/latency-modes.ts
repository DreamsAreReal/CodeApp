...
export const latencyModes: LessonData = {
  ...
  segments: [
    ...
    },
    {
      ...
      explain: 'Опровержение мифов MM5/MM6. Подавление не абсолютно (verbatim): «During low latency periods, generation 2 collections are suppressed <span class="hl">unless</span> the following occurs: The system receives a low memory notification from the operating system. Application code induces a collection by calling the <code>GC.Collect</code> method and specifying 2 for the <code>generation</code> parameter». Замер подтверждает: под <code>SustainedLowLatency</code> явный <code>GC.Collect(2)</code> реально поднимает <code>CollectionCount(2)</code>. Цена режима (verbatim): «This mode results in a <span class="hl">larger managed heap size</span> than other modes. Because it does not compact the managed heap, <b>higher fragmentation is possible</b>. Ensure that sufficient memory is available». И критично: «Because the <code>LatencyMode</code> property setting is <span class="hl">process-wide</span>, <code>OutOfMemoryException</code> exceptions can be generated on any thread that is allocating». Вывод: low-latency — не «выключить GC», а сместить компромисс в сторону памяти.',
      sources: ["ms-latency"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 3, out: "lowLatencyStuck=False sustainedStuck=True", caption: 'Панель: <b>lowLatencyStuck=False sustainedStuck=True</b> (замер 3/3) — прямое доказательство «This setting is available only for workstation garbage collection». И <code>GC.Collect(2)</code> под Sustained всё равно даёт gen2.', nodes: [{ id: "s", kind: "gate", at: { zone: "stick", row: 0 }, state: "fail", label: "lowLatencyStuck", detail: "False" }, { id: "b", kind: "gate", at: { zone: "still", row: 0 }, state: "ok", label: "sustainedStuck", detail: "True", accent: true }], edges: [] },
      ],
      explain: 'Машинная панель — ограничение «LowLatency — только Workstation», снятое в рантайме на <b>Server GC</b>-процессе бэкенда. Замер: попытка выставить <code>GCSettings.LatencyMode = LowLatency</code> <b>не прилипает</b> (режим остаётся <code>Interactive</code>), а <code>SustainedLowLatency</code> — <b>прилипает</b>: <b>lowLatencyStuck=False sustainedStuck=True</b> (детерминированно 3/3). Это прямое исполняемое подтверждение doc-фразы «This setting is available <b>only for workstation garbage collection</b>» — не пересказ, а поведение рантайма. Второй замер (карточка) подтверждает не-абсолютность подавления: под <code>SustainedLowLatency</code> явный <code>GC.Collect(2)</code> поднимает <code>CollectionCount(2)</code> → <b>gen2StillRanOnExplicitCollect=True</b>. Итог: latency-режимы — это конфигурируемый компромисс, а не выключатель GC; ограничения флейвора и «unless»-условия действуют реально.',
      sources: ["ms-latency"],
    },
  ],
  ...
  cards: [
    {
      ...
      okText: 'Дефолтный режим для workstation и server GC — <span class="hl">Interactive</span> («default mode for workstation and server garbage collection»). Печать: <b>default=Interactive</b>. (Batch — при отключённом background GC.)',
    ...
    },
    {
      ...
      okText: '<code>LowLatency</code> — «available <span class="hl">only for workstation</span> garbage collection» → на Server GC не прилипает; <code>SustainedLowLatency</code> — «for both workstation and server» → прилипает. Печать: <b>lowLatencyStuck=False sustainedStuck=True</b>.',
    ...
    },
    {
      ...
      okText: 'Подавление gen2 <span class="hl">не абсолютно</span>: «suppressed <b>unless</b>… GC.Collect… specifying 2». Явный <code>GC.Collect(2)</code> под Sustained всё равно запускает gen2. Печать: <b>gen2StillRanOnExplicitCollect=True</b>.',
  ...
  takeaways: [
    { icon: "why", k: "latency = пауза GC", v: '«To reclaim objects, the garbage collector (GC) <span class="hl">must stop all the executing threads</span> in an application»; период = latency, регулируется <code>GCSettings.LatencyMode</code>. Дефолт — <b>Interactive</b> (замер). Режим — компромисс, а не выключатель GC.' },
    ...
    { icon: "avoid", k: "не абсолютно + цена", v: 'gen2 подавлен «<span class="hl">unless</span>» low-memory ИЛИ <code>GC.Collect(2)</code> (замер: gen2 всё равно). Sustained → «larger managed heap size» + фрагментация; <code>LatencyMode</code> process-wide → <b>OOM на любом потоке</b>.' },
  ],
