...
export const syncPrimitivesOverview: LessonData = {
  ...
  spec: [
    { text: "Единственное общее правило: «Use the same synchronization primitive instance to protect access of a shared resource. If you use different synchronization primitive instances to protect the same resource, you'll circumvent the protection provided by a synchronization primitive».", source: "ms-sync" },
    { text: "Две большие группы: наследники «System.Threading.WaitHandle class, which encapsulates a native operating system synchronization handle», и lightweight-типы, что «don't rely on underlying operating system handles and typically provide better performance».", source: "ms-sync" },
  ],
  edgeCases: [
    { text: "Lightweight — только внутри процесса: «they cannot be used for the inter-process synchronization. Use those types for thread synchronization within one application». Пример: «SemaphoreSlim is a lightweight alternative to Semaphore».", source: "ms-sync" },
    { text: "AutoReset — турникет на одного: «resets automatically after releasing a single waiting thread. It's like a turnstile that allows only one thread through each time it's signaled». ManualReset — ворота: «remains signaled until its Reset method is called».", source: "ms-sync" },
    { text: "Monitor лучше звать через lock: «To synchronize access to a shared resource, use the lock statement in C# … instead of using the Monitor class directly».", source: "ms-sync" },
  ],

  misconceptions: [
    {
      wrong: "примитивы синхронизации — это «куча взаимозаменяемых замков», бери любой",
      hook: 'Они <b>не</b> взаимозаменяемы, и делятся на группы. Первая ось: наследники «<span class="hl">System.Threading.WaitHandle</span> class, which <b>encapsulates a native operating system synchronization handle</b>» (Mutex, Semaphore, EventWaitHandle — можно между процессами на Windows) против lightweight-типов, что «<span class="hl">don\'t rely on underlying operating system handles and typically provide better performance</span>» (SemaphoreSlim, ManualResetEventSlim — только внутри процесса). Вторая ось: одни защищают <b>доступ к ресурсу</b> (Monitor/lock, Mutex, SpinLock, ReaderWriterLockSlim, Semaphore), другие — <b>сигналинг</b> между потоками (EventWaitHandle, CountdownEvent, Barrier). И железное правило: «<span class="hl">Use the same synchronization primitive instance to protect access of a shared resource</span>». Дальше <b>пять разборов</b>: карта ролей, WaitHandle vs lightweight, guard-примитивы, Auto vs Manual reset, и <b>машинная панель</b> — кто наследует WaitHandle реальным прогоном.',
      source: "ms-sync",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 1, out: "", caption: 'Другие — <b>сигналинг</b>: поток ждёт уведомления. «a thread must <span class="hl">wait for notification, or a signal</span>, from one or more threads in order to proceed».', nodes: [{ id: "g", kind: "gate", at: { zone: "map", row: 0 }, state: "ok", label: "доступ к ресурсу", detail: "Monitor · Mutex · Semaphore" }, { id: "s", kind: "gate", at: { zone: "map", row: 1 }, state: "ok", label: "сигналинг", detail: "EventWaitHandle · Countdown · Barrier", accent: true }], edges: [] },
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'Наследники <code>WaitHandle</code> оборачивают <b>нативный OS-handle</b>: «encapsulates a native operating system synchronization handle».', nodes: [{ id: "w", kind: "obj", at: { zone: "wh", row: 0 }, typeTag: "WaitHandle", value: "OS handle", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Lightweight-типы OS-handle <span class="hl">не</span> держат: «don\'t rely on underlying operating system handles and typically provide better performance».', nodes: [{ id: "w", kind: "obj", at: { zone: "wh", row: 0 }, typeTag: "WaitHandle", value: "OS handle" }, { id: "l", kind: "obj", at: { zone: "light", row: 0 }, typeTag: "SemaphoreSlim", value: "без handle", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: 'Плата за скорость — <b>только in-process</b>: «cannot be used for the inter-process synchronization». WaitHandle-типы (на Windows) — межпроцессны через имя.', nodes: [{ id: "w", kind: "gate", at: { zone: "wh", row: 0 }, state: "ok", label: "WaitHandle", detail: "cross-process (named)", accent: true }, { id: "l", kind: "gate", at: { zone: "light", row: 0 }, state: "", label: "lightweight", detail: "in-process only", accent: true }], edges: [] },
      ],
      explain: 'Первая ось выбора — тяжёлый OS-примитив или лёгкий CLR-примитив. «Multiple .NET synchronization primitives derive from the <code>System.Threading.WaitHandle</code> class, which <span class="hl">encapsulates a native operating system synchronization handle and uses a signaling mechanism for thread interaction</span>» — сюда входят Mutex, Semaphore, EventWaitHandle (+ Auto/ManualResetEvent). На Windows они «can represent named system synchronization handles… <span class="hl">for the inter-process synchronization</span>». Lightweight-типы иные: «<span class="hl">Lightweight synchronization types don\'t rely on underlying operating system handles and typically provide better performance. However, they cannot be used for the inter-process synchronization</span>. Use those types for thread synchronization within one application». Прямая пара: «<span class="hl">SemaphoreSlim is a lightweight alternative to Semaphore</span>». Правило: не нужен межпроцессный сценарий — бери Slim.',
      sources: ["ms-sync"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<b>Monitor</b> — база: «grants mutually exclusive access… by acquiring or releasing a lock». Звать через <code>lock</code>, не напрямую.', nodes: [{ id: "mon", kind: "gate", at: { zone: "guard", row: 0 }, state: "ok", label: "Monitor / lock", detail: "взаимное исключение", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: '<b>Mutex</b> — как Monitor, но «can be used for <span class="hl">inter-process synchronization</span>» (named). Имеет thread affinity: освобождает владелец.', nodes: [{ id: "mon", kind: "gate", at: { zone: "guard", row: 0 }, state: "ok", label: "Monitor / lock", detail: "взаимное исключение" }, { id: "mx", kind: "gate", at: { zone: "guard", row: 1 }, state: "ok", label: "Mutex", detail: "+ межпроцессно", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: '<b>ReaderWriterLockSlim</b> — «allows <span class="hl">multiple threads to access the resource simultaneously for reading</span>», но writer эксклюзивен.', nodes: [{ id: "mon", kind: "gate", at: { zone: "guard", row: 0 }, state: "ok", label: "Monitor", detail: "1 поток" }, { id: "rw", kind: "gate", at: { zone: "guard", row: 1 }, state: "ok", label: "RWLockSlim", detail: "N читателей / 1 писатель", accent: true }], edges: [] },
        { codeLine: 3, out: "", caption: '<b>Semaphore(Slim)</b> — «<span class="hl">limit the number of threads that can access</span> a shared resource… concurrently»: не более N. SpinLock крутится в цикле, не блокируя.', nodes: [{ id: "rw", kind: "gate", at: { zone: "guard", row: 0 }, state: "ok", label: "RWLockSlim", detail: "чтение параллельно" }, { id: "sem", kind: "gate", at: { zone: "guard", row: 1 }, state: "ok", label: "Semaphore(Slim)", detail: "не более N", accent: true }], edges: [] },
      ],
      explain: 'Guard-семейство — по «форме» ограничения. <b>Monitor</b>: «grants mutually exclusive access to a shared resource by acquiring or releasing a lock» — но «<span class="hl">use the lock statement in C#… instead of using the Monitor class directly</span>». <b>Mutex</b>: как Monitor, «has thread affinity» и «<span class="hl">can be used for inter-process synchronization</span>» через named. <b>SpinLock</b>: «When <code>SpinLock</code> attempts to acquire a lock that is unavailable, it <span class="hl">waits in a loop, repeatedly checking</span> until the lock becomes available» — дёшево при коротком ожидании. <b>ReaderWriterLockSlim</b>: «grants exclusive access… for writing and <span class="hl">allows multiple threads to access the resource simultaneously for reading</span>». <b>Semaphore/SemaphoreSlim</b>: «<span class="hl">limit the number of threads that can access a shared resource or a pool of resources concurrently</span>» — семафор без thread affinity: «a thread can acquire the semaphore and another one can release it».',
      sources: ["ms-sync"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>AutoResetEvent</code> — «<span class="hl">turnstile that allows only one thread through</span> each time it\'s signaled»: сбрасывается автоматически.', nodes: [{ id: "a", kind: "gate", at: { zone: "auto", row: 0 }, state: "ok", label: "AutoReset", detail: "1 поток / Set", accent: true }], edges: [] },
    ...
    },
    {
      ...
      explain: 'Машинная панель — граница WaitHandle рефлексией. Прогон: <code>Mutex</code> и <code>Semaphore</code> — <code>IsSubclassOf(WaitHandle) == True</code>, <code>SemaphoreSlim</code> — <code>False</code> (печать <b>True True False</b>). Это ровно та граница из доков: WaitHandle-наследники «<span class="hl">encapsulates a native operating system synchronization handle</span>» и (на Windows) годятся для межпроцессной синхронизации, а <code>SemaphoreSlim</code> — «<span class="hl">a lightweight alternative to Semaphore</span>» без OS-handle: «<span class="hl">SemaphoreSlim doesn\'t support named system semaphores</span>». Практический вывод: <code>WaitOne</code>/<code>WaitAll</code>/<code>WaitAny</code> (методы <code>WaitHandle</code>) есть только у WaitHandle-типов; у Slim — свои <code>Wait</code>/<code>WaitAsync</code>.',
      sources: ["ms-sync"],
    },
  ],
  ...
  cards: [
    {
      ...
      okText: '<code>SemaphoreSlim(2)</code> «limit the number of threads that can access… concurrently» — не более <b>2</b> в критической секции одновременно, сколько бы задач ни стартовало. Итог <b>maxConcurrent=2</b>.',
    ...
    },
    {
      ...
      okText: '<code>Mutex</code> и <code>Semaphore</code> наследуют <code>WaitHandle</code> (OS-handle, межпроцессны) — True/True; <code>SemaphoreSlim</code> — «lightweight alternative» без handle — False.',
      noText: 'WaitHandle «encapsulates a native operating system synchronization handle»; Mutex и Semaphore его наследуют, Slim — нет. Реальный вывод: <b>True True False</b>.',
    ...
    },
    {
      ...
      okText: '<code>AutoResetEvent</code> — «turnstile that allows only one thread through each time it\'s signaled»: один <code>Set</code> пропускает ровно одного (afterOneSet=1); чтобы пройти троим — три <code>Set</code> (total=3).',
  ...
  takeaways: [
    { icon: "why", k: "Две оси", v: 'По роли: guard-примитивы (Monitor/lock, Mutex, SpinLock, RWLockSlim, Semaphore) vs сигналинг (EventWaitHandle, Countdown, Barrier). По реализации: WaitHandle (OS-handle) vs lightweight. Правило: «<span class="hl">Use the same synchronization primitive instance to protect access</span>».' },
    { icon: "cost", k: "WaitHandle / Slim", v: 'WaitHandle «encapsulates a native operating system synchronization handle» → межпроцессно (named). Lightweight «don\'t rely on underlying operating system handles… <span class="hl">cannot be used for the inter-process synchronization</span>» → быстрее, но in-process. Панель: True True False.' },
    { icon: "avoid", k: "reset-режим", v: 'AutoReset — «turnstile that allows only one thread through» (1 Set → 1 поток, afterOneSet=1 total=3). ManualReset — «gate… stays open» до <code>Reset</code>. Semaphore(Slim) ограничивает N разом (maxConcurrent=2).' },
  ],
