...
export const choosingCollection: LessonData = {
  ...
  spec: [
    { text: "«Be sure to choose your collection class carefully. Using the wrong type can restrict your use of the collection.»", source: "ms-selecting" },
    { text: "«Avoid using the types in the System.Collections namespace. The generic and concurrent versions of the collections are recommended because of their greater type safety and other improvements.»", source: "ms-selecting" },
  ],
  edgeCases: [
    ...
    { text: "«<code>Dictionary&lt;TKey,TValue&gt;</code> … provides faster lookup than the <code>SortedDictionary&lt;TKey,TValue&gt;</code>»: хеш быстрее дерева на поиск, но не хранит порядок.", source: "ms-selecting" },
  ],

  misconceptions: [
    {
      wrong: "любая коллекция ищет элемент примерно одинаково — разница только в удобстве API",
      hook: 'Кажется, что <code>list.Contains(x)</code> и <code>set.Contains(x)</code> — «одно и то же с разным синтаксисом». Машинно это <span class="wrong">разные алгоритмы</span>: <code>List&lt;T&gt;</code> перебирает элементы по одному (O(n)), <code>HashSet&lt;T&gt;</code> прыгает сразу в бакет по хешу (O(1)). Дока прямо предупреждает: «Be sure to <span class="hl">choose your collection class carefully</span>. Using the wrong type can restrict your use of the collection». Дальше <b>четыре разбора</b>: воронка вопросов, таблица amortized vs worst-case, линейный обход против бакета и <b>машинная панель</b> — реально снятый счётчик <code>Equals</code>: поиск отсутствующего среди 1000 = <b>1000</b> сравнений у <code>List</code> против <b>0</b> у <code>HashSet</code>.',
      source: "ms-selecting",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, caption: '«Do you need to <b>access each element by index</b>?» → <code>List&lt;T&gt;</code> (и <code>ArrayList</code>): доступ по zero-based индексу.', nodes: [{ id: "qi", kind: "chip", at: { zone: "q", row: 0 }, value: "по индексу?" }, { id: "list", kind: "obj", at: { zone: "pick", row: 0 }, typeTag: "List<T>", value: "index O(1)", accent: true }], edges: [{ id: "e1", from: "qi", to: "list", accent: true }] },
        { codeLine: 1, caption: '«access to their elements <b>by the key</b>» → <code>Dictionary&lt;TKey,TValue&gt;</code>: доступ по ключу, поиск через хеш.', nodes: [{ id: "qi", kind: "chip", at: { zone: "q", row: 0 }, value: "по индексу?" }, { id: "list", kind: "obj", at: { zone: "pick", row: 0 }, typeTag: "List<T>", value: "index O(1)" }, { id: "qk", kind: "chip", at: { zone: "q", row: 1 }, value: "по ключу?" }, { id: "dict", kind: "obj", at: { zone: "pick", row: 1 }, typeTag: "Dictionary", value: "key O(1)", accent: true }], edges: [{ id: "e1", from: "qi", to: "list" }, { id: "e2", from: "qk", to: "dict", accent: true }] },
        { codeLine: 2, caption: '«Do you need <b>fast searches and retrieval</b>?» → хеш-типы: <code>Dictionary</code> «faster lookup than the <code>SortedDictionary</code>». Общий совет: избегать <code>System.Collections</code>, брать generic.', nodes: [{ id: "qk", kind: "chip", at: { zone: "q", row: 0 }, value: "по ключу?" }, { id: "dict", kind: "obj", at: { zone: "pick", row: 0 }, typeTag: "Dictionary", value: "key O(1)" }, { id: "qs", kind: "chip", at: { zone: "q", row: 1 }, value: "быстрый поиск?" }, { id: "hash", kind: "obj", at: { zone: "pick", row: 1 }, typeTag: "hash-типы", value: "O(1)", accent: true }], edges: [{ id: "e2", from: "qk", to: "dict" }, { id: "e3", from: "qs", to: "hash", accent: true }] },
      ],
      explain: 'Страница «Selecting a Collection Class» устроена как воронка вопросов: «Be sure to choose your collection class carefully. Using the wrong type can restrict your use of the collection». Дальше «Consider the following questions»: нужен доступ по индексу → <code>List&lt;T&gt;</code>; по ключу → <code>Dictionary&lt;TKey,TValue&gt;</code>; «Do you need fast searches and retrieval of information?» → хеш-типы, где «The <code>Dictionary&lt;TKey,TValue&gt;</code> generic class provides <span class="hl">faster lookup</span> than the <code>SortedDictionary&lt;TKey,TValue&gt;</code>». И общий императив: «Avoid using the types in the <code>System.Collections</code> namespace».',
  ...
  takeaways: [
    { icon: "why", k: "Выбор от вопроса", v: 'Дока: «choose your collection class carefully… Consider the following questions». Индекс → <code>List&lt;T&gt;</code>, ключ → <code>Dictionary</code>, быстрый поиск/членство → хеш-типы. И «Avoid… <code>System.Collections</code>» — бери generic.' },
