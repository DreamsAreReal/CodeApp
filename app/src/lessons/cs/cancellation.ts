...
export const cancellation: LessonData = {
  ...
  spec: [
    { text: "«The .NET Framework provides a unified model for cooperative cancellation of asynchronous or long-running synchronous operations.» — модель отмены КООПЕРАТИВНА: фреймворк не убивает поток, слушатели сами останавливаются.", source: "ms-cancel" },
  ],
  edgeCases: [
    { text: 'Отмена <b>кооперативна</b>: только запрашивающий объект (CTS) выдаёт запрос; каждый слушатель <b>сам</b> замечает <code>IsCancellationRequested</code> и «gracefully terminate». Фреймворк <span class="hl">не прерывает</span> поток (это не <code>Thread.Abort</code>) и не гарантирует немедленную остановку.', source: "ms-cancel" },
