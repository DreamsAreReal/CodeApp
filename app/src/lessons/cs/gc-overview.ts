...
export const gcOverview: LessonData = {
  ...
  spec: [
    { text: "«In the common language runtime (CLR), the garbage collector (GC) serves as an automatic memory manager. The garbage collector manages the allocation and release of memory for an application.» <span class=\"ru-tr\">«В общеязыковой среде выполнения (CLR) сборщик мусора (GC) работает как автоматический менеджер памяти. Сборщик мусора управляет выделением и освобождением памяти для приложения.»</span>", source: "ms-gc-fund" },
  ],
  edgeCases: [
    { text: "GC определяет живость по <b>корням</b> (verbatim): «An application\'s roots include <span class=\"hl\">static fields, local variables on a thread\'s stack, CPU registers, GC handles, and the finalize queue</span>». <span class=\"ru-tr\">«Корни приложения включают статические поля, локальные переменные в стеке потока, регистры процессора, дескрипторы GC и очередь финализации».</span> Недостижимое из корней — мусор.", source: "ms-gc-fund" },
    { text: "Две гарантии GC: «Managed objects <span class=\"hl\">automatically get clean content to start with</span>, so their constructors don\'t have to initialize every data field» <span class=\"ru-tr\">«Управляемые объекты изначально автоматически получают очищенное содержимое, поэтому их конструкторам не нужно инициализировать каждое поле данных».</span> и «memory safety by making sure that an object can\'t use for itself the memory allocated for another object». <span class=\"ru-tr\">«безопасность памяти за счёт гарантии того, что объект не может использовать для себя память, выделенную для другого объекта».</span>", source: "ms-gc-fund" },
    { text: "Аллокация из managed heap дешёвая: «Because the runtime allocates memory for an object by <span class=\"hl\">adding a value to a pointer</span>, it\'s almost as fast as allocating memory from the stack». <span class=\"ru-tr\">«Поскольку среда выполнения выделяет память для объекта, прибавляя значение к указателю, это почти так же быстро, как выделение памяти из стека».</span>", source: "ms-gc-fund" },
  ],

  misconceptions: [
    {
      wrong: "GC детерминирован — срабатывает в предсказуемый момент / по расписанию, и это можно контролировать",
      hook: 'Живучий миф: «<span class="wrong">GC срабатывает в предсказуемый момент</span>» (по таймеру/расписанию, и я это контролирую). Нет: «The garbage collector\'s <span class="hl">optimizing engine determines the best time</span> to perform a collection <b>based on the allocations being made</b>» <span class="ru-tr">«Оптимизирующий движок сборщика мусора определяет наилучший момент для выполнения сбора <b>на основе выполняемых аллокаций</b>»</span>; порог «is <span class="hl">continuously adjusted</span> as the process runs». <span class="ru-tr">«непрерывно подстраивается по мере работы процесса».</span> Момент сбора выбирает движок, не пользователь и не фиксированное расписание. Ниже <b>пять разборов</b>: GC как авто-менеджер, корни и достижимость, аллокация = сдвиг указателя, когда случается сбор, и <b>машинная панель</b> — реальный замер аллокации (byte[1000] = 1024 байта) + сбор без <code>GC.Collect()</code>.',
      source: "ms-gc-fund",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: 'Итог: «Frees developers from having to manually release memory» <span class="ru-tr">«Освобождает разработчиков от необходимости вручную освобождать память»</span> — <span class="hl">нет утечек «забыл free»</span> и нет доступа к освобождённой памяти.', nodes: [{ id: "a", kind: "gate", at: { zone: "gc", row: 0 }, state: "ok", label: "авто-менеджер", detail: "alloc + release", accent: true }], edges: [] },
      ],
      explain: 'Ядро S7: дословно «In the common language runtime (CLR), the garbage collector (GC) serves as an <span class="hl">automatic memory manager</span>. The garbage collector manages the <b>allocation and release</b> of memory for an application. Therefore, developers working with managed code don\'t have to write code to perform memory management tasks». <span class="ru-tr">«В общеязыковой среде выполнения (CLR) сборщик мусора (GC) работает как автоматический менеджер памяти. Сборщик мусора управляет <b>выделением и освобождением</b> памяти для приложения. Поэтому разработчикам, работающим с управляемым кодом, не нужно писать код для выполнения задач управления памятью».</span> Выгоды (verbatim, страница overview/fundamentals): «Frees developers from having to manually release memory» <span class="ru-tr">«Освобождает разработчиков от необходимости вручную освобождать память»</span>; «Allocates objects on the managed heap efficiently» <span class="ru-tr">«Эффективно выделяет объекты в управляемой куче»</span>; «Reclaims objects that are no longer being used, clears their memory». <span class="ru-tr">«Восстанавливает объекты, которые больше не используются, очищает их память».</span> Это устраняет два класса ошибок из C/C++: утечку «забыл освободить» и обращение к уже освобождённой памяти. Как GC решает, что объект больше не нужен — по достижимости из корней (разбор 02).',
      sources: ["ms-gc-fund"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: 'Стал <span class="hl">недостижим</span> из всех корней — «considers unreachable objects garbage and releases the memory». <span class="ru-tr">«считает недостижимые объекты мусором и освобождает память».</span> Память освободится в следующий сбор.', nodes: [{ id: "r", kind: "gate", at: { zone: "roots", row: 0 }, state: "ok", label: "root = null", detail: "не ссылается" }, { id: "o", kind: "obj", at: { zone: "heap", row: 0 }, typeTag: "object", value: "мусор", accent: true }], edges: [] },
      ],
      explain: 'Как GC определяет, что живо (verbatim): «It determines which objects are no longer being used by <span class="hl">examining the application\'s <i>roots</i></span>. An application\'s roots include <b>static fields, local variables on a thread\'s stack, CPU registers, GC handles, and the finalize queue</b>. … The garbage collector uses this list to create a graph that contains all the objects that are reachable from the roots». <span class="ru-tr">«Он определяет, какие объекты больше не используются, исследуя <i>корни</i> приложения. Корни приложения включают <b>статические поля, локальные переменные в стеке потока, регистры процессора, дескрипторы GC и очередь финализации</b>. … Сборщик мусора использует этот список, чтобы построить граф, содержащий все объекты, достижимые из корней».</span> И далее: «Objects that aren\'t in the graph are unreachable from the application\'s roots. The garbage collector <span class="hl">considers unreachable objects garbage</span> and releases the memory allocated for them». <span class="ru-tr">«Объекты, которых нет в графе, недостижимы из корней приложения. Сборщик мусора считает недостижимые объекты мусором и освобождает выделенную для них память».</span> Ключ: живость — это НЕ «есть ли ещё переменная в исходнике», а <b>достижимость из активных корней в рантайме</b>. Обнулил последнюю ссылку → объект стал кандидатом на сбор.',
      sources: ["ms-gc-fund"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: 'Выделить объект = положить по next и <b>сдвинуть указатель</b> на размер: «adding a value to a pointer». <span class="ru-tr">«прибавление значения к указателю».</span>', nodes: [{ id: "p", kind: "gate", at: { zone: "alloc", row: 0 }, state: "ok", label: "a", detail: "next += размер" }, { id: "n", kind: "gate", at: { zone: "alloc", row: 1 }, state: "ok", label: "next двигается", detail: "почти как стек", accent: true }], edges: [] },
      ...
      explain: 'Почему аллокация в .NET дешёвая (verbatim): «When you initialize a new process, the runtime reserves a contiguous region of address space… The <span class="hl">managed heap maintains a pointer to the address where the next object in the heap will be allocated</span>». <span class="ru-tr">«Когда вы инициализируете новый процесс, среда выполнения резервирует непрерывную область адресного пространства… Управляемая куча хранит указатель на адрес, по которому будет выделен следующий объект в куче».</span> И про стоимость: «Allocating memory from the managed heap is faster than unmanaged memory allocation. <span class="hl">Because the runtime allocates memory for an object by adding a value to a pointer, it\'s almost as fast as allocating memory from the stack</span>. In addition, because new objects that are allocated consecutively are <b>stored contiguously</b> in the managed heap, an application can access the objects quickly». <span class="ru-tr">«Выделение памяти из управляемой кучи быстрее, чем выделение неуправляемой памяти. Поскольку среда выполнения выделяет память для объекта, прибавляя значение к указателю, это почти так же быстро, как выделение памяти из стека. Кроме того, поскольку новые объекты, выделяемые подряд, <b>хранятся непрерывно</b> в управляемой куче, приложение может обращаться к этим объектам быстро».</span> То есть в норме выделение — это инкремент указателя, а не поиск свободного блока, как в классическом <code>malloc</code>. Реальную цену в байтах измерим в панели (разбор 05).',
      sources: ["ms-gc-fund"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: 'Условие 2 — аллокации <span class="hl">превысили порог</span>; сам порог «is <b>continuously adjusted</b> as the process runs» <span class="ru-tr">«<b>непрерывно подстраивается</b> по мере работы процесса»</span> — не фиксирован.', nodes: [{ id: "m", kind: "gate", at: { zone: "when", row: 0 }, state: "ok", label: "low memory", detail: "1" }, { id: "t", kind: "gate", at: { zone: "when", row: 1 }, state: "ok", label: "порог аллокаций", detail: "подстраивается", accent: true }], edges: [] },
        { codeLine: 3, caption: 'Условие 3 — явный <code>GC.Collect()</code>: «In almost all cases, <span class="hl">you don\'t have to call this method</span>» <span class="ru-tr">«Почти во всех случаях вам не нужно вызывать этот метод»</span> — движок работает continuously. Момент — за движком.', nodes: [{ id: "t", kind: "gate", at: { zone: "when", row: 0 }, state: "ok", label: "порог", detail: "2" }, { id: "g", kind: "gate", at: { zone: "when", row: 1 }, state: "fail", label: "GC.Collect()", detail: "почти никогда", accent: true }], edges: [] },
      ],
      explain: 'Опровержение мифа «GC детерминирован». Дословно (раздел Conditions for a garbage collection): сбор происходит когда истинно одно из: «The system has <b>low physical memory</b>… The <span class="hl">memory that\'s used by allocated objects on the managed heap surpasses an acceptable threshold. This threshold is continuously adjusted as the process runs</span>… The <code>GC.Collect</code> method is called. In almost all cases, <span class="hl">you don\'t have to call this method because the garbage collector runs continuously</span>». <span class="ru-tr">«В системе <b>мало физической памяти</b>… Память, используемая выделенными объектами в управляемой куче, превышает приемлемый порог. Этот порог непрерывно подстраивается по мере работы процесса… Вызывается метод <code>GC.Collect</code>. Почти во всех случаях вам не нужно вызывать этот метод, потому что сборщик мусора работает непрерывно».</span> И про выбор момента: «The garbage collector\'s <b>optimizing engine determines the best time</b> to perform a collection based on the allocations being made». <span class="ru-tr">«<b>Оптимизирующий движок</b> сборщика мусора <b>определяет наилучший момент</b> для выполнения сбора на основе выполняемых аллокаций».</span> Вывод: момент сбора — недетерминированный, его выбирает движок по аллокациям; полагаться на «когда именно» нельзя, и звать <code>GC.Collect()</code> для «ускорения» — почти всегда антипаттерн.',
      sources: ["ms-gc-fund"],
    },
    {
  ...
  cards: [
    {
      ...
      noText: '«the garbage collector runs continuously» <span class="ru-tr">«сборщик мусора работает непрерывно»</span> — сбор идёт сам при превышении порога. Реальный вывод: <b>ranAutomatically=True</b>.',
    ...
    },
    {
      ...
      okText: 'Гарантия GC: «Managed objects <span class="hl">automatically get clean content to start with</span>» <span class="ru-tr">«Управляемые объекты изначально автоматически получают очищенное содержимое»</span> — новый массив обнулён. <b>zeroed=True</b>. Конструктору не нужно инициализировать каждое поле.',
  ...
  takeaways: [
    { icon: "why", k: "авто-менеджер", v: 'GC — «automatic memory manager» <span class="ru-tr">«автоматический менеджер памяти»</span>: сам аллоцирует (сдвиг next-указателя, «almost as fast as… the stack» <span class="ru-tr">«почти так же быстро, как… стек»</span>) и сам освобождает недостижимое. Нет ручного <code>free</code> → нет утечек «забыл» и use-after-free.' },
    { icon: "cost", k: "живость = достижимость", v: 'Живо то, что <span class="hl">достижимо из корней</span> (static, локальные на стеке, регистры, GC handles, finalize queue). Обнулил последнюю ссылку → GC «considers unreachable objects garbage» <span class="ru-tr">«считает недостижимые объекты мусором»</span>, память освободится в сбор.' },
    { icon: "avoid", k: "момент — за движком", v: '«optimizing engine determines the best time… based on the allocations» <span class="ru-tr">«оптимизирующий движок определяет наилучший момент… на основе аллокаций»</span>; порог «continuously adjusted» <span class="ru-tr">«непрерывно подстраивается»</span>. Сбор недетерминирован (замер: gen0 растёт сам). <code>GC.Collect()</code> «you don\'t have to call» <span class="ru-tr">«вам не нужно вызывать»</span> — почти всегда антипаттерн.' },
  ],
