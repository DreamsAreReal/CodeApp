...
export const tplParallel: LessonData = {
  ...
  spec: [
    ...
    { text: "Data parallelism = одна операция над элементами: «the same operation is performed concurrently (that is, in parallel) on elements in a source collection or array… the source collection is partitioned so that multiple threads can operate on different segments concurrently».", source: "ms-data" },
  ],
  edgeCases: [
    { text: "Не всё стоит параллелить: «if a loop performs only a small amount of work on each iteration, or it doesn't run for many iterations, then the overhead of parallelization can cause the code to run more slowly».", source: "ms-tpl" },
    { text: "Планировщик балансирует сам: «Behind the scenes, the Task Scheduler partitions the task based on system resources and workload. When possible, the scheduler redistributes work among multiple threads and processors if the workload becomes unbalanced».", source: "ms-data" },
    { text: "Overload-ы дают контроль: перегрузки «let you stop or break loop execution, monitor the state of the loop… maintain thread-local state… control the degree of concurrency» через ParallelLoopState, ParallelOptions, ParallelLoopResult.", source: "ms-data" },
  ],

  misconceptions: [
    {
      wrong: "Parallel.For — это «просто быстрее foreach», всегда и без трейдофов",
      hook: 'Нет: <code>Parallel.For</code>/<code>ForEach</code> — это <b>data parallelism</b> с реальной ценой. Что он делает: «<span class="hl">the same operation is performed concurrently… on elements in a source collection</span>… the source collection is partitioned so that multiple threads can operate on different segments concurrently». И он прячет всю кухню: «<span class="hl">the TPL handles the partitioning of the work, the scheduling of threads on the ThreadPool</span>… and other low-level details», плюс «<span class="hl">dynamically scales the degree of concurrency</span>». Но выигрыш не гарантирован: «if a loop performs only a <span class="hl">small amount of work</span> on each iteration, or it doesn\'t run for many iterations, then the <span class="hl">overhead of parallelization can cause the code to run more slowly</span>». А степень параллелизма можно ограничить (<code>MaxDegreeOfParallelism</code>). Дальше <b>пять разборов</b>: что такое TPL, seq→parallel-цикл, партиционирование и ребаланс, overhead (когда НЕ параллелить), и <b>машинная панель</b> — <code>MaxDegreeOfParallelism=1</code> = один поток реальным прогоном.',
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: 'TPL берёт на себя всё низкоуровневое: «<span class="hl">the partitioning of the work, the scheduling of threads on the <code>ThreadPool</code></span>… and other low-level details».', nodes: [{ id: "t", kind: "gate", at: { zone: "tpl", row: 0 }, state: "ok", label: "Parallel.ForEach", detail: "что" }, { id: "r", kind: "gate", at: { zone: "runtime", row: 0 }, state: "ok", label: "TPL runtime", detail: "партиции · планировщик · пул", accent: true }], edges: [{ id: "e1", from: "t", to: "r", accent: true }] },
        { codeLine: 0, out: "", caption: 'И масштабирует сам: «<span class="hl">dynamically scales the degree of concurrency to use all the available processors most efficiently</span>».', nodes: [{ id: "t", kind: "gate", at: { zone: "tpl", row: 0 }, state: "ok", label: "твой код", detail: "операция" }, { id: "r", kind: "gate", at: { zone: "runtime", row: 0 }, state: "ok", label: "auto-scale", detail: "по числу ядер", accent: true }], edges: [] },
      ],
      explain: 'TPL — рекомендованный высокий уровень многопоточности: «The Task Parallel Library (TPL) is a set of public types and APIs in the <code>System.Threading</code> and <code>System.Threading.Tasks</code> namespaces». Ценность — в сокрытии: «<span class="hl">The TPL dynamically scales the degree of concurrency to use all the available processors most efficiently. In addition, the TPL handles the partitioning of the work, the scheduling of threads on the ThreadPool</span>… <span class="hl">cancellation support, state management, and other low-level details</span>». Это «data parallelism»: «Data parallelism refers to scenarios in which <span class="hl">the same operation is performed concurrently (that is, in parallel) on elements in a source collection or array</span>». Реализация — «the <code>System.Threading.Tasks.Parallel</code> class. This class provides method-based parallel implementations of <code>for</code> and <code>foreach</code> loops».',
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, out: "", caption: 'Режется на <b>партиции</b>: «<span class="hl">the TPL partitions the data source so that the loop can operate on multiple parts concurrently</span>».', nodes: [{ id: "src", kind: "obj", at: { zone: "source", row: 0 }, typeTag: "[0..n)", value: "источник" }, { id: "p", kind: "obj", at: { zone: "parts", row: 0 }, typeTag: "partitions", value: "по потокам", accent: true }], edges: [{ id: "e1", from: "src", to: "p", accent: true }] },
        { codeLine: 2, out: "", caption: 'При перекосе — <b>ребаланс</b>: «the scheduler <span class="hl">redistributes work among multiple threads and processors if the workload becomes unbalanced</span>».', nodes: [{ id: "p", kind: "obj", at: { zone: "parts", row: 0 }, typeTag: "partitions", value: "работают" }, { id: "rb", kind: "gate", at: { zone: "parts", row: 1 }, state: "ok", label: "rebalance", detail: "при дисбалансе", accent: true }], edges: [] },
      ],
      explain: 'Партиционирование — сердце data parallelism: «When a parallel loop runs, the TPL partitions the data source so that the loop can operate on multiple parts concurrently. <span class="hl">Behind the scenes, the Task Scheduler partitions the task based on system resources and workload. When possible, the scheduler redistributes work among multiple threads and processors if the workload becomes unbalanced</span>». То есть число и размер партиций — не фиксированы: планировщик подбирает их под ядра и нагрузку и умеет докидывать работу простаивающим потокам (work-stealing). Можно и переопределить: «You can also supply your own <span class="hl">custom partitioner or scheduler</span>». Порядок обработки при этом <b>не</b> гарантирован — параллельный цикл не сохраняет последовательность (в отличие от <code>foreach</code>).',
      sources: ["ms-data"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'Крошечное тело / мало итераций: «<span class="hl">the overhead of parallelization can cause the code to run more slowly</span>».', nodes: [{ id: "s", kind: "gate", at: { zone: "small", row: 0 }, state: "fail", label: "мелкое тело", detail: "overhead > выигрыш", accent: true }], edges: [] },
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>ParallelOptions.MaxDegreeOfParallelism</code> — потолок конкурентности: одна из перегрузок, что «<span class="hl">control the degree of concurrency</span>».', nodes: [{ id: "o", kind: "gate", at: { zone: "mdop", row: 0 }, state: "ok", label: "MaxDegreeOfParallelism = 1", detail: "потолок = 1", accent: true }], edges: [] },
  ...
  cards: [
    ...
    },
    {
      ...
      okText: '<code>MaxDegreeOfParallelism = 1</code> «control the degree of concurrency» → потолок 1: все 20 итераций идут на <span class="hl">одном</span> потоке. Различных <code>ManagedThreadId</code> — <b>1</b>.',
  ...
  takeaways: [
    { icon: "why", k: "Что это", v: 'TPL data parallelism: «<span class="hl">the same operation is performed concurrently… on elements in a source collection</span>». <code>Parallel.For/ForEach</code> прячут «the partitioning of the work, the scheduling of threads on the <code>ThreadPool</code>» и «dynamically scales the degree of concurrency».' },
    { icon: "cost", k: "Не бесплатно", v: '«if a loop performs only a <span class="hl">small amount of work</span>… the overhead of parallelization can cause the code to run more slowly». Тело на нескольких потоках → синхронизируй общее состояние (sum=5050 через Interlocked). Порядок не сохраняется.' },
    { icon: "avoid", k: "degree of parallelism", v: '<code>MaxDegreeOfParallelism</code> ограничивает конкурентность; =1 → один поток (distinctThreads=1), фактически последовательно. Перегрузки «control the degree of concurrency», дают <code>ParallelLoopState</code>, отмену, thread-local.' },
  ],
