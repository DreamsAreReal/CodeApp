...
export const aggregateException: LessonData = {
  ...
  spec: [
    { text: "«AggregateException is used to consolidate multiple failures into a single, throwable exception object.»", source: "ms-aggapi" },
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 1, out: "", caption: '<code>AggregateException</code> — «<span class="hl">consolidate multiple failures into a single, throwable exception object</span>». Обёртка над списком настоящих причин.', nodes: [{ id: "m1", kind: "obj", at: { zone: "many", row: 0 }, typeTag: "task1", value: "IOException" }, { id: "m2", kind: "obj", at: { zone: "many", row: 1 }, typeTag: "task2", value: "ArgumentException" }, { id: "a", kind: "obj", at: { zone: "one", row: 0 }, typeTag: "AggregateException", value: "InnerExceptions", accent: true }], edges: [{ id: "e1", from: "m1", to: "a" }, { id: "e2", from: "m2", to: "a", accent: true }] },
        { codeLine: 2, out: "", caption: 'Перебираешь <code>InnerExceptions</code> — «<span class="hl">a read-only collection of the Exception instances that caused the current exception</span>» — и обрабатываешь каждую.', nodes: [{ id: "a", kind: "obj", at: { zone: "one", row: 0 }, typeTag: "AggregateException", value: "InnerExceptions" }, { id: "i", kind: "chip", at: { zone: "one", row: 1 }, value: "[IOException, ArgumentException]", accent: true }], edges: [] },
      ],
      explain: '<code>AggregateException</code> решает проблему «много ошибок — один канал возврата». «<span class="hl">AggregateException is used to consolidate multiple failures into a single, throwable exception object</span>. It is used extensively in the Task Parallel Library (TPL) and Parallel LINQ (PLINQ)». Когда «you are waiting on multiple tasks, <span class="hl">multiple exceptions could be thrown</span>» — TPL не может бросить их по одному, поэтому «the Task infrastructure <span class="hl">wraps them in an AggregateException instance</span>». Реальные причины лежат в <code>InnerExceptions</code> — «a <span class="hl">read-only collection of the Exception instances that caused the current exception</span>», которую «can be enumerated to examine all the original exceptions that were thrown, and handle (or not handle) each one individually». То есть агрегат — не «ошибка», а <b>перечислимый контейнер</b> ошибок.',
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: 'Нет: <code>.Wait()</code> всё равно оборачивает. «<span class="hl">Even if only one exception is thrown, it is still wrapped in an AggregateException</span>». Ловишь <code>AggregateException</code>.', nodes: [{ id: "t", kind: "obj", at: { zone: "task", row: 0 }, typeTag: "Task.Run", value: "throw InvalidOp" }, { id: "a", kind: "obj", at: { zone: "wrap", row: 0 }, typeTag: "AggregateException", value: "Count=1", accent: true }], edges: [{ id: "e1", from: "t", to: "a", accent: true }] },
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: '<code>Flatten()</code> — «<span class="hl">Flattens an AggregateException instances into a single, new instance</span>» — убирает вложенные агрегаты, оставляя настоящие причины.', nodes: [{ id: "o", kind: "obj", at: { zone: "nested", row: 0 }, typeTag: "outer", value: "агрегат внутри" }, { id: "f", kind: "obj", at: { zone: "flat", row: 0 }, typeTag: "Flatten()", value: "x, y", accent: true }], edges: [{ id: "e1", from: "o", to: "f", accent: true }] },
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'Поймать агрегат и <b>не</b> разбирать — плохо: «analogous to <span class="hl">catching the base Exception type in non-parallel scenarios</span>» — прячет реальные ошибки.', nodes: [{ id: "b", kind: "gate", at: { zone: "bad", row: 0 }, state: "fail", label: "catch без разбора", detail: "indeterminate state", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Правильно — перебрать <code>InnerExceptions</code> и «<span class="hl">handle (or not handle) each one individually</span>», или отдать в <code>Handle</code>. Настоящие причины — внутри.', nodes: [{ id: "b", kind: "gate", at: { zone: "bad", row: 0 }, state: "fail", label: "catch без разбора", detail: "плохо" }, { id: "g", kind: "gate", at: { zone: "good", row: 0 }, state: "ok", label: "foreach InnerExceptions", detail: "разобрать", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: 'А совсем не наблюдать задачу — ошибка всплывёт поздно: «the exception is <span class="hl">escalated</span>… when the task is garbage-collected». «Забыл дождаться» = скрытый сбой.', nodes: [{ id: "g", kind: "gate", at: { zone: "good", row: 0 }, state: "ok", label: "наблюдай", detail: "InnerExceptions" }, { id: "e", kind: "gate", at: { zone: "bad", row: 1 }, state: "fail", label: "не наблюдал", detail: "эскалация @ GC", accent: true }], edges: [] },
      ],
      explain: 'Агрегат надо <b>наблюсти по существу</b>, а не просто поймать. «You could avoid an unhandled exception by just catching the <code>AggregateException</code> and not observing any of the inner exceptions. However, we <span class="hl">recommend that you do not do this</span> because it is analogous to catching the base <code>Exception</code> type in non-parallel scenarios. To catch an exception without taking specific actions to recover from it can <span class="hl">leave your program in an indeterminate state</span>». Наблюдать можно и без ожидания — «you can also retrieve the <code>AggregateException</code>… from the task\'s <code>Exception</code> property» при <code>TaskStatus.Faulted</code>. А полное игнорирование опасно: «If you do <span class="hl">not wait on a task that propagates an exception, or access its Exception property, the exception is escalated</span>… when the task is garbage-collected». Правило: перебери <code>InnerExceptions</code> (или зови <code>Handle</code>), обработай каждую причину, неизвестное — переброс.',
      sources: ["ms-tplexc"],
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: '<code>Handle</code> зовёт делегат на <b>каждую</b> внутреннюю. Вернул <code>true</code> для <code>C</code> — «<span class="hl">treat as handled</span>»: одна обработана.', nodes: [{ id: "h", kind: "gate", at: { zone: "handle", row: 0, col: 0 }, state: "ok", label: "C → true", detail: "обработано", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Для <code>InvalidOperationException</code> делегат вернул <code>false</code>: «Any exceptions for which the delegate returns <code>false</code> are <span class="hl">rethrown in a new AggregateException instance</span>».', nodes: [{ id: "h", kind: "gate", at: { zone: "handle", row: 0, col: 0 }, state: "ok", label: "C → true", detail: "обработано" }, { id: "f", kind: "gate", at: { zone: "handle", row: 0, col: 1 }, state: "fail", label: "InvalidOp → false", detail: "переброс", accent: true }], edges: [] },
  ...
  cards: [
    {
      ...
      okText: '«<span class="hl">Even if only one exception is thrown, it is still wrapped in an AggregateException</span>»: <code>.Wait()</code> ловится как агрегат, <code>Count==1</code>, а настоящий тип — в <code>InnerExceptions[0]</code>. Печать: <b>AggregateException|count=1|inner=InvalidOperationException</b>.',
    ...
    },
    {
      ...
      okText: '<code>Handle</code> зовёт делегат на каждую: <code>C</code>→<code>true</code> (обработано, handled=1), <code>InvalidOp</code>→<code>false</code>. «Any exceptions for which the delegate returns <code>false</code> are <span class="hl">rethrown in a new AggregateException</span>» → 1 переброшена. Печать: <b>handled=1 rethrown=1</b>.',
    ...
    },
    {
      ...
      okText: '<code>outer</code> держит один вложенный агрегат (x, y). <code>Flatten()</code> «<span class="hl">Flattens an AggregateException instances into a single, new instance</span>»: 2 настоящих <code>Exception</code>, вложенных агрегатов нет. Печать: <b>count=2 anyNested=False</b>.',
  ...
  takeaways: [
    { icon: "why", k: "Контейнер, не ошибка", v: '«<span class="hl">AggregateException is used to consolidate multiple failures into a single, throwable exception object</span>». TPL «<span class="hl">wraps them in an AggregateException</span>» — «even if only one» (замер: AggregateException|count=1). Причины — в <code>InnerExceptions</code>.' },
    { icon: "cost", k: "Flatten", v: 'Attached child → «<span class="hl">one or more AggregateException instances, not the original exceptions</span>». <code>Flatten()</code> «<span class="hl">Flattens… into a single, new instance</span>» → один цикл (замер: count=2 anyNested=False).' },
    { icon: "avoid", k: "Handle / наблюдай", v: '<code>Handle(ex=>bool)</code>: <code>true</code>=обработано, <code>false</code>=«<span class="hl">rethrown in a new AggregateException</span>» (замер: handled=1 rethrown=1). Не глотай агрегат целиком (indeterminate state); не наблюдёно → эскалация при GC.' },
  ],
