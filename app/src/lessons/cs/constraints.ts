...
export const constraints: LessonData = {
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'Без constraint <code>T</code> может быть <b>любым</b> типом, поэтому компилятор «can only assume the <span class="hl">members of System.Object</span>»: <code>ToString</code>, <code>Equals</code>, <code>GetHashCode</code> — и всё.', nodes: [{ id: "n", kind: "gate", at: { zone: "none", row: 0 }, state: "fail", label: "x.Foo()", detail: "нет доступа", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: '<code>where T : Employee</code> — гарантия: «only objects of this type or derived from this type can replace that type argument». Теперь <span class="hl">члены Employee</span> доступны.', nodes: [{ id: "n", kind: "gate", at: { zone: "none", row: 0 }, state: "fail", label: "x.Foo()", detail: "нет доступа" }, { id: "w", kind: "gate", at: { zone: "with", row: 0 }, state: "ok", label: "e.Name", detail: "разблокирован", accent: true }], edges: [{ id: "e", from: "n", to: "w", accent: true }] },
      ],
      explain: 'Constraint — это <b>контракт возможностей</b> типа-аргумента. Дословно: «Constraints inform the compiler about the capabilities a type argument must have. Without any constraints, the type argument could be any type. <span class="hl">The compiler can only assume the members of System.Object</span>, which is the ultimate base class for any .NET type». Смысл прямой: «You apply constraints to the type parameter when your generic class or method uses any operation on the generic members beyond simple assignment, which includes calling any methods not supported by <code>System.Object</code>». Пример базового constraint: «the base class constraint tells the compiler that only objects of this type or derived from this type can replace that type argument. Once the compiler has this guarantee, it can allow methods of that type to be called in the generic class». Дальше — весь словарь гейтов.',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>class</code> — «The type argument must be a <span class="hl">reference type</span>». Пускает класс, интерфейс, делегат, массив.', nodes: [{ id: "c", kind: "gate", at: { zone: "cat", row: 0 }, state: "ok", label: "class", detail: "reference type", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: '<code>struct</code> — «a <span class="hl">non-nullable value type</span>, which includes <code>record struct</code>». <code>int?</code> отсекается (CS0453). Подразумевает <code>new()</code>.', nodes: [{ id: "c", kind: "gate", at: { zone: "cat", row: 0 }, state: "ok", label: "class", detail: "reference type" }, { id: "s", kind: "gate", at: { zone: "cat", row: 1 }, state: "ok", label: "struct", detail: "non-nullable value", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: '<code>notnull</code> — «a <span class="hl">non-nullable type</span>»: либо non-nullable value, либо non-nullable reference. Нарушение — <b>warning</b>, не error.', nodes: [{ id: "c", kind: "gate", at: { zone: "cat", row: 0 }, state: "ok", label: "class", detail: "reference type" }, { id: "s", kind: "gate", at: { zone: "cat", row: 1 }, state: "ok", label: "struct", detail: "non-nullable value" }, { id: "nn", kind: "gate", at: { zone: "cat", row: 2, col: 0 }, state: "ok", label: "notnull", detail: "value или ref", accent: true }], edges: [] },
        { codeLine: 3, out: "", caption: '<code>unmanaged</code> — «a non-nullable <span class="hl">unmanaged type</span>»: блоки памяти без ссылок (int, Guid, свой struct из value-полей). Подразумевает <code>struct</code>.', nodes: [{ id: "c", kind: "gate", at: { zone: "cat", row: 0 }, state: "ok", label: "class", detail: "reference type" }, { id: "s", kind: "gate", at: { zone: "cat", row: 1 }, state: "ok", label: "struct", detail: "non-nullable value" }, { id: "nn", kind: "gate", at: { zone: "cat", row: 2, col: 0 }, state: "ok", label: "notnull", detail: "value или ref" }, { id: "um", kind: "gate", at: { zone: "cat", row: 2, col: 1 }, state: "ok", label: "unmanaged", detail: "блок памяти", accent: true }], edges: [] },
      ],
      explain: 'Четыре <b>гейта категории</b> отвечают на вопрос «какого рода тип». <code>class</code>: «The type argument must be a reference type. This constraint applies also to any class, interface, delegate, or array type». <code>struct</code>: «The type argument must be a non-nullable value type, which includes <code>record struct</code> types… the <code>struct</code> constraint implies the <code>new()</code> constraint». <code>notnull</code>: «The type argument must be a non-nullable type. The argument can be a non-nullable reference type or a non-nullable value type» — причём нарушение даёт warning, не error. <code>unmanaged</code>: «The type argument must be a non-nullable unmanaged type… The <code>unmanaged</code> constraint implies the <code>struct</code> constraint». Правило комбинирования: «You can apply <b>at most one</b> of the <code>struct</code>, <code>class</code>, <code>class?</code>, <code>notnull</code>, and <code>unmanaged</code> constraints. If you supply any of these constraints, it must be the <span class="hl">first constraint</span> specified».',
      sources: ["ms-constraints"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<b>Базовый тип</b>: «The type argument must be <span class="hl">or derive from the specified base class</span>». <code>where T : Employee</code> открывает <code>e.Name</code> для T и наследников.', nodes: [{ id: "b", kind: "gate", at: { zone: "base", row: 0 }, state: "ok", label: "T : Employee", detail: "e.Name открыт", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: '<b>Интерфейс</b>: «must be <span class="hl">or implement the specified interface</span>». <code>where T : IComparable&lt;T&gt;</code> открывает <code>CompareTo</code>. «Multiple interface constraints can be specified».', nodes: [{ id: "b", kind: "gate", at: { zone: "base", row: 0 }, state: "ok", label: "T : Employee", detail: "e.Name открыт" }, { id: "i", kind: "gate", at: { zone: "iface", row: 0 }, state: "ok", label: "T : IComparable<T>", detail: "CompareTo открыт", accent: true }], edges: [] },
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>new()</code> — «The type argument must have a <span class="hl">public parameterless constructor</span>». Позволяет <code>T t = new();</code> внутри дженерика.', nodes: [{ id: "n", kind: "gate", at: { zone: "newc", row: 0 }, state: "ok", label: "new()", detail: "T t = new()", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Порядок жёсткий: гейт категории <b>первым</b>, <code>new()</code> — <span class="hl">последним</span>. «the <code>new()</code> constraint must be specified last».', nodes: [{ id: "n", kind: "gate", at: { zone: "newc", row: 0 }, state: "ok", label: "new()", detail: "последним ✓", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: '<b>Naked</b> <code>T : U</code> — связывает два параметра: «must be <span class="hl">or derive from the argument supplied for U</span>». <code>Add&lt;U&gt;</code> берёт только <code>U</code>, совместимые с <code>T</code>.', nodes: [{ id: "n", kind: "gate", at: { zone: "newc", row: 0 }, state: "ok", label: "new()", detail: "последним" }, { id: "u", kind: "gate", at: { zone: "naked", row: 0 }, state: "ok", label: "U : T", detail: "U ⊆ T", accent: true }], edges: [] },
      ],
      explain: '<code>new()</code> и naked-type-parameter constraint — про <b>создание</b> и <b>связь параметров</b>. <code>new()</code>: «The type argument must have a public parameterless constructor. When used together with other constraints, <span class="hl">the <code>new()</code> constraint must be specified last</span>». Он не комбинируется со <code>struct</code>/<code>unmanaged</code> (те его уже подразумевают). Naked <code>T : U</code>: «The type argument supplied for <code>T</code> must be or derive from the argument supplied for <code>U</code>» — используется, «when a member function with its own type parameter has to constrain that parameter to the type parameter of the containing type», как <code>Add&lt;U&gt;(List&lt;U&gt;) where U : T</code>. Полный пример из доки: <code>class EmployeeList&lt;T&gt; where T : notnull, Employee, IComparable&lt;T&gt;, new()</code> — категория, база, интерфейс, ctor в правильном порядке.',
      sources: ["ms-constraints"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>allows ref struct</code> — <b>анти-ограничение</b>: «declares that the type argument for <code>T</code> <span class="hl">can be a <code>ref struct</code> type</span>» (например <code>Span&lt;T&gt;</code>). Расширяет, а не сужает.', nodes: [{ id: "a", kind: "gate", at: { zone: "allow", row: 0 }, state: "ok", label: "allows ref struct", detail: "T = Span<T> ок", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Цена: «The generic type or method must <span class="hl">obey ref safety rules</span>» — <code>T</code> нельзя боксить, нельзя класть в static-поля. Идёт «must follow all constraints».', nodes: [{ id: "a", kind: "gate", at: { zone: "allow", row: 0 }, state: "ok", label: "allows ref struct", detail: "ref safety" }, { id: "r", kind: "gate", at: { zone: "allow", row: 1 }, state: "fail", label: "box(T)", detail: "запрещён", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: '<code>default</code> — тонкий: «resolves the ambiguity when you need to specify an <span class="hl">unconstrained type parameter</span> when you override a method or provide an explicit interface implementation». Ни <code>class</code>, ни <code>struct</code>.', nodes: [{ id: "a", kind: "gate", at: { zone: "allow", row: 0 }, state: "ok", label: "allows ref struct", detail: "ref safety" }, { id: "d", kind: "gate", at: { zone: "defc", row: 0 }, state: "ok", label: "default", detail: "снимает ambiguity", accent: true }], edges: [] },
      ],
      explain: 'Два новых члена словаря — часто пропускаемые. <code>allows ref struct</code> (C# 13) — это <b>анти-ограничение</b>: «This anti-constraint declares that the type argument for <code>T</code> can be a <code>ref struct</code> type. The generic type or method must <span class="hl">obey ref safety rules</span> for any instance of <code>T</code> because it might be a <code>ref struct</code>». Правила для такого <code>T</code>: «It can\'t be boxed», «Instances can\'t be used where a <code>ref struct</code> type isn\'t allowed, such as <code>static</code> fields», и оно «must follow all constraints for that type parameter» и «can\'t be combined with the <code>class</code> or <code>class?</code> constraint». <code>default</code> — редкий: «This constraint resolves the ambiguity when you need to specify an unconstrained type parameter when you override a method or provide an explicit interface implementation… The <code>default</code> constraint implies the base method without either the <code>class</code> or <code>struct</code> constraint».',
      sources: ["ms-constraints"],
    },
    {
  ...
  cards: [
    {
      ...
      okText: '<code>where T : Employee</code> разблокирует <code>e.Name</code> (без него — CS1061: компилятор «can only assume the members of System.Object»). <code>Manager</code> — наследник, проходит гейт → <span class="hl">Ann</span>.',
    ...
    },
    {
      ...
      okText: '<code>where T : unmanaged</code> позволяет <code>sizeof(T)</code> в unsafe-контексте: «reusable routines to work with types that can be manipulated as <span class="hl">blocks of memory</span>». <code>int</code>=4, <code>double</code>=8, <code>Guid</code>=16.',
  ...
  takeaways: [
    { icon: "why", k: "Гейт возможностей", v: 'Без constraint компилятор «can only assume the <span class="hl">members of System.Object</span>». Базовый/интерфейсный constraint разблокирует члены типа (замер: <code>e.Name</code> → Ann, <code>CompareTo</code> → 9/pear).' },
    ...
    { icon: "avoid", k: "Импликации и порядок", v: '<code>struct</code> ⇒ <code>new()</code>, <code>unmanaged</code> ⇒ <code>struct</code> (не комбинировать). <code>allows ref struct</code> «must follow all constraints», запрещает боксинг T. Правило: «maximum constraints possible that will still let you handle the types you must handle».' },
  ],
