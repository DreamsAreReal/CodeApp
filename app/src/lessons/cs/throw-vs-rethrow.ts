...
export const throwVsRethrow: LessonData = {
  ...
  spec: [
    { text: "«Inside a catch block, use a throw; statement to re-throw the exception that the catch block handles.» <span class=\"ru-tr\">«Внутри блока catch используйте инструкцию throw;, чтобы перебросить исключение, которое обрабатывает этот блок catch.»</span>", source: "ms-stmts" },
    { text: "«throw; preserves the original stack trace of the exception, which is stored in the Exception.StackTrace property. In contrast, throw e; updates the StackTrace property of e.» <span class=\"ru-tr\">«throw; сохраняет исходную трассировку стека исключения, которая хранится в свойстве Exception.StackTrace. Напротив, throw e; обновляет свойство StackTrace объекта e.»</span>", source: "ms-stmts" },
  ],
  edgeCases: [
    { text: "Пойманное можно бросить снова: «You can also throw a caught exception again using the <code>throw</code> statement». <span class=\"ru-tr\">«Вы также можете снова бросить пойманное исключение с помощью инструкции <code>throw</code>».</span>", source: "ms-rethrow" },
    { text: "При перебросе добавляй контекст: «It's good coding practice to <b>add information to an exception that's rethrown</b> to provide more information when debugging». <span class=\"ru-tr\">«Это хорошая практика программирования — <b>добавлять информацию к исключению, которое перебрасывается</b>, чтобы дать больше сведений при отладке».</span>", source: "ms-rethrow" },
    { text: "<code>throw e;</code> требует конвертируемости: «In a <code>throw e;</code> statement, the result of expression <code>e</code> must be implicitly convertible to <code>System.Exception</code>». <span class=\"ru-tr\">«В инструкции <code>throw e;</code> результат выражения <code>e</code> должен быть неявно приводим к <code>System.Exception</code>».</span>", source: "ms-stmts" },
  ],

  misconceptions: [
    {
      wrong: "throw ex; и throw; внутри catch — одно и то же; логируй и перебрось через throw ex;",
      hook: 'Разница <b>критична</b> для диагностики. «Inside a <code>catch</code> block, use a <span class="hl">throw; statement to re-throw the exception that the catch block handles</span>» <span class="ru-tr">«Внутри блока <code>catch</code> используйте инструкцию throw;, чтобы перебросить исключение, которое обрабатывает этот блок catch».</span>. Отличие от <code>throw e;</code> — в стеке: «<span class="hl">throw; preserves the original stack trace of the exception, which is stored in the Exception.StackTrace property</span>. In contrast, <span class="hl">throw e; updates the StackTrace property of e</span>» <span class="ru-tr">«throw; сохраняет исходную трассировку стека исключения, которая хранится в свойстве Exception.StackTrace. Напротив, throw e; обновляет свойство StackTrace объекта e».</span>. То есть <code>throw e;</code> <b>переписывает</b> место падения на строку переброса — исходный кадр теряется. Третий путь — <b>обёртка</b>: бросить новое, передав пойманное как <code>innerException</code>, чтобы «<span class="hl">add information to an exception that\'s rethrown</span>» <span class="ru-tr">«добавить информацию к исключению, которое перебрасывается».</span>. Дальше <b>пять разборов</b>: <code>throw;</code> как переброс того же, preserve vs update, обёртка через inner, потеря кадра Origin, и <b>машинная панель</b> — жив ли метод-источник в <code>StackTrace</code> (реальный прогон: throw;→True, throw e;→False).',
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: '<code>throw;</code> <b>без операнда</b> — «a <span class="hl">throw; statement to re-throw the exception that the catch block handles</span>» <span class="ru-tr">«инструкция throw;, чтобы перебросить исключение, которое обрабатывает этот блок catch».</span>. Перебрасывается <b>то же самое</b> исключение.', nodes: [{ id: "c", kind: "gate", at: { zone: "caught", row: 0 }, state: "fail", label: "catch (e)", detail: "log" }, { id: "r", kind: "gate", at: { zone: "reth", row: 0 }, state: "ok", label: "throw;", detail: "тот же объект", accent: true }], edges: [{ id: "e1", from: "c", to: "r", accent: true }] },
      ...
      explain: 'Переброс без операнда — базовый инструмент «поймать, что-то сделать, пустить дальше». «Inside a <code>catch</code> block, use a <span class="hl">throw; statement to re-throw the exception that the catch block handles</span>» <span class="ru-tr">«Внутри блока <code>catch</code> используйте инструкцию throw;, чтобы перебросить исключение, которое обрабатывает этот блок catch».</span>. То же и в reference по <code>try-catch</code>: «To re-throw a caught exception, use the <code>throw</code> statement» <span class="ru-tr">«Чтобы перебросить пойманное исключение, используйте инструкцию <code>throw</code>».</span>. Смысл — не <b>обработать</b> (оставить приложение в известном состоянии ты не можешь/не хочешь), а вклиниться с побочным эффектом (лог, метрика, очистка) и пропустить исключение к тому, кто действительно его обработает. Ключевое — <code>throw;</code> отдаёт <b>тот же объект</b> с его состоянием, а не создаёт новый. Разница с <code>throw e;</code> — в следующем разборе.',
      sources: ["ms-stmts"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>throw;</code> — «<span class="hl">preserves the original stack trace of the exception</span>, which is stored in the <code>Exception.StackTrace</code> property» <span class="ru-tr">«сохраняет исходную трассировку стека исключения, которая хранится в свойстве <code>Exception.StackTrace</code>».</span>. Стек указывает на <b>настоящее</b> место падения.', nodes: [{ id: "k", kind: "gate", at: { zone: "keep", row: 0 }, state: "ok", label: "throw;", detail: "стек ИСХОДНЫЙ", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: '<code>throw e;</code> — «In contrast, <span class="hl">throw e; updates the StackTrace property of e</span>» <span class="ru-tr">«Напротив, throw e; обновляет свойство StackTrace объекта e».</span>. Стек <b>переписывается</b> на строку с <code>throw e;</code>.', nodes: [{ id: "k", kind: "gate", at: { zone: "keep", row: 0 }, state: "ok", label: "throw;", detail: "стек ИСХОДНЫЙ" }, { id: "r", kind: "gate", at: { zone: "reset", row: 0 }, state: "fail", label: "throw e;", detail: "стек ПЕРЕЗАПИСАН", accent: true }], edges: [] },
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, out: "", caption: '<code>Middle</code> ловит и делает <code>throw e;</code>. Рантайм «<span class="hl">updates the StackTrace property of e</span>» <span class="ru-tr">«обновляет свойство StackTrace объекта e».</span> — снимок стека начинается заново <b>отсюда</b>.', nodes: [{ id: "o", kind: "gate", at: { zone: "stackw", row: 0 }, state: "fail", label: "Origin()", detail: "стёрт из стека" }, { id: "m", kind: "gate", at: { zone: "stackw", row: 1 }, state: "fail", label: "Middle: throw e;", detail: "новый старт стека", accent: true }], edges: [{ id: "e1", from: "m", to: "o", accent: true }] },
      ...
      explain: 'Наглядная цена <code>throw e;</code> — потеря кадра источника. Поскольку «<code>throw e;</code> <span class="hl">updates the StackTrace property of e</span>» <span class="ru-tr">«<code>throw e;</code> обновляет свойство StackTrace объекта e».</span>, снимок стека начинается со строки <code>throw e;</code> в <code>Middle</code>, а кадр <code>Origin</code> (где действительно бросили) в него не попадает. В проде это означает «стектрейс показывает обработчик, а не баг» — часы отладки не туда. С <code>throw;</code> «<span class="hl">preserves the original stack trace</span>» <span class="ru-tr">«сохраняет исходную трассировку стека».</span>, и <code>Origin</code> остаётся в трейсе. Детерминированный признак (следующий разбор): проверяем, <b>есть</b> ли подстрока <code>"Origin"</code> в <code>StackTrace</code>, — с <code>throw;</code> она есть, с <code>throw e;</code> её нет. Практика ревью: <code>throw ex;</code> внутри <code>catch</code> — почти всегда ошибка; либо <code>throw;</code>, либо осознанная обёртка с <code>innerException</code>.',
      sources: ["ms-stmts"],
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: '<code>throw;</code> «preserves the original stack trace» <span class="ru-tr">«сохраняет исходную трассировку стека».</span> → кадр <code>Origin</code> <b>на месте</b>: <code>StackTrace.Contains("Origin") == True</code>.', nodes: [{ id: "t1", kind: "gate", at: { zone: "t1", row: 0 }, state: "ok", label: "throw;", detail: "True", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: '<code>throw e;</code> «updates the StackTrace property of e» <span class="ru-tr">«обновляет свойство StackTrace объекта e».</span> → кадр <code>Origin</code> <b>стёрт</b>: <code>StackTrace.Contains("Origin") == False</code>.', nodes: [{ id: "t1", kind: "gate", at: { zone: "t1", row: 0 }, state: "ok", label: "throw;", detail: "True" }, { id: "t2", kind: "gate", at: { zone: "t2", row: 0 }, state: "fail", label: "throw e;", detail: "False", accent: true }], edges: [] },
  ...
  cards: [
    {
      ...
      okText: '<code>throw;</code> «<span class="hl">preserves the original stack trace</span>» <span class="ru-tr">«сохраняет исходную трассировку стека».</span> — кадр метода-источника <code>Origin</code> остаётся в <code>StackTrace</code>. Проверка <code>Contains("Origin")</code> → <b>True</b>.',
    ...
    },
    {
      ...
      okText: '<code>throw e;</code> «<span class="hl">updates the StackTrace property of e</span>» <span class="ru-tr">«обновляет свойство StackTrace объекта e».</span> — снимок стека переписывается со строки переброса; кадр <code>Origin</code> стёрт. <code>Contains("Origin")</code> → <b>False</b>.',
    ...
    },
    {
      ...
      okText: 'Один источник, два способа переброса: <code>throw;</code> хранит кадр <code>Origin</code> (True), <code>throw e;</code> его теряет (False). «throw; preserves… throw e; updates the StackTrace» <span class="ru-tr">«throw; сохраняет… throw e; обновляет StackTrace».</span>. Печать: <b>rethrow=True throwEx=False</b>.',
  ...
  takeaways: [
    { icon: "why", k: "throw;", v: '«Inside a <code>catch</code> block, use a <span class="hl">throw; statement to re-throw the exception that the catch block handles</span>» <span class="ru-tr">«Внутри блока <code>catch</code> используйте инструкцию throw;, чтобы перебросить исключение, которое обрабатывает этот блок catch».</span> — перебрасывает ТОТ ЖЕ объект. Паттерн «залогировать и пропустить».' },
    { icon: "cost", k: "preserve vs update", v: '«<span class="hl">throw; preserves the original stack trace</span>… In contrast, <span class="hl">throw e; updates the StackTrace property of e</span>» <span class="ru-tr">«throw; сохраняет исходную трассировку стека… Напротив, throw e; обновляет свойство StackTrace объекта e».</span>. Признак сохранения (замер): rethrow=True throwEx=False.' },
    { icon: "avoid", k: "throw ex; = потеря", v: '<code>throw ex;</code> стирает кадр источника — «упало будто в обработчике». Либо <code>throw;</code>, либо обёртка <code>throw new …(msg, e)</code> с <code>innerException</code> — «add information to an exception that\'s rethrown» <span class="ru-tr">«добавить информацию к исключению, которое перебрасывается».</span>.' },
  ],
