...
export const asyncEventHandlers: LessonData = {
  ...
  spec: [
    { text: "«Event handlers must declare <code>void</code> return types and can't use or return <code>Task</code> and <code>Task&lt;T&gt;</code> objects as other methods do.» <span class=\"ru-tr\">«Обработчики событий обязаны объявлять возвращаемый тип <code>void</code> и не могут использовать или возвращать объекты <code>Task</code> и <code>Task&lt;T&gt;</code>, как это делают другие методы.»</span>", source: "ms-async-void" },
  ],
  edgeCases: [
    { text: "Исключение из <code>async void</code> не поймать снаружи: «Exceptions thrown in an <code>async void</code> method <b>can't be caught outside of that method</b>» <span class=\"ru-tr\">«Исключения, брошенные в методе <code>async void</code>, <b>невозможно поймать за пределами этого метода</b>»</span> — реальный прогон: незакрытое исключение из async void <b>роняет процесс</b>.", source: "ms-async-void" },
    { text: "<code>async void</code> трудно тестировать: «<code>async void</code> methods are difficult to test» <span class=\"ru-tr\">«методы <code>async void</code> трудно тестировать»</span>.", source: "ms-async-void" },
    { text: "<code>async void</code> может дать скрытые эффекты: «<code>async void</code> methods can cause negative side effects if the caller isn't expecting them to be asynchronous» <span class=\"ru-tr\">«методы <code>async void</code> могут вызвать нежелательные побочные эффекты, если вызывающий код не ожидает, что они асинхронны»</span>.", source: "ms-async-void" },
  ],

  misconceptions: [
    {
      wrong: "async void — это просто async-метод без результата, использовать его можно везде",
      hook: '<code>async void</code> — это <b>не</b> «async без результата»; это опасная форма, законная лишь в одном месте — в обработчике события. «<span class="hl">Return \'async void\' only from event handlers</span>» <span class="ru-tr">«Возвращайте <code>async void</code> только из обработчиков событий»</span>. Причина — контракт обработчика: «Event handlers must declare <code>void</code> return types and can\'t use or return <code>Task</code> and <code>Task&lt;T&gt;</code> objects as other methods do. When you write asynchronous event handlers, you need to use the <code>async</code> modifier on a <code>void</code> returning method for the handlers» <span class="ru-tr">«Обработчики событий обязаны объявлять возвращаемый тип <code>void</code> и не могут использовать или возвращать объекты <code>Task</code> и <code>Task&lt;T&gt;</code>, как это делают другие методы. Когда вы пишете асинхронные обработчики событий, вам нужно применить модификатор <code>async</code> к методу с возвращаемым типом <code>void</code>»</span>. А цена — исключения: «Exceptions thrown in an <code>async void</code> method <b>can\'t be caught outside of that method</b>» <span class="ru-tr">«Исключения, брошенные в методе <code>async void</code>, <b>невозможно поймать за пределами этого метода</b>»</span> (реальный прогон: незакрытое исключение из async void роняет процесс). Дальше <b>шесть разборов</b>: контракт void, ловля исключения у <code>async Task</code>, утечка исключения у <code>async void</code>, безопасный паттерн (catch внутри), <b>машинная панель</b> — raise возвращается <b>до</b> завершения async-обработчика (реальный прогон), и ловушка async-лямбд в LINQ.',
  ...
  segments: [
    {
      ...
      explain: 'Обработчики событий — исключение из правила «async всегда возвращает Task». Дословно: «Event handlers must declare <code>void</code> return types and can\'t use or return <code>Task</code> and <code>Task&lt;T&gt;</code> objects as other methods do. When you write asynchronous event handlers, you need to use the <code>async</code> modifier on a <code>void</code> returning method for the handlers» <span class="ru-tr">«Обработчики событий обязаны объявлять возвращаемый тип <code>void</code> и не могут использовать или возвращать объекты <code>Task</code> и <code>Task&lt;T&gt;</code>, как это делают другие методы. Когда вы пишете асинхронные обработчики событий, вам нужно применить модификатор <code>async</code> к методу с возвращаемым типом <code>void</code>»</span>. Отсюда правило-заголовок из доков: «<b>Return \'async void\' only from event handlers</b>» <span class="ru-tr">«<b>Возвращайте <code>async void</code> только из обработчиков событий</b>»</span>. Причина проста: делегат события (<code>EventHandler</code>) объявляет возврат <code>void</code>, а метод обязан иметь совместимый возврат — <code>Task</code> вернуть нельзя. Во всех прочих async-методах возвращают <code>Task</code>/<code>Task&lt;T&gt;</code>, чтобы вызывающий мог дождаться (<code>await</code>) и увидеть исключения. У <code>async void</code> этой ручки нет — что и создаёт ловушки следующих разборов.',
    ...
    },
    {
      ...
      explain: 'Практический вывод из невозможности поймать исключение снаружи: async-обработчик должен ловить свои исключения <b>внутри себя</b>. Раз «Exceptions thrown in an <code>async void</code> method can\'t be caught outside of that method» <span class="ru-tr">«Исключения, брошенные в методе <code>async void</code>, невозможно поймать за пределами этого метода»</span>, единственная безопасная стратегия — <code>try/catch</code> вокруг всего тела обработчика (или маршалинг ошибки в свой канал логирования/UI). Реальный прогон подтверждает: <code>async void</code> с внутренним <code>catch</code> печатает <code>handler caught: boom</code> и спокойно доходит до <code>done</code> — процесс жив. Сравните с разбором 03, где то же исключение <b>без</b> внутреннего <code>catch</code> уронило хост. Отсюда и мотив «трудно тестировать» — «<code>async void</code> methods are difficult to test» <span class="ru-tr">«методы <code>async void</code> трудно тестировать»</span>: у них нет <code>Task</code>, за который можно ухватиться в тесте, поэтому логику выносят в отдельный <code>async Task</code>-метод, а обработчик делают тонкой обёрткой.',
      sources: ["ms-async-void"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Финальная ловушка — async-лямбды в LINQ. Дословно: «It\'s important to use caution when you implement asynchronous lambdas in LINQ expressions. <b>Lambda expressions in LINQ use deferred execution, which means the code can execute at an unexpected time</b>. The introduction of blocking tasks into this scenario can easily result in a <span class="hl">deadlock</span>, if the code isn\'t written correctly. Moreover, the nesting of asynchronous code can also make it difficult to reason about the execution of the code» <span class="ru-tr">«Важно проявлять осторожность, когда вы применяете асинхронные лямбды в выражениях LINQ. <b>Лямбда-выражения в LINQ используют отложенное выполнение, а значит код может выполниться в неожиданный момент</b>. Ввод блокирующих задач в такой сценарий легко приводит к взаимной блокировке (deadlock), если код написан неверно. Более того, вложенность асинхронного кода также может затруднить понимание того, как этот код выполняется»</span>. Две беды складываются: отложенное выполнение LINQ (запрос — рецепт, исполняется при перечислении, см. урок про deferred execution) плюс async-лямбда, чей тип возврата — <code>Task</code>/<code>async void</code> в неожиданном контексте. Итог доков трезвый: «Async and LINQ are powerful, but these techniques should be used together as carefully and clearly as possible» <span class="ru-tr">«Async и LINQ мощны, но эти приёмы следует применять вместе настолько аккуратно и ясно, насколько это возможно»</span>. На практике: не прячьте <code>await</code> внутрь <code>Select</code>/<code>Where</code>; материализуйте данные и обрабатывайте их асинхронно явно.',
      sources: ["ms-async-void"],
    },
  ],
  ...
  takeaways: [
    { icon: "why", k: "async void = только handler", v: 'Обработчик обязан вернуть <code>void</code>, потому async-обработчик = <code>async void</code>: «Event handlers must declare <code>void</code> return types and can\'t use or return <code>Task</code>…» <span class="ru-tr">«Обработчики событий обязаны объявлять возвращаемый тип <code>void</code> и не могут использовать или возвращать <code>Task</code>…»</span>. Правило: «<span class="hl">Return \'async void\' only from event handlers</span>» <span class="ru-tr">«Возвращайте <code>async void</code> только из обработчиков событий»</span>.' },
    { icon: "avoid", k: "Исключения утекают", v: 'Исключение из <code>async void</code> <b>не поймать снаружи</b>: «can\'t be caught outside of that method» <span class="ru-tr">«невозможно поймать за пределами этого метода»</span> — необработанное <span class="hl">роняет процесс</span> (реальный прогон уронил хост). Обработчик обязан ловить свои исключения <b>внутри</b> (реальный прогон: <code>handler caught: boom</code>).' },
    { icon: "cost", k: "Fire-and-forget + LINQ", v: 'Raise <b>не ждёт</b> async-обработчик: <code>Click()</code> возвращается на первом <code>await</code> (реальный прогон: <code>Click returned</code> раньше). А async-λ в LINQ — deferred execution + риск <span class="hl">deadlock</span>: «can easily result in a deadlock» <span class="ru-tr">«легко приводит к взаимной блокировке (deadlock)»</span>.' },
  ],
