...
export const valueTypesCopy: LessonData = {
  ...
  spec: [
    { text: "«A variable of a value type contains an instance of the type.» <span class=\"ru-tr\">«Переменная value-типа содержит экземпляр этого типа.»</span>", source: "ms-value-types" },
  ],
  edgeCases: [
  ...
  segments: [
    {
      ...
      explain: 'Value-тип не отделяет «переменную» от «объекта»: «A variable of a value type <b>contains an instance</b> of the type. This behavior differs from a variable of a reference type, which contains a <span class="hl">reference to an instance</span> of the type». <span class="ru-tr">«Переменная value-типа <b>содержит экземпляр</b> этого типа. Это поведение отличается от переменной reference-типа, которая содержит ссылку на экземпляр этого типа».</span> Поэтому поля структуры лежат <b>inline</b> — на стеке для локальной переменной, или внутри содержащего объекта для поля. Ни заголовка объекта, ни разыменования: адрес переменной — это и есть адрес данных.',
    ...
    },
    {
      ...
      explain: 'Присваивание value-типа копирует значение целиком: «By default, on <b>assignment</b>, passing an argument to a method, and returning a method result, you copy variable values. In the case of value-type variables, you copy the corresponding type instances». <span class="ru-tr">«По умолчанию при <b>присваивании</b>, передаче аргумента в метод и возврате результата метода вы копируете значения переменных. В случае переменных value-типов вы копируете соответствующие экземпляры типа».</span> Поэтому <code>b.X = 99</code> трогает только b: «operations on a value-type variable affect <span class="hl">only that instance</span> of the value type, stored in the variable». <span class="ru-tr">«операции над переменной value-типа затрагивают <b>только тот экземпляр</b> value-типа, что хранится в переменной».</span> Реальный вывод — <code>a=(1,2) b=(99,2)</code>: у класса тут было бы <code>a=(99,2)</code> — вот граница между value и reference.',
      sources: ["ms-value-types"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Это машинная панель урока — реально снятое число, не легенда. Табло: <code>GC.GetAllocatedBytesForCurrentThread()</code> вокруг двух операций даёт <b>struct copy: 0 bytes</b> и <b>class new: 24 bytes</b> (собственный прогон, evidence урока). Метод «returns the number of bytes allocated on the current managed thread» <span class="ru-tr">«возвращает число байт, выделенных в текущем управляемом потоке»</span> — прямой измеритель давления на кучу. Отсюда практический смысл value-типов: копия структуры не идёт в gen 0, тогда как каждый <code>new</code> класса — аллокация и будущая работа для сборщика. Тот же «Point», две разные цены.',
    ...
    },
    {
      ...
      explain: 'Передача аргумента — тот же копирующий механизм: «on assignment, <b>passing an argument to a method</b>, and returning a method result, you copy variable values». <span class="ru-tr">«при присваивании, <b>передаче аргумента в метод</b> и возврате результата метода вы копируете значения переменных».</span> Дока показывает это прямо: после <code>MutateAndDisplay(p2)</code> вызывающий видит <code>p2</code> неизменным, хотя внутри метода <code>p.X = 100</code>. Значит struct-параметр — независимая копия в кадре метода; чтобы метод менял оригинал, нужен <code>ref</code>/<code>out</code>/<code>in</code>. Это и делает большие struct дорогими в передаче — копируется весь экземпляр.',
      sources: ["ms-value-types"],
    },
  ],
  ...
  cards: [
    {
      ...
      okText: '<code>b = a</code> для структуры — <b>копия</b>: <code>b.X = 99</code> трогает только b. Оригинал <code>a</code> остался <span class="hl">(1,2)</span> — «separate memory locations». <span class="ru-tr">«отдельные области памяти».</span>',
    ...
    },
    {
      ...
      okText: 'Копия <b>мелкая</b>: <code>Number</code> расходится (0 vs 7), но <code>Tags</code>-ссылка общая — <code>Add("B")</code> виден через оба. «Both … have access to the same reference-type instance». <span class="ru-tr">«И то, и другое… имеют доступ к одному и тому же экземпляру reference-типа».</span>',
