...
export const tapModel: LessonData = {
  ...
  spec: [
    { text: "«The `async` and `await` keywords don't cause extra threads to be created. Async methods don't require multithreading because an async method doesn't run on its own thread.» <span class=\"ru-tr\">«Ключевые слова `async` и `await` не приводят к созданию дополнительных потоков. Асинхронным методам не нужна многопоточность, потому что async-метод не выполняется в собственном потоке.»</span>", source: "ms-tap" },
  ],
  edgeCases: [
    { text: "Приостановка на <code>await</code> — <b>НЕ выход</b> из метода: «doesn't constitute an exit from the method, and <code>finally</code> blocks <span class=\"hl\">don't run</span>» <span class=\"ru-tr\">«не является выходом из метода, и блоки <code>finally</code> не выполняются»</span>. Метод возобновится в той же точке.", source: "ms-tap" },
  ...
  misconceptions: [
    {
      ...
      hook: 'Самый живучий миф: «<span class="wrong">async делает код параллельным</span>» и «<span class="wrong">await создаёт поток</span>». На деле <code>async</code>/<code>await</code> — это <b>компиляторная абстракция</b>, не потоки: async-метод бежит <span class="hl">синхронно на потоке вызывающего</span> до первого <code>await</code>, а «The <code>async</code> and <code>await</code> keywords don\'t cause extra threads to be created» <span class="ru-tr">«Ключевые слова <code>async</code> и <code>await</code> не приводят к созданию дополнительных потоков»</span>. Ниже <b>пять разборов</b>: <b>машинная панель</b> — реально снятый таймлайн (до <code>await</code> — поток вызывающего; после — текущий sync-context, в консоли может отличаться, но нового потока НЕТ), что делает <code>await</code> (приостановка, не выход), <code>Task</code> как обещание, <code>async</code> без <code>await</code> = синхронно, и почему «async ≠ поток».',
      source: "ms-tap",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "before await == caller: true\ngot task, result later", caption: 'На <code>await</code> метод <b>приостановлен</b>, <span class="hl">control возвращается вызывающему</span> — тот получает <code>Task</code> и идёт дальше («got task, result later» <span class="ru-tr">«получил задачу, результат позже»</span>).', nodes: [{ id: "c", kind: "gate", at: { zone: "caller", row: 0 }, state: "ok", label: "caller продолжает", detail: "task на руках", accent: true }, { id: "a", kind: "gate", at: { zone: "asyncm", row: 0 }, state: "fail", label: "await", detail: "приостановлен" }], edges: [] },
      ...
      explain: 'Это машинная панель урока — таймлайн исполнения, снятый по <code>Thread.CurrentThread.ManagedThreadId</code>. Что <b>детерминированно верно</b> (замер 5/5): <code>before await</code> == <code>caller</code> — async-метод исполняется <span class="hl">синхронно на потоке вызывающего</span> до первого <code>await</code>. Про поток <b>после</b> <code>await</code> важно не соврать: дословно «The method <span class="hl">runs on the current synchronization context</span> and uses time on the thread only when the method is active» <span class="ru-tr">«Метод выполняется в текущем контексте синхронизации и использует время потока только тогда, когда метод активен»</span>. То есть continuation возобновляется на <b>текущем контексте синхронизации</b>: при <b>захваченном</b> контексте (UI, legacy ASP.NET) — на исходном потоке; <b>без</b> контекста (консоль, ASP.NET Core) — на потоке пула, который <b>МОЖЕТ отличаться</b> от вызвавшего (в замере встречается и <code>same=False</code>). Ключевое, что верно всегда: «The <code>async</code> and <code>await</code> keywords <b>don\'t cause extra threads to be created</b>» <span class="ru-tr">«Ключевые слова <code>async</code> и <code>await</code> <b>не приводят к созданию дополнительных потоков</b>»</span> и «an async method <span class="hl">doesn\'t run on its own thread</span>» <span class="ru-tr">«async-метод не выполняется в собственном потоке»</span> — <code>await</code> не создаёт поток и не блокирует. Миф «await держит тот же поток» верен лишь при захваченном контексте, а не как общая истина.',
      sources: ["ms-tap"],
    },
    {
      ...
      explain: 'Опровержение мифа «await разматывает стек / это return». Дословно: «The <code>await</code> operator tells the compiler that the async method can\'t continue past that point until the awaited asynchronous process is complete. In the meantime, control returns to the caller» <span class="ru-tr">«Оператор <code>await</code> сообщает компилятору, что async-метод не может продолжиться дальше этой точки, пока ожидаемый асинхронный процесс не завершится. Тем временем управление возвращается вызывающему»</span>. И критично: «The suspension of an async method at an <code>await</code> expression <span class="hl">doesn\'t constitute an exit from the method, and <code>finally</code> blocks don\'t run</span>» <span class="ru-tr">«Приостановка async-метода на выражении <code>await</code> не является выходом из метода, и блоки <code>finally</code> не выполняются»</span>. То есть <code>await</code> — не выход: компилятор регистрирует остаток метода как continuation, метод возобновится в той же точке. <code>finally</code> отработает только при реальном завершении/исключении, не при паузе.',
      sources: ["ms-tap"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, caption: '<code>Work()</code> вернул <code>Task&lt;int&gt;</code> <span class="hl">в момент приостановки</span>, а не завершения — «a promise to produce an integer result» <span class="ru-tr">«обещание произвести целочисленный результат»</span>.', nodes: [{ id: "t", kind: "obj", at: { zone: "task", row: 0 }, typeTag: "Task<int>", value: "не готов", accent: true }], edges: [] },
      ...
      explain: 'Задача — это обещание: «an async method returns a task value when its work is <b>suspended</b>… When the async method eventually completes its work, the task is marked as completed and the result, if any, is stored in the task» <span class="ru-tr">«async-метод возвращает значение-задачу, когда его работа <b>приостановлена</b>… Когда async-метод в итоге завершает свою работу, задача помечается как завершённая, и результат, если он есть, сохраняется в задаче»</span>. Дословно про содержимое: «A task <span class="hl">encapsulates information about the state</span> of the asynchronous process and, eventually, either the final result from the process or the <b>exception</b> that the process raises if it doesn\'t succeed» <span class="ru-tr">«Задача инкапсулирует информацию о состоянии асинхронного процесса и, в конечном счёте, либо итоговый результат процесса, либо <b>исключение</b>, которое процесс выбрасывает, если он не удаётся»</span>. Отсюда две вещи: (1) задача возвращается рано (на паузе), результат появляется позже; (2) исключение живёт В задаче и всплывает на <code>await</code> — фундамент урока про исключения (S2.7).',
      sources: ["ms-tap"],
    },
    {
      ...
      explain: 'Опровержение мифа «async без await всё равно асинхронен». Дословно: «the absence of <code>await</code> expressions doesn\'t cause a compiler error. If an async method <b>doesn\'t use an <code>await</code></b> operator to mark a suspension point, the method <span class="hl">executes as a synchronous method does, despite the <code>async</code> modifier</span>. The compiler issues a warning for such methods» <span class="ru-tr">«отсутствие выражений <code>await</code> не вызывает ошибки компиляции. Если async-метод <b>не использует оператор <code>await</code></b> для обозначения точки приостановки, метод выполняется так же, как синхронный метод, несмотря на модификатор <code>async</code>. Компилятор выдаёт предупреждение для таких методов»</span>. То есть <code>async</code> сам по себе ничего не «распараллеливает» и не делает асинхронным: он лишь <b>включает</b> <code>await</code> (и позволяет ждать этот метод). Асинхронность создаёт именно <code>await</code> на незавершённой операции.',
      sources: ["ms-tap"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: '<code>await</code> <b>не блокирует</b> текущий поток: «signs up the rest of the method as a continuation and returns control» <span class="ru-tr">«регистрирует оставшуюся часть метода как continuation и возвращает управление»</span>.', nodes: [{ id: "io", kind: "gate", at: { zone: "onethread", row: 0 }, state: "ok", label: "await", detail: "не блокирует" }, { id: "c", kind: "chip", at: { zone: "onethread", row: 1 }, value: "continuation вместо блокировки", accent: true }], edges: [] },
      ...
      explain: 'Ядро анти-мифа. Дословно: «An <code>await</code> expression in an async method <span class="hl">doesn\'t block the current thread</span> while the awaited task is running. Instead, the expression signs up the rest of the method as a continuation and returns control to the caller» <span class="ru-tr">«Выражение <code>await</code> в async-методе не блокирует текущий поток, пока выполняется ожидаемая задача. Вместо этого выражение регистрирует оставшуюся часть метода как continuation и возвращает управление вызывающему»</span>. И прямо: «The <code>async</code> and <code>await</code> keywords <b>don\'t cause extra threads to be created</b>. Async methods don\'t require multithreading because an async method <span class="hl">doesn\'t run on its own thread</span>. The method runs on the current synchronization context and uses time on the thread only when the method is active» <span class="ru-tr">«Ключевые слова <code>async</code> и <code>await</code> <b>не приводят к созданию дополнительных потоков</b>. Асинхронным методам не нужна многопоточность, потому что async-метод не выполняется в собственном потоке. Метод выполняется в текущем контексте синхронизации и использует время потока только тогда, когда метод активен»</span>. Для CPU-bound работы поток берут явно через <code>Task.Run</code>, «but a background thread doesn\'t help with a process that\'s just waiting» <span class="ru-tr">«но фоновый поток не помогает процессу, который просто ждёт»</span> (детали — S2.3). Асинхронность — про освобождение потока на время ожидания, а не про добавление потоков.',
      sources: ["ms-tap"],
    },
  ],
  ...
  cards: [
    {
      ...
      okText: 'Async-метод бежит <span class="hl">синхронно на потоке вызывающего</span> до первого <code>await</code> — тот же поток («same» <span class="ru-tr">«тот же»</span>). Новый поток НЕ создаётся.',
      noText: '«an async method doesn\'t run on its own thread. The method runs on the current synchronization context and uses time on the thread only when the method is active» <span class="ru-tr">«async-метод не выполняется в собственном потоке. Метод выполняется в текущем контексте синхронизации и использует время потока только тогда, когда метод активен»</span>. До <code>await</code> — тот же поток, что у caller. Реальный вывод: <b>same</b>.',
