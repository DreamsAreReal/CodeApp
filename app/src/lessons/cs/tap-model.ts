...
export const tapModel: LessonData = {
  ...
  segments: [
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, caption: '<code>Work()</code> вернул <code>Task&lt;int&gt;</code> <span class="hl">в момент приостановки</span>, а не завершения — «a promise to produce a result».', nodes: [{ id: "t", kind: "obj", at: { zone: "task", row: 0 }, typeTag: "Task<int>", value: "не готов", accent: true }], edges: [] },
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: 'Компилятор выдаёт <span class="hl">warning</span> (CS1998): «This async method lacks await… will run synchronously».', nodes: [{ id: "m", kind: "obj", at: { zone: "noawait", row: 0 }, typeTag: "async NoAwait", value: "нет await" }, { id: "w", kind: "gate", at: { zone: "noawait", row: 1 }, state: "fail", label: "CS1998", detail: "warning · синхронно", accent: true }], edges: [] },
      ],
  ...
  cards: [
    {
      ...
      noText: '«an async method doesn\'t run on its own thread… uses time on the thread only when active». До <code>await</code> — тот же поток, что у caller. Реальный вывод: <b>same</b>.',
  ...
  takeaways: [
    { icon: "why", k: "async ≠ поток", v: 'Async-метод бежит <span class="hl">синхронно на потоке вызывающего</span> до первого <code>await</code>; <code>await</code> «doesn\'t cause extra threads». Замер: caller и before-await — один поток T. Асинхронность освобождает поток на время ожидания, а не добавляет его.' },
