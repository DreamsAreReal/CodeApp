...
export const reflectionEmit: LessonData = {
  ...
  spec: [
    { text: "«…a set of managed types in the System.Reflection.Emit namespace that allow a compiler or tool to emit metadata and common intermediate language (CIL) at runtime.» <span class=\"ru-tr\">«…набор управляемых типов в пространстве имён System.Reflection.Emit, которые позволяют компилятору или инструменту эмитировать метаданные и общий промежуточный язык (CIL) в рантайме.»</span>", source: "ms-emit" },
  ],
  edgeCases: [
    { text: "<code>DynamicMethod</code> — «Define <b>lightweight global methods</b> at runtime… and execute them using <b>delegates</b>» <span class=\"ru-tr\">«Определяет <b>лёгкие глобальные методы</b> в рантайме… и выполняет их через <b>делегаты</b>»</span> (метод без класса-владельца).", source: "ms-emit" },
    { text: "<code>OpCodes</code> «Catalogs the <b>CIL instruction codes</b> you can use to build method bodies» <span class=\"ru-tr\">«Каталогизирует <b>коды инструкций CIL</b>, которыми можно строить тела методов»</span> — у каждого реальный байт (<code>add</code>=0x58).", source: "ms-emit" },
    { text: "Managed emit строже unmanaged: «provides stronger <b>semantic error checking</b> and a higher level of abstraction of the metadata than the unmanaged metadata interfaces» <span class=\"ru-tr\">«обеспечивает более строгую <b>семантическую проверку ошибок</b> и более высокий уровень абстракции метаданных, чем неуправляемые интерфейсы метаданных»</span>.", source: "ms-emit" },
  ],

  misconceptions: [
    {
      wrong: "reflection умеет только читать метаданные — сгенерировать метод на лету нельзя",
      hook: 'Reflection умеет и <span class="hl">писать код</span>. «This section describes a set of managed types in the <b>System.Reflection.Emit</b> namespace that allow a compiler or tool to <b>emit metadata and common intermediate language (CIL) at runtime</b>». <span class="ru-tr">«В этом разделе описан набор управляемых типов в пространстве имён <b>System.Reflection.Emit</b>, которые позволяют компилятору или инструменту <b>эмитировать метаданные и общий промежуточный язык (CIL) в рантайме</b>».</span> Ты собираешь тело метода <b>опкод за опкодом</b> через <code>ILGenerator</code>, а <code>DynamicMethod</code> даёт «lightweight global methods at runtime… execute them using <b>delegates</b>» <span class="ru-tr">«лёгкие глобальные методы в рантайме… выполняет их через <b>делегаты</b>»</span>. Тот же CIL, что выдаёт компилятор C#. Дальше <b>пять разборов</b>: read vs emit, конвейер сборки метода, стек вычислений под IL, <code>CreateDelegate</code>, и <b>машинная панель</b> — рукописный IL (<code>ldarg.0/ldc.i4.1/add/ret</code>), скомпилированный JIT-ом и <b>выполненный</b>: <code>f(41)=42</code> (реальный прогон, реальные байты опкодов).',
      source: "ms-emit",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Reflection — двусторонняя. Уроки 1–4 были про <b>интроспекцию</b> (чтение метаданных). <code>System.Reflection.Emit</code> — про <b>генерацию</b>: «a set of managed types in the <code>System.Reflection.Emit</code> namespace that allow a compiler or tool to <span class="hl">emit metadata and common intermediate language (CIL) at runtime</span>». <span class="ru-tr">«набор управляемых типов в пространстве имён <code>System.Reflection.Emit</code>, которые позволяют компилятору или инструменту эмитировать метаданные и общий промежуточный язык (CIL) в рантайме».</span> Главные потребители — «Script engines and compilers». <span class="ru-tr">«Скриптовые движки и компиляторы».</span> Managed-вариант удобнее сырых интерфейсов: «Managed reflection emit provides <b>stronger semantic error checking</b> and a higher level of abstraction of the metadata than the unmanaged metadata interfaces». <span class="ru-tr">«Управляемый reflection emit обеспечивает более строгую <b>семантическую проверку ошибок</b> и более высокий уровень абстракции метаданных, чем неуправляемые интерфейсы метаданных».</span> Итог: reflection не только смотрит на код — она умеет его создавать.',
      sources: ["ms-emit"],
    },
    {
      ...
      explain: 'Сборка динамического метода — три шага. <code>DynamicMethod</code> — «Define <b>lightweight global methods</b> at runtime… and execute them using delegates» <span class="ru-tr">«Определяет <b>лёгкие глобальные методы</b> в рантайме… и выполняет их через делегаты»</span>: это метод <b>без класса-владельца</b>, живёт сам по себе. Конструктор задаёт имя, тип возврата и типы параметров — сигнатуру. <code>GetILGenerator()</code> возвращает <code>ILGenerator</code>, которым ты эмитишь тело по инструкции за раз. Наконец <code>CreateDelegate(delegateType)</code> компилирует IL в исполнимый код и возвращает делегат нужной сигнатуры. После этого вызов делегата — обычный вызов метода, только тело собрано в рантайме.',
  ...
  takeaways: [
    { icon: "why", k: "Reflection пишет код", v: '<code>System.Reflection.Emit</code> — «emit… CIL at runtime» <span class="ru-tr">«эмитировать… CIL в рантайме»</span>. Не только читать метаданные: собрать тело метода <b>опкод за опкодом</b> через <code>ILGenerator</code>.' },
