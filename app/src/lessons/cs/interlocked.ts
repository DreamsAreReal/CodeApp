...
export const interlocked: LessonData = {
  ...
  spec: [
    { text: "Проблема, которую решает: «On most computers, incrementing a variable is not an atomic operation, requiring the following steps: Load a value from an instance variable into a register. Increment or decrement the value. Store the value in the instance variable». <span class=\"ru-tr\">«На большинстве компьютеров инкремент переменной не является атомарной операцией и требует следующих шагов: загрузить значение из переменной экземпляра в регистр. Увеличить или уменьшить значение. Сохранить значение в переменную экземпляра».</span>", source: "ms-interlocked" },
    { text: "Атомарность инкремента: «The Increment and Decrement methods increment or decrement a variable and store the resulting value in a single operation». <span class=\"ru-tr\">«Методы Increment и Decrement увеличивают или уменьшают переменную и сохраняют получившееся значение одной операцией».</span>", source: "ms-interlocked" },
  ],
  edgeCases: [
    { text: "Exchange и CompareExchange: «The Exchange method atomically exchanges the values of the specified variables. The CompareExchange method combines two operations: comparing two values and storing a third value in one of the variables, based on the outcome of the comparison». <span class=\"ru-tr\">«Метод Exchange атомарно обменивает значения указанных переменных. Метод CompareExchange объединяет две операции: сравнение двух значений и запись третьего значения в одну из переменных — в зависимости от результата сравнения».</span>", source: "ms-interlocked" },
    { text: "Не бросают и потокобезопасны: «The members of this class do not throw exceptions». <span class=\"ru-tr\">«Члены этого класса не бросают исключений».</span> Тип: «This type is thread safe». <span class=\"ru-tr\">«Этот тип потокобезопасен».</span>", source: "ms-interlocked" },
    { text: "Для простых состояний — быстрее lock: «the Interlocked class provides better performance for updates that must be atomic. Internally, it executes a single lock prefix if there is no contention». <span class=\"ru-tr\">«класс Interlocked обеспечивает лучшую производительность для обновлений, которые должны быть атомарными. Внутри он выполняет один префикс lock, если нет конкуренции».</span>", source: "ms-best" },
  ],

  misconceptions: [
    {
      wrong: "objCt++ атомарен, а если нет — надо обязательно брать lock",
      hook: '<code>objCt++</code> — <b>не</b> атомарен: «On most computers, incrementing a variable is not an atomic operation, requiring the following steps: <span class="hl">Load a value… Increment or decrement… Store the value</span>». <span class="ru-tr">«На большинстве компьютеров инкремент переменной не является атомарной операцией и требует следующих шагов: загрузить значение… увеличить или уменьшить… сохранить значение».</span> Вытеснение между шагами теряет обновление. Но <code>lock</code> тут избыточен: «<span class="hl">The Increment and Decrement methods increment or decrement a variable and store the resulting value in a single operation</span>» <span class="ru-tr">«Методы Increment и Decrement увеличивают или уменьшают переменную и сохраняют получившееся значение одной операцией».</span> — атомарно и <b>без блокировки</b>. Есть ещё <code>Exchange</code> (атомарный swap) и <code>CompareExchange</code> — CAS, примитив под lock-free-алгоритмами: «combines two operations: comparing two values and storing a third value… <span class="hl">based on the outcome of the comparison</span>». <span class="ru-tr">«объединяет две операции: сравнение двух значений и запись третьего значения… в зависимости от результата сравнения».</span> И это дёшево: «the <code>Interlocked</code> class provides <span class="hl">better performance for updates that must be atomic</span>. Internally, it executes a single lock prefix if there is no contention». <span class="ru-tr">«класс <code>Interlocked</code> обеспечивает лучшую производительность для обновлений, которые должны быть атомарными. Внутри он выполняет один префикс lock, если нет конкуренции».</span> Дальше <b>пять разборов</b>: почему ++ не атомарен, атомарный Increment, Exchange, CompareExchange (CAS), и <b>машинная панель</b> — 8 потоков × 100000 = ровно 800000 реальным прогоном.',
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: 'Шаг 1: «<span class="hl">Load a value from an instance variable into a register</span>». <span class="ru-tr">«Загрузить значение из переменной экземпляра в регистр».</span> Значение уходит в регистр CPU.', nodes: [{ id: "l", kind: "gate", at: { zone: "steps", row: 0 }, state: "ok", label: "1. load", detail: "в регистр", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: 'Шаг 2: «<span class="hl">Increment or decrement the value</span>» <span class="ru-tr">«Увеличить или уменьшить значение».</span> — в регистре. Оригинал в памяти ещё старый.', nodes: [{ id: "l", kind: "gate", at: { zone: "steps", row: 0 }, state: "ok", label: "1. load", detail: "в регистр" }, { id: "i", kind: "gate", at: { zone: "steps", row: 1, col: 0 }, state: "ok", label: "2. increment", detail: "в регистре", accent: true }], edges: [] },
        { codeLine: 3, out: "", caption: 'Шаг 3: «<span class="hl">Store the value in the instance variable</span>». <span class="ru-tr">«Сохранить значение в переменную экземпляра».</span> Между 1–2 и 3 поток можно вытеснить → lost update.', nodes: [{ id: "l", kind: "gate", at: { zone: "steps", row: 0 }, state: "ok", label: "1. load", detail: "регистр" }, { id: "i", kind: "gate", at: { zone: "steps", row: 1, col: 0 }, state: "ok", label: "2. increment", detail: "регистр" }, { id: "s", kind: "gate", at: { zone: "steps", row: 1, col: 1 }, state: "fail", label: "3. store", detail: "тут вытеснят → потеря", accent: true }], edges: [] },
      ],
      explain: 'Interlocked существует ради одной вещи — сделать read-modify-write неделимым. «<span class="hl">On most computers, incrementing a variable is not an atomic operation, requiring the following steps: Load a value from an instance variable into a register. Increment or decrement the value. Store the value in the instance variable</span>». <span class="ru-tr">«На большинстве компьютеров инкремент переменной не является атомарной операцией и требует следующих шагов: загрузить значение из переменной экземпляра в регистр. Увеличить или уменьшить значение. Сохранить значение в переменную экземпляра».</span> Дальше — механика гонки: «If you do not use <code>Increment</code> and <code>Decrement</code>… a thread can be preempted after executing the first two steps. Another thread can then execute all three steps. When the first thread resumes execution, it <span class="hl">overwrites the value in the instance variable</span>, and the effect of the increment or decrement performed by the second thread is lost». <span class="ru-tr">«Если не использовать <code>Increment</code> и <code>Decrement</code>… поток может быть вытеснен после выполнения первых двух шагов. Другой поток может тогда выполнить все три шага. Когда первый поток возобновляет выполнение, он перезаписывает значение в переменной экземпляра, и результат инкремента или декремента, выполненного вторым потоком, теряется».</span> Это тот же lost-update из урока про managed threading, но здесь у него точечное лекарство — атомарные методы <code>Interlocked</code>.',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>Increment</code> делает все три шага <span class="hl">неделимо</span>: «increment or decrement a variable and store the resulting value in a single operation». <span class="ru-tr">«увеличивает или уменьшает переменную и сохраняет получившееся значение одной операцией».</span>', nodes: [{ id: "inc", kind: "gate", at: { zone: "atomic", row: 0 }, state: "ok", label: "Increment(ref counter)", detail: "load+inc+store = 1 атом", accent: true }], edges: [] },
        ...
        { codeLine: 2, out: "", caption: 'Для инкремента больше 1 — <code>Add</code>: «<span class="hl">atomically adds an integer value to an integer variable and returns the new value</span>». <span class="ru-tr">«атомарно добавляет целочисленное значение к целочисленной переменной и возвращает новое значение».</span>', nodes: [{ id: "inc", kind: "gate", at: { zone: "atomic", row: 0 }, state: "ok", label: "Increment", detail: "+1" }, { id: "add", kind: "gate", at: { zone: "atomic", row: 1 }, state: "ok", label: "Add(ref, 5)", detail: "атомарный += , вернёт новое", accent: true }], edges: [] },
      ],
      explain: 'Лекарство от гонки инкремента: «<span class="hl">The Increment and Decrement methods increment or decrement a variable and store the resulting value in a single operation</span>». <span class="ru-tr">«Методы Increment и Decrement увеличивают или уменьшают переменную и сохраняют получившееся значение одной операцией».</span> Для шагов больше 1: «The <code>Add</code> method <span class="hl">atomically adds an integer value to an integer variable and returns the new value of the variable</span>» <span class="ru-tr">«Метод <code>Add</code> атомарно добавляет целочисленное значение к целочисленной переменной и возвращает новое значение переменной».</span> (в best-practices прямо: «Use the <code>Add</code> method for atomic increments larger than 1»). <span class="ru-tr">«Используйте метод <code>Add</code> для атомарных инкрементов больше 1».</span> Почему не всегда <code>lock</code>: «the <code>Interlocked</code> class provides <span class="hl">better performance for updates that must be atomic</span>. Internally, it <span class="hl">executes a single lock prefix if there is no contention</span>» <span class="ru-tr">«класс <code>Interlocked</code> обеспечивает лучшую производительность для обновлений, которые должны быть атомарными. Внутри он выполняет один префикс lock, если нет конкуренции».</span> — это аппаратная инструкция с префиксом <code>lock</code>, а не полновесная блокировка ОС. Бонус: «The members of this class <span class="hl">do not throw exceptions</span>», <span class="ru-tr">«Члены этого класса не бросают исключений»,</span> и «This type is thread safe». <span class="ru-tr">«Этот тип потокобезопасен».</span>',
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, out: "old=5 new=10", caption: '<code>Exchange(ref x, 10)</code>: «<span class="hl">Sets a 32-bit signed integer to a specified value and returns the original value</span>, as an atomic operation». <span class="ru-tr">«Устанавливает 32-битное знаковое целое в заданное значение и возвращает исходное значение — как атомарную операцию».</span> Вернул <b>5</b>, x стал <b>10</b>.', nodes: [{ id: "b", kind: "obj", at: { zone: "before", row: 0 }, typeTag: "x", value: "5" }, { id: "a", kind: "gate", at: { zone: "after", row: 0 }, state: "ok", label: "Exchange → 5", detail: "x = 10", accent: true }], edges: [{ id: "e1", from: "b", to: "a", accent: true }] },
      ...
      explain: 'Атомарный обмен: «<span class="hl">The Exchange method atomically exchanges the values of the specified variables</span>». <span class="ru-tr">«Метод Exchange атомарно обменивает значения указанных переменных».</span> Конкретно для <code>int</code>: «Sets a 32-bit signed integer to a specified value and <span class="hl">returns the original value</span>, as an atomic operation». <span class="ru-tr">«Устанавливает 32-битное знаковое целое в заданное значение и возвращает исходное значение — как атомарную операцию».</span> Возврат прежнего значения делает <code>Exchange</code> строительным блоком lock-free-паттернов: в примере из доков переменная <code>usingResource</code> (0/1) служит замком без reentrancy — «<span class="hl">if(0 == Interlocked.Exchange(ref usingResource, 1))</span>»: атомарно поставили 1 и, если прежнее было 0, значит именно мы «захватили» ресурс. Есть перегрузки для <code>object</code>, <code>double</code>, <code>&lt;T&gt;</code> и т.д. — тип-безопасно и атомарно.',
      sources: ["ms-interlocked"],
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: 'CAS: «<span class="hl">Compares two 32-bit signed integers for equality and, if they are equal, replaces the first value</span>, as an atomic operation». <span class="ru-tr">«Сравнивает два 32-битных знаковых целых на равенство и, если они равны, заменяет первое значение — как атомарную операцию».</span> comparand 7 == x → пишет 100, возвращает старое 7.', nodes: [{ id: "m", kind: "gate", at: { zone: "match", row: 0 }, state: "ok", label: "x==7 == comparand", detail: "swap → 100, r1=7", accent: true }], edges: [] },
  ...
  cards: [
    {
      ...
      okText: '<code>Interlocked.Increment</code> делает load+inc+store «in a single operation» <span class="ru-tr">«одной операцией»</span> — потерянных обновлений нет; 8 × 100000 = ровно <b>800000</b>, воспроизводимо, без <code>lock</code>.',
    ...
    },
    {
      ...
      okText: '<code>Exchange</code> «Sets a 32-bit signed integer to a specified value and <b>returns the original value</b>» <span class="ru-tr">«Устанавливает 32-битное знаковое целое в заданное значение и <b>возвращает исходное значение</b>»</span> — вернул прежнее <b>5</b>, а <code>x</code> стал <b>10</b>. Атомарный swap.',
  ...
  takeaways: [
    { icon: "why", k: "++ не атомарен", v: '«incrementing a variable is not an atomic operation, requiring… <span class="hl">Load a value… Increment or decrement the value… Store the value</span>» <span class="ru-tr">«инкремент переменной не является атомарной операцией и требует… загрузить значение… увеличить или уменьшить значение… сохранить значение».</span> — вытеснение теряет обновление. <code>Interlocked.Increment</code> делает всё «<span class="hl">store the resulting value in a single operation</span>». <span class="ru-tr">«сохранить получившееся значение одной операцией».</span>' },
    { icon: "cost", k: "Exchange / CAS", v: '<code>Exchange</code> — атомарный swap, «returns the original value». <span class="ru-tr">«возвращает исходное значение».</span> <code>CompareExchange</code> — CAS: «comparing two values and storing a third value… <span class="hl">based on the outcome of the comparison</span>» <span class="ru-tr">«сравнение двух значений и запись третьего значения… в зависимости от результата сравнения»</span> (r1=7 afterMatch=100 r2=100). Примитив lock-free.' },
    { icon: "avoid", k: "vs lock", v: 'Для одиночных атомарных обновлений — <code>Interlocked</code>, не <code>lock</code>: «<span class="hl">better performance… executes a single lock prefix if there is no contention</span>», <span class="ru-tr">«лучшая производительность… выполняет один префикс lock, если нет конкуренции»,</span> «do not throw exceptions», <span class="ru-tr">«не бросают исключений»,</span> «thread safe». <span class="ru-tr">«потокобезопасен».</span> Панель: 8×100000 = 800000.' },
  ],
