...
export const deferredExecution: LessonData = {
  ...
  spec: [
    { text: "«Deferred execution means that the operation isn't performed at the point in the code where the query is declared. The operation is performed only when the query variable is enumerated, for example by using a foreach statement.» <span class=\"ru-tr\">«Отложенное выполнение означает, что операция не выполняется в той точке кода, где запрос объявлен. Операция выполняется только тогда, когда переменная запроса перечисляется, например с помощью оператора foreach.»</span>", source: "ms-intro" },
  ],
  edgeCases: [
    { text: "Immediate = скаляр: «All the standard query operators that <b>return a scalar result execute immediately</b>. Examples of such queries are <code>Count</code>, <code>Max</code>, <code>Average</code>, and <code>First</code>». <span class=\"ru-tr\">«Все стандартные операторы запросов, которые <b>возвращают скалярный результат, выполняются немедленно</b>. Примеры таких запросов — <code>Count</code>, <code>Max</code>, <code>Average</code> и <code>First</code>».</span>", source: "ms-intro" },
    { text: "Streaming не читает всё: «<span class=\"hl\">Streaming operators don't have to read all the source data</span> before they yield elements… a streaming operator performs its operation on each source element as it is read and yields the element if appropriate». <span class=\"ru-tr\">«Потоковым операторам не нужно читать все данные источника, прежде чем они начнут отдавать элементы… потоковый оператор выполняет свою операцию над каждым элементом источника по мере его чтения и отдаёт элемент, если это уместно».</span>", source: "ms-intro" },
    { text: "Nonstreaming читает всё: «Nonstreaming operators <span class=\"hl\">must read all the source data before they can yield a result element</span>. Operations such as sorting or grouping fall into this category». <span class=\"ru-tr\">«Непотоковые операторы должны прочитать все данные источника, прежде чем смогут отдать элемент результата. В эту категорию попадают такие операции, как сортировка или группировка».</span>", source: "ms-intro" },
  ],

  misconceptions: [
    {
      wrong: "LINQ-запрос выполняется на строке, где он написан, и результат кешируется в переменной",
      hook: 'Опасное заблуждение: <span class="wrong">запрос выполняется там, где написан</span>, и переменная держит закешированный результат. Нет: операторы, возвращающие <code>IEnumerable&lt;T&gt;</code>, <b>откладывают</b> работу. Дословно: «<span class="hl">Deferred execution</span> means that the operation isn\'t performed at the point in the code where the query is declared. The operation is performed <b>only when the query variable is enumerated</b>». <span class="ru-tr">«Отложенное выполнение означает, что операция не выполняется в той точке кода, где запрос объявлен. Операция выполняется <b>только тогда, когда переменная запроса перечисляется</b>».</span> И есть два вида отложенных: <b>streaming</b> (по элементу) и <b>nonstreaming</b> (сначала читает весь источник). Ниже <b>шесть разборов</b> и <b>машинная панель</b> — живой счётчик итераций: deferred <b>0→3</b>, streaming <b>читает 7 из 100</b>, nonstreaming <b>читает все 10</b> до первого результата (реальные замеры).',
      source: "ms-intro",
    },
  ],
  ...
  segments: [
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: 'Итог: «performed only when the query variable is <span class="hl">enumerated</span>». <span class="ru-tr">«выполняется только тогда, когда переменная запроса перечисляется».</span> Тяжёлый запрос без перечисления — бесплатен.', nodes: [{ id: "r", kind: "gate", at: { zone: "tl", row: 0 }, state: "ok", label: "перечисление", detail: "= исполнение", accent: true }], edges: [] },
      ],
      explain: 'Где именно исполняется отложенный запрос — дословно: «Deferred execution means that the operation <b>isn\'t performed at the point in the code where the query is declared</b>. The operation is performed <span class="hl">only when the query variable is enumerated</span>, for example by using a <code>foreach</code> statement». <span class="ru-tr">«Отложенное выполнение означает, что операция <b>не выполняется в той точке кода, где запрос объявлен</b>. Операция выполняется только тогда, когда переменная запроса перечисляется, например с помощью оператора <code>foreach</code>».</span> Перечисление запускают: <code>foreach</code>, <code>ToList/ToArray/ToDictionary</code>, и immediate-операторы (<code>Count/First/…</code>). Следствие для отладки: если <code>Select</code> с побочным эффектом «не сработал» — вероятно, запрос ни разу не перечислили. Машинная панель покажет это счётчиком.',
      sources: ["ms-intro"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Nonstreaming (буферизующие) операторы обязаны прочитать источник целиком. Дословно: «Nonstreaming operators <span class="hl">must read all the source data before they can yield a result element</span>. Operations such as sorting or grouping fall into this category. At the time of execution, nonstreaming query operators read all the source data, put it into a data structure, perform the operation, and yield the resulting elements». <span class="ru-tr">«Непотоковые операторы должны прочитать все данные источника, прежде чем смогут отдать элемент результата. В эту категорию попадают такие операции, как сортировка или группировка. В момент выполнения непотоковые операторы запросов читают все данные источника, помещают их в структуру данных, выполняют операцию и отдают результирующие элементы».</span> К ним относятся <code>OrderBy/OrderByDescending, GroupBy, Reverse, ThenBy</code>. Логика неизбежна: чтобы вернуть <b>максимум</b> (первый после <code>OrderByDescending</code>), нужно увидеть все элементы. Поэтому <code>OrderBy(...).First()</code> — это полный проход источника, а не streaming-«стоп на первом». Разница — в панели: 10 из 10 против 7 из 100.',
      sources: ["ms-intro"],
    },
    {
      ...
      explain: 'Обратная сторона deferred — <b>повторное перечисление</b>. Дословно: «If the query variable is enumerated multiple times, the <span class="hl">results might differ every time</span>» <span class="ru-tr">«Если переменная запроса перечисляется несколько раз, результаты каждый раз могут различаться»</span> — и каждый раз это новый проход источника со всеми побочными эффектами/затратами. «Immediate execution provides <b>reuse of query results</b>, not query declaration. The results are retrieved once, then stored for future use». <span class="ru-tr">«Немедленное выполнение обеспечивает <b>повторное использование результатов запроса</b>, а не объявления запроса. Результаты извлекаются один раз, а затем сохраняются для дальнейшего использования».</span> То есть если запрос дорогой или источник может измениться/иссякнуть (сетевой поток), материализуй один раз через <code>ToList/ToArray</code> и работай с коллекцией. Классический баг: два <code>foreach</code> по одному <code>q</code> дважды бьют в БД. Deferred — мощь и грабли одновременно.',
      sources: ["ms-intro"],
    },
    {
  ...
  cards: [
    {
      ...
      okText: '<code>Select</code> отложен: на строке объявления счётчик <span class="hl">0</span> (before=0). <code>ToList()</code> перечисляет → лямбда бежит 3 раза (after=3). «performed only when the query variable is enumerated». <span class="ru-tr">«выполняется только тогда, когда переменная запроса перечисляется».</span>',
    ...
    },
    {
      ...
      noText: '«Streaming operators don\'t have to read all the source data». <span class="ru-tr">«Потоковым операторам не нужно читать все данные источника».</span> Первое кратное 7 — само число 7, на 7-м шаге. Реальный вывод: <b>first=7 seen=7</b> (не 100).',
    ...
    },
    {
      ...
      noText: '«Nonstreaming operators must read all the source data before they can yield a result element». <span class="ru-tr">«Непотоковые операторы должны прочитать все данные источника, прежде чем смогут отдать элемент результата».</span> Сортировка видит все 10 → seen=10, максимум 10. Реальный вывод: <b>first=10 seen=10</b>.',
      verify: { kind: "exec", run: "dotnet run", expect: "first=10 seen=10" }, sourceRefs: ["ms-intro"],
    },
  ],

  takeaways: [
    { icon: "why", k: "deferred = при перечислении", v: '«the operation is performed only when the query variable is enumerated». <span class="ru-tr">«операция выполняется только тогда, когда переменная запроса перечисляется».</span> Возврат <code>IEnumerable&lt;T&gt;</code> → отложено (замер: before=0 after=3). Возврат скаляра (<code>Count/First</code>) → immediate.' },
    ...
    { icon: "avoid", k: "не перечисляй дважды", v: 'Каждый <code>foreach</code>/<code>Count</code> — новый прогон источника (побочные эффекты и запросы к БД повторяются). Дорогой запрос → <code>ToList()</code> один раз: «reuse of query results». <span class="ru-tr">«повторное использование результатов запроса».</span>' },
  ],
