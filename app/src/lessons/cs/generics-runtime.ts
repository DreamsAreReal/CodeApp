...
export const genericsRuntime: LessonData = {
  ...
  spec: [
    { text: "«When you create an instance of a generic class, you specify the actual types to substitute for the type parameters. This establishes a new generic class, referred to as a constructed generic class, with your chosen types substituted everywhere that the type parameters appear.» <span class=\"ru-tr\">«Когда вы создаёте экземпляр обобщённого класса, вы указываете конкретные типы, которые нужно подставить вместо параметров типа. Это создаёт новый обобщённый класс, называемый сконструированным обобщённым классом, где ваши выбранные типы подставлены везде, где встречаются параметры типа.»</span>", source: "ms-generics-std" },
  ],
  edgeCases: [
    { text: "Каждый закрытый тип — <b>отдельный</b> runtime-тип: <code>typeof(List&lt;int&gt;) != typeof(List&lt;object&gt;)</code>, хотя определение одно (собственный прогон: <code>True False</code>). «This establishes a <b>new</b> generic class, referred to as a constructed generic class». <span class=\"ru-tr\">«Это создаёт <b>новый</b> обобщённый класс, называемый сконструированным обобщённым классом».</span>", source: "ms-generics-std" },
    ...
    { text: "Value-типы не боксятся при хранении в закрытом типе: «Generic collection types generally perform better for storing and manipulating value types because there is <span class=\"hl\">no need to box the value types</span>». <span class=\"ru-tr\">«Обобщённые типы коллекций обычно работают лучше при хранении и обработке value-типов, потому что нет необходимости боксить value-типы».</span>", source: "ms-generics-std" },
  ],

  misconceptions: [
    {
      wrong: "у всех List<T> в рантайме один общий тип, а T стирается до object — как в Java",
      hook: 'Нет: .NET <b>реифицирует</b> дженерики, а не стирает их. Когда ты пишешь <code>new List&lt;int&gt;()</code>, рантайм строит <b>отдельный</b> тип: «When you create an instance of a generic class, you specify the actual types to substitute for the type parameters. This establishes a <span class="hl">new generic class, referred to as a constructed generic class</span>». <span class="ru-tr">«Когда вы создаёте экземпляр обобщённого класса, вы указываете конкретные типы, которые нужно подставить вместо параметров типа. Это создаёт новый обобщённый класс, называемый сконструированным обобщённым классом».</span> Поэтому <code>typeof(T)</code> внутри дженерика знает реальный тип, а <code>typeof(List&lt;int&gt;) != typeof(List&lt;object&gt;)</code>. И это не просто метаданные: <code>List&lt;int&gt;</code> хранит <code>int</code> <span class="hl">inline</span> — «no need to box the value types» <span class="ru-tr">«нет необходимости боксить value-типы»</span>, — тогда как <code>List&lt;object&gt;</code> боксит каждый. Дальше <b>пять разборов</b>: одно определение → много закрытых типов, реификация <code>typeof(T)</code>, две дорожки (ref-типы делят layout, value-типы специализируются), CIL знает о generics, и <b>машинная панель</b> — реально снятая цена боксинга <code>List&lt;object&gt;</code> (32056 B) против <code>List&lt;int&gt;</code> (4056 B).',
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 1, out: "", caption: '<code>new List&lt;int&gt;()</code> — рантайм <span class="hl">строит новый тип</span>: «This establishes a new generic class, referred to as a constructed generic class». <span class="ru-tr">«Это создаёт новый обобщённый класс, называемый сконструированным обобщённым классом».</span>', nodes: [{ id: "d", kind: "obj", at: { zone: "def", row: 0 }, typeTag: "List`1", value: "шаблон" }, { id: "ci", kind: "obj", at: { zone: "cons", row: 0 }, typeTag: "List<int>", value: "закрыт", accent: true }], edges: [{ id: "e1", from: "ci", to: "d", accent: true }] },
      ...
      explain: 'Дженерик в сборке хранится как <b>одно определение</b> (<code>List`1</code>, суффикс <code>`1</code> — арность), но в рантайме каждый <code>new List&lt;T&gt;()</code> порождает свой закрытый тип. Дословно: «When you create an instance of a generic class, you specify the actual types to substitute for the type parameters. <span class="hl">This establishes a new generic class, referred to as a constructed generic class</span>, with your chosen types substituted everywhere that the type parameters appear». <span class="ru-tr">«Когда вы создаёте экземпляр обобщённого класса, вы указываете конкретные типы, которые нужно подставить вместо параметров типа. Это создаёт новый обобщённый класс, называемый сконструированным обобщённым классом, где ваши выбранные типы подставлены везде, где встречаются параметры типа».</span> Слово <b>new</b> здесь буквальное: <code>List&lt;int&gt;</code> и <code>List&lt;string&gt;</code> — разные типы (<code>typeof</code> их не равняет), хотя оба идут от одного шаблона. Это фундамент всей секции: рантайм <b>знает</b> аргументы, а не забывает их.',
    ...
    },
    {
      ...
      explain: 'Это машинная панель урока — цена двух специализаций одного определения, снятая числом. <code>List&lt;int&gt;</code> и <code>List&lt;object&gt;</code> идут от одного <code>List`1</code>, но рантайм раскладывает их по-разному: <code>int</code> хранится <b>inline</b> (value-специализация), а в <code>List&lt;object&gt;</code> элемент — ссылка, поэтому каждый <code>int</code> <b>боксится</b> в объект на куче. Прогон <code>GC.GetAllocatedBytesForCurrentThread()</code> вокруг 1000 вставок: <code>List&lt;object&gt;</code> — <b>32056 байт</b> (1000 боксов + рост), <code>List&lt;int&gt;</code> — <b>4056 байт</b> (только int-массив). Дословно почему так: «Generic collection types generally perform better for storing and manipulating value types because there is <span class="hl">no need to box the value types</span>». <span class="ru-tr">«Обобщённые типы коллекций обычно работают лучше при хранении и обработке value-типов, потому что нет необходимости боксить value-типы».</span> Тот же исходник, но представление в рантайме — разное: это и есть специализация.',
      sources: ["ms-generics-std"],
    },
  ],
  ...
  cards: [
    ...
    },
    {
      ...
      okText: 'Определение <b>одно</b> (<code>List`1</code>) → <code>sameDef=True</code>. Но каждый закрытый тип <b>новый</b>: «This establishes a <span class="hl">new generic class</span>» <span class="ru-tr">«Это создаёт новый обобщённый класс»</span> → <code>List&lt;string&gt; != List&lt;object&gt;</code>, <code>sameClosed=False</code>.',
  ...
  takeaways: [
    { icon: "why", k: "Реификация, не erasure", v: 'Каждый <code>new List&lt;T&gt;()</code> — «a <span class="hl">new</span> generic class, referred to as a constructed generic class» <span class="ru-tr">«<b>новый</b> обобщённый класс, называемый сконструированным обобщённым классом»</span>. <code>typeof(T)</code> в рантайме знает реальный тип; <code>typeof(List&lt;int&gt;) != typeof(List&lt;object&gt;)</code> (замер: True False).' },
    { icon: "cost", k: "Две специализации", v: 'Ref-типы делят один layout (элемент = указатель); каждый value-тип — свой макет, значение <b>inline</b>. Отсюда «no need to box the value types» <span class="ru-tr">«нет необходимости боксить value-типы»</span> (замер: List<int> 4056 B против List<object> 32056 B).' },
    { icon: "avoid", k: "CIL под generics", v: 'Дженерики — возможность рантайма, не сахар над <code>object</code>: «new opcodes and prefixes to support generic types in common intermediate language (CIL)» <span class="ru-tr">«новые опкоды и префиксы для поддержки обобщённых типов в общем промежуточном языке (CIL)»</span> (<code>Stelem</code>/<code>Ldelem</code>/<code>Constrained</code>/<code>Unbox_Any</code>). Тип <code>T</code> присутствует в IL, а не стёрт.' },
  ],
