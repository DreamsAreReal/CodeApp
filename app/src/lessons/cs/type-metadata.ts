...
export const typeMetadata: LessonData = {
  ...
  spec: [
    { text: "«The System.Reflection namespace contains many methods for obtaining information about types and their members.» <span class=\"ru-tr\">«Пространство имён System.Reflection содержит множество методов для получения информации о типах и их членах.»</span>", source: "ms-getmembers" },
  ],
  edgeCases: [
    { text: "<code>BindingFlags</code> — «Specifies flags that control binding and the way in which the search for members and types is conducted by reflection» <span class=\"ru-tr\">«Задаёт флаги, управляющие связыванием и тем, как reflection выполняет поиск членов и типов»</span>.", source: "ms-bindingflags" },
    { text: "Ловушка: «You must specify <b>Instance</b> or <b>Static</b> along with <b>Public</b> or <b>NonPublic</b> or no members will be returned» <span class=\"ru-tr\">«Вы должны указать <b>Instance</b> или <b>Static</b> вместе с <b>Public</b> или <b>NonPublic</b>, иначе не будет возвращено ни одного члена»</span> — только <code>Public</code> вернёт <span class=\"hl\">пустой</span> массив.", source: "ms-bindingflags" },
    { text: "<code>DeclaredOnly</code> — «only members declared at the level of the supplied type's hierarchy should be considered. Inherited members are not considered» <span class=\"ru-tr\">«следует учитывать только члены, объявленные на уровне иерархии указанного типа. Унаследованные члены не учитываются»</span> — отсекает унаследованное от <code>object</code>.", source: "ms-bindingflags" },
  ],

  misconceptions: [
    {
      wrong: "GetMembers() возвращает все члены типа — публичные и приватные",
      hook: 'Нет: <code>GetMembers()</code> без аргументов даёт только <b>public</b> члены (и с учётом наследования от <code>object</code>). Приватные, статические, declared-only — это <span class="hl">параметры запроса</span>, а не поведение по умолчанию. <code>BindingFlags</code>: «Specifies flags that control binding and the way in which the search for members and types is conducted by reflection» <span class="ru-tr">«Задаёт флаги, управляющие связыванием и тем, как reflection выполняет поиск членов и типов»</span>. А главная ловушка прямо в доке: «You must specify <b>Instance</b> or <b>Static</b> along with <b>Public</b> or <b>NonPublic</b> or <span class="hl">no members will be returned</span>» <span class="ru-tr">«Вы должны указать <b>Instance</b> или <b>Static</b> вместе с <b>Public</b> или <b>NonPublic</b>, иначе не будет возвращено ни одного члена»</span>. Дальше <b>пять разборов</b>: Type как ручка к метаданным, <code>MemberInfo[]</code> и <code>MemberType</code>, оси фильтра, и <b>машинная панель</b> — как один флаг <code>NonPublic</code> меняет счётчик членов (реальный прогон: 3 → 5).',
  ...
  segments: [
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: 'Ось <b>видимости</b>: <code>Public</code> и/или <code>NonPublic</code>. «public members are to be included in the search» <span class="ru-tr">«публичные члены должны быть включены в поиск»</span>.', nodes: [{ id: "pub", kind: "chip", at: { zone: "filter", row: 0 }, value: "Public", accent: true }, { id: "npub", kind: "chip", at: { zone: "filter", row: 1 }, value: "NonPublic" }], edges: [] },
        ...
        { codeLine: 3, out: "", caption: '<code>DeclaredOnly</code> — модификатор: «Inherited members are not considered» <span class="ru-tr">«Унаследованные члены не учитываются»</span>. Убирает члены <code>object</code>.', nodes: [{ id: "pub", kind: "chip", at: { zone: "filter", row: 0 }, value: "Public" }, { id: "npub", kind: "chip", at: { zone: "filter", row: 1 }, value: "NonPublic" }, { id: "inst", kind: "chip", at: { zone: "result", row: 0 }, value: "Instance" }, { id: "decl", kind: "gate", at: { zone: "result", row: 1 }, state: "ok", label: "DeclaredOnly", detail: "без наследия", accent: true }], edges: [] },
      ],
      explain: '<code>BindingFlags</code> — не «настройка», а сам <b>запрос</b>: «Specifies flags that control binding and the way in which the search for members and types is conducted by reflection» <span class="ru-tr">«Задаёт флаги, управляющие связыванием и тем, как reflection выполняет поиск членов и типов»</span>. Две ортогональные оси обязательны вместе — доступность (<code>Public</code>/<code>NonPublic</code>) и время жизни (<code>Instance</code>/<code>Static</code>). Прямое предупреждение: «You must specify <b>Instance</b> or <b>Static</b> along with <b>Public</b> or <b>NonPublic</b> or <span class="hl">no members will be returned</span>» <span class="ru-tr">«Вы должны указать <b>Instance</b> или <b>Static</b> вместе с <b>Public</b> или <b>NonPublic</b>, иначе не будет возвращено ни одного члена»</span> — классический баг «reflection ничего не находит». <code>DeclaredOnly</code> сверху отсекает унаследованное: «only members declared at the level of the supplied type\'s hierarchy… Inherited members are not considered» <span class="ru-tr">«учитываются только члены, объявленные на уровне иерархии указанного типа… Унаследованные члены не учитываются»</span>.',
  ...
  cards: [
    ...
    },
    {
      ...
      okText: 'Только <code>Public</code> без оси времени жизни → <span class="hl">0</span>: «You must specify Instance or Static along with Public or NonPublic or no members will be returned» <span class="ru-tr">«Вы должны указать Instance или Static вместе с Public или NonPublic, иначе не будет возвращено ни одного члена»</span>. С <code>Instance | DeclaredOnly</code> → <b>2</b> (поле <code>A</code> + <code>.ctor</code>).',
