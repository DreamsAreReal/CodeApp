...
export const genericMath: LessonData = {
  ...
  spec: [
    { text: "«An interface can declare static abstract and static virtual members for all member types except fields. By declaring these members, an interface can require that implementing types define operators or other static members.»", source: "ms-iface" },
  ],
  edgeCases: [
    ...
    { text: "Self-constraint обязателен: «most interfaces that declare <code>static virtual</code> or <code>static abstract</code> methods declare that <span class=\"hl\">one of the type parameters must implement the declared interface</span>. For example, the <code>INumber&lt;T&gt;</code> interface declares that <code>T</code> must implement <code>INumber&lt;T&gt;</code>».", source: "ms-iface" },
    { text: "Один generic на все числа: «Now you can write a <b>single, generic method</b>, where the type parameter is constrained to be a number-like type» — «any of .NET's built-in numeric types, because they've all been updated to implement <code>INumber&lt;TSelf&gt;</code> in .NET 7».", source: "ms-math" },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Отправная точка — старая боль: обобщить арифметику было нельзя, потому что операторы — <b>static</b>-члены, а до C# 11 интерфейс не мог их требовать. Дословно: «if you wanted to write a method that adds two numbers, previously you had to add an overload of the method <span class="hl">for each type</span> (for example, <code>static int Add(int first, int second)</code> and <code>static float Add(float first, float second)</code>)». Решение — два нововведения вместе: «.NET 7 introduces new math-related generic interfaces… C# 11 and later lets you define <code>static virtual</code> interface members. Because operators must be declared as <code>static</code>, this new C# feature lets operators be declared in the new interfaces for number-like types». Итог: «Now you can write a <span class="hl">single, generic method</span>, where the type parameter is constrained to be a number-like type».',
      sources: ["ms-math"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Почему сигнатура именно <code>T : INumber&lt;T&gt;</code>, а не <code>T : INumber</code>. Дословно: «most interfaces that declare <code>static virtual</code> or <code>static abstract</code> methods declare that <span class="hl">one of the type parameters must implement the declared interface</span>. For example, the <code>INumber&lt;T&gt;</code> interface declares that <code>T</code> must implement <code>INumber&lt;T&gt;</code>». Это self-constraint (шаблон CRTP): он даёт компилятору <b>конкретный тип</b>, на котором объявлены static-операторы. Как разрешается вызов: «The compiler uses the type argument to resolve calls to the methods and operators declared in the interface declaration. For example, the <code>int</code> type implements <code>INumber&lt;int&gt;</code>. When the type parameter <code>T</code> denotes the type argument <code>int</code>, the <span class="hl">static members declared on <code>int</code> are invoked</span>». Поэтому <code>Add(3,4)</code> вызывает именно <code>int</code>-оператор сложения и даёт <code>7</code>.',
      sources: ["ms-iface"],
    },
    {
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>INumber&lt;TSelf&gt;</code> — «APIs common to <span class="hl">comparable number types</span> (effectively the "real" number domain)». Самый частый constraint.', nodes: [{ id: "n", kind: "obj", at: { zone: "hier", row: 0 }, typeTag: "INumber<TSelf>", value: "real-числа", accent: true }], edges: [] },
  ...
  takeaways: [
    { icon: "why", k: "static abstract члены", v: '«An interface can declare <span class="hl">static abstract and static virtual members</span>… require that implementing types define operators or other static members». Это выносит операторы и <code>Zero</code>/<code>One</code> в контракт (C# 11).' },
    { icon: "cost", k: "Один generic на все числа", v: 'Раньше — перегрузка на каждый тип; теперь «a <span class="hl">single, generic method</span>» с <code>where T : INumber&lt;T&gt;</code>. Работает для всех встроенных чисел (замер: <code>Add&lt;T&gt;</code> → 7 4 30; <code>Sum</code> с <code>T.Zero</code> → 10 7).' },
    { icon: "avoid", k: "Диспетчер — компиляция", v: 'static abstract «<span class="hl">don\'t have a runtime dispatch mechanism</span>»: разрешается по compile-time типу-аргументу, не через vtable. Нужен self-constraint <code>T : INumber&lt;T&gt;</code> (замер своего <code>IZero&lt;Money&gt;</code>: 0).' },
  ],
