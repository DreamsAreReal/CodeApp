...
export const genericStaticMembers: LessonData = {
  ...
  spec: [
    { text: "«client code can reference the class either by specifying a type argument - to create a closed constructed type (Node<int>); or by leaving the type parameter unspecified - for example when you specify a generic base class, to create an open constructed type (Node<T>).»", source: "ms-gen-classes" },
  ],
  edgeCases: [
    { text: "Каждый <b>закрытый</b> тип — отдельный: «to create a <span class=\"hl\">closed constructed type</span> (<code>Node&lt;int&gt;</code>)». Его статика независима от <code>Node&lt;string&gt;</code> — собственный прогон: <code>Counter&lt;int&gt;.Count</code>=2, <code>Counter&lt;string&gt;.Count</code>=1.", source: "ms-gen-classes" },
  ...
  takeaways: [
    { icon: "why", k: "Static per closed type", v: 'Static-поле generic-типа принадлежит <b>закрытому</b> типу («a <span class="hl">closed constructed type</span> (<code>Node&lt;int&gt;</code>)»), не определению. <code>Counter&lt;int&gt;.Count</code> и <code>Counter&lt;string&gt;.Count</code> независимы (замер: 2 и 1).' },
