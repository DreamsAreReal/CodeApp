...
export const iteratorsOverview: LessonData = {
  ...
  spec: [
    { text: "«An iterator method defines how to generate the objects in a sequence when requested. You use the yield return contextual keywords to define an iterator method.» <span class=\"ru-tr\">«Итератор-метод определяет, как порождать объекты последовательности по запросу. Для определения итератор-метода используются контекстные ключевые слова yield return.»</span>", source: "ms-iterators" },
  ],
  edgeCases: [
    { text: 'Вызов итератор-метода возвращает <b>объект</b> и <span class="hl">не исполняет тело</span>: «Calling an iterator doesn\'t execute it immediately» <span class="ru-tr">«Вызов итератора не исполняет его немедленно»</span>. Тело стартует при начале обхода.', source: "ms-yield" },
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: 'Вызов метода даёт <b>iterator</b> — объект обхода. «An iterator is an object that traverses a container, particularly lists» <span class="ru-tr">«Итератор — это объект, который обходит контейнер, в частности списки»</span>.', nodes: [{ id: "m", kind: "gate", at: { zone: "method", row: 0 }, state: "ok", label: "GetSingleDigits()", detail: "iterator-метод" }, { id: "o", kind: "obj", at: { zone: "iobj", row: 0 }, typeTag: "IEnumerable<int>", value: "объект обхода", accent: true }], edges: [{ id: "e1", from: "m", to: "o" }] },
      ...
      explain: 'Итератор-метод — способ построить источник последовательности, не материализуя её. «An iterator method defines how to generate the objects in a sequence when requested. You use the <code>yield return</code> contextual keywords to define an iterator method» <span class="ru-tr">«Итератор-метод определяет, как порождать объекты последовательности по запросу. Для определения итератор-метода используются контекстные ключевые слова <code>yield return</code>»</span>. Сам итератор — «An iterator is an object that traverses a container, particularly lists» <span class="ru-tr">«Итератор — это объект, который обходит контейнер, в частности списки»</span>. Ключевая идея: метод с <code>yield return</code> возвращает не готовый список, а <b>объект</b>, который умеет отдавать элементы по одному по запросу. Как именно «по запросу» работает механически — следующие разборы: ленивость (02), пауза/возобновление (03) и машинная панель (05).',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: '<code>var seq = Produce()</code>: получили <b>объект</b>. Тело метода <span class="hl">не исполнялось</span> — «Iterator: start.» <span class="ru-tr">«Итератор: старт.»</span> ещё не напечатано.', nodes: [{ id: "c", kind: "gate", at: { zone: "call", row: 0 }, state: "ok", label: "seq = Produce()", detail: "объект, тело спит", accent: true }], edges: [] },
        ...
        { codeLine: 3, out: "Iterator: start.\nCaller: 1", caption: '<code>foreach</code> начинает обход — <span class="hl">теперь</span> тело исполняется до первого <code>yield return</code>: печать «Iterator: start.» <span class="ru-tr">«Итератор: старт.»</span>, затем отдаётся 1.', nodes: [{ id: "c", kind: "gate", at: { zone: "call", row: 0 }, state: "ok", label: "seq = Produce()", detail: "объект" }, { id: "i", kind: "gate", at: { zone: "iter", row: 0 }, state: "ok", label: "foreach", detail: "тело стартовало", accent: true }], edges: [{ id: "e1", from: "c", to: "i" }] },
      ],
      explain: 'Центральный факт S18.1 — <b>ленивость</b>: «Calling an iterator doesn\'t execute it immediately» <span class="ru-tr">«Вызов итератора не исполняет его немедленно»</span>. Строка <code>var seq = Produce()</code> лишь создаёт объект-итератор; тело метода не запускается. Реальный порядок вывода это доказывает: «Caller: got the object, nothing ran yet.» <span class="ru-tr">«Вызывающий: получил объект, пока ничего не исполнилось.»</span> печатается <b>раньше</b> «Iterator: start.» <span class="ru-tr">«Итератор: старт.»</span> из тела. Дальше: «when you start to iterate over an iterator\'s result, the iterator executes until the first <code>yield return</code> statement is reached. Then, the execution of the iterator is suspended and the caller gets the first iteration value» <span class="ru-tr">«когда вы начинаете обходить результат итератора, итератор исполняется до достижения первого оператора <code>yield return</code>. Затем исполнение итератора приостанавливается, и вызывающий получает первое итерационное значение»</span>. То есть тело оживает только на <code>foreach</code> и работает до первого <code>yield</code>. Практический смысл — можно строить конвейеры LINQ, где ничего не считается, пока не пошёл обход.',
      sources: ["ms-yield"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: 'Конец тела → итерация завершена. «The iteration completes when control reaches the end of an iterator or a <code>yield break</code> statement» <span class="ru-tr">«Итерация завершается, когда управление доходит до конца итератора или до оператора <code>yield break</code>»</span>.', nodes: [{ id: "y2", kind: "gate", at: { zone: "flow", row: 0, col: 0 }, state: "ok", label: "yield return 2", detail: "отдал 2" }, { id: "end", kind: "gate", at: { zone: "flow", row: 0, col: 1 }, state: "fail", label: "конец тела", detail: "MoveNext → false", accent: true }], edges: [{ id: "e2", from: "y2", to: "end" }] },
      ],
      explain: 'Механизм отдачи — <b>пауза на каждом</b> <code>yield return</code>, а не буферизация. «the iterator executes until the first <code>yield return</code> statement is reached. Then, the execution of the iterator is suspended... On each subsequent iteration, the execution of the iterator resumes <b>after</b> the <code>yield return</code> statement that caused the previous suspension and continues until the next <code>yield return</code> statement is reached» <span class="ru-tr">«итератор исполняется до достижения первого оператора <code>yield return</code>. Затем исполнение итератора приостанавливается... На каждой последующей итерации исполнение итератора возобновляется <b>после</b> оператора <code>yield return</code>, вызвавшего предыдущую приостановку, и продолжается до достижения следующего оператора <code>yield return</code>»</span>. Поэтому элементы приходят по одному: печать «A» → отдать 1 → (пауза) → печать «B» → отдать 2. «The iteration completes when control reaches the end of an iterator or a <code>yield break</code> statement» <span class="ru-tr">«Итерация завершается, когда управление доходит до конца итератора или до оператора <code>yield break</code>»</span>. Это опровергает миф «буферизует всё сразу»: между двумя элементами тело реально останавливается и ждёт следующего запроса.',
      sources: ["ms-yield"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: 'Выбор по типу последовательности: «When a sequence is an …<code>IEnumerable&lt;T&gt;</code>, you use <code>foreach</code>. When a sequence is an …<code>IAsyncEnumerable&lt;T&gt;</code>, you use <code>await foreach</code>» <span class="ru-tr">«Когда последовательность — это …<code>IEnumerable&lt;T&gt;</code>, вы используете <code>foreach</code>. Когда последовательность — это …<code>IAsyncEnumerable&lt;T&gt;</code>, вы используете <code>await foreach</code>»</span> (в доке — с квалификатором <code>System.Collections.Generic.</code>).', nodes: [{ id: "f", kind: "gate", at: { zone: "sync", row: 0 }, state: "ok", label: "IEnumerable<T>", detail: "→ foreach", w: 120 }, { id: "af", kind: "gate", at: { zone: "async", row: 0 }, state: "ok", label: "IAsyncEnumerable<T>", detail: "→ await foreach", accent: true, w: 144 }], edges: [] },
      ],
      explain: 'Обход стоит на двух парах интерфейсов. Синхронный <code>foreach</code>: «It relies on two generic interfaces defined in the .NET core library to generate the code necessary to iterate a collection: <code>IEnumerable&lt;T&gt;</code> and <code>IEnumerator&lt;T&gt;</code>» <span class="ru-tr">«Он опирается на два обобщённых интерфейса, определённых в основной библиотеке .NET, чтобы сгенерировать код, необходимый для обхода коллекции: <code>IEnumerable&lt;T&gt;</code> и <code>IEnumerator&lt;T&gt;</code>»</span>. Асинхронный — <code>await foreach</code> над <code>IAsyncEnumerable&lt;T&gt;</code>. Правило выбора дословно: «When a sequence is an <code>System.Collections.Generic.IEnumerable&lt;T&gt;</code>, you use <code>foreach</code>. When a sequence is an <code>System.Collections.Generic.IAsyncEnumerable&lt;T&gt;</code>, you use <code>await foreach</code>» <span class="ru-tr">«Когда последовательность — это <code>System.Collections.Generic.IEnumerable&lt;T&gt;</code>, вы используете <code>foreach</code>. Когда последовательность — это <code>System.Collections.Generic.IAsyncEnumerable&lt;T&gt;</code>, вы используете <code>await foreach</code>»</span>. А любой синхронный итератор имеет async-двойник: «All of these preceding examples would have an asynchronous counterpart. In each case, you\'d replace the return type of <code>IEnumerable&lt;T&gt;</code> with an <code>IAsyncEnumerable&lt;T&gt;</code>» <span class="ru-tr">«У всех предыдущих примеров был бы асинхронный аналог. В каждом случае вы бы заменили тип возврата <code>IEnumerable&lt;T&gt;</code> на <code>IAsyncEnumerable&lt;T&gt;</code>»</span> — это тема S18.4. Как <code>foreach</code> разворачивается в <code>GetEnumerator</code>/<code>MoveNext</code>/<code>Current</code> — машинная тема S18.3.',
  ...
  cards: [
    {
      ...
      okText: 'Итератор <span class="hl">ленив</span>: <code>var seq = Produce()</code> не запускает тело, поэтому «Caller: got the object…» <span class="ru-tr">«Вызывающий: получил объект…»</span> печатается ПЕРВЫМ. Тело оживает на <code>foreach</code>. «Calling an iterator doesn\'t execute it immediately» <span class="ru-tr">«Вызов итератора не исполняет его немедленно»</span>.',
      noText: 'Вызов итератора возвращает объект и не исполняет тело — строка вызывающего печатается раньше «Iterator: start.» <span class="ru-tr">«Итератор: старт.»</span>. Реальный вывод: сначала «Caller: got the object, nothing ran yet.» <span class="ru-tr">«Вызывающий: получил объект, пока ничего не исполнилось.»</span>, затем «Iterator: start.» <span class="ru-tr">«Итератор: старт.»</span>, «Caller: 1» <span class="ru-tr">«Вызывающий: 1»</span>, «Iterator: after 1» <span class="ru-tr">«Итератор: после 1»</span>.',
  ...
  takeaways: [
    { icon: "why", k: "вызов ≠ исполнение", v: 'Метод с <code>yield return</code> при вызове возвращает <span class="hl">объект</span> и не исполняет тело: «Calling an iterator doesn\'t execute it immediately» <span class="ru-tr">«Вызов итератора не исполняет его немедленно»</span>. Тело стартует при начале обхода (замер: 2 итератора, 0 тел исполнено).' },
