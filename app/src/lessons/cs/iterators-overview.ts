...
const Z_SYNC: Zone = { id: "sync", x: 14, y: 34, w: 150, h: 168, cls: "vz-zone", label: "foreach", labelCls: "vz-zlabel sm", lx: 89, ly: 24, sub: "IEnumerable<T> / IEnumerator<T>", subCls: "vz-zsub", subY: 47 };
const Z_ASYNC: Zone = { id: "async", x: 176, y: 34, w: 150, h: 168, cls: "vz-zone good", label: "await foreach", labelCls: "vz-zlabel good sm", lx: 251, ly: 24, sub: "IAsyncEnumerable<T>", subCls: "vz-zsub good", subY: 47 };
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: 'Вызов метода даёт <b>iterator</b> — объект обхода. «An iterator is an object that traverses a container».', nodes: [{ id: "m", kind: "gate", at: { zone: "method", row: 0 }, state: "ok", label: "GetSingleDigits()", detail: "iterator-метод" }, { id: "o", kind: "obj", at: { zone: "iobj", row: 0 }, typeTag: "IEnumerable<int>", value: "объект обхода", accent: true }], edges: [{ id: "e1", from: "m", to: "o" }] },
        { codeLine: 2, caption: 'Элементы генерируются <span class="hl">по запросу</span>, элемент за элементом — не разом. Это делает итератор ленивым (разбор 02).', nodes: [{ id: "o", kind: "obj", at: { zone: "iobj", row: 0 }, typeTag: "IEnumerable<int>", value: "по запросу" }, { id: "seq", kind: "chip", at: { zone: "iobj", row: 1 }, value: "0,1,2,… по одному", accent: true }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'Синхронный обход — <code>foreach</code> над <span class="hl"><code>IEnumerable&lt;T&gt;</code></span>. «It relies on two generic interfaces… <code>IEnumerable&lt;T&gt;</code> and <code>IEnumerator&lt;T&gt;</code>».', nodes: [{ id: "f", kind: "gate", at: { zone: "sync", row: 0 }, state: "ok", label: "foreach", detail: "IEnumerable<T>", accent: true }, { id: "en", kind: "chip", at: { zone: "sync", row: 1 }, value: "IEnumerator<T>" }], edges: [] },
        { codeLine: 1, caption: 'Асинхронный обход — <code>await foreach</code> над <span class="hl"><code>IAsyncEnumerable&lt;T&gt;</code></span>: элементы добываются асинхронно.', nodes: [{ id: "f", kind: "gate", at: { zone: "sync", row: 0 }, state: "ok", label: "foreach", detail: "IEnumerable<T>" }, { id: "af", kind: "gate", at: { zone: "async", row: 0 }, state: "ok", label: "await foreach", detail: "IAsyncEnumerable<T>", accent: true }], edges: [] },
        { codeLine: 2, caption: 'Выбор по типу последовательности: «When a sequence is an <code>IEnumerable&lt;T&gt;</code>, you use <code>foreach</code>. When a sequence is an <code>IAsyncEnumerable&lt;T&gt;</code>, you use <code>await foreach</code>».', nodes: [{ id: "f", kind: "gate", at: { zone: "sync", row: 0 }, state: "ok", label: "IEnumerable<T>", detail: "→ foreach" }, { id: "af", kind: "gate", at: { zone: "async", row: 0 }, state: "ok", label: "IAsyncEnumerable<T>", detail: "→ await foreach", accent: true }], edges: [] },
      ],
      explain: 'Обход стоит на двух парах интерфейсов. Синхронный <code>foreach</code>: «It relies on two generic interfaces defined in the .NET core library to generate the code necessary to iterate a collection: <code>IEnumerable&lt;T&gt;</code> and <code>IEnumerator&lt;T&gt;</code>». Асинхронный — <code>await foreach</code> над <code>IAsyncEnumerable&lt;T&gt;</code>. Правило выбора дословно: «When a sequence is an <code>IEnumerable&lt;T&gt;</code>, you use <code>foreach</code>. When a sequence is an <code>IAsyncEnumerable&lt;T&gt;</code>, you use <code>await foreach</code>». А любой синхронный итератор имеет async-двойник: «All of these preceding examples would have an asynchronous counterpart. In each case, you\'d replace the return type of <code>IEnumerable&lt;T&gt;</code> with an <code>IAsyncEnumerable&lt;T&gt;</code>» — это тема S18.4. Как <code>foreach</code> разворачивается в <code>GetEnumerator</code>/<code>MoveNext</code>/<code>Current</code> — машинная тема S18.3.',
