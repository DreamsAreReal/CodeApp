...
export const boxingAndGenerics: LessonData = {
  ...
  spec: [
    { text: "«Generic collection types generally perform better for storing and manipulating value types because there is no need to box the value types.»", source: "ms-generics-std" },
  ],
  edgeCases: [
    ...
    { text: "Выигрыш обобщений — не только скорость, но и <b>аллокации</b>: «Generic collection types generally perform better for storing and manipulating value types because there is <span class=\"hl\">no need to box the value types</span>».", source: "ms-generics-std" },
  ],

  misconceptions: [
    {
      wrong: "боксинг — мелочь: ArrayList и List<int> хранят int примерно одинаково",
      hook: 'Разница <b>измерима и большая</b>. Необобщённый <code>ArrayList</code> хранит слот как <code>object</code>, поэтому каждый <code>int</code> при <code>Add</code> <span class="hl">упаковывается</span> в отдельный объект на куче (заголовок + значение) — собственный прогон: <b>24 байта на бокс</b>. Обобщённый <code>List&lt;int&gt;</code> хранит <code>int</code> <b>inline</b> в массиве — 0 боксов. Дословно почему: «Generic collection types generally perform better for storing and manipulating value types because there is <span class="hl">no need to box the value types</span>». На 1000 int это <code>ArrayList</code> 32056 B против <code>List&lt;int&gt;</code> 4056 B — <b>~8×</b>. Дальше <b>пять разборов</b>: почему ArrayList боксит (слот object), цена одного бокса (24 B), два бокса одного значения = два объекта, <code>List&lt;int&gt;</code> inline, и <b>машинная панель</b> — замер аллокаций обеих коллекций (реальный прогон: 32056 vs 4056).',
  ...
  cards: [
    {
      ...
      okText: '<code>ArrayList</code> боксит каждый int (слот <code>object</code>) → <b>32056 B</b>; <code>List&lt;int&gt;</code> хранит inline → <b>4056 B</b>. «no need to box the value types» — <span class="hl">~8×</span> дешевле.',
  ...
  takeaways: [
    { icon: "why", k: "Почему боксит", v: '<code>ArrayList</code> хранит слот как <code>object</code> → каждый <code>int</code> при <code>Add</code> упаковывается в объект на куче. <code>List&lt;int&gt;</code> хранит <code>int</code> <b>inline</b>: «<span class="hl">no need to box the value types</span>».' },
