...
export const records: LessonData = {
  ...
  spec: [
    { text: "«For `class` types, two objects are equal if they refer to the same object in memory… For types with the `record` modifier (`record class`, `record struct`, and `readonly record struct`), two objects are equal if they are of the same type and store the same values.» <span class=\"ru-tr\">«Для типов `class` два объекта равны, если они ссылаются на один и тот же объект в памяти… Для типов с модификатором `record` (`record class`, `record struct` и `readonly record struct`) два объекта равны, если они одного типа и хранят одинаковые значения.»</span>", source: "ms-record" },
  ],
  edgeCases: [
    ...
    { text: "Объявлять override <code>Object.Equals</code> и <code>==</code>/<code>!=</code> вручную — <b>ОШИБКА компиляции</b>; а <code>Equals(R?)</code>, <code>GetHashCode</code> и <code>EqualityContract</code> объявлять <span class=\"hl\">можно</span> («You can declare this property explicitly» <span class=\"ru-tr\">«Ты можешь объявить это свойство явно»</span>). Свой <code>Clone</code>/член с именем <code>Clone</code> — запрещён.", source: "ms-record" },
    { text: "Для <code>record class</code> равенство требует <b>совпадения runtime-типа</b>: две записи-переменные равны, только если реальный тип объекта совпадает («the run-time type must be equal» <span class=\"ru-tr\">«тип времени выполнения должен совпадать»</span>).", source: "ms-record" },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Это машинная панель урока — равенство, снятое числом. Дословно из спеки: «For <code>class</code> types, two objects are equal if they refer to the <span class="hl">same object in memory</span>… For types with the <code>record</code> modifier…, two objects are equal if they are of the same type and <b>store the same values</b>». <span class="ru-tr">«Для типов <code>class</code> два объекта равны, если они ссылаются на один и тот же объект в памяти… Для типов с модификатором <code>record</code>… два объекта равны, если они одного типа и хранят одинаковые значения».</span> Прогон: <code>r1==r2</code> → <b>True</b> (одинаковые X,Y), <code>c1==c2</code> → <b>False</b> (разные объекты). Именно поэтому record удобен для data-моделей: два «одинаковых» объекта считаются равными без ручного <code>Equals</code>. Это ключевая фича, а не сахар.',
      sources: ["ms-record"],
    },
    {
      ...
      explain: 'Суть record — <b>синтез компилятора</b>, не новый рантайм-механизм: «The features unique to record types are implemented by <span class="hl">compiler-synthesized methods</span>». <span class="ru-tr">«Особенности, уникальные для типов record, реализованы методами, синтезированными компилятором».</span> Он эмитит override <code>Object.Equals</code>, <code>Equals(R?)</code> (реализует <code>IEquatable&lt;T&gt;</code>), <code>GetHashCode</code>, <code>operator ==</code>/<code>!=</code>, <code>ToString</code>+<code>PrintMembers</code>, <code>Deconstruct</code> (для позиционных). Важная граница: «It\'s an error if you declare the override [<code>Object.Equals</code>] explicitly» <span class="ru-tr">«Объявить переопределение [<code>Object.Equals</code>] явно — ошибка»</span> и то же для <code>==</code>/<code>!=</code>. А вот <code>Equals(R?)</code>, <code>GetHashCode</code> и <code>EqualityContract</code> объявить <b>можно</b> — про последний Learn прямо пишет «<span class="hl">You can declare this property explicitly</span>» <span class="ru-tr">«Ты можешь объявить это свойство явно»</span> (напр., чтобы влиять на равенство в иерархии). И нельзя создать член <code>Clone</code>: «you can\'t create a member named <code>Clone</code> in any record type». <span class="ru-tr">«Ты не можешь создать член с именем <code>Clone</code> ни в одном типе record».</span>',
      sources: ["ms-record"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Опровержение мифа «with делает deep-копию». Дословно: «The result of a <code>with</code> expression is a <b>shallow copy</b>. For a reference property, the expression copies only the reference to an instance. Both the original record and the copy end up with a <span class="hl">reference to the same instance</span>». <span class="ru-tr">«Результат выражения <code>with</code> — <b>поверхностная копия</b>. Для ссылочного свойства выражение копирует только ссылку на экземпляр. И исходная запись, и копия в итоге имеют ссылку на один и тот же экземпляр».</span> Прогон: <code>b2.Items.Add(2)</code> изменил список, виден через <code>b1</code> — оба <code>Count == 2</code>, <code>ReferenceEquals(b1.Items, b2.Items)</code> == <b>True</b>. Тот же принцип, что shallow-иммутабельность init-свойств: заменить ссылку нельзя, а содержимое объекта за ней — можно.',
      sources: ["ms-record"],
    },
    {
      ...
      explain: 'Опровержение мифа «два record с одинаковыми полями равны». Для <code>record class</code>: «For two record variables to be equal, the <span class="hl">run-time type must be equal</span>». <span class="ru-tr">«Чтобы две переменные-записи были равны, тип времени выполнения должен совпадать».</span> Прогон: <code>A(1).Equals(B(1))</code> → <b>False</b>, потому что типы разные — компилятор синтезирует <code>EqualityContract</code> (возвращает <code>Type</code> записи) и «enables the equality methods to <b>compare the runtime type</b> of objects when they\'re checking for equality». <span class="ru-tr">«позволяет методам равенства <b>сравнивать тип времени выполнения</b> объектов при проверке на равенство».</span> Даже в иерархии: <code>Teacher</code> и <code>Student</code> с одинаковыми полями через ссылку <code>Person</code> не равны — «The equality test depends on the runtime type of the actual object, not the declared type of the variable». <span class="ru-tr">«Проверка равенства зависит от типа времени выполнения реального объекта, а не от объявленного типа переменной».</span> Равенство value-based, но с проверкой типа.',
      sources: ["ms-record"],
    },
  ],
  ...
  cards: [
    {
      ...
      noText: '«record… two objects are equal if… store the same values» <span class="ru-tr">«record… два объекта равны, если… хранят одинаковые значения»</span>; «class… equal if they refer to the same object in memory» <span class="ru-tr">«class… равны, если ссылаются на один и тот же объект в памяти»</span>. Реальный вывод: <b>True False</b>.',
    ...
    },
    {
      ...
      noText: '«A with expression creates a new record instance… with specified properties… modified» <span class="ru-tr">«Выражение with создаёт новый экземпляр записи… с изменёнными указанными свойствами»</span>. Оригинал цел: <b>p1={X=1,Y=2}</b>, копия <b>p2={X=99,Y=2}</b>.',
    ...
    },
    {
      ...
      noText: '«For two record variables to be equal, the run-time type must be equal» <span class="ru-tr">«Чтобы две переменные-записи были равны, тип времени выполнения должен совпадать»</span>. A ≠ B по типу, поэтому <code>a.Equals(b)</code> → <b>False</b>, хотя <code>X=1</code> совпадает.',
  ...
  takeaways: [
    ...
    { icon: "avoid", k: "Типы и синтез", v: 'Два разных record с одинаковыми полями <b>не равны</b> (EqualityContract, runtime-тип). <code>record struct</code> — value type. Ручной override <code>Object.Equals</code>/<code>==</code>/<code>!=</code> и член <code>Clone</code> — <span class="hl">ошибка</span>; а <code>EqualityContract</code>, <code>Equals(R?)</code>, <code>GetHashCode</code> объявить можно.' },
  ],
