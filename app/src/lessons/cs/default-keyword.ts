...
export const defaultKeyword: LessonData = {
  ...
  spec: [
    { text: "«A default value expression produces the default value of a type. Two kinds of default value expressions exist: the default operator call and a default literal.»", source: "ms-default-op" },
  ],
  edgeCases: [
    { text: "Аргумент — <b>тип или параметр типа</b>: «The argument to the <code>default</code> operator must be the name of a <span class=\"hl\">type or a type parameter</span>». Значит внутри дженерика законно <code>default(T)</code>.", source: "ms-default-op" },
    ...
    { text: "<code>default</code>-<b>литерал</b> = то же значение, где тип выводим: «The <code>default</code> literal expression produces the <span class=\"hl\">same value as the <code>default(T)</code> expression</span> where <code>T</code> is the inferred type».", source: "ms-default-op" },
  ],

  misconceptions: [
    {
      wrong: "внутри дженерика можно вернуть null как нейтральное значение T — компилятор разберётся",
      hook: 'Нельзя: <code>null</code> не годится, если <code>T</code> — value-тип, а <code>0</code> не годится, если <code>T</code> — ссылочный. Нужен <b>тип-параметр-безопасный ноль</b>. Это <code>default(T)</code>: «A default value expression produces the <span class="hl">default value of a type</span>… the <code>default</code> operator call and a <code>default</code> literal». Аргумент может быть <b>параметром типа</b>: «The argument to the <code>default</code> operator must be the name of a type or a type parameter». Для value-типов это ноль по всем полям, для ссылочных — <code>null</code>, для <code>int?</code> — тоже <code>null</code> (не 0!). Дальше <b>пять разборов</b>: сама проблема, value-дефолт (ноль по полям), reference/Nullable-дефолт (null), <code>default</code>-литерал в выводимых контекстах, и <b>машинная панель</b> — одно <code>default(T)</code>, четыре правильных нуля по типу аргумента (реальный прогон: <code>0|False|null|null</code>).',
  ...
  segments: [
    {
      ...
      explain: 'Дженерики создают конкретную проблему: у тебя нет универсального «пустого» значения, потому что <code>T</code> может быть и value-, и reference-типом. <code>default</code> решает это: «A default value expression produces the <span class="hl">default value of a type</span>. Two kinds of default value expressions exist: the <code>default</code> operator call and a <code>default</code> literal». Ключевое — аргументом может быть <b>параметр типа</b>: «The argument to the <code>default</code> operator must be the name of a <code>type</code> or a <code>type parameter</code>». Поэтому <code>default(T)</code> внутри <code>First&lt;T&gt;</code> легально и даёт корректный нейтральный элемент независимо от того, чем окажется <code>T</code>. Это стандартный паттерн для <code>TryGet</code>-подобных API и «нет результата».',
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: 'При <b>инициализации</b> переменной тип известен из объявления: <code>int x = default;</code> → 0. «produces the <span class="hl">same value as the default(T) expression</span>».', nodes: [{ id: "p", kind: "gate", at: { zone: "lit", row: 0 }, state: "ok", label: "T fill = default", detail: "тип = T" }, { id: "r", kind: "gate", at: { zone: "lit", row: 1 }, state: "ok", label: "return default", detail: "тип = возврат" }, { id: "v", kind: "gate", at: { zone: "lit", row: 2 }, state: "ok", label: "int x = default", detail: "0", accent: true }], edges: [] },
      ],
      explain: 'Когда тип понятен из контекста, можно опустить <code>(T)</code> и писать просто <code>default</code>: «You can use the <code>default</code> literal to produce the default value of a type <span class="hl">when the compiler can infer the expression type</span>. The <code>default</code> literal expression produces the same value as the <code>default(T)</code> expression where <code>T</code> is the inferred type». Контексты из доки: «In the assignment or initialization of a variable», «In the declaration of the default value for an optional method parameter», «In a method call to provide an argument value», «In a <code>return</code> statement or as an expression in an expression-bodied member». Самый частый — optional-параметр <code>T fill = default</code>: без <code>default</code>-литерала для обобщённого параметра дефолт не задать иначе.',
      sources: ["ms-default-op"],
    },
    {
  ...
  cards: [
    ...
    },
    {
      ...
      noText: 'Литерал <code>default</code> «produces the same value as the <code>default(T)</code> expression». Для <code>int</code> это 0, для <code>bool</code> — False. Реальный вывод: <b>0,0,0 | False,False</b>.',
  ...
  takeaways: [
    { icon: "why", k: "Зачем default(T)", v: 'Внутри дженерика нет универсального «пустого»: <code>null</code> не годится для value, <code>0</code> — для ref. «The argument to the <code>default</code> operator must be the name of a <span class="hl">type or a type parameter</span>» — <code>default(T)</code> даёт верный ноль.' },
    ...
    { icon: "avoid", k: "Литерал + три контекста", v: '<code>default</code>-литерал = «same value as the <code>default(T)</code> expression» там, где тип выводим (init/optional-параметр/return). Ключевое слово <code>default</code> живёт и в <code>switch</code>, и как <b>type-constraint</b> на override.' },
  ],
