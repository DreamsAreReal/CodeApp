...
export const switchExpressions: LessonData = {
  ...
  spec: [
    { text: "«Each switch expression arm contains a pattern, an optional case guard, the => token, and an expression.» <span class=\"ru-tr\">«Каждый рукав switch-выражения содержит паттерн, необязательный case guard, токен => и выражение.»</span>", source: "ms-switch-expr" },
    { text: "«The result of a switch expression is the value of the expression of the first switch expression arm whose pattern matches the input expression and whose case guard, if present, evaluates to true. The switch expression arms are evaluated in text order.» <span class=\"ru-tr\">«Результат switch-выражения — это значение выражения первого рукава switch-выражения, чей паттерн совпадает с входным выражением и чей case guard, если он есть, вычисляется в true. Рукава switch-выражения вычисляются в порядке текста.»</span>", source: "ms-switch-expr" },
  ],
  edgeCases: [
    { text: "Case guard — «another condition that must be satisfied together with a matched pattern. A case guard <b>must be a Boolean expression</b>» <span class=\"ru-tr\">«ещё одно условие, которое должно выполняться вместе с совпавшим паттерном. Case guard <b>должен быть булевым выражением</b>»</span> после <code>when</code>.", source: "ms-switch-expr" },
    { text: "«The compiler generates an error when a lower switch expression arm can't be chosen because a higher switch expression arm <span class=\"hl\">matches all its values</span>» <span class=\"ru-tr\">«Компилятор выдаёт ошибку, когда нижний рукав switch-выражения не может быть выбран, потому что вышестоящий рукав switch-выражения совпадает со всеми его значениями»</span> — недостижимый рукав ловится на компиляции.", source: "ms-switch-expr" },
    { text: "«List patterns don't generate a warning when all possible inputs aren't handled» <span class=\"ru-tr\">«List-паттерны не порождают предупреждение, когда обработаны не все возможные входы»</span> — исключение из общего правила проверки полноты.", source: "ms-switch-expr" },
  ],

  misconceptions: [
    {
      wrong: "switch-выражение — это тот же switch-statement, только короче",
      hook: 'Это <span class="hl">выражение</span>, а не оператор: оно <b>возвращает значение</b>, а не выполняет ветки. Рукав — не <code>case</code>: «Each switch expression arm contains a <b>pattern</b>, an optional <b>case guard</b>, the <code>=&gt;</code> token, and an <b>expression</b>». <span class="ru-tr">«Каждый рукав switch-выражения содержит <b>паттерн</b>, необязательный <b>case guard</b>, токен <code>=&gt;</code> и <b>выражение</b>».</span> Результат — «the value of the expression of the <span class="hl">first</span> switch expression arm whose pattern matches… and whose case guard, if present, evaluates to <code>true</code>», <span class="ru-tr">«значение выражения <b>первого</b> рукава switch-выражения, чей паттерн совпадает… и чей case guard, если он есть, вычисляется в <code>true</code>»,</span> причём рукава «are evaluated in <b>text order</b>». <span class="ru-tr">«вычисляются в <b>порядке текста</b>».</span> А ниже абстракции живёт exhaustiveness: неполный switch — рантайм-мина. Дальше <b>пять разборов</b> — анатомия рукава, порядок текста, or/and-диапазоны, полнота и <b>машинная панель</b> when-guard.',
  ...
  segments: [
    {
      ...
      explain: 'Switch-выражение — «Use the <code>switch</code> expression to evaluate a single expression from a list of candidate expressions. The evaluation is based on a <b>pattern match</b> with an input expression». <span class="ru-tr">«Используйте выражение <code>switch</code>, чтобы вычислить одно выражение из списка выражений-кандидатов. Вычисление основано на <b>сопоставлении с паттерном</b> входного выражения».</span> Его строительный блок — рукав, а не <code>case</code>: «The switch expression arms, separated by commas. Each switch expression arm contains a <b>pattern</b>, an optional <b>case guard</b>, the <code>=&gt;</code> token, and an <b>expression</b>». <span class="ru-tr">«Рукава switch-выражения, разделённые запятыми. Каждый рукав switch-выражения содержит <b>паттерн</b>, необязательный <b>case guard</b>, токен <code>=&gt;</code> и <b>выражение</b>».</span> Отличие от statement принципиальное: switch-выражение <span class="hl">возвращает значение</span>, поэтому его можно присвоить или вернуть из метода (как <code>=&gt;</code> тело).',
    ...
    },
    {
      ...
      explain: 'Правило выбора рукава — дословно: «The result of a switch expression is the value of the expression of the <b>first</b> switch expression arm whose pattern matches the input expression and whose case guard, if present, evaluates to <code>true</code>. The switch expression arms are evaluated in <span class="hl">text order</span>». <span class="ru-tr">«Результат switch-выражения — это значение выражения <b>первого</b> рукава switch-выражения, чей паттерн совпадает с входным выражением и чей case guard, если он есть, вычисляется в <code>true</code>. Рукава switch-выражения вычисляются в порядке текста».</span> Порядок — это семантика: верхний рукав может закрыть все входы нижнего. Компилятор это охраняет: «The compiler generates an <b>error</b> when a lower switch expression arm can\'t be chosen because a higher switch expression arm matches all its values». <span class="ru-tr">«Компилятор выдаёт <b>ошибку</b>, когда нижний рукав switch-выражения не может быть выбран, потому что вышестоящий рукав switch-выражения совпадает со всеми его значениями».</span> <code>ToOrientation(Direction.Right)</code> → <code>East</code>.',
      sources: ["ms-switch-expr"],
    },
    {
      ...
      explain: 'Логические комбинаторы собирают диапазоны прямо в рукаве. Conjunctive: «<b>Conjunctive</b> <code>and</code> pattern that matches an expression when <span class="hl">both patterns match</span> the expression». <span class="ru-tr">«<b>Конъюнктивный</b> паттерн <code>and</code>, который совпадает с выражением, когда оба паттерна совпадают с выражением».</span> Disjunctive: «<b>Disjunctive</b> <code>or</code> pattern that matches an expression when <span class="hl">either pattern matches</span> the expression». <span class="ru-tr">«<b>Дизъюнктивный</b> паттерн <code>or</code>, который совпадает с выражением, когда любой из паттернов совпадает с выражением».</span> Отсюда <code>&gt;= 6 and &lt; 9</code> — диапазон лета, <code>12 or (&gt;= 1 and &lt; 3)</code> — зима через декабрь + янв/фев. Прогон: <code>summer</code>, <code>winter</code>. Скобки нужны для ясности связывания (детальнее — урок relational/logical).',
      sources: ["ms-patterns"],
    },
    {
      ...
      explain: 'Exhaustiveness — обещание, которое проверяет компилятор и охраняет рантайм: «If none of a <code>switch</code> expression\'s patterns matches an input value, the runtime <b>throws an exception</b>. In .NET Core 3.0 and later versions, the exception is a <code>System.Runtime.CompilerServices.SwitchExpressionException</code>. … In most cases, the compiler generates a <span class="hl">warning</span> if a switch expression doesn\'t handle all possible input values». <span class="ru-tr">«Если ни один из паттернов выражения <code>switch</code> не совпадает с входным значением, рантайм <b>бросает исключение</b>. В .NET Core 3.0 и более поздних версиях это исключение — <code>System.Runtime.CompilerServices.SwitchExpressionException</code>. … В большинстве случаев компилятор выдаёт предупреждение, если switch-выражение обрабатывает не все возможные входные значения».</span> Исключение: «List patterns don\'t generate a warning». <span class="ru-tr">«List-паттерны не порождают предупреждение».</span> Совет доки прямой: «To guarantee that a switch expression handles all possible input values, provide a switch expression arm with a <b>discard pattern</b>». <span class="ru-tr">«Чтобы гарантировать, что switch-выражение обрабатывает все возможные входные значения, добавьте рукав switch-выражения с <b>discard-паттерном</b>».</span> <code>_</code> закрывает дыру.',
      sources: ["ms-switch-expr"],
    },
    {
      ...
      explain: 'Это машинная панель урока — case guard как разветвитель, снятый прогоном. Дока: «A pattern might not be expressive enough to specify the condition… In such a case, use a <b>case guard</b>. A case guard is <span class="hl">another condition that must be satisfied together with a matched pattern</span>. A case guard must be a Boolean expression. Specify a case guard after the <code>when</code> keyword». <span class="ru-tr">«Паттерн может быть недостаточно выразителен, чтобы задать условие… В таком случае используйте <b>case guard</b>. Case guard — это ещё одно условие, которое должно выполняться вместе с совпавшим паттерном. Case guard должен быть булевым выражением. Задавайте case guard после ключевого слова <code>when</code>».</span> Здесь три рукава несут <b>один и тот же</b> паттерн <code>var (a, b)</code>, а различает их только <code>when</code>: <code>(1,2)→a&lt;b</code>, <code>(5,2)→a&gt;b</code>, <code>(3,3)→equal</code>. Прогон подтверждает. Значит выбор рукава — это «pattern <b>И</b> guard», а не только паттерн.',
      sources: ["ms-switch-expr"],
    },
  ],
  ...
  cards: [
    ...
    },
    {
      ...
      noText: 'Case guard — «another condition that must be satisfied together with a matched pattern». <span class="ru-tr">«ещё одно условие, которое должно выполняться вместе с совпавшим паттерном».</span> Три рукава с одним паттерном разводит <code>when</code>: <code>a&lt;b / a&gt;b / equal</code>.',
