...
export const workstationServer: LessonData = {
  ...
  spec: [
    { text: "«Workstation garbage collection (GC), which is designed for client apps. It\'s the default GC flavor for standalone apps.» / «Server garbage collection, which is intended for server applications that need high throughput and scalability.»", source: "ms-wks-svr" },
  ],
  edgeCases: [
    { text: "Server GC (verbatim): «<span class=\"hl\">A heap and a dedicated thread to perform garbage collection are provided for each logical CPU, and the heaps are collected at the same time</span>. Each heap contains a small object heap and a large object heap, and all heaps can be accessed by user code».", source: "ms-wks-svr" },
    { text: "Concurrent = background (verbatim): «<span class=\"hl\">Concurrent (or <i>background</i>) garbage collection enables managed threads to continue operations during a garbage collection</span>. Background garbage collection replaces concurrent garbage collection in .NET Framework 4 and later versions».", source: "ms-wks-svr" },
    { text: "На 1 CPU (verbatim): «<span class=\"hl\">Workstation garbage collection is always used on a computer that has only one logical CPU</span>, regardless of the configuration setting». Независимо от конфигурации.", source: "ms-wks-svr" },
  ],

  misconceptions: [
    {
      wrong: "Server GC всегда быстрее и всегда лучше Workstation — включай его везде",
      hook: 'Миф: «<span class="wrong">Server GC всегда быстрее / всегда лучше</span>». Реальность узкая: «<span class="hl">server garbage collection is faster than workstation garbage collection <b>on the same size heap</b></span>» — и только за счёт ресурсов: «Server garbage collection <span class="hl">can be resource-intensive</span>». Доки прямо предупреждают: «If you\'re running <b>hundreds of instances</b> of an application, consider using <span class="hl">workstation garbage collection with concurrent garbage collection disabled</span>». Ниже <b>пять разборов</b>: два флейвора, server = heap+поток на каждый CPU, workstation = пользовательский поток (и всегда на 1-CPU), concurrent/background, и <b>машинная панель</b> — реальный флейвор и latency-режим текущего процесса.',
      source: "ms-wks-svr",
    },
  ],
  ...
  segments: [
    ...
    },
    {
      ...
      explain: 'Дословно: «Workstation garbage collection can be concurrent or non-concurrent. <span class="hl">Concurrent (or <i>background</i>) garbage collection enables managed threads to continue operations during a garbage collection</span>. Background garbage collection replaces concurrent garbage collection in .NET Framework 4 and later versions». И про Server: «In .NET Core, server garbage collection can be non-concurrent or background». Смысл background/concurrent — уменьшить паузы: пока GC работает в фоне, потоки приложения не полностью остановлены (важная оговорка: «Threads that run native code are not suspended on either server or workstation garbage collection»). Практика: отключение concurrent (для сотен инстансов) уменьшает context switching (разбор мифа) — то есть у режима есть и цена.',
      sources: ["ms-wks-svr"],
    },
    {
  ...
  cards: [
    ...
    },
    {
      ...
      noText: '«A heap and a dedicated thread… for each logical CPU». Реальный вывод: <b>server=True logicalCpus>=1=True</b>.',
  ...
  takeaways: [
    { icon: "why", k: "два флейвора", v: '<b>Workstation</b> — клиентские, «default GC flavor for standalone apps»; <b>Server</b> — «high throughput and scalability». Для hosted (ASP.NET) флейвор задаёт хост. Замер: этот процесс — <code>IsServerGC=True</code>.' },
    { icon: "cost", k: "server = heap на CPU", v: 'Server GC: «a heap and a dedicated thread… for each logical CPU», собираются одновременно (throughput). WKS: сбор на пользовательском потоке, конкурирует за CPU; на <b>1 CPU — всегда Workstation</b>.' },
    { icon: "avoid", k: "не «всегда лучше»", v: 'Server быстрее лишь «on the same size heap» и «can be resource-intensive». Для <b>сотен инстансов</b> доки советуют Workstation + concurrent disabled (меньше context switching). Background/concurrent — managed-потоки работают во время сбора.' },
  ],
