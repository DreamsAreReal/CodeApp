...
export const finalizersDispose: LessonData = {
  ...
  spec: [
    { text: "«The exact time when the finalizer executes is undefined. To ensure deterministic release of resources for instances of your class, implement a Close method or provide a IDisposable.Dispose implementation.»", source: "ms-finalize" },
  ],
  edgeCases: [
    { text: "Финализация дорогая (verbatim, Object.Finalize): «Reclaiming memory tends to take much longer if a finalization operation runs, because it <span class=\"hl\">requires at least two garbage collections</span>. … you should override the Finalize method for reference types only. The common language runtime only finalizes reference types. <b>It ignores finalizers on value types</b>».", source: "ms-finalize" },
    { text: "Может не выполниться (verbatim): «The Finalize method <span class=\"hl\">might not run to completion or might not run at all</span>» — при блокировке другого финализатора или аварийном завершении процесса. Порядок и поток финализатора не гарантированы.", source: "ms-finalize" },
    { text: "SuppressFinalize (verbatim, Dispose): «the call to the SuppressFinalize method <span class=\"hl\">prevents the garbage collector from running the finalizer</span>. If the type has no finalizer, the call to GC.SuppressFinalize has no effect. <b>The actual cleanup is performed by the Dispose(bool) method overload</b>».", source: "ms-dispose" },
  ],

  misconceptions: [
    {
      wrong: "финализатор гарантированно вызовется; Dispose — это то же, что финализатор; SuppressFinalize освобождает ресурсы",
      hook: 'Три мифа. «<span class="wrong">финализатор гарантированно вызовется</span>» — нет: «<span class="hl">The exact time when the finalizer executes is undefined</span>» и «might not run at all». «<span class="wrong">Dispose = финализатор</span>» — нет: <code>Dispose()</code> <b>детерминирован</b> (зовёт потребитель), финализатор — недетерминированный fallback GC; различаются флагом <code>disposing</code>. «<span class="wrong">SuppressFinalize освобождает ресурсы</span>» — нет: он лишь снимает объект с финализации, «<span class="hl">The actual cleanup is performed by the Dispose(bool)</span>». Ниже <b>пять разборов</b>: финализатор как fallback, Dispose детерминирован, канон-паттерн (SuppressFinalize + disposing), SafeHandle вместо своего финализатора, и <b>машинная панель</b> — реальный замер.',
      source: "ms-finalize",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 1, caption: 'Тип с финализатором (<code>~F()</code>) GC добавляет в <span class="hl">finalization queue</span> — очередь объектов, чей финализатор должен отработать.', nodes: [{ id: "q", kind: "gate", at: { zone: "fin", row: 0 }, state: "ok", label: "finalization queue", detail: "запись на тип", accent: true }], edges: [] },
      ...
      explain: 'Дословно (Object.Finalize): «If a type does override the <code>Finalize</code> method, the garbage collector <span class="hl">adds an entry for each instance of the type to an internal structure called the finalization queue</span>… The garbage collector then calls the <code>Finalize</code> method automatically when it discovers that an object is inaccessible». Ограничения: «<span class="hl">The exact time when the finalizer executes is undefined</span>»; «The finalizers of two objects are not guaranteed to run in any specific order»; «The thread on which the finalizer runs is unspecified»; и «The <code>Finalize</code> method <span class="hl">might not run to completion or might not run at all</span>» (если другой финализатор завис или процесс аварийно завершился). Вывод (анти-миф MM11/MM15): финализатор — это <b>недетерминированный fallback</b>, на который нельзя полагаться для своевременной очистки. Для детерминизма — <code>Dispose</code> (разбор 02).',
      sources: ["ms-finalize"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: '<code>Dispose</code> должен быть <span class="hl">идемпотентен</span>: «callable multiple times without throwing», повторные вызовы «do nothing».', nodes: [{ id: "d", kind: "gate", at: { zone: "disp", row: 0 }, state: "ok", label: "Dispose × N", detail: "идемпотентен" }, { id: "i", kind: "gate", at: { zone: "disp", row: 1 }, state: "ok", label: "2-й вызов", detail: "no-op", accent: true }], edges: [] },
      ...
      explain: 'Дословно (implementing-dispose): «Because the <code>public</code>, non-virtual … parameterless <code>Dispose</code> method is <span class="hl">called when it\'s no longer needed (by a consumer of the type)</span>, its purpose is to free unmanaged resources, perform general cleanup, and to indicate that the finalizer, if one is present, doesn\'t have to run». И про идемпотентность: «a <code>Dispose</code> method <span class="hl">should be idempotent, such that it\'s callable multiple times without throwing an exception. Furthermore, subsequent invocations of <code>Dispose</code> should do nothing</span>». Ключевое различие (анти-миф MM12): <code>Dispose</code> — <b>детерминированная</b> очистка от потребителя (ты знаешь, когда); финализатор — <b>недетерминированный</b> fallback от GC (ты не знаешь, когда и выполнится ли). Замер (панель): повторный <code>Dispose</code> не бросает и делает no-op.',
      sources: ["ms-dispose"],
    },
    {
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "finalizerRan=False · disposingWhenCalledFromDispose=True", caption: 'Панель: <b>finalizerRan=False</b> (SuppressFinalize сработал, 3/3) и <b>disposingWhenCalledFromDispose=True</b> (3/3). Финализация «requires at least two garbage collections», reference-types-only — из доки (не runnable).', nodes: [{ id: "s", kind: "gate", at: { zone: "suppr", row: 0 }, state: "ok", label: "finalizerRan", detail: "False" }, { id: "f", kind: "gate", at: { zone: "flag", row: 0 }, state: "ok", label: "disposing", detail: "True", accent: true }], edges: [] },
      ],
      explain: 'Машинная панель — механика SuppressFinalize и флага disposing, снятая в рантайме. (1) <b>SuppressFinalize</b>: у объекта с финализатором вызов <code>GC.SuppressFinalize(f)</code> убирает его из очереди финализации → финализатор НЕ бежит после сбора: замер <code>finalizerRan=False</code> (3/3), ровно как «prevents the garbage collector from running the finalizer». (2) <b>disposing</b>: канон <code>Dispose() → Dispose(true)</code> передаёт <code>disposing = true</code> (детерминированный путь) — замер <code>disposingWhenCalledFromDispose=True</code> (3/3); из финализатора было бы <code>false</code>. Две вещи оставлены как <b>Learn-факты</b>, а не runnable-карты, потому что недетерминированы во времени: «requires at least two garbage collections» (финализируемый объект собирается минимум за 2 GC) и «The common language runtime only finalizes reference types. It ignores finalizers on value types». Числа панели реальны; недетерминированные факты честно помечены как цитаты доки.',
      sources: ["ms-dispose", "ms-finalize"],
    },
  ],
  ...
  cards: [
    {
      ...
      noText: '«SuppressFinalize… prevents the garbage collector from running the finalizer». Реальный вывод: <b>finalizerRan=False</b>.',
    ...
    },
    {
      ...
      okText: '<code>Dispose</code> <span class="hl">идемпотентен</span>: «callable multiple times without throwing an exception. Furthermore, subsequent invocations of Dispose should do nothing». Три вызова → работа один раз. Печать: <b>disposeCount=1 idempotent=True</b>.',
  ...
  takeaways: [
    { icon: "why", k: "финализатор = fallback", v: 'Финализатор недетерминирован: «<span class="hl">exact time… undefined</span>», «might not run at all», порядок/поток не гарантированы, <b>≥2 GC</b> на сбор, только reference-типы (CLR «ignores finalizers on value types»). Не полагайся на него для своевременной очистки.' },
