...
export const nullable: LessonData = {
  ...
  spec: [
    { text: "«If HasValue returns true, the boxing operation boxes the corresponding value of the underlying value type T, not the instance of Nullable<T>.»", source: "ms-nullable" },
  ],
  edgeCases: [
  ...
  segments: [
    ...
    },
    {
      ...
      explain: 'Lifted operators распространяют null. Дословно: «These operators, also known as <b>lifted operators</b>, <span class="hl">return null if one or both operands are null</span>». Поэтому <code>a + b</code> при <code>b==null</code> даёт <code>null</code> (то есть <code>== null</code> True). А сравнения: «For the comparison operators <code>&lt;</code>, <code>&gt;</code>, <code>&lt;=</code>, and <code>&gt;=</code>, if one or both operands are <code>null</code>, the result is <b>false</b>». Ключевая ловушка из доки: «Don\'t assume that because a particular comparison (for example, <code>&lt;=</code>) returns <code>false</code>, the opposite comparison (<code>&gt;</code>) returns <code>true</code>» — прогон: и <code>a&gt;b</code>, и <code>a&lt;b</code> дают False. (<code>bool?</code> <code>&amp;</code>/<code>|</code> — отдельное исключение.)',
      sources: ["ms-nullable"],
    },
    {
      ...
      explain: 'Три способа извлечь значение — два безопасных, один бросающий. «Use the null-coalescing operator <code>??</code>… You can also use the <code>Nullable&lt;T&gt;.GetValueOrDefault(T)</code> method»; <code>GetValueOrDefault()</code> без аргумента даёт <code>default</code> базового типа. А явный каст рискован: «you can also explicitly cast… <code>int n2 = (int)n;</code> Compiles, but <span class="hl">throws an exception if n is null</span>». И асимметрия конверсий: «A non-nullable value type <code>T</code> is <b>implicitly convertible</b> to the corresponding nullable value type <code>T?</code>» — а обратно только явно/через безопасные методы. Правило: из <code>T?</code> в <code>T</code> — через <code>??</code>/<code>GetValueOrDefault</code>, не голым кастом.',
      sources: ["ms-nullable"],
    },
  ],
  ...
  cards: [
    {
      ...
      noText: '«the boxing operation boxes the corresponding value of the underlying value type T, not the instance of Nullable<T>». Реальный вывод: <b>Int32 True</b>.',
    ...
    },
    {
      ...
      noText: '«Any nullable value type is an instance of the generic Nullable<T> structure». <code>int?</code> — value type: <b>True False 0</b>, не reference type.',
    ...
    },
    {
      ...
      noText: '«lifted operators return null if… operands are null»; сравнения с null → false. Ловушка: <code>a&gt;b</code> false НЕ значит <code>a&lt;b</code> true. Реальный вывод: <b>True False False</b>.',
