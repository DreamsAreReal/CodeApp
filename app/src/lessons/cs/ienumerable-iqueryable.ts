...
export const ienumerableIqueryable: LessonData = {
  ...
  spec: [
    { text: "«Query expressions can be compiled to expression trees or to delegates, depending on the type that the query is applied to. The compiler compiles IEnumerable<T> queries to delegates. The compiler compiles IQueryable and IQueryable<T> queries to expression trees.» <span class=\"ru-tr\">«Запросы-выражения компилируются в деревья выражений или в делегаты — в зависимости от типа, к которому применён запрос. Запросы IEnumerable<T> компилятор компилирует в делегаты. Запросы IQueryable и IQueryable<T> компилятор компилирует в деревья выражений.»</span>", source: "ms-linq" },
  ],
  edgeCases: [
    { text: "<code>IQueryable&lt;T&gt;</code> — для провайдеров: «The <code>IQueryable&lt;T&gt;</code> interface is <span class=\"hl\">intended for implementation by query providers</span>». <span class=\"ru-tr\">«Интерфейс <code>IQueryable&lt;T&gt;</code> предназначен для реализации провайдерами запросов».</span> Он «inherits the <code>IEnumerable&lt;T&gt;</code> interface so that if it represents a query, the results of that query can be enumerated». <span class=\"ru-tr\">«наследует интерфейс <code>IEnumerable&lt;T&gt;</code>, так что если он представляет запрос, результаты этого запроса можно перечислить».</span>", source: "ms-iqueryable" },
    { text: "Тип решает способ исполнения: «The distinction between <code>IEnumerable&lt;T&gt;</code> and <code>IQueryable&lt;T&gt;</code> sequences <span class=\"hl\">determines how the query is executed at runtime</span>». <span class=\"ru-tr\">«Различие между последовательностями <code>IEnumerable&lt;T&gt;</code> и <code>IQueryable&lt;T&gt;</code> определяет, как запрос исполняется во время выполнения».</span>", source: "ms-sqo" },
    { text: "Провайдер транслирует дерево: «For <code>IQueryable&lt;T&gt;</code>, the query is translated into an <span class=\"hl\">expression tree</span>… Libraries such as Entity Framework translate LINQ queries into <b>native SQL queries</b> that execute at the database». <span class=\"ru-tr\">«Для <code>IQueryable&lt;T&gt;</code> запрос транслируется в дерево выражения… Библиотеки вроде Entity Framework переводят LINQ-запросы в <b>нативные SQL-запросы</b>, которые исполняются в базе данных».</span>", source: "ms-sqo" },
  ],

  misconceptions: [
    {
      wrong: "IEnumerable и IQueryable — одно и то же, разница только в имени интерфейса",
      hook: 'Дорогое заблуждение: <span class="wrong"><code>IEnumerable</code> и <code>IQueryable</code> взаимозаменяемы</span>. На деле от типа зависит, <b>во что компилятор превращает лямбду</b> и <b>где исполнится запрос</b>. Дословно: «The compiler compiles <span class="hl">IEnumerable&lt;T&gt; queries to delegates</span>. The compiler compiles IQueryable and IQueryable&lt;T&gt; queries to <span class="hl">expression trees</span>». <span class="ru-tr">«Компилятор компилирует запросы IEnumerable&lt;T&gt; в делегаты. Компилятор компилирует запросы IQueryable и IQueryable&lt;T&gt; в деревья выражений».</span> <code>IEnumerable</code> = делегат, бежит в процессе; <code>IQueryable</code> = дерево выражения, которое EF Core транслирует в SQL и гоняет в БД. Ниже <b>четыре разбора</b> и <b>машинная панель</b> — реальный IL: одна лямбда <code>x =&gt; x &gt; 2</code> компилируется в <code>Func&lt;&gt;</code> для <code>Enumerable.Where</code> и в <code>Expression&lt;Func&lt;&gt;&gt;</code> для <code>Queryable.Where</code>.',
      source: "ms-linq",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Оба интерфейса дают LINQ-синтаксис, но исполняются по-разному. Дословно (IQueryable API): «The <code>IQueryable&lt;T&gt;</code> interface is <span class="hl">intended for implementation by query providers</span>… This interface <b>inherits the <code>IEnumerable&lt;T&gt;</code> interface</b> so that if it represents a query, the results of that query can be enumerated». <span class="ru-tr">«Интерфейс <code>IQueryable&lt;T&gt;</code> предназначен для реализации провайдерами запросов… Этот интерфейс <b>наследует интерфейс <code>IEnumerable&lt;T&gt;</code></b>, так что если он представляет запрос, результаты этого запроса можно перечислить».</span> И (standard-query-operators): «The distinction between <code>IEnumerable&lt;T&gt;</code> and <code>IQueryable&lt;T&gt;</code> sequences <b>determines how the query is executed at runtime</b>». <span class="ru-tr">«Различие между последовательностями <code>IEnumerable&lt;T&gt;</code> и <code>IQueryable&lt;T&gt;</code> <b>определяет, как запрос исполняется во время выполнения</b>».</span> Так что выбор типа — не косметика: он решает, поедет ли лямбда делегатом в вашем процессе или деревом в БД.',
    ...
    },
    {
      ...
      explain: 'Для <code>IEnumerable&lt;T&gt;</code> лямбда — это обычный делегат. Дословно: «The compiler compiles <span class="hl">IEnumerable&lt;T&gt; queries to delegates</span>». <span class="ru-tr">«Компилятор компилирует запросы IEnumerable&lt;T&gt; в делегаты».</span> И (standard-query-operators): «For <code>IEnumerable&lt;T&gt;</code>, the returned enumerable object captures the arguments that were passed to the method. When that object is enumerated, the logic of the query operator is employed and the query results are returned». <span class="ru-tr">«Для <code>IEnumerable&lt;T&gt;</code> возвращаемый перечислимый объект захватывает аргументы, переданные методу. Когда этот объект перечисляется, применяется логика оператора запроса и возвращаются результаты запроса».</span> Это LINQ to Objects: фильтр/проекция — исполняемый .NET-код, бегущий в вашем процессе. Поэтому работают замыкания над локальными переменными (проверим прогоном) — делегат видит их значение <b>в момент перечисления</b>, а не объявления.',
    ...
    },
    {
      ...
      explain: 'Для <code>IQueryable&lt;T&gt;</code> лямбда превращается в дерево выражения. Дословно: «The compiler compiles IQueryable and IQueryable&lt;T&gt; queries to <span class="hl">expression trees</span>». <span class="ru-tr">«Компилятор компилирует запросы IQueryable и IQueryable&lt;T&gt; в деревья выражений».</span> И (standard-query-operators): «For <code>IQueryable&lt;T&gt;</code>, the query is <b>translated into an expression tree</b>. The expression tree can be translated to a native query when the data source can optimize the query. Libraries such as Entity Framework translate LINQ queries into <b>native SQL queries</b> that execute at the database». <span class="ru-tr">«Для <code>IQueryable&lt;T&gt;</code> запрос <b>транслируется в дерево выражения</b>. Дерево выражения может быть переведено в нативный запрос, когда источник данных способен оптимизировать запрос. Библиотеки вроде Entity Framework переводят LINQ-запросы в <b>нативные SQL-запросы</b>, которые исполняются в базе данных».</span> Дерево — это <b>описание</b> запроса как данные (узлы Parameter/Constant/GreaterThan…), а не исполняемый код. Плата за это — ограничения: «Expression trees have <b>limitations</b> on the allowed C# syntax» <span class="ru-tr">«У деревьев выражений есть <b>ограничения</b> на допустимый синтаксис C#»</span> — в <code>IQueryable</code>-лямбду нельзя пихать произвольный код, только переводимое подмножество.',
      sources: ["ms-linq", "ms-sqo"],
    },
    {
  ...
  takeaways: [
    { icon: "why", k: "делегат vs дерево", v: '«compiles IEnumerable&lt;T&gt; queries to delegates… compiles IQueryable and IQueryable&lt;T&gt; queries to expression trees». <span class="ru-tr">«компилирует запросы IEnumerable&lt;T&gt; в делегаты… компилирует запросы IQueryable и IQueryable&lt;T&gt; в деревья выражений».</span> Замер (IL): одна лямбда → <code>Func&lt;&gt;</code> для <code>Enumerable.Where</code>, <code>Expression&lt;Func&lt;&gt;&gt;</code> для <code>Queryable.Where</code>.' },
    { icon: "cost", k: "где исполняется", v: '<code>IEnumerable</code> — делегат в вашем процессе (LINQ to Objects). <code>IQueryable</code> — дерево транслируется провайдером «into native SQL queries that execute at the database». <span class="ru-tr">«в нативные SQL-запросы, которые исполняются в базе данных».</span> Тип «determines how the query is executed at runtime». <span class="ru-tr">«определяет, как запрос исполняется во время выполнения».</span>' },
