...
export const stringComparison: LessonData = {
  ...
  spec: [
    { text: "«The StringComparison enumeration is used to specify whether a string comparison should use the current culture or the invariant culture, word or ordinal sort rules, and be case-sensitive or case-insensitive.»", source: "ms-sc" },
  ],
  edgeCases: [
    { text: "Ordinal — это <b>сравнение по коду</b>: «An operation that uses ordinal sort rules performs a comparison based on the <span class=\"hl\">numeric value (Unicode code point)</span> of each Char in the string. An ordinal comparison is <b>fast but culture-insensitive</b>». Собственный прогон: <code>Compare(\"abc\",\"ABC\",Ordinal)</code> = 32 ('a'−'A').", source: "ms-sc" },
    { text: "Culture-sensitive применяет <b>лингвистические</b> правила: «An operation that uses word sort rules performs a <span class=\"hl\">culture-sensitive comparison</span> wherein certain nonalphanumeric Unicode characters might have special weights assigned to them». Собственный прогон: <code>\"i\"</code> и <code>\"I\"</code> без учёта регистра — равны в invariant, но <b>не</b> в <code>tr-TR</code>.", source: "ms-sc" },
    { text: "Всегда передавай <code>StringComparison</code>: «you should <b>always</b> call an overload that includes a parameter of type StringComparison so that you can specify the type of comparison that the method performs». Дефолт для нелингвистичного матчинга — <code>Ordinal</code>/<code>OrdinalIgnoreCase</code>.", source: "ms-sc" },
    { text: "Ordinal — «non-linguistic»: «Methods that are invoked with these StringComparison values base string operation decisions on <span class=\"hl\">simple byte comparisons</span> instead of casing or equivalence tables that are parameterized by culture». Быстрее и надёжнее для идентификаторов/символов.", source: "ms-bps" },
  ],
  ...
  misconceptions: [
    {
      ...
      hook: 'Нет — «сравнение» строк — это <b>несколько разных операций</b>, и результат зависит от выбранной политики. «The <b>StringComparison</b> enumeration is used to specify whether a string comparison should use the <span class="hl">current culture or the invariant culture</span>, <span class="hl">word or ordinal sort rules</span>, and be case-sensitive or case-insensitive». Есть два мира: <b>ordinal</b> — «a comparison based on the numeric value (Unicode code point) of each Char… fast but culture-insensitive», и <b>culture-sensitive</b> word sort — «wherein certain nonalphanumeric Unicode characters might have special weights». Они дают <b>разные</b> вердикты: классический пример — Turkish-I, где <code>"i"</code> и <code>"I"</code> без регистра равны в инвариантной культуре, но <b>не равны</b> в турецкой. Поэтому правило: «you should <b>always</b> call an overload that includes a parameter of type <b>StringComparison</b>», а безопасный дефолт для нелингвистичного матчинга — <code>Ordinal</code>/<code>OrdinalIgnoreCase</code>. Дальше <b>пять разборов</b>: два мира сравнения, ordinal как code point, culture word-sort и Turkish-I, правило «always StringComparison», и <b>машинная панель</b> — три вердикта для одной пары <code>"i"/"I"</code> (реальный прогон).',
      source: ["ms-sc", "ms-bps"],
    },
  ],
  ...
  segments: [
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 3, out: "", caption: 'С явным <code>Ordinal</code> — <span class="hl">нелингвистичное</span> сравнение по коду. Для схемы URL, пути, ключа это и нужно: «simple byte comparisons».', nodes: [{ id: "bad", kind: "gate", at: { zone: "bad", row: 0 }, state: "fail", label: "без флага", detail: "культура" }, { id: "good", kind: "gate", at: { zone: "good", row: 0 }, state: "ok", label: "StartsWith(..., Ordinal)", detail: "byte compare", accent: true }], edges: [] },
  ...
  cards: [
    {
      ...
      noText: '«a comparison based on the numeric value (Unicode code point)»: разность кодов = 32. Без регистра abc == ABC. Реальный вывод: <b>32 / 0 / True</b>.',
    ...
    },
    {
      ...
      noText: '«assumptions… about capitalizing "i"… aren\'t valid among all cultures». Три политики → три вердикта: <b>True / 1 / 0</b> (реальный прогон).',
  ...
  takeaways: [
    { icon: "why", k: "Два мира", v: 'Ordinal = «comparison based on the numeric value (Unicode code point)… <b>fast but culture-insensitive</b>». Culture word-sort = «culture-sensitive comparison» с лингвистическими весами. Разные семантика и вердикты (замер: i/I → True/1/0).' },
    { icon: "cost", k: "Turkish-I", v: '«assumptions… about capitalizing "i"… <b>aren\'t valid among all cultures</b>». <code>"i"</code>/<code>"I"</code> без регистра равны в invariant, но не в <code>tr-TR</code> (замер: 0 vs 1). Культурозависимое сравнение нелингвистичных строк = баги.' },
    { icon: "avoid", k: "Always StringComparison", v: '«you should <b>always</b> call an overload that includes a parameter of type StringComparison». Дефолт для матчинга — <code>Ordinal</code>/<code>OrdinalIgnoreCase</code> (safe default + better performance). Culture — только для текста, что читает человек.' },
  ],
