...
export const interfaceVariance: LessonData = {
  ...
  spec: [
    { text: "«Variance is a property of a generic interface or delegate type's type parameter, and it controls which implicit conversions exist between constructed types that use different type arguments.»", source: "ms-var-std" },
  ],
  edgeCases: [
    { text: "По умолчанию — <b>инвариантно</b>: «By default, generic type parameters are <span class=\"hl\">invariant</span>—even if one type argument derives from another, the corresponding constructed generic types, such as <code>List&lt;Derived&gt;</code> and <code>List&lt;Base&gt;</code>, are unrelated».", source: "ms-var-std" },
    { text: "Вариантность — <b>только для ссылочных</b> типов: «Variance applies only to reference types; if you specify a value type for a variant type parameter, that type parameter is <span class=\"hl\">invariant</span> for the resulting constructed type».", source: "ms-var-std" },
    { text: "<code>out</code> — только в позиции возврата, <code>in</code> — только в позиции параметра: «covariant type parameters can be used as the return types of the interface's methods, and contravariant type parameters can be used as the <span class=\"hl\">parameter types</span> of the interface's methods».", source: "ms-var-std" },
  ],

  misconceptions: [
    {
      wrong: "IEnumerable<string> и List<string> одинаково приводятся к <...object>: раз string : object, значит и обёртки совместимы",
      hook: 'Нет — по умолчанию дженерик-типы <b>инвариантны</b>: «By default, generic type parameters are <span class="hl">invariant</span>—even if one type argument derives from another, the corresponding constructed generic types, such as <code>List&lt;Derived&gt;</code> and <code>List&lt;Base&gt;</code>, are unrelated». <code>List&lt;string&gt;</code> к <code>List&lt;object&gt;</code> <b>не</b> приводится (CS0029). А вот <code>IEnumerable&lt;string&gt;</code> к <code>IEnumerable&lt;object&gt;</code> — <b>да</b>, потому что <code>IEnumerable&lt;out T&gt;</code> объявлен <span class="hl">ковариантным</span>. Разница — в модификаторе <code>out</code>/<code>in</code> на параметре интерфейса. Это отдельная тема от вариантности <b>делегатов</b> (соседний трек S4). Дальше <b>шесть разборов</b>: инвариантность по умолчанию, ковариантность <code>out T</code> (IEnumerable), контравариантность <code>in T</code> (IComparer), правило позиций (<code>out</code>=возврат, <code>in</code>=параметр), <b>машинная панель</b> — свой ковариантный <code>IProducer&lt;out T&gt;</code> (реальный прогон), и контраст с небезопасной вариантностью массивов.',
      source: "ms-var-std",
    },
  ],
  ...
  segments: [
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, out: "", caption: 'Безопасно, потому что <code>IEnumerable</code> только <b>отдаёт</b> элементы (позиция возврата). Читать строки как object можно всегда — «Assignment compatibility is preserved».', nodes: [{ id: "d", kind: "obj", at: { zone: "enud", row: 0 }, typeTag: "IEnumerable<string>", value: "3 строки" }, { id: "b", kind: "gate", at: { zone: "enub", row: 0 }, state: "ok", label: "только читаем", detail: "string как object", accent: true }], edges: [] },
      ...
      explain: 'Ковариантность интерфейса включается модификатором <code>out</code>. Дословно: «<i>Covariant</i> type parameters let you substitute a more derived type for the original type argument. For example, you can assign an instance of <code>IEnumerable&lt;Derived&gt;</code> to a variable of type <code>IEnumerable&lt;Base&gt;</code>». Из C#-обзора — тот же приём с сохранением совместимости: «Covariance… Assignment compatibility is preserved» и прямой пример «<code>IEnumerable&lt;object&gt; objects = strings;</code>». Почему безопасно: «All the type parameters of these interfaces are covariant, so the type parameters are used <span class="hl">only for the return types</span> of the members» — интерфейс лишь отдаёт <code>T</code>, никогда не принимает, поэтому расширение <code>string</code>→<code>object</code> не создаёт дыры. Реальный прогон подтверждает: элементы те же (3, «a»), меняется только статический тип ссылки.',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>out T</code> можно ставить <b>только там, где T «выходит»</b> — в возврате метода. «covariant type parameters can be used as the <span class="hl">return types</span>».', nodes: [{ id: "o", kind: "gate", at: { zone: "outpos", row: 0 }, state: "ok", label: "T Produce()", detail: "возврат ✓", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: '<code>in T</code> — <b>только там, где T «входит»</b> — в параметре. «contravariant type parameters can be used as the <span class="hl">parameter types</span>».', nodes: [{ id: "o", kind: "gate", at: { zone: "outpos", row: 0 }, state: "ok", label: "T Produce()", detail: "возврат ✓" }, { id: "i", kind: "gate", at: { zone: "inpos", row: 0 }, state: "ok", label: "Consume(T)", detail: "параметр ✓", accent: true }], edges: [] },
        { codeLine: 3, out: "", caption: 'Два ограничения: нарушение позиции → ошибка компиляции; и «Variance applies <span class="hl">only to reference types</span>» — для value-аргумента параметр снова инвариантен.', nodes: [{ id: "o", kind: "gate", at: { zone: "outpos", row: 0 }, state: "ok", label: "out", detail: "возврат" }, { id: "i", kind: "gate", at: { zone: "inpos", row: 0 }, state: "ok", label: "in", detail: "параметр" }, { id: "v", kind: "gate", at: { zone: "inpos", row: 1 }, state: "fail", label: "value-тип", detail: "инвариант", accent: true }], edges: [] },
      ],
      explain: 'Компилятор проверяет вариантность <b>структурно</b> по позициям. Дословно: «For an interface, <span class="hl">covariant type parameters can be used as the return types of the interface\'s methods, and contravariant type parameters can be used as the parameter types</span> of the interface\'s methods». Отсюда мнемоника: <code>out</code> — тип «выходит» (возврат, ковариант), <code>in</code> — тип «входит» (параметр, контравариант); поставить <code>out T</code> в параметр или <code>in T</code> в возврат нельзя — компилятор откажет. Плюс два жёстких факта: «<span class="hl">Only interface types and delegate types</span> can have variant type parameters» (у классов вариантности нет) и «Variance applies only to reference types; if you specify a value type for a variant type parameter, that type parameter is invariant» — <code>IEnumerable&lt;int&gt;</code> к <code>IEnumerable&lt;object&gt;</code> не приводится, потому что <code>int</code> — value-тип.',
      sources: ["ms-var-std"],
    },
    {
  ...
  takeaways: [
    { icon: "why", k: "Инвариантно по умолчанию", v: '«By default, generic type parameters are <span class="hl">invariant</span>… such as <code>List&lt;Derived&gt;</code> and <code>List&lt;Base&gt;</code>, are unrelated». <code>List&lt;string&gt;</code> → <code>List&lt;object&gt;</code> = CS0029. Вариантность включает только явный <code>out</code>/<code>in</code>.' },
    ...
    { icon: "avoid", k: "Только ref, только интерфейс/делегат", v: '«Variance applies only to <span class="hl">reference types</span>» (<code>IEnumerable&lt;int&gt;</code>↛<code>IEnumerable&lt;object&gt;</code>); «Only interface types and delegate types can have variant type parameters» — у классов вариантности нет. Замер своего <code>IProducer&lt;out T&gt;</code>: woof/True.' },
  ],
