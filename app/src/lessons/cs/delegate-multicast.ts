...
export const delegateMulticast: LessonData = {
  ...
  spec: [
    { text: "«A useful property of delegate objects is that you can assign multiple methods to one delegate instance by using the + operator.»", source: "ms-combine" },
  ],
  edgeCases: [
    { text: "Комбинировать можно только <b>однотипные</b> делегаты: «You can only combine delegates of the same type».", source: "ms-combine" },
    { text: "Дубликаты допустимы и вызываются все: «You can add the same delegate to a multicast delegate multiple times… it invokes all the delegates in the list, including duplicates».", source: "ms-combine" },
    { text: "Исключение обрывает список: «No subsequent methods in the invocation list are called» — незакрытое в методе исключение уходит вызывающему, остаток не выполняется.", source: "ms-using" },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Делегат — двухшаговая штука: сначала объявляют <b>тип</b>, потом создают <b>экземпляр</b>, заворачивающий метод. «A delegate object is normally constructed by providing the name of the method the delegate wraps, or with a <b>lambda expression</b>. A delegate can be <span class="hl">invoked once instantiated</span> in this manner. Invoking a delegate calls the method attached to the delegate instance». Экземпляр — это настоящий объект: «Because the instantiated delegate is an object, it can be passed as an argument, or assigned to a property». И типы делегатов особые: «Delegate types are derived from the <code>Delegate</code> class in .NET. Delegate types are <b>sealed</b>, they can\'t be derived from». Именно потому, что экземпляр — объект, к нему применимы операторы <code>+</code>/<code>-</code> — следующий разбор.',
      sources: ["ms-using"],
    },
    {
  ...
  cards: [
    {
      ...
      okText: '<code>+=</code> строит invocation list из трёх методов; один <code>a()</code> вызывает <span class="hl">все по порядку</span> → <code>ABC</code>, длина списка <b>3</b>. «invokes the delegates in the list, in order».',
    ...
    },
    {
      ...
      options: ["XX\\n2", "\\n0", "XXX\\n3", "X\\n2"], correctIndex: 0, xp: 10,
      okText: 'Один метод добавлен трижды (дубликаты разрешены), <code>-= m</code> снимает <span class="hl">самый правый</span> — одну копию. Осталось два → <code>XX</code>, список <b>2</b>. «removes the rightmost matching entry».',
  ...
  takeaways: [
    { icon: "why", k: "Invocation list", v: 'Один делегат держит <span class="hl">список методов</span>: <code>+</code>/<code>+=</code> добавляют, вызов прогоняет весь список <b>по порядку</b>. «invokes the delegates in the list, in order». <code>-</code>/<code>-=</code> снимают самый правый совпадающий (реальный прогон: <code>XX</code>).' },
    ...
    { icon: "avoid", k: "Ловушки multicast", v: 'Multicast-<code>Func</code> возвращает <span class="hl">только результат последнего</span> метода (реальный прогон: <code>3</code>) — остальные отброшены. Исключение в методе <b>обрывает</b> остаток списка: «No subsequent methods in the invocation list are called».' },
  ],
