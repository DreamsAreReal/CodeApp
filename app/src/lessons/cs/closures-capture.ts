...
export const closuresCapture: LessonData = {
  ...
  spec: [
    { text: "«If you capture variables in this way, the lambda expression stores them for use even if the variables go out of scope and would normally be garbage collected.» <span class=\"ru-tr\">«Если вы захватываете переменные таким образом, лямбда-выражение сохраняет их для использования, даже если переменные выходят из области видимости и обычно были бы удалены сборщиком мусора.»</span>", source: "ms-lambda" },
  ],
  edgeCases: [
    { text: "Захваченная переменная переживает scope: «A variable that you capture isn't garbage collected until the delegate that references it becomes eligible for garbage collection» <span class=\"ru-tr\">«Захваченная вами переменная не удаляется сборщиком мусора до тех пор, пока делегат, который на неё ссылается, не станет пригодным для сборки мусора»</span> — замыкание держит объект живым.", source: "ms-lambda" },
    { text: "Лямбда <span class=\"hl\">не может</span> напрямую захватить <code>in</code>/<code>ref</code>/<code>out</code>-параметр: «A lambda expression can't directly capture an <code>in</code>, <code>ref</code>, or <code>out</code> parameter from the enclosing method». <span class=\"ru-tr\">«Лямбда-выражение не может напрямую захватить параметр <code>in</code>, <code>ref</code> или <code>out</code> из объемлющего метода».</span>", source: "ms-lambda" },
  ...
  segments: [
    {
      ...
      explain: 'Лямбда — это способ записать анонимную функцию: «Use a <b>lambda expression</b> to create an anonymous function. Use the lambda declaration operator <code>=&gt;</code> to separate the lambda\'s parameter list from its body». <span class="ru-tr">«Используйте <b>лямбда-выражение</b>, чтобы создать анонимную функцию. Используйте оператор объявления лямбды <code>=&gt;</code>, чтобы отделить список параметров лямбды от её тела».</span> Сама по себе типа она не имеет («A lambda expression doesn\'t have a type because the common type system has no intrinsic concept of \'lambda expression.\'» <span class="ru-tr">«Лямбда-выражение не имеет типа, потому что в общей системе типов нет собственного понятия "лямбда-выражение".»</span>), но конвертируется в делегат: «If a lambda expression doesn\'t return a value, convert it to one of the <code>Action</code> delegate types. If it returns a value, convert it to one of the <code>Func</code> delegate types». <span class="ru-tr">«Если лямбда-выражение не возвращает значение, преобразуйте его в один из типов делегатов <code>Action</code>. Если оно возвращает значение, преобразуйте его в один из типов делегатов <code>Func</code>».</span> А делегат — «a type that represents references to methods with a particular parameter list and return type»; <span class="ru-tr">«тип, представляющий ссылки на методы с определённым списком параметров и типом возвращаемого значения»;</span> «Lambda expressions (in certain contexts) are compiled to delegate types». <span class="ru-tr">«Лямбда-выражения (в определённых контекстах) компилируются в типы делегатов».</span> Эта связка лямбда→делегат и есть каркас, на который дальше сядет захват.',
    ...
    },
    {
      ...
      explain: 'Центральный тезис темы, дословно от участника дизайна C#: «Because <code>()=&gt;v</code> means "return <b>the current value of variable v</b>", not "return the value v was back when the delegate was created". <span class="hl">Closures close over variables, not over values.</span>» <span class="ru-tr">«Потому что <code>()=&gt;v</code> означает "вернуть <b>текущее значение переменной v</b>", а не "вернуть значение, которое v имела в момент создания делегата". <b>Замыкания замыкаются над переменными, а не над значениями.</b>»</span> Лямбда ссылается на <i>outer variable</i>, и «If you capture variables in this way, the lambda expression stores them for use even if the variables go out of scope and would normally be garbage collected». <span class="ru-tr">«Если вы захватываете переменные таким образом, лямбда-выражение сохраняет их для использования, даже если переменные выходят из области видимости и обычно были бы удалены сборщиком мусора».</span> Поэтому <code>f()</code> после <code>v = 10</code> печатает <code>10</code>, а не <code>5</code> (реальный прогон: <code>5</code>, затем <code>10</code>). И захваченная переменная переживает scope: «A variable that you capture isn\'t garbage collected until the delegate that references it becomes eligible for garbage collection» <span class="ru-tr">«Захваченная вами переменная не удаляется сборщиком мусора до тех пор, пока делегат, который на неё ссылается, не станет пригодным для сборки мусора»</span> — делегат держит её живой.',
    ...
    },
    {
      ...
      explain: 'Это машинный слой захвата — как компилятор реализует «общую переменную». Из design-doc компилятора Roslyn: чтобы замыкание пережило метод, «we also have to create an <b>"environment"</b> to hold its captured variables and somehow deliver that environment into the context of the rewritten method». <span class="ru-tr">«нам также приходится создать <b>"окружение" (environment)</b>, чтобы хранить его захваченные переменные, и как-то доставить это окружение в контекст переписанного метода».</span> Захваченные локали <b>перестают быть локалями</b>: «Since the local variables are now <span class="hl">fields in a class instance</span> we can keep the same delegate signature and rely on field access to read and write the free variables», <span class="ru-tr">«Поскольку локальные переменные теперь <b>поля экземпляра класса</b>, мы можем сохранить ту же сигнатуру делегата и полагаться на доступ к полям для чтения и записи свободных переменных»,</span> и «Instead of referring to local variables, the rewritten closure now references <b>fields on the environment</b>». <span class="ru-tr">«Вместо обращения к локальным переменным переписанное замыкание теперь обращается к <b>полям окружения</b>».</span> Поэтому «замыкание копирует переменную» — миф: <code>read</code> и <code>write</code> делят <b>одно поле</b> одного экземпляра. В декомпиляторе этот класс виден как <code>&lt;&gt;c__DisplayClass…</code> (деталь реализации Roslyn, не публичный API).',
    ...
    },
    {
      ...
      explain: 'Одна и та же на вид петля даёт разный результат, потому что <code>for</code> и <code>foreach</code> захватывают по-разному. <code>foreach</code> изменили в C# 5: «Beginning with C# 5 (Visual Studio 2012), <code>foreach</code> iterator variables are <b>scoped within the iteration</b>» <span class="ru-tr">«Начиная с C# 5 (Visual Studio 2012), переменные итератора <code>foreach</code> <b>ограничены областью видимости в пределах итерации</b>»</span> — на каждой итерации логически свежая переменная, поэтому каждый делегат захватывает свою (<code>foreach: 0,1,2</code>). А <code>for</code> НЕ меняли: Липперт прямо ограничил изменение — «closures will close over a fresh copy of the variable each time. <span class="hl">The "for" loop will not be changed.</span>», <span class="ru-tr">«замыкания будут замыкаться над свежей копией переменной каждый раз. <b>Цикл "for" изменён не будет.</b>»,</span> потому что «there is one loop variable for the whole loop, not a new fresh variable "i" every time through». <span class="ru-tr">«есть одна переменная цикла на весь цикл, а не новая свежая переменная "i" на каждом проходе».</span> В <code>for</code> все делегаты делят одну <code>i</code>, после цикла равную 3 → <code>for: 3,3,3</code> (реальный прогон). Воркэраунд для <code>for</code>: <code>int copy = i;</code> в теле цикла и захватывать <code>copy</code>.',
    ...
    },
    {
      ...
      explain: 'С C# 9 у лямбды есть модификатор, управляющий захватом: «To prevent unintentional capture of local variables or instance state by the lambda, apply the <code>static</code> modifier to a lambda expression». <span class="ru-tr">«Чтобы предотвратить непреднамеренный захват локальных переменных или состояния экземпляра лямбдой, примените модификатор <code>static</code> к лямбда-выражению».</span> Что именно запрещено — тоже дословно: «A static lambda <b>can\'t capture local variables or instance state</b> from enclosing scopes, but it can reference static members and constant definitions». <span class="ru-tr">«Статическая лямбда <b>не может захватывать локальные переменные или состояние экземпляра</b> из объемлющих областей видимости, но может ссылаться на статические члены и определения констант».</span> Попытка захватить локаль из static-лямбды — ошибка компиляции (реальный прогон эндпоинта: <code>error CS8820: A static anonymous function cannot contain a reference to \'local\'</code>), а не тихая аллокация в рантайме. Тонкость: захват локали/параметра — это <code>CS8820</code>, а ссылка на <code>this</code>/<code>base</code> — отдельный <code>CS8821</code>. Практический смысл двойной: в горячем пути <code>static</code> гарантирует <b>0</b> closure-аллокаций на повторных вызовах (разбор 05), и он же ловит случайный захват <code>this</code> — частую причину неожиданного удержания объекта в памяти. Не забываем границу: даже обычная лямбда «can\'t directly capture an <code>in</code>, <code>ref</code>, or <code>out</code> parameter from the enclosing method». <span class="ru-tr">«не может напрямую захватить параметр <code>in</code>, <code>ref</code> или <code>out</code> из объемлющего метода».</span>',
      sources: ["ms-lambda"],
    },
  ],
  ...
  cards: [
    {
      ...
      okText: 'Лямбда захватила <b>переменную</b> <code>v</code>, а не число 5. Второй <code>f()</code> читает <span class="hl">текущее</span> значение → <code>10</code>. «Closures close over variables, not over values». <span class="ru-tr">«Замыкания замыкаются над переменными, а не над значениями».</span>',
    ...
    },
    {
      ...
      okText: '<code>for</code> делит <b>одну</b> <code>i</code> (после цикла = 3) → <code>3,3,3</code>. С C# 5 переменная <code>foreach</code> «scoped within the iteration» <span class="ru-tr">«ограничена областью видимости в пределах итерации»</span> — свежая на каждой → <code>0,1,2</code>. Изменение затронуло только foreach.',
  ...
  takeaways: [
    { icon: "why", k: "Что захватывается", v: 'Лямбда захватывает <span class="hl">переменную</span>, не её значение: читает текущее значение в момент вызова. «Closures close over variables, not over values». <span class="ru-tr">«Замыкания замыкаются над переменными, а не над значениями».</span> Компилятор поднимает локаль в <b>поле</b> общего display-класса — потому делегаты делят хранилище.' },
