...
export const reflectionGenerics: LessonData = {
  ...
  spec: [
    { text: "«…a generic type has associated with it a set of type parameters (if it's a generic type definition) or type arguments (if it's a constructed type).»", source: "ms-refgen" },
  ],
  edgeCases: [
    { text: "Инстанцировать можно <b>только закрытый</b> тип: «You can only create an instance of a generic type if it's <span class=\"hl\">closed</span>» — открытый бросает.", source: "ms-refgen" },
    { text: "<code>IsGenericParameter</code> различает <code>T</code> и <code>int</code>: «The <code>IsGenericParameter</code> property is <b>true</b> if the element is a type parameter».", source: "ms-refgen" },
    { text: "Открытый ≠ определение: базовый тип <code>D&lt;V,W&gt; : B&lt;int,V&gt;</code> даёт <code>B&lt;int,V&gt;</code> — «open, but it's not a generic type definition» (смесь аргумента и параметра).", source: "ms-refgen" },
  ],

  misconceptions: [
    {
      wrong: "typeof(List<>) и typeof(List<int>) — по сути один и тот же тип",
      hook: 'Это <b>разные</b> типы для рантайма. «From the point of view of reflection, the difference between a generic type and an ordinary type is that a generic type has associated with it a set of <b>type parameters</b> (if it\'s a generic type definition) or <b>type arguments</b> (if it\'s a constructed type)». <code>List&lt;&gt;</code> — <span class="hl">открытое определение</span> (шаблон), <code>List&lt;int&gt;</code> — закрытый тип. И это не педантизм: «You can only create an instance of a generic type <b>if it\'s closed</b>» — из <code>List&lt;&gt;</code> объект не создать. Дальше <b>пять разборов</b>: параметры vs аргументы, флаги <code>Is*</code>, round-trip <code>GetGenericTypeDefinition</code>↔<code>MakeGenericType</code>, <code>GetGenericArguments</code>, и <b>машинная панель</b> — как <code>MakeGenericType</code> делает открытый тип инстанцируемым (реальный прогон: флаги переключаются).',
      source: "ms-refgen",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>Dictionary&lt;,&gt;</code> — <b>открытое определение</b>: несёт <span class="hl">параметры</span> <code>TKey</code>, <code>TValue</code> — заглушки.', nodes: [{ id: "o", kind: "obj", at: { zone: "open", row: 0 }, w: 144, typeTag: "Dictionary<,>", value: "TKey, TValue", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: '<code>Dictionary&lt;int,string&gt;</code> — <b>закрытый</b> тип: параметры заменены <span class="hl">аргументами</span> <code>int</code>, <code>string</code>.', nodes: [{ id: "o", kind: "obj", at: { zone: "open", row: 0 }, w: 144, typeTag: "Dictionary<,>", value: "TKey, TValue" }, { id: "c", kind: "obj", at: { zone: "closed", row: 0 }, typeTag: "Dictionary<int,string>", value: "int, string", accent: true }], edges: [{ id: "e", from: "o", to: "c", accent: true }] },
      ],
      explain: 'Для reflection generic-тип — это тип <b>плюс список типов-спутников</b>: «a generic type has associated with it a set of <b>type parameters</b> (if it\'s a generic type definition) or <b>type arguments</b> (if it\'s a constructed type)». Параметры (<code>TKey</code>, <code>TValue</code>) — заглушки открытого определения; аргументы (<code>int</code>, <code>string</code>) — конкретные типы закрытого. И то, и другое — «represented by instances of the <code>Type</code> class»: даже <code>T</code> в рантайме это объект <code>Type</code>, просто особый. Определения — «the <b>templates</b> from which instantiable types are created»; <code>Dictionary&lt;TKey,TValue&gt;</code> из BCL — именно такое определение.',
      sources: ["ms-refgen"],
    },
    {
  ...
  cards: [
    ...
    },
    {
      ...
      noText: 'Только закрытый тип создаваем: «You can only create an instance… if it\'s closed». Имя закрытого — <code>List`1</code> (суффикс <code>`1</code> — число параметров), и он инстанцируем.',
  ...
  takeaways: [
    ...
    { icon: "cost", k: "Только закрытый создаваем", v: '«You can only create an instance… if it\'s <span class="hl">closed</span>». <code>ContainsGenericParameters=True</code> = открыт = <code>CreateInstance</code> бросит <code>ArgumentException</code>.' },
