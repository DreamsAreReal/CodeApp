...
export const composition: LessonData = {
  ...
  spec: [
    { text: "«The WhenAny method… returns a `Task<Task>` object that completes when any of its arguments complete… to retrieve that task's result or ensure any exceptions are properly thrown, you must `await` the completed task itself.» <span class=\"ru-tr\">«Метод WhenAny… возвращает объект `Task<Task>`, который завершается, когда завершается любой из его аргументов… чтобы получить результат этой задачи или гарантировать, что все исключения будут корректно выброшены, необходимо сделать `await` самой завершившейся задачи.»</span>", source: "ms-async-over" },
  ],
  edgeCases: [
    { text: "<b>Композиция</b>: «if any portion of an operation is asynchronous, the <span class=\"hl\">entire operation is asynchronous</span>» <span class=\"ru-tr\">«если хоть какая-то часть операции асинхронна, то вся операция асинхронна»</span> — async + последующая синхронная работа остаётся async.", source: "ms-async-over" },
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "2x", caption: 'Замер: последовательный вариант — <b>~2x</b>; старт-обеих-then-WhenAll — <span class="hl">~1x</span>. «total cook time is reduced because some tasks run concurrently» <span class="ru-tr">«общее время готовки сокращается, потому что некоторые задачи выполняются конкурентно»</span> (реальный прогон).', nodes: [{ id: "s", kind: "gate", at: { zone: "seq", row: 0 }, state: "fail", label: "await каждого", detail: "~2x" }, { id: "cg", kind: "gate", at: { zone: "conc", row: 0 }, state: "ok", label: "WhenAll", detail: "~1x", accent: true }], edges: [] },
      ],
      explain: 'Это машинная панель урока — конкурентность, снятая по времени. Паттерн из доки: «You start all the asynchronous tasks at once. You wait on each task <span class="hl">only when you need the results</span>» <span class="ru-tr">«Вы запускаете все асинхронные задачи сразу. Вы ожидаете каждую задачу только тогда, когда вам нужны её результаты»</span>. Прогон: <code>await V(); await V();</code> (await сразу после старта каждой) — <b>~2x</b> времени одной задачи (последовательно), а <code>var x=V(); var y=V(); await WhenAll(x,y)</code> — <b>~1x</b>: «The total cook time is reduced because some tasks run concurrently» <span class="ru-tr">«Общее время готовки сокращается, потому что некоторые задачи выполняются конкурентно»</span>. Ключ: <code>async</code> сам НЕ делает код конкурентным — конкурентность появляется, когда ты стартуешь задачи и <code>await</code>-ишь их в точке, где нужен результат. И это конкурентность, не параллелизм (один поток ведёт обе задачи).',
    ...
    },
    {
      ...
      explain: 'Ключевая тонкость <code>WhenAny</code>. Дословно: «the <code>WhenAny</code> method… returns a <span class="hl"><code>Task&lt;Task&gt;</code></span> object that completes when any of its arguments complete… <code>Task.WhenAny</code> returns a <code>Task&lt;Task&gt;</code> — a wrapper task that contains the completed task» <span class="ru-tr">«метод <code>WhenAny</code>… возвращает объект <span class="hl"><code>Task&lt;Task&gt;</code></span>, который завершается, когда завершается любой из его аргументов… <code>Task.WhenAny</code> возвращает <code>Task&lt;Task&gt;</code> — задачу-обёртку, содержащую завершившуюся задачу»</span>. То есть <code>await Task.WhenAny(...)</code> даёт не результат, а завершившуюся ЗАДАЧУ; «to retrieve that task\'s result or ensure any exceptions are properly thrown, you must <b>await the completed task itself</b>» <span class="ru-tr">«чтобы получить результат этой задачи или гарантировать, что все исключения будут корректно выброшены, необходимо <b>сделать await самой завершившейся задачи</b>»</span>. Поэтому два await: первый — дождаться первой финишировавшей, второй — <code>await finished</code> за результатом. Прогон: <code>fast</code> (10мс) обгоняет <code>slow</code> (100мс).',
      sources: ["ms-async-over"],
    },
    {
      ...
      explain: 'Паттерн «обработка по мере завершения»: «You use the <code>WhenAny</code> method to wait on the first task to finish and then process its result. After you process the result… you <b>remove the completed task</b> from the list of tasks» <span class="ru-tr">«Вы используете метод <code>WhenAny</code>, чтобы дождаться первой завершившейся задачи, а затем обработать её результат. После обработки результата… вы <b>удаляете завершившуюся задачу</b> из списка задач»</span>. Прогон: задачи <code>V(30,1), V(10,2), V(20,3)</code> обрабатываются в порядке <b>2,3,1</b> — по времени завершения (10, 20, 30 мс), а не по позиции в списке. Внутри цикла обязателен <code>await done</code> (второй await) — достать результат/исключение завершившейся задачи. Это то, чем <code>WhenAny</code> в цикле ценнее <code>WhenAll</code>: реагируешь сразу, не ждёшь самую медленную.',
      sources: ["ms-async-over"],
    },
    {
  ...
  cards: [
    {
      ...
      noText: '«WhenAll… completes when all the tasks… are complete» <span class="ru-tr">«WhenAll… завершается, когда завершены все задачи»</span>. Массив результатов — по порядку задач-аргументов. Реальный вывод: <b>1,2,3</b>.',
    ...
    },
    {
      ...
      noText: '«WhenAny… wait on the first task to finish… then process its result» <span class="ru-tr">«WhenAny… дождаться первой завершившейся задачи… затем обработать её результат»</span>. Порядок — по времени (10,20,30 мс), не по списку. Реальный вывод: <b>2,3,1</b>.',
