...
export const concurrentCollections: LessonData = {
  ...
  spec: [
    { text: "«The System.Collections.Concurrent namespace includes several collection classes that are both thread-safe and scalable. Multiple threads can safely and efficiently add or remove items from these collections, without requiring additional synchronization in user code.» <span class=\"ru-tr\">«Пространство имён System.Collections.Concurrent включает несколько классов коллекций, которые одновременно потокобезопасны и масштабируемы. Несколько потоков могут безопасно и эффективно добавлять или удалять элементы из этих коллекций без дополнительной синхронизации в пользовательском коде.»</span>", source: "ms-threadsafe" },
    { text: "«The ConcurrentQueue&lt;T&gt; and ConcurrentStack&lt;T&gt; classes don't use locks at all. Instead, they rely on Interlocked operations to achieve thread safety.» <span class=\"ru-tr\">«Классы ConcurrentQueue&lt;T&gt; и ConcurrentStack&lt;T&gt; вообще не используют блокировки. Вместо этого они полагаются на операции Interlocked для обеспечения потокобезопасности.»</span>", source: "ms-threadsafe" },
  ],
  edgeCases: [
    { text: "Старая <code>Synchronized</code>-обёртка «works by <b>locking the entire collection</b> on every add or remove operation» <span class=\"ru-tr\">«работает, <b>блокируя всю коллекцию</b> при каждой операции добавления или удаления»</span> — не масштабируется и не защищена от гонок.", source: "ms-threadsafe" },
    { text: "Только чтение из общей коллекции? «If you're <b>only reading</b> from a shared collection, use the classes in the <code>System.Collections.Generic</code> namespace» <span class=\"ru-tr\">«Если вы <b>только читаете</b> из общей коллекции, используйте классы из пространства имён <code>System.Collections.Generic</code>»</span> — синхронизация не нужна.", source: "ms-threadsafe" },
  ...
  misconceptions: [
    {
      ...
      hook: 'Соблазнительно считать <code>GetOrAdd</code> одной неделимой операцией с гарантией «фабрика вызовется один раз». Дока говорит обратное: «the <code>valueFactory</code> delegate is called <span class="wrong">outside the locks</span>… Therefore, <code>GetOrAdd</code> is <b>not atomic</b>». <span class="ru-tr">«делегат <code>valueFactory</code> вызывается вне локов… Поэтому <code>GetOrAdd</code> <b>не атомарен</b>».</span> Под гонкой «<code>valueFactory</code> may be called <span class="hl">multiple times</span>, but only one key/value pair will be added». <span class="ru-tr">«<code>valueFactory</code> может быть вызвана несколько раз, но добавлена будет только одна пара ключ/значение».</span> Concurrent-коллекции быстры не потому, что «lock вокруг обычной»: у части — fine-grained locking, а <code>ConcurrentQueue</code>/<code>Stack</code> «don\'t use locks at all… Interlocked». <span class="ru-tr">«вообще не используют локи… Interlocked».</span> Дальше <b>четыре разбора</b> — до <b>машинной панели</b>: реальный снимок энумератора <code>ConcurrentQueue</code> — считает <b>3</b>, пока живая очередь уже <b>4</b>.',
      source: "ms-getoradd",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 1, caption: '<code>Synchronized</code>-обёртка «works by <span class="wrong">locking the entire collection</span> on every add or remove». <span class="ru-tr">«работает, блокируя всю коллекцию при каждом добавлении или удалении».</span> Каждый поток ждёт <b>один</b> общий лок.', nodes: [{ id: "ht", kind: "obj", at: { zone: "old", row: 0 }, typeTag: "Synchronized", value: "1 лок на всё", accent: true }, { id: "wait", kind: "gate", at: { zone: "old", row: 1 }, state: "fail", label: "потоки", detail: "в очереди" }], edges: [] },
        { codeLine: 3, caption: '<code>Concurrent</code>-типы — «both thread-safe and <span class="hl">scalable</span>»: <span class="ru-tr">«одновременно потокобезопасны и масштабируемы»:</span> часть использует fine-grained locking (лок на сегмент, не на всё).', nodes: [{ id: "ht", kind: "obj", at: { zone: "old", row: 0 }, typeTag: "Synchronized", value: "1 лок" }, { id: "cd", kind: "obj", at: { zone: "conc", row: 0 }, typeTag: "ConcurrentDict", value: "лок на сегмент", accent: true }], edges: [] },
        { codeLine: 3, caption: '<code>ConcurrentQueue</code>/<code>ConcurrentStack</code> — <span class="hl">вообще без локов</span>: «rely on <code>Interlocked</code> operations». <span class="ru-tr">«полагаются на операции <code>Interlocked</code>».</span> Атомарные CAS-операции вместо блокировок.', nodes: [{ id: "cd", kind: "obj", at: { zone: "conc", row: 0 }, typeTag: "ConcurrentDict", value: "fine-grained" }, { id: "cq", kind: "gate", at: { zone: "conc", row: 1 }, state: "ok", label: "Queue/Stack", detail: "Interlocked", accent: true }], edges: [] },
      ],
      explain: 'Ось урока — цена синхронизации. Старый путь: «These classes provide some thread safety through the <code>Synchronized</code> property… The wrapper works by <b>locking the entire collection</b> on every add or remove operation… This process isn\'t scalable». <span class="ru-tr">«Эти классы обеспечивают некоторую потокобезопасность через свойство <code>Synchronized</code>… Обёртка работает, <b>блокируя всю коллекцию</b> при каждой операции добавления или удаления… Этот подход не масштабируется».</span> Новый: «The <code>System.Collections.Concurrent</code> namespace includes several collection classes that are both <b>thread-safe and scalable</b>… without requiring additional synchronization in user code». <span class="ru-tr">«Пространство имён <code>System.Collections.Concurrent</code> включает несколько классов коллекций, которые одновременно <b>потокобезопасны и масштабируемы</b>… без дополнительной синхронизации в пользовательском коде».</span> Внутри — «lightweight synchronization mechanisms such as <code>SpinLock</code>, <code>SpinWait</code>…» <span class="ru-tr">«лёгкие механизмы синхронизации, такие как <code>SpinLock</code>, <code>SpinWait</code>…»</span> (busy-spinning вместо дорогого kernel-перехода), а <code>ConcurrentQueue&lt;T&gt;</code> и <code>ConcurrentStack&lt;T&gt;</code> «don\'t use locks at all. Instead, they rely on <code>Interlocked</code> operations». <span class="ru-tr">«вообще не используют локи. Вместо этого они полагаются на операции <code>Interlocked</code>».</span> Миф «concurrent = lock вокруг обычной» — ровно то, чего эти типы избегают.',
    ...
    },
    {
      ...
      explain: 'Пять ролей в <code>System.Collections.Concurrent</code>: <code>ConcurrentDictionary&lt;TKey,TValue&gt;</code> — «Thread-safe implementation of a dictionary of key-value pairs» <span class="ru-tr">«Потокобезопасная реализация словаря пар ключ-значение»</span>; <code>ConcurrentQueue&lt;T&gt;</code> — FIFO; <code>ConcurrentStack&lt;T&gt;</code> — LIFO; <code>ConcurrentBag&lt;T&gt;</code> — «unordered collection of elements» <span class="ru-tr">«неупорядоченная коллекция элементов»</span> (допускает дубликаты, оптимизирован под «тот же поток produce+consume»); <code>BlockingCollection&lt;T&gt;</code> — «Provides bounding and blocking functionality for any type that implements <code>IProducerConsumerCollection&lt;T&gt;</code>». <span class="ru-tr">«Предоставляет функциональность ограничения ёмкости и блокировки для любого типа, реализующего <code>IProducerConsumerCollection&lt;T&gt;</code>».</span> Важное уточнение: <code>BlockingCollection</code> — не самостоятельное хранилище, а обёртка над внутренней коллекцией (по умолчанию <code>ConcurrentQueue</code>, можно <code>ConcurrentStack</code>), добавляющая ёмкость и блокировку на пустой/полной.',
      sources: ["ms-threadsafe"],
    },
    {
      ...
      explain: 'Тонкость <code>GetOrAdd</code>: «For modifications and write operations… <code>ConcurrentDictionary</code> uses <b>fine-grained locking</b>… However, the <code>valueFactory</code> delegate is called <b>outside the locks</b> to avoid the problems that can arise from executing unknown code under a lock. Therefore, <code>GetOrAdd</code> is <b>not atomic</b> with regards to all other operations». <span class="ru-tr">«Для изменений и операций записи… <code>ConcurrentDictionary</code> использует <b>мелкозернистую блокировку</b>… Однако делегат <code>valueFactory</code> вызывается <b>вне локов</b>, чтобы избежать проблем, которые могут возникнуть при выполнении неизвестного кода под локом. Поэтому <code>GetOrAdd</code> <b>не атомарен</b> по отношению ко всем остальным операциям».</span> Следствие: «If you call <code>GetOrAdd</code> simultaneously on different threads, <code>valueFactory</code> may be called <b>multiple times</b>, but only <b>one</b> key/value pair will be added». <span class="ru-tr">«Если вы вызываете <code>GetOrAdd</code> одновременно из разных потоков, <code>valueFactory</code> может быть вызвана <b>несколько раз</b>, но добавлена будет только <b>одна</b> пара ключ/значение».</span> В один поток всё просто: свежий ключ → фабрика выполняется, значение вставляется; существующий → «The existing value is returned», <span class="ru-tr">«Возвращается существующее значение»,</span> фабрика не зовётся (реальный прогон: <code>100 100 factoryRuns=1</code>). Практический вывод: фабрика должна быть <b>идемпотентной и дешёвой</b> — на неё нельзя вешать побочные эффекты «ровно один раз».',
      sources: ["ms-getoradd"],
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: 'В очереди <b>1, 2, 3</b>. Вызов <code>GetEnumerator()</code> берёт <span class="hl">снимок</span> содержимого — «a moment-in-time snapshot». <span class="ru-tr">«снимок на момент времени».</span>', nodes: [{ id: "e1", kind: "slot", at: { zone: "enum", row: 0 }, name: "снимок", value: "1 2 3", accent: true }, { id: "l1", kind: "slot", at: { zone: "live", row: 0 }, name: "очередь", value: "1 2 3" }], edges: [] },
        ...
        { codeLine: 3, out: "snapshot-count=3 live-count=4", caption: 'Энумератор насчитал <span class="hl">3</span>, а <code>q.Count</code> = <b>4</b>: «It does <b>not reflect any updates</b> to the collection after <code>GetEnumerator</code> was called». <span class="ru-tr">«Он <b>не отражает никаких изменений</b> коллекции после вызова <code>GetEnumerator</code>».</span> Снимок ≠ живая коллекция (реальный прогон).', nodes: [{ id: "s", kind: "gate", at: { zone: "enum", row: 0 }, state: "ok", label: "snapshot", detail: "3", accent: true }, { id: "lv", kind: "gate", at: { zone: "live", row: 0 }, state: "fail", label: "live Count", detail: "4", accent: true }], edges: [] },
      ],
      explain: 'Машинная панель — реально снятое поведение энумерации. У <code>ConcurrentQueue&lt;T&gt;</code>: «The enumeration represents a <b>moment-in-time snapshot</b> of the contents of the queue. It does <b>not reflect any updates</b> to the collection after <code>GetEnumerator</code> was called. The enumerator is <b>safe to use concurrently</b> with reads from and writes to the queue». <span class="ru-tr">«Перечисление представляет собой <b>снимок содержимого очереди на момент времени</b>. Оно <b>не отражает никаких изменений</b> коллекции после вызова <code>GetEnumerator</code>. Энумератор <b>безопасно использовать параллельно</b> с чтениями из очереди и записями в неё».</span> Реальный прогон: взяли энумератор на очереди <code>{1,2,3}</code>, добавили 4-й — энумератор насчитал <b>3</b>, а <code>Count</code> уже <b>4</b>. Важная асимметрия: у <code>ConcurrentQueue</code>/<code>ConcurrentBag</code> — снимок, а у <code>ConcurrentDictionary</code> — <b>наоборот</b>: энумератор не снимок и может отражать изменения после <code>GetEnumerator</code>. Поэтому «<code>foreach</code> по любой concurrent-коллекции даёт консистентный снимок» — миф: зависит от типа.',
  ...
  cards: [
    {
      ...
      okText: 'Свежий ключ 2 → фабрика даёт 100 (f=1). Второй вызов: ключ уже есть → «The existing value is returned» <span class="ru-tr">«Возвращается существующее значение»</span> = 100, фабрика <b>не вызвана</b>. Итог: <b>100 100 factoryRuns=1</b>.',
    ...
    },
    {
      ...
      okText: 'Энумератор — «moment-in-time snapshot» <span class="ru-tr">«снимок на момент времени»</span>: заморожен на <b>3</b> элементах до <code>GetEnumerator</code>. <code>Enqueue(4)</code> после не виден → snapshot=3, а <code>Count</code>=<b>4</b>.',
      noText: 'Снимок «does not reflect any updates to the collection after GetEnumerator was called». <span class="ru-tr">«не отражает никаких изменений коллекции после вызова GetEnumerator».</span> 4-й элемент добавлен позже — энумератор его не видит (3), живая очередь — 4. Вывод: <b>snapshot-count=3 live-count=4</b>.',
  ...
  takeaways: [
    { icon: "why", k: "Цена синхронизации", v: 'Старая <code>Synchronized</code>-обёртка лочит <b>всю</b> коллекцию (не масштабируется). Concurrent-типы — fine-grained locking, а <code>ConcurrentQueue</code>/<code>Stack</code> «don\'t use locks at all… <code>Interlocked</code>». <span class="ru-tr">«вообще не используют локи… <code>Interlocked</code>».</span> Только читаешь? Бери <code>System.Collections.Generic</code>.' },
    { icon: "cost", k: "GetOrAdd не атомарен", v: '<code>valueFactory</code> зовётся <b>вне лока</b>: «not atomic», <span class="ru-tr">«не атомарен»,</span> под гонкой «may be called multiple times, but only one key/value pair will be added». <span class="ru-tr">«может быть вызвана несколько раз, но добавлена будет только одна пара ключ/значение».</span> Фабрика должна быть идемпотентной. В один поток: <b>100 100 factoryRuns=1</b>.' },
    { icon: "avoid", k: "Снимок зависит от типа", v: 'Энумератор <code>ConcurrentQueue</code>/<code>Bag</code> — «moment-in-time snapshot» <span class="ru-tr">«снимок на момент времени»</span> (реальный замер: <b>3</b> vs live <b>4</b>). У <code>ConcurrentDictionary</code> — <b>не</b> снимок. <code>IsSynchronized</code>=false; <code>SyncRoot</code> в .NET 10 бросает.' },
  ],
