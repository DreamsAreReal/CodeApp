...
export const standardOperators: LessonData = {
  ...
  spec: [
    { text: "«The standard query operators are the keywords and methods that form the LINQ pattern.»", source: "ms-sqo" },
  ],
  edgeCases: [
    { text: "Операторы — <b>extension-члены</b>: «The methods that make up each set are <span class=\"hl\">extension members</span> defined in the <code>Enumerable</code> and <code>Queryable</code> classes… where the receiver type is either the <code>IEnumerable&lt;T&gt;</code> or <code>IQueryable&lt;T&gt;</code> type».", source: "ms-sqo" },
    { text: "Тайминг зависит от типа возврата: «Those methods that return a singleton value (such as <code>Average</code> and <code>Sum</code>) <span class=\"hl\">execute immediately</span>. Methods that return a sequence <b>defer</b> the query execution and return an enumerable object».", source: "ms-sqo" },
    { text: "Композиция: «You can use the output sequence of one query as the <span class=\"hl\">input sequence to another query</span>. You chain query methods together in one query, which enables queries to become arbitrarily complex».", source: "ms-sqo" },
  ],

  misconceptions: [
    {
      wrong: "Where/Select/GroupBy/Join — встроенные конструкции языка, как for или if",
      hook: 'Заблуждение: <span class="wrong"><code>Where/Select/GroupBy/Join</code> — это языковые конструкции</span>, часть синтаксиса C#. На деле это <b>обычные методы</b> — «The standard query operators are the <span class="hl">keywords and methods</span> that form the LINQ pattern» — extension-члены на <code>IEnumerable&lt;T&gt;</code>/<code>IQueryable&lt;T&gt;</code>. Они делятся на семейства: фильтр, проекция, группировка, join, сортировка; выход одного — вход следующего. Ниже <b>пять разборов</b>: операторы как extension-методы, фильтр→проекция, группировка, и <b>машинная панель</b> — детерминированный порядок <code>Join</code>: outer сохраняется, затем совпавшие inner (реальный прогон: <b>Ann→Eng, Cy→Eng, Bob→Math</b>).',
      source: "ms-sqo",
    },
  ],
  ...
  segments: [
    ...
    },
    {
      ...
      explain: 'Два базовых семейства. <b>Фильтрация</b>: «Filter data using the <code>where</code> keyword». <b>Проекция</b>: «Project data using the <code>select</code> keyword». Ключ к их силе — композиция: «You can use the <span class="hl">output sequence of one query as the input sequence to another query</span>. You chain query methods together in one query, which enables queries to become arbitrarily complex». Поэтому <code>Where(...).Select(...)</code> — конвейер: отфильтровали {3,4,5,6}, спроецировали в квадраты {9,16,25,36}. Каждый оператор возвращает последовательность, к которой цепляется следующий.',
      sources: ["ms-sqo"],
    },
    {
      ...
      explain: '<b>Группировка</b> — дословно: «Group data using the <code>group</code> and optionally <code>into</code> keywords» (метод — <code>GroupBy</code>). Каждая группа — <code>IGrouping&lt;TKey, TElement&gt;</code>: имеет <code>Key</code> и сама перечислима. В примере из доки слова группируются по длине: <code>"the quick brown fox"</code> → длина 3: {the, fox}, длина 5: {quick, brown}. После <code>OrderBy(x =&gt; x.Key)</code> и проекции в <code>"{Key}:{Count}"</code> получаем <b>3:2 5:2</b>. GroupBy — <b>nonstreaming</b> оператор: чтобы построить группы, он обязан прочитать <span class="hl">весь</span> источник (детально о streaming/nonstreaming — урок S3.4).',
  ...
  takeaways: [
    { icon: "why", k: "операторы — это методы", v: '«The standard query operators are the keywords and methods that form the LINQ pattern» — extension-члены на <code>IEnumerable&lt;T&gt;</code>/<code>IQueryable&lt;T&gt;</code>, не синтаксис языка. Семейства: фильтр/проекция/группировка/join/сортировка.' },
    { icon: "cost", k: "композиция конвейером", v: '«output sequence of one query as the input sequence to another» — <code>Where().Select().GroupBy()</code> цепляются. Скаляры (<code>Count/Sum</code>) исполняются сразу; последовательности — отложенно.' },
