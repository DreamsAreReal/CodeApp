...
export const weakReferences: LessonData = {
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'Пока есть <span class="hl">strong reference</span>, GC не может собрать объект — «The application is said to have a strong reference».', nodes: [{ id: "s", kind: "gate", at: { zone: "strong", row: 0 }, state: "fail", label: "strong ref", detail: "GC не тронет", accent: true }], edges: [] },
        { codeLine: 2, caption: 'Weak reference — <b>наоборот</b>: «<span class="hl">permits the garbage collector to collect the object</span> while still allowing the application to access» его.', nodes: [{ id: "s", kind: "gate", at: { zone: "strong", row: 0 }, state: "fail", label: "strong", detail: "мешает" }, { id: "w", kind: "gate", at: { zone: "weak", row: 0 }, state: "ok", label: "weak ref", detail: "разрешает сбор", accent: true }], edges: [] },
    ...
    },
    {
      ...
      explain: 'Short weak reference (verbatim): «The target of a short weak reference <span class="hl">becomes <code>null</code> when the object is reclaimed by garbage collection</span>. The weak reference is itself a managed object, and is subject to garbage collection just like any other managed object. <span class="hl">A short weak reference is the parameterless constructor for <code>WeakReference</code></span>». То есть <code>new WeakReference(obj)</code> (без второго булева-аргумента) — «short»: как только объект собран, <code>Target</code> обнуляется, и это твой сигнал «объект пропал». Это дефолтный и самый частый вид. Если у типа нет <code>Finalize</code>, «short weak reference functionality applies» независимо от того, как создавали. Long — для отслеживания сквозь финализацию (разбор 04).',
      sources: ["ms-weak"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Машинная панель — поведение weak-ссылок, снятое в рантайме. (1) <b>Weak разрешает сбор</b>: держим объект только через weak (проверка через <code>Target</code>-каст в helper-методе, чтобы не оставить strong-ссылку на стеке) → после полного GC <code>Target == null</code>: <code>beforeGC=True afterGC=False</code> (5/5), ровно как «permits the garbage collector to collect». (2) <b>Strong удерживает</b>: если из <code>Target</code> достать strong-ссылку и держать её — объект переживает GC: <code>strongRefKeepsAlive=True</code> (3/3), «obtain a strong reference… prevents it from being collected». (3) <b>Short vs long</b>: <code>new WeakReference(o).TrackResurrection</code> → <b>False</b> (short), <code>new WeakReference(o, true).TrackResurrection</code> → <b>True</b> (long). Числа реальны; главный факт — weak ≠ удержание, а разрешение сбора с сохранением доступа (анти-миф MM17/MM18).',
      sources: ["ms-weak"],
    },
  ],
  ...
  cards: [
    {
      ...
      okText: 'Weak reference «<span class="hl">permits the garbage collector to collect</span>»: без strong-ссылки объект собран, <code>Target</code> → null. Печать: <b>beforeGC=True afterGC=False</b>. (Weak НЕ предотвращает сбор — миф наоборот.)',
    ...
    },
    {
      ...
      okText: 'Достали strong-ссылку из <code>Target</code> и держим (<code>KeepAlive</code>) → «obtain a strong reference… <span class="hl">prevents it from being collected</span>». Объект жив. Печать: <b>strongRefKeepsAlive=True</b>.',
    ...
    },
    {
      ...
      okText: '<b>Short</b> (parameterless-ish ctor) → <code>TrackResurrection=False</code>; <b>long</b> (второй арг <code>true</code>) → <code>True</code> — «retained after… <code>Finalize</code>… called». Печать: <b>shortTracksResurrection=False longTracksResurrection=True</b>.',
  ...
  takeaways: [
    { icon: "why", k: "weak разрешает сбор", v: '«A weak reference <span class="hl">permits the garbage collector to collect the object</span> while still allowing… access» — в отличие от strong, weak НЕ мешает сбору (замер: afterGC=False). Для тяжёлых, но пересоздаваемых объектов.' },
    ...
    { icon: "avoid", k: "short vs long", v: '<b>short</b> (parameterless): <code>Target→null</code> при сборе (<code>TrackResurrection=False</code>). <b>long</b> (<code>true</code>): «retained after… <code>Finalize</code>», state «unpredictable» (<code>True</code>). Guidelines: long по необходимости, не мелким, <span class="hl">не вместо кэша</span>.' },
  ],
