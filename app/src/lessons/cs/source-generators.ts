...
export const sourceGenerators: LessonData = {
  ...
  spec: [
    { text: "«Source generators aim to enable compile time metaprogramming, that is, code that can be created at compile time and added to the compilation.» <span class=\"ru-tr\">«Source-генераторы призваны обеспечить метапрограммирование на этапе компиляции, то есть код, который может быть создан во время компиляции и добавлен в компиляцию.»</span>", source: "ms-roslyn" },
  ],
  edgeCases: [
    { text: "Генератор <b>интроспектит компиляцию</b>: «Source generators are able to read the contents of the compilation before running… introspect both user C# code and generator-specific files». <span class=\"ru-tr\">«Source-генераторы способны читать содержимое компиляции до запуска… интроспектировать как пользовательский код на C#, так и специфичные для генератора файлы».</span>", source: "ms-roslyn" },
    ...
    { text: "Reflection делает то же в рантайме: «You can use reflection to dynamically create an instance of a type… invoke the type's methods» <span class=\"ru-tr\">«С помощью reflection можно динамически создать экземпляр типа… вызвать методы этого типа»</span> — но платит на каждом вызове; генератор платит один раз при билде.", source: "ms-reflection" },
  ],

  misconceptions: [
    {
      wrong: "source generator — это как reflection, только вызывается вручную; разница косметическая",
      hook: 'Разница фундаментальна — <b>момент</b>. Reflection работает <span class="hl">в рантайме</span>: читает метаданные и вызывает на каждом запуске. Source generator работает <b>на компиляции</b>: «Source generators aim to enable <b>compile time metaprogramming</b>, that is, code that can be created at compile time and added to the compilation». <span class="ru-tr">«Source-генераторы призваны обеспечить <b>метапрограммирование на этапе компиляции</b>, то есть код, который может быть создан во время компиляции и добавлен в компиляцию».</span> Он «read the contents of the compilation before running… introspect both user C# code», <span class="ru-tr">«читает содержимое компиляции до запуска… интроспектирует пользовательский код на C#»,</span> а затем <b>эмитит обычный C#</b>, который компилятор превращает в такой же IL, как рукописный — <span class="hl">без единого reflection-вызова</span> в рантайме. Дальше <b>пять разборов</b>: когда работает каждый, конвейер генератора, additive-only, эмитированный код = обычный IL, и <b>машинная панель</b> — та же строка двумя путями: reflection делает N вызовов <code>GetValue</code>, генератор — 0 (реальный прогон).',
      source: "ms-roslyn",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Оба решают одну задачу — «код, который знает о других типах» — но в <b>разные моменты</b>. Reflection — рантайм-интроспекция: «You can use reflection to dynamically create an instance of a type… invoke the type\'s methods or access its fields and properties», <span class="ru-tr">«С помощью reflection можно динамически создать экземпляр типа… вызвать методы этого типа или обратиться к его полям и свойствам»,</span> и это происходит на каждом вызове. Source generator — <b>compile-time</b>: «Source generators aim to enable <span class="hl">compile time metaprogramming</span>, that is, code that can be created at compile time and added to the compilation». <span class="ru-tr">«Source-генераторы призваны обеспечить <b>метапрограммирование на этапе компиляции</b>, то есть код, который может быть создан во время компиляции и добавлен в компиляцию».</span> Он анализирует программу до её запуска и дописывает готовый исходник. В рантайме этот код — обычный, без reflection-накладных. Так работают <code>[LoggerMessage]</code>, <code>[GeneratedRegex]</code>, System.Text.Json — генерируют то, что раньше делал reflection.',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: 'Шаг 1–2: компилятор строит модель кода; генератор <span class="hl">читает</span> её — «read the contents of the compilation before running». <span class="ru-tr">«читает содержимое компиляции до запуска».</span>', nodes: [{ id: "read", kind: "gate", at: { zone: "gen", row: 0 }, state: "ok", label: "read compilation", detail: "интроспекция", accent: true }], edges: [] },
      ...
      explain: 'Конвейер генератора — читать и эмитить. Чтение: «Source generators are able to <b>read the contents of the compilation before running</b>, as well as access any <i>additional files</i>. This ability enables them to <span class="hl">introspect both user C# code and generator-specific files</span>». <span class="ru-tr">«Source-генераторы способны <b>читать содержимое компиляции до запуска</b>, а также обращаться к любым дополнительным файлам. Эта возможность позволяет им интроспектировать как пользовательский код на C#, так и специфичные для генератора файлы».</span> Эмиссия: сгенерированный исходник «added to the compilation» <span class="ru-tr">«добавляется в компиляцию»</span> — попадает в ту же сборку. Современный API — <code>IIncrementalGenerator</code> с методом <code>Initialize</code>: он строит инкрементальный конвейер, чтобы не пересчитывать всё при каждом нажатии клавиши в IDE. Итог: генератор — участник компиляции, а не отдельный шаг постобработки.',
  ...
  takeaways: [
    { icon: "why", k: "Момент, не задача", v: 'Reflection интроспектит <b>в рантайме</b>; source generator — «<span class="hl">compile time metaprogramming</span>… added to the compilation». <span class="ru-tr">«<b>метапрограммирование на этапе компиляции</b>… добавлено в компиляцию».</span> Та же цель, разный момент.' },
    { icon: "cost", k: "Additive + обычный IL", v: 'Генератор «read the contents of the compilation before running» <span class="ru-tr">«читает содержимое компиляции до запуска»</span> и <b>только добавляет</b> <code>.cs</code> (partial), не переписывая твой код. Эмитированное = обычный IL, <span class="hl">0 reflection-вызовов</span>.' },
