...
export const compilingExpressions: LessonData = {
  ...
  segments: [
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: '<code>add.Compile()</code> — «Create Delegate» <span class="ru-tr">«Создать делегат»</span>: <span class="hl">создаёт делегат</span> из дерева. Пока ничего не исполнено.', nodes: [{ id: "l", kind: "gate", at: { zone: "lam", row: 0 }, state: "ok", label: "add.Compile()", detail: "делегат func", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: '<code>func()</code> — «Invoke Delegate» <span class="ru-tr">«Вызвать делегат»</span>: <span class="hl">вызов</span> исполняет скомпилированный код. <code>() =&gt; 1 + 2</code> → результат.', nodes: [{ id: "l", kind: "gate", at: { zone: "lam", row: 0 }, state: "ok", label: "func()", detail: "исполнение", accent: true }], edges: [] },
