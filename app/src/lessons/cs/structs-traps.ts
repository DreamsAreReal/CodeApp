...
export const structsTraps: LessonData = {
  ...
  spec: [
    { text: "«a `readonly` member can call a non-`readonly` member. In that case, the compiler creates a copy of the structure instance and calls the non-`readonly` member on that copy. As a result, the original structure instance isn't modified.» <span class=\"ru-tr\">«`readonly`-член может вызвать не-`readonly` член. В этом случае компилятор создаёт копию экземпляра структуры и вызывает не-`readonly` член на этой копии. В результате исходный экземпляр структуры не изменяется.»</span>", source: "ms-struct" },
  ],
  edgeCases: [
    { text: "<code>default(T)</code> даёт <b>0-битный</b> паттерн и <span class=\"hl\">игнорирует</span> parameterless-конструктор; <code>new T()</code> — выполняет его. «the default value expression ignores a parameterless constructor» <span class=\"ru-tr\">«выражение значения по умолчанию игнорирует конструктор без параметров»</span>.", source: "ms-struct" },
  ...
  misconceptions: [
    {
      ...
      hook: 'Две ложные интуиции разом. Первая — «<span class="wrong">readonly не влияет на перф</span>»: на деле <code>readonly</code>-член, зовущий не-<code>readonly</code> член, заставляет компилятор сделать <b>защитную копию</b> — «the compiler creates a copy of the structure instance and calls the non-<code>readonly</code> member on that copy» <span class="ru-tr">«компилятор создаёт копию экземпляра структуры и вызывает не-<code>readonly</code> член на этой копии»</span>. Вторая — «<span class="wrong">readonly struct иммутабелен насквозь</span>»: нет, <b>shallow</b> — ссылочный член внутри мутирует. Ниже <b>пять разборов</b>: мутация через индексатор бьёт в копию, <b>машинная панель</b> защитной копии (реально снятое 11 vs 10), <code>default</code> против <code>new</code>, и readonly-поле ссылочного типа.',
      source: "ms-struct",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Структуры имеют value-семантику дословно: «Structure types have <b>value semantics</b>. That is, a variable of a structure type <span class="hl">contains an instance</span> of the type. By default, the system copies variable values on assignment, when passing an argument to a method, and when returning a method result» <span class="ru-tr">«Структурные типы обладают <b>семантикой значения</b>. То есть переменная структурного типа содержит экземпляр этого типа. По умолчанию система копирует значения переменных при присваивании, при передаче аргумента в метод и при возврате результата метода»</span>. Поэтому поле <code>N</code> живёт inline — на стеке для локальной переменной, внутри содержащего объекта для поля. И ровно из-за копирующей семантики офдок советует делать структуры иммутабельными: «Because structure types have value semantics, we recommend you define <b>immutable</b> structure types» <span class="ru-tr">«Поскольку структурные типы обладают семантикой значения, мы рекомендуем определять <b>неизменяемые</b> структурные типы»</span> — но сделать это правильно мешают ловушки ниже.',
      sources: ["ms-struct"],
    },
    {
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: '<span class="hl">Защитная копия</span>: компилятор копирует <code>V</code> и вызывает <code>Bump()</code> на копии — «creates a copy … and calls the non-<code>readonly</code> member on that copy» <span class="ru-tr">«создаёт копию … и вызывает не-<code>readonly</code> член на этой копии»</span>.', nodes: [{ id: "v", kind: "obj", at: { zone: "field", row: 0 }, typeTag: "V (readonly)", value: "N:10" }, { id: "cp", kind: "obj", at: { zone: "copy", row: 0 }, typeTag: "копия V", value: "N:11", accent: true }], edges: [] },
      ...
      explain: 'Это машинная панель урока — защитная копия, снятая числом. Поле <code>V</code> — <code>readonly</code>, а <code>Bump()</code> — не-<code>readonly</code> (меняет <code>N</code>). Правило дословно: «a <code>readonly</code> member can call a non-<code>readonly</code> member. In that case, the compiler <span class="hl">creates a copy</span> of the structure instance and calls the non-<code>readonly</code> member <b>on that copy</b>. As a result, the original structure instance isn\'t modified» <span class="ru-tr">«<code>readonly</code>-член может вызвать не-<code>readonly</code> член. В этом случае компилятор создаёт копию экземпляра структуры и вызывает не-<code>readonly</code> член <b>на этой копии</b>. В результате исходный экземпляр структуры не изменяется»</span>. Прогон это подтверждает: <code>Touch()</code> вернул <b>11</b>, но <code>V.N == 10</code>. Практический вывод: скрытые defensive copies стоят перф; пометка <code>readonly struct</code>/<code>readonly</code>-члены их <b>устраняет</b> — «The compiler can make use of the <code>readonly</code> modifier for performance optimizations» <span class="ru-tr">«Компилятор может использовать модификатор <code>readonly</code> для оптимизаций производительности»</span>. Это и опровергает миф «readonly — чисто про корректность».',
  ...
  cards: [
    ...
    },
    {
      ...
      noText: '«the compiler creates a copy of the structure instance and calls the non-readonly member on that copy… the original isn\'t modified» <span class="ru-tr">«компилятор создаёт копию экземпляра структуры и вызывает не-readonly член на этой копии… исходный экземпляр не изменяется»</span>. Реальный вывод: <b>11 10</b> — метод инкрементил копию, поле цело.',
    ...
    },
    {
      ...
      noText: '«the default value expression ignores a parameterless constructor» <span class="ru-tr">«выражение значения по умолчанию игнорирует конструктор без параметров»</span>. <code>default</code> ≠ <code>new T()</code>: реальный вывод <b>0 42</b>. Массивы структур (<code>new P[n]</code>) тоже обнуляются битово, минуя ctor.',
