...
export const dictionaryInternals: LessonData = {
  ...
  edgeCases: [
    { text: "«A hash function must always return the same hash code for the same key» — поэтому <b>мутация живого ключа</b>, меняющая хеш, ломает поиск (ключ уходит не в свой бакет).", source: "ms-hashtable-dict" },
    { text: "«A key cannot be <code>null</code>, but a value can be, if its type <code>TValue</code> is a reference type» — ключ <code>null</code> бросит <code>ArgumentNullException</code>.", source: "ms-dictionary" },
    { text: "«The order in which the items are returned is <b>undefined</b>» — <code>foreach</code> по <code>Dictionary</code> не хранит порядок вставки; полагаться нельзя.", source: "ms-dictionary" },
  ],

  misconceptions: [
    {
      wrong: "хеш-код уникален для каждого ключа, а Dictionary сохраняет порядок вставки",
      hook: 'Два соблазнительных, но ложных допущения. Первое: «хеш уникален» — нет, «It is possible for a hash function to generate the <span class="wrong">same hash code for two different keys</span>» (это коллизия). Второе: «Dictionary помнит порядок» — нет, «The order in which the items are returned is <span class="hl">undefined</span>». Dictionary — это хеш-таблица: ключ через <code>GetHashCode</code> выбирает <b>бакет</b>, коллизии делят бакет, а внутренний массив бакетов реаллоцируется по мере роста. Дальше <b>пять разборов</b> — от бакета по хешу до <b>машинной панели</b>: реально снятый рост внутреннего массива <code>_buckets</code> <b>0 → 3 → 7 → 17 → 37</b> (рефлексия, .NET 10).',
  ...
  segments: [
    {
      ...
      explain: 'Dictionary — это хеш-таблица: «A <code>Hashtable</code> object consists of <b>buckets</b> that contain the elements… Each bucket is associated with a hash code, which is generated using a hash function and is based on the key». Механика put/get симметрична: «When an object is added… it is stored in the bucket that is associated with the hash code that matches the object\'s hash code. When a value is being searched for… the hash code is generated for that value, and the bucket associated with that hash code is searched». Поэтому поиск по ключу — «very fast, close to O(1), because the <code>Dictionary&lt;TKey,TValue&gt;</code> class is implemented as a hash table», и «depends on the quality of the hashing algorithm».',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "picnic=630 basket=634", caption: '«"picnic" and "basket" would be in <span class="hl">different buckets</span>»: суммы ASCII 630 и 634 — разные хеши, разные бакеты.', nodes: [{ id: "p", kind: "chip", at: { zone: "hash2", row: 0 }, value: "picnic 630" }, { id: "bk", kind: "chip", at: { zone: "hash2", row: 1 }, value: "basket 634" }, { id: "bp", kind: "slot", at: { zone: "chain", row: 0 }, name: "бакет", value: "picnic", accent: true }, { id: "bb", kind: "slot", at: { zone: "chain", row: 1 }, name: "бакет", value: "basket" }], edges: [{ id: "e1", from: "p", to: "bp", accent: true }, { id: "e2", from: "bk", to: "bb" }] },
        { codeLine: 2, out: "stressed=877 desserts=877", caption: '«"stressed" and "desserts" would have the <span class="wrong">same hash code</span> and would be in the <b>same bucket</b>»: 877 == 877 — коллизия.', nodes: [{ id: "s", kind: "chip", at: { zone: "hash2", row: 0 }, value: "stressed 877", accent: true }, { id: "de", kind: "chip", at: { zone: "hash2", row: 1 }, value: "desserts 877", accent: true }, { id: "bc", kind: "slot", at: { zone: "chain", row: 0 }, name: "бакет 877", value: "stressed", accent: true }], edges: [{ id: "e1", from: "s", to: "bc", accent: true }] },
      ...
      explain: 'Хеш не уникален по построению: «It is possible for a hash function to generate the same hash code for two different keys… but a hash function that generates a unique hash code for each unique key results in better performance». Дока даёт наглядный пример на сумме ASCII-кодов: «The string "picnic" would have a hash code that is different from… "basket"; therefore… in different buckets. In contrast, "stressed" and "desserts" would have the <b>same hash code</b> and would be in the <b>same bucket</b>». Реальный прогон подтверждает: 630 ≠ 634, но 877 == 877. Коллизия не ошибка — бакет держит цепочку, и поиск в нём идёт через <code>Equals</code>; много коллизий в один бакет и есть тот самый worst-case O(n).',
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: '«The order in which the items are returned is <span class="wrong">undefined</span>». Полагаться на порядок нельзя; нужен порядок — <code>SortedDictionary</code> или <code>List</code> отдельно.', nodes: [{ id: "e", kind: "gate", at: { zone: "enum", row: 0 }, state: "fail", label: "order", detail: "undefined", accent: true }, { id: "kvp", kind: "chip", at: { zone: "enum", row: 1 }, value: "KeyValuePair" }], edges: [] },
      ],
      explain: 'Порядок перечисления <code>Dictionary</code> — не порядок вставки. «The <code>foreach</code> statement… returns an object of the type of the elements… the element type is a <code>KeyValuePair&lt;TKey,TValue&gt;</code>… The order in which the items are returned is <b>undefined</b>». Причина машинная: элементы разложены по бакетам согласно хешу, а итератор обходит внутреннюю структуру, не журнал вставок. Практический вывод: любой код, который «неявно» опирается на порядок ключей словаря, — баг, который может проявиться после resize или на другой версии рантайма. Нужен детерминированный порядок — берите <code>SortedDictionary&lt;TKey,TValue&gt;</code> (по ключу) или храните последовательность отдельно.',
  ...
  cards: [
    ...
    },
    {
      ...
      noText: '«it must not change in any way that affects its hash value». Меняем <code>Id</code> после вставки — новый хеш ведёт в другой (пустой) бакет. before=True, after=<b>False</b>: запись стала сиротой.',
    ...
    },
    {
      ...
      okText: 'Тот же ключ — <b>перезапись</b>, не добавление: <code>Count</code> = 1, значение = 2. «Every key… must be unique»; повторный индексатор обновляет.',
  ...
  takeaways: [
    { icon: "why", k: "Хеш → бакет", v: 'Dictionary — хеш-таблица: <code>GetHashCode</code> ключа выбирает <b>бакет</b>, <code>Equals</code> находит элемент внутри него. Поиск «close to O(1)» — идёт в один бакет, не по всей таблице.' },
