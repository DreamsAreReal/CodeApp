...
export const linqQuerySyntax: LessonData = {
  ...
  spec: [
    { text: "«At compile time, the compiler converts query expressions to standard query operator method calls according to the rules defined in the C# specification. You can express any query that uses query syntax by using method syntax.»", source: "ms-linq" },
  ],
  edgeCases: [
    { text: "Обе формы эквивалентны: «There's no <span class=\"hl\">semantic or performance difference</span> between the two different forms». Компилятор переписывает query в те же вызовы методов.", source: "ms-linq" },
    { text: "Некоторые операции не имеют query-клаузы: «Some query operations, such as <code>Count</code> or <code>Max</code>, have <b>no equivalent query expression clause</b> and must be expressed as a <span class=\"hl\">method call</span>».", source: "ms-linq" },
    { text: "Формы можно смешивать: «You can <b>combine method syntax with query syntax</b> in various ways» — обернуть query в скобки и позвать <code>.Count()</code>.", source: "ms-linq" },
  ],

  misconceptions: [
    {
      wrong: "query-синтаксис — это отдельный «язык запросов» внутри C#, отличный от вызовов методов LINQ",
      hook: 'Ходовое заблуждение: <span class="wrong">query-синтаксис (<code>from…where…select</code>) — это отдельный «мини-язык»</span>, работающий иначе, чем <code>.Where().Select()</code>. Нет: query-выражение — это <b>синтаксический сахар</b>, который компилятор переписывает в вызовы стандартных операторов. Дословно: «At compile time, the compiler <span class="hl">converts query expressions to standard query operator method calls</span>… There\'s no semantic or performance difference between the two different forms». Ниже <b>пять разборов</b>: анатомия query-выражения, эквивалентность двум формам, где смешивать, где метод обязателен, и <b>машинная панель</b> — реальный Release-IL, в котором <code>from x where select</code> превращается в <code>Enumerable.Where</code> + <code>Enumerable.Select</code>.',
      source: "ms-linq",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Query-выражение читается почти как SQL, но с обратным порядком клауз. Дословно: «You write query expressions in a declarative <i>query syntax</i>. By using query syntax, you perform <span class="hl">filtering, ordering, and grouping</span> operations on data sources with a minimum of code». Клаузы: <code>from</code> вводит источник и range-переменную, <code>where</code> фильтрует, <code>select</code> проецирует. Важно: «The variables in a query expression are all <b>strongly typed</b>» — тип элементов выводится компилятором, IntelliSense работает. И ключ к разделу: «A query isn\'t executed until you <b>iterate over the query variable</b>» — переменная <code>q</code> держит только описание запроса (детально — урок S3.2).',
      sources: ["ms-linq"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Query и method-синтаксис свободно комбинируются. Дословно: «You can <span class="hl">combine method syntax with query syntax</span> in various ways». Практический приём: пишешь фильтрацию/проекцию в читаемой query-форме, оборачиваешь в скобки и продолжаешь методом-агрегатом — <code>(from x in n where … select x).Count()</code>. Причина, почему это вообще нужно, — в следующем разборе: у ряда операций <b>нет</b> query-клаузы, поэтому чистой query-формой их не выразить.',
      sources: ["ms-linq"],
    },
    {
  ...
  takeaways: [
    { icon: "why", k: "две формы — один запрос", v: 'Query-синтаксис (<code>from…where…select</code>) — сахар: компилятор «converts query expressions to standard query operator method calls». <b>No semantic or performance difference</b> с <code>.Where().Select()</code>.' },
    ...
    { icon: "avoid", k: "агрегаты — только метод", v: '<code>Count/Max/Average/First</code> «have no equivalent query expression clause» — оборачивай query в скобки и зови метод: <code>(from…select).Count()</code>. Скаляр ≠ последовательность.' },
  ],
