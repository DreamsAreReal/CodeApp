...
export const exceptionFilters: LessonData = {
  ...
  spec: [
    { text: "«An exception filter is a Boolean expression that follows the when keyword» — он «further examines an exception and decides if the corresponding catch block handles that exception».", source: "ms-stmts" },
    { text: "«Exception filters (when): The filter expression is evaluated before the stack is unwound. This means the original call stack and all local variables remain intact during filter evaluation.»", source: "ms-stmts" },
  ],
  edgeCases: [
    { text: "Несколько <code>catch</code> одного типа различаются фильтрами: «You can provide several <code>catch</code> clauses for the same exception type <b>if they distinguish by exception filters</b>».", source: "ms-stmts" },
    { text: "С фильтром порядок гибче: «if an exception filter is present, a <code>catch (Exception e)</code> clause <b>doesn't need to be the last clause</b>».", source: "ms-stmts" },
    { text: "<code>false</code>-фильтр не портит стек: «The <code>when</code> filter doesn't unwind the stack, so <b>if a when filter is false, the original stack trace isn't changed</b>».", source: "ms-stmts" },
    { text: "Каждый <code>catch</code> в how-to называет тип: «Each <code>catch</code> block includes the exception type and can contain additional statements needed to handle that exception type».", source: "ms-howto" },
  ],

  misconceptions: [
    {
      wrong: "when — это просто сахар над if внутри catch; разницы нет, стек всё равно уже размотан",
      hook: 'Разница — <b>машинная</b>, во времени вычисления. «An <span class="hl">exception filter is a Boolean expression that follows the when keyword</span>», который «<span class="hl">further examines an exception and decides if the corresponding catch block handles that exception</span>». Ключ: «<span class="hl">The filter expression is evaluated before the stack is unwound</span>. This means the <span class="hl">original call stack and all local variables remain intact during filter evaluation</span>» — тогда как обычный <code>catch</code> «runs <span class="hl">after the stack is unwound</span>». И если фильтр <code>false</code>, «the <code>when</code> filter <span class="hl">doesn\'t unwind the stack</span>, so if a when filter is false, the original stack trace isn\'t changed» — исключение летит дальше нетронутым. Дальше <b>пять разборов</b>: <code>when</code> как булев решатель, несколько <code>catch</code> одного типа по фильтрам, <code>false</code>→поиск продолжается, оценка <b>до</b> раскрутки (стек цел), и <b>машинная панель</b> — порядок throw→filter→catch (реальный прогон).',
      source: "ms-stmts",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 1, out: "", caption: '<code>when (…)</code> — «a <span class="hl">Boolean expression that follows the when keyword</span>». Он «further examines an exception and decides if the corresponding <code>catch</code> block handles that exception».', nodes: [{ id: "e", kind: "obj", at: { zone: "ex", row: 0 }, typeTag: "Exception", value: "тип совпал" }, { id: "w", kind: "gate", at: { zone: "when", row: 0 }, state: "ok", label: "when (e is …)", detail: "true → ловим", accent: true }], edges: [{ id: "e1", from: "e", to: "w", accent: true }] },
        { codeLine: 2, out: "", caption: 'Один <code>catch</code>-блок через фильтр обрабатывает <b>два</b> типа: «uses an exception filter to provide a <span class="hl">single catch block to handle exceptions of two specified types</span>».', nodes: [{ id: "e", kind: "obj", at: { zone: "ex", row: 0 }, typeTag: "Exception", value: "Argument | DivByZero" }, { id: "w", kind: "gate", at: { zone: "when", row: 0 }, state: "ok", label: "when → true", detail: "единый catch", accent: true }], edges: [{ id: "e1", from: "e", to: "w" }] },
      ],
      explain: 'Фильтр — второй, необязательный критерий отбора после типа. «Along with an exception type, you can also specify an exception filter that <span class="hl">further examines an exception and decides if the corresponding catch block handles that exception</span>. An <span class="hl">exception filter is a Boolean expression that follows the when keyword</span>». В теле фильтра — любое булево выражение над пойманным <code>e</code>: проверка типа (<code>e is …</code>), содержимого <code>Message</code>, внешнего состояния. Практический эффект — один <code>catch</code> покрывает несколько условий без <code>if</code>-лестницы внутри: «The preceding example uses an exception filter to provide a <span class="hl">single catch block to handle exceptions of two specified types</span>». Обычный <code>catch</code> по типу решает «мой ли это тип», <code>when</code> добавляет «и подходит ли он по условию».',
      sources: ["ms-stmts"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: 'Ветка <b>без</b> фильтра — по правилу последняя из веток этого типа: «If such a clause exists, it must be the <span class="hl">last of the clauses that specify that exception type</span>».', nodes: [{ id: "a", kind: "gate", at: { zone: "split", row: 0 }, state: "ok", label: 'when "access denied"', detail: "1" }, { id: "b", kind: "gate", at: { zone: "split", row: 1 }, state: "ok", label: 'when "not found"', detail: "2" }, { id: "c", kind: "gate", at: { zone: "split", row: 2 }, state: "ok", label: "catch (IOException)", detail: "без when · последним", accent: true }], edges: [] },
      ],
      explain: 'Фильтры снимают запрет «один тип — один <code>catch</code>»: «<span class="hl">You can provide several catch clauses for the same exception type if they distinguish by exception filters</span>. One of those clauses might have no exception filter. If such a clause exists, it must be the <span class="hl">last of the clauses that specify that exception type</span>». Так один <code>IOException</code> обрабатывается по-разному в зависимости от условия (доступ запрещён / файл не найден / сеть), а «catch-all» этого типа замыкает список. Ещё следствие фильтров для порядка: «if an exception filter is present, a <code>catch (Exception e)</code> clause <span class="hl">doesn\'t need to be the last clause</span>» — обычно широкий <code>catch (Exception)</code> обязан быть последним, но при наличии фильтра это правило смягчается.',
      sources: ["ms-stmts"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 3, out: "", caption: 'Исключение <span class="hl">продолжает поиск</span> — как будто этой ветки не было. «The <code>when</code> filter doesn\'t unwind the stack, so if a when filter is false, the original stack trace isn\'t changed».', nodes: [{ id: "f", kind: "gate", at: { zone: "false", row: 0 }, state: "fail", label: "when false", detail: "не ловит" }, { id: "c", kind: "gate", at: { zone: "cont", row: 0 }, state: "ok", label: "внешний catch", detail: "перехватит", accent: true }], edges: [{ id: "e1", from: "f", to: "c", accent: true }] },
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>when</code> вычисляется <span class="hl">до</span> размотки: «The filter expression is evaluated <b>before the stack is unwound</b>». Стек броска ещё на месте.', nodes: [{ id: "i", kind: "gate", at: { zone: "intact", row: 0 }, state: "ok", label: "when-фильтр", detail: "стек ЦЕЛ", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Обычный <code>catch</code>+<code>if</code> — тело бежит <span class="hl">после</span> размотки: «The catch block runs <b>after the stack is unwound</b>, potentially losing valuable debugging information».', nodes: [{ id: "i", kind: "gate", at: { zone: "intact", row: 0 }, state: "ok", label: "when", detail: "стек ЦЕЛ" }, { id: "u", kind: "gate", at: { zone: "unw", row: 0 }, state: "fail", label: "catch + if", detail: "стек размотан", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: 'Потому фильтр — точка для логирования из места сбоя: «<span class="hl">the original call stack and all local variables remain intact during filter evaluation</span>». Debugger покажет исходную точку падения.', nodes: [{ id: "i", kind: "gate", at: { zone: "intact", row: 0 }, state: "ok", label: "when: log", detail: "локальные видны", accent: true }, { id: "u", kind: "gate", at: { zone: "unw", row: 0 }, state: "fail", label: "catch: log", detail: "инфо потеряна" }], edges: [] },
      ],
      explain: 'Это и есть машинная суть <code>when</code>: момент вычисления. «Exception filters (<code>when</code>): <span class="hl">The filter expression is evaluated before the stack is unwound</span>. This means the <span class="hl">original call stack and all local variables remain intact during filter evaluation</span>» — против «Traditional <code>catch</code> blocks: The catch block runs <span class="hl">after the stack is unwound</span>, potentially losing valuable debugging information». Отсюда преимущества: «<b>Better debugging experience</b>: Since the stack isn\'t unwound until a filter matches, debuggers can show the original point of failure with all local variables intact» и «<b>Performance benefits</b>: If no filter matches, the exception continues propagating without the <span class="hl">overhead of stack unwinding and restoration</span>». Логирование из <code>when</code> происходит там, где стек ещё описывает реальное место сбоя.',
      sources: ["ms-stmts"],
    },
    {
      ...
      explain: 'Машинная панель — временной порядок <code>throw → when-фильтр → тело catch</code>. Реальный прогон печатает <code>throw;filter;catch;</code>: фильтр <code>Filter()</code> выполнился <b>после</b> броска, но <b>до</b> тела обработчика — ровно потому, что «The <span class="hl">filter expression is evaluated before the stack is unwound</span>». Если бы фильтр вернул <code>false</code>, тело <code>catch</code> не выполнилось бы вовсе, а исключение продолжило поиск с целым стеком. Практический вывод раздела: <code>when</code> — не косметика над <code>if</code> внутри <code>catch</code>. Оценка до раскрутки даёт три вещи: точную диагностику из места сбоя, отсутствие оверхеда размотки при несовпадении и возможность «осмотреть-и-пропустить». «The exception filter approach is valuable in applications where <span class="hl">preserving debugging information is crucial for diagnosing code errors</span>».',
      sources: ["ms-stmts"],
    },
  ],
  ...
  cards: [
    ...
    },
    {
      ...
      okText: '«The filter expression is <span class="hl">evaluated before the stack is unwound</span>»: <code>throw</code> → <code>Filter()</code> → (true) тело <code>catch</code>. Фильтр строго <b>между</b> броском и обработкой. Печать: <b>throw;filter;catch;</b>.',
    ...
    },
    {
      ...
      okText: 'Внутренний фильтр <code>false</code> (сообщение «x» ≠ «nomatch») → этот <code>catch</code> не ловит; «if a <span class="hl">when filter is false</span>» исключение продолжает поиск и ловится внешним. Печать: <b>outer</b>.',
  ...
  takeaways: [
    { icon: "why", k: "when", v: '«An <span class="hl">exception filter is a Boolean expression that follows the when keyword</span>» — «further examines an exception and decides if the corresponding catch block handles that exception». Один <code>catch</code> может покрыть несколько условий.' },
    { icon: "cost", k: "До раскрутки", v: '«The filter expression is <span class="hl">evaluated before the stack is unwound</span>… the original call stack and all local variables remain intact» — точная диагностика из места сбоя (замер порядка: throw;filter;catch;).' },
    { icon: "avoid", k: "false → дальше", v: '<code>false</code>-фильтр не глотает исключение: «if a <span class="hl">when filter is false, the original stack trace isn\'t changed</span>» — оно продолжает поиск (замер: outer). Несколько <code>catch</code> одного типа различают по <code>when</code>.' },
  ],
