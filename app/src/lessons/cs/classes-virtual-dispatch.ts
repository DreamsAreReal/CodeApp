...
export const classesVirtualDispatch: LessonData = {
  ...
  spec: [
    { text: "«At run-time, when client code calls the method, the CLR looks up the run-time type of the object, and invokes that override of the virtual method.» <span class=\"ru-tr\">«Во время выполнения, когда клиентский код вызывает метод, CLR определяет run-time тип объекта и вызывает именно тот override виртуального метода.»</span>", source: "ms-poly" },
  ],
  edgeCases: [
  ...
  misconceptions: [
    {
      ...
      hook: 'На вид <code>override</code> и <code>new</code> делают одно — «свою версию метода». На машинном уровне это <span class="wrong">две разные механики</span>. <code>override</code> — <b>виртуальный</b> вызов: «the CLR looks up the <b>run-time type</b> of the object, and invokes that override». <span class="ru-tr">«CLR определяет <b>run-time type</b> объекта и вызывает именно этот override».</span> <code>new</code> — <b>сокрытие</b>: «the method that gets called depends on the <span class="hl">compile-time type</span> of the variable». <span class="ru-tr">«вызываемый метод зависит от compile-time type переменной».</span> Один и тот же объект через <code>Base</code>-ссылку: <code>b.V()</code> → <b>Derived.V</b>, а <code>b.N()</code> → <b>Base.N</b> (реальный прогон). Дальше <b>пять разборов</b>: наследование, vtable-панель и <code>base</code>.',
      source: "ms-poly",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Наследование даёт derived-классу все члены базы: «A class that derives from another class, called the <b>base class</b>, automatically contains all the public, protected, and internal members of the base class except its constructors and finalizers». <span class="ru-tr">«Класс, производный от другого класса, называемого <b>base class</b>, автоматически содержит все public-, protected- и internal-члены базового класса, кроме его конструкторов и финализаторов».</span> Переопределять можно только помеченное <code>virtual</code>/<code>abstract</code>, и обязательно через <code>override</code>: «A derived class can override a base class member <span class="hl">only if</span> the base class member is declared as <code>virtual</code> or <code>abstract</code>. The derived member must use the <code>override</code> keyword». <span class="ru-tr">«Производный класс может переопределить член базового класса, только если этот член базового класса объявлен как <code>virtual</code> или <code>abstract</code>. Производный член обязан использовать ключевое слово <code>override</code>».</span> Так строится полиморфизм: «every type is polymorphic because all types… inherit from <code>Object</code>». <span class="ru-tr">«каждый тип полиморфен, потому что все типы… наследуются от <code>Object</code>».</span>',
    ...
    },
    {
      ...
      explain: 'Виртуальный вызов диспетчеризуется по объекту, не по переменной: «At run-time, when client code calls the method, the CLR looks up the <span class="hl">run-time type</span> of the object, and invokes that override of the virtual method». <span class="ru-tr">«Во время выполнения, когда клиентский код вызывает метод, CLR определяет run-time type объекта и вызывает именно тот override виртуального метода».</span> И это работает даже через базовую ссылку: «When a derived class overrides a virtual member, that member is called <b>even when</b> an instance of that class is being accessed as an instance of the base class». <span class="ru-tr">«Когда производный класс переопределяет виртуальный член, этот член вызывается <b>даже тогда</b>, когда к экземпляру этого класса обращаются как к экземпляру базового класса».</span> Поэтому <code>Base b = new Derived(); b.V()</code> печатает <code>Derived.V</code>. Это и есть полиморфизм: один вызов через базовый тип бьёт в нужный override.',
      sources: ["ms-poly"],
    },
    {
      ...
      explain: 'Это машинная панель урока — как именно вызов находит override. У каждого объекта есть <b>метод-таблица</b> (MethodTable), и в ней — <b>слоты</b> виртуальных методов; дословно из Book of the Runtime рантайма: «Each MethodDesc has a <span class="hl">slot</span>, which contains the current entry point of the method» <span class="ru-tr">«У каждого MethodDesc есть slot, который содержит текущую точку входа метода».</span> и «The slot is stored in <b>MethodTable</b> for methods that require efficient lookup via slot index, e.g. <span class="hl">virtual methods</span>». <span class="ru-tr">«Слот хранится в <b>MethodTable</b> для методов, которым нужен быстрый поиск по индексу слота, например virtual methods».</span> Для виртуального члена компилятор эмитит не прямой вызов, а обращение к слоту таблицы; <code>override</code> в Derived <b>перезаписывает</b> этот слот адресом своей реализации, поэтому <code>b.V()</code> через слот попадает в <code>Derived.V</code> (собственный прогон: <b>Derived.V</b>). Ключ: слот выбирается по типу переменной (Base знает, где слот V), а адрес в слоте — от run-time типа объекта (ECMA-335 §I.8.10: «types can provide their own implementation of a virtual method… this is known as overriding» <span class="ru-tr">«типы могут предоставлять собственную реализацию виртуального метода… это называется overriding (переопределением)»</span>). Отсюда цена и сила виртуальности: один indirect-переход, но истинный полиморфизм.',
    ...
    },
    {
      ...
      explain: '<code>new</code> — это <b>сокрытие</b>, не переопределение: «you\'re creating a method that <span class="hl">hides</span> the base class method rather than overriding it. This is different from virtual methods. With method hiding, the method that gets called depends on the <b>compile-time type</b> of the variable, not the run-time type of the object». <span class="ru-tr">«вы создаёте метод, который скрывает метод базового класса, а не переопределяет его. Это не то же самое, что виртуальные методы. При сокрытии метода вызываемый метод зависит от <b>compile-time type</b> переменной, а не от run-time типа объекта».</span> Поэтому один и тот же <code>Derived</code>-объект даёт <code>Base.N</code> через <code>Base</code>-ссылку и <code>Derived.N</code> через <code>Derived</code>-ссылку (реальный прогон): «the method that gets called depends on the variable\'s <b>declared type</b>». <span class="ru-tr">«вызываемый метод зависит от <b>declared type</b> переменной».</span> Это тихий баг: обновил метод в derived через <code>new</code> — а старый код с базовой ссылкой зовёт старую версию.',
      sources: ["ms-poly"],
    },
    {
