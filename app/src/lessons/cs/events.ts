  ...
  segments: [
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 3, out: "handler A\nhandler B", caption: 'Оба обработчика выполнились <b>по порядку подписки</b> → <span class="hl">handler A</span>, затем <span class="hl">handler B</span> (реальный прогон).', nodes: [{ id: "click", kind: "obj", at: { zone: "raise", row: 0 }, typeTag: "raise", value: "→ done" }, { id: "ha", kind: "gate", at: { zone: "handlers", row: 0 }, state: "ok", label: "A", detail: "✓" }, { id: "hb", kind: "gate", at: { zone: "handlers", row: 1 }, state: "ok", label: "B", detail: "✓", accent: true }], edges: [] },
      ],
