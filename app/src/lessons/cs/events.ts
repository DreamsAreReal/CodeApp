...
export const events: LessonData = {
  ...
  spec: [
    { text: "«The <code>event</code> keyword declares an event.» <span class=\"ru-tr\">«Ключевое слово <code>event</code> объявляет событие.»</span>", source: "ms-event-kw" },
  ],
  edgeCases: [
    { text: "Событие — это multicast-делегат с оградой: «Events are multicast delegates that you can only invoke from within the class (or derived classes) or struct where you declare them (the publisher class)» <span class=\"ru-tr\">«События — это multicast-делегаты, которые можно вызывать только изнутри класса (или производных классов) либо структуры, где они объявлены (класс-издатель)»</span> — реальный прогон подтверждает <code>CS0070</code> при вызове снаружи.", source: "ms-event-kw" },
    { text: "Издатель не знает получателя: «The event sender doesn\'t know the object or method that receives (handles) the events it raises». <span class=\"ru-tr\">«Отправитель события не знает объект или метод, который получает (обрабатывает) поднимаемые им события».</span>", source: "ms-events" },
    { text: "Событие можно объявить статическим: «Use the <code>static</code> keyword to declare an event as a static event. Static events are available to callers at any time, even if no instance of the class exists». <span class=\"ru-tr\">«Используйте ключевое слово <code>static</code>, чтобы объявить событие как статическое. Статические события доступны вызывающим в любой момент, даже если ни одного экземпляра класса не существует».</span>", source: "ms-event-kw" },
  ],

  misconceptions: [
    {
      wrong: "событие — это отдельная сущность, а публичный делегат-поле сделал бы то же самое",
      hook: 'Событие <b>построено на делегате</b>, но добавляет ключевое — <span class="hl">инкапсуляцию</span>. «An <b>event</b> is a member that enables an object to trigger notifications… The <code>event</code> keyword declares an <b>event</b>. The event is of a <b>delegate type</b>. While an object triggers an event, the event invokes all supplied event handlers». <span class="ru-tr">«<b>Событие</b> — это член, позволяющий объекту запускать уведомления. Пользователи события могут прикреплять исполняемый код к событиям, предоставляя <b>обработчики событий</b>. Ключевое слово <code>event</code> объявляет событие. Событие имеет <b>делегатный тип</b>. Когда объект запускает событие, событие вызывает все предоставленные обработчики событий».</span> Публичный делегат-поле кто угодно мог бы обнулить или вызвать; событие — нет: «Events are multicast delegates that you can <span class="hl">only invoke from within the class</span> (or derived classes) or struct where you declare them (the <b>publisher class</b>)». <span class="ru-tr">«События — это multicast-делегаты, которые можно вызывать только изнутри класса (или производных классов) либо структуры, где они объявлены (<b>класс-издатель</b>)».</span> Это паттерн «наблюдатель»: «The delegate model follows the <b>observer design pattern</b>, which enables a <b>subscriber</b> to register with and receive notifications from a <b>provider</b>». <span class="ru-tr">«Модель делегатов следует <b>паттерну проектирования наблюдатель</b>, который позволяет <b>подписчику</b> зарегистрироваться у <b>поставщика</b> и получать от него уведомления».</span> Дальше <b>шесть разборов</b>: event поверх делегата, publisher/subscriber, raise → все обработчики, стандартная сигнатура <code>EventHandler</code>, <code>EventHandler&lt;T&gt;</code> с данными, и <b>машинная панель</b> — ограда компилятора (реальный прогон: <code>CS0070</code> снаружи).',
  ...
  segments: [
    {
      ...
      explain: 'Событие — это <b>член</b>, дающий объекту слать уведомления, поверх делегатного типа: «An <b>event</b> is a member that enables an object to trigger notifications. Event users can attach executable code for events by supplying <b>event handlers</b>. The <code>event</code> keyword declares an event. The <span class="hl">event is of a delegate type</span>. While an object triggers an event, the event invokes all supplied event handlers. <b>Event handlers are delegate instances</b> added to the event and executed when the event is raised». <span class="ru-tr">«<b>Событие</b> — это член, позволяющий объекту запускать уведомления. Пользователи события могут прикреплять к событиям исполняемый код, предоставляя <b>обработчики событий</b>. Ключевое слово <code>event</code> объявляет событие. Событие имеет делегатный тип. Когда объект запускает событие, событие вызывает все предоставленные обработчики событий. <b>Обработчики событий — это экземпляры делегата</b>, добавленные к событию и выполняемые при его поднятии».</span> То есть <code>Clicked</code> — не магия, а фасад над multicast-делегатом: «Delegates are <b>multicast</b> class objects, which means they can hold references to more than one event-handling method». <span class="ru-tr">«Делегаты — это <b>multicast</b>-объекты класса, а значит они могут хранить ссылки более чем на один метод-обработчик события».</span> Поднять событие = вызвать этот делегат (обычно через <code>?.Invoke</code> — защита от отсутствия подписчиков, разбор 03).',
    ...
    },
    {
      ...
      explain: 'События реализуют паттерн «наблюдатель» через делегатную модель: «Events in .NET are based on the <b>delegate model</b>. The delegate model follows the <b>observer design pattern</b>, which enables a <span class="hl">subscriber to register with and receive notifications from a provider</span>. An event sender pushes a notification when an event occurs. An event receiver defines the response». <span class="ru-tr">«События в .NET основаны на <b>модели делегатов</b>. Модель делегатов следует <b>паттерну проектирования наблюдатель</b>, который позволяет подписчику зарегистрироваться у поставщика и получать от него уведомления. Отправитель события шлёт уведомление, когда событие происходит. Получатель события определяет реакцию».</span> Ключевое свойство развязки: «The <b>event sender doesn\'t know the object or method that receives</b> (handles) the events it raises». <span class="ru-tr">«<b>Отправитель события не знает объект или метод, который получает</b> (обрабатывает) поднимаемые им события».</span> Подписка — оператором <code>+=</code>, и делегат под событием ведёт список: «A delegate acts as an event dispatcher for the class that raises the event by <b>maintaining a list of registered event handlers</b> for the event». <span class="ru-tr">«Делегат выступает диспетчером событий для класса, поднимающего событие, <b>поддерживая список зарегистрированных обработчиков событий</b> этого события».</span> Так издатель и подписчики не связаны напрямую — это и есть развязка через событие.',
      sources: ["ms-events"],
    },
    {
      ...
      explain: 'Поднять событие — вызвать делегат, и «While an object triggers an event, the event <b>invokes all supplied event handlers</b>». <span class="ru-tr">«Когда объект запускает событие, событие <b>вызывает все предоставленные обработчики событий</b>».</span> Издатель обычно оборачивает вызов в защищённый метод: «To raise an event, invoke the delegate associated with the event. The event sender typically wraps the invocation in a <code>protected virtual</code> method… named <code>On&lt;EventName&gt;</code>». <span class="ru-tr">«Чтобы поднять событие, вызовите делегат, связанный с событием. Отправитель события обычно оборачивает вызов в метод <code>protected virtual</code>… с именем <code>On&lt;EventName&gt;</code>».</span> Реальный прогон: два обработчика → <code>handler A</code>, <code>handler B</code> (порядок подписки; это multicast invocation list из соседнего урока). Важно про <code>?.Invoke</code>: если подписчиков нет, событие равно <code>null</code>, и вызов без <code>?.</code> кинул бы <code>NullReferenceException</code> — реальный прогон подтверждает, что при нуле подписчиков событие <code>is null</code>.',
      sources: ["ms-event-kw", "ms-events"],
    },
    {
      ...
      explain: 'BCL стандартизует форму обработчика, чтобы не плодить свои делегаты: «.NET provides the <code>EventHandler</code> and <code>EventHandler&lt;TEventArgs&gt;</code> delegates to support most event scenarios. <b>Use the <code>EventHandler</code> delegate for all events that don\'t include event data</b>. Use the <code>EventHandler&lt;TEventArgs&gt;</code> delegate for events that include data about the event. These delegates have <span class="hl">no return type value and take two parameters</span> (an object for the source of the event and an object for event data)». <span class="ru-tr">«.NET предоставляет делегаты <code>EventHandler</code> и <code>EventHandler&lt;TEventArgs&gt;</code> для поддержки большинства сценариев событий. <b>Используйте делегат <code>EventHandler</code> для всех событий, которые не несут данных события</b>. Используйте делегат <code>EventHandler&lt;TEventArgs&gt;</code> для событий, которые несут данные о событии. У этих делегатов нет возвращаемого значения, и они принимают два параметра (объект — источник события и объект — данные события)».</span> Первый параметр — <code>sender</code> (кто поднял), второй — данные: «The <code>EventArgs</code> class is typically the base type for event data classes. You also use this class if an event doesn\'t have any data associated with it… You can pass the <code>EventArgs.Empty</code> value when no data is provided». <span class="ru-tr">«Класс <code>EventArgs</code> обычно является базовым типом для классов данных события. Этот класс также используют, если с событием не связано никаких данных… Можно передать значение <code>EventArgs.Empty</code>, когда данные не предоставлены».</span> Возврата нет — обработчик реагирует, а не отвечает.',
      sources: ["ms-events"],
    },
    {
      ...
      explain: 'Когда событию есть что сообщить, берут обобщённый вариант: «Use the <code>EventHandler&lt;TEventArgs&gt;</code> delegate for events that include <b>data about the event</b>». <span class="ru-tr">«Используйте делегат <code>EventHandler&lt;TEventArgs&gt;</code> для событий, которые несут <b>данные о событии</b>».</span> Данные оформляют классом-наследником <code>EventArgs</code>: «Data associated with an event can be provided through an event data class… <b>.NET follows a naming pattern where all event data classes end with the <code>EventArgs</code> suffix</b>». <span class="ru-tr">«Данные, связанные с событием, можно передать через класс данных события… <b>.NET следует соглашению об именовании, по которому все классы данных события заканчиваются суффиксом <code>EventArgs</code></b>».</span> Обработчик получает эти данные вторым параметром (<code>e.Value</code>) — реальный прогон: <code>s.Set(42)</code> → <code>temp = 42</code>. Так подписчик узнаёт не только «что-то случилось», но и <b>детали</b>: какое значение, какой источник (<code>sender</code>), — не завися при этом от кода издателя.',
      sources: ["ms-events"],
    },
    {
  ...
  cards: [
    {
      ...
      okText: 'Поднять событие = вызвать multicast-делегат под ним → <span class="hl">все</span> обработчики по порядку подписки: <code>handler A</code>, затем <code>handler B</code>. «the event invokes all supplied event handlers». <span class="ru-tr">«событие вызывает все предоставленные обработчики событий».</span>',
  ...
  takeaways: [
    { icon: "why", k: "event = делегат + ограда", v: 'Событие — член поверх multicast-делегата: «The <code>event</code> keyword declares an event. The event is of a <span class="hl">delegate type</span>». <span class="ru-tr">«Ключевое слово <code>event</code> объявляет событие. Событие имеет делегатный тип».</span> Поднять = вызвать делегат → «invokes all supplied event handlers» <span class="ru-tr">«вызывает все предоставленные обработчики событий»</span> (реальный прогон: A, B).' },
    { icon: "cost", k: "Observer / publish-subscribe", v: 'Паттерн «наблюдатель»: подписчик сам регистрируется (<code>+=</code>), издатель <b>не знает</b> получателя. «The event sender doesn\'t know the object or method that receives… the events it raises». <span class="ru-tr">«Отправитель события не знает объект или метод, который получает… поднимаемые им события».</span> Форма обработчика — <code>EventHandler</code>: <code>(object sender, EventArgs e)</code>, без возврата.' },
    { icon: "avoid", k: "Инкапсуляция (CS0070)", v: 'Снаружи класса событие можно <span class="hl">только += / -=</span>, не <code>=</code> и не raise: «only invoke from within the class… (the publisher class)» <span class="ru-tr">«вызывать только изнутри класса… (класс-издатель)»</span> — реальный прогон даёт <code>CS0070</code>. Это и отличает событие от публичного делегат-поля. Raise — через <code>?.Invoke</code> (0 подписчиков = null).' },
  ],
