...
export const yieldContract: LessonData = {
  ...
  spec: [
    { text: "«Use the yield statement in an iterator to provide the next value or signal the end of an iteration.» <span class=\"ru-tr\">«Используйте оператор yield в итераторе, чтобы предоставить следующее значение или сигнализировать о конце итерации.»</span>", source: "ms-yield" },
  ],
  edgeCases: [
    { text: 'Тип возврата итератора — один из четырёх: «The return type of an iterator method or <code>get</code> accessor can be <code>IEnumerable</code>, <code>IEnumerable&lt;T&gt;</code>, <code>IEnumerator</code>, or <code>IEnumerator&lt;T&gt;</code>» <span class="ru-tr">«Типом возврата метода-итератора или <code>get</code>-аксессора может быть <code>IEnumerable</code>, <code>IEnumerable&lt;T&gt;</code>, <code>IEnumerator</code> или <code>IEnumerator&lt;T&gt;</code>»</span>; для async — <code>IAsyncEnumerable&lt;T&gt;</code>.', source: "ms-iter-pg" },
    { text: '<code>yield</code> — не зарезервированное слово: «In C#, <code>yield</code> is not a reserved word and has special meaning only when it is used before a <code>return</code> or <code>break</code> keyword». <span class="ru-tr">«В C# <code>yield</code> не является зарезервированным словом и имеет особое значение только когда стоит перед ключевым словом <code>return</code> или <code>break</code>».</span> Итератор — метод или <code>get</code>-аксессор, но не event/конструктор/финалайзер.', source: "ms-iter-pg" },
  ...
  misconceptions: [
    {
      ...
      hook: 'Два мифа о правилах <code>yield</code>. «<span class="wrong">yield в любом try/catch</span>» — нет: <code>yield return</code>/<code>yield break</code> запрещены в <code>catch</code>/<code>finally</code> и в <code>try</code> <b>с</b> <code>catch</code>; разрешены только в <code>try</code> без <code>catch</code> (с <code>finally</code>) — вот почему <code>using</code> в итераторе работает (он компилируется в <code>try/finally</code>). «<span class="wrong">свободно мешать return и yield return</span>» — нет: «you can\'t have both a <code>return</code> statement and a <code>yield return</code> statement in the same method» <span class="ru-tr">«нельзя иметь одновременно оператор <code>return</code> и оператор <code>yield return</code> в одном и том же методе»</span> (страница Iterators, C#) — компилятор бросает <b>CS1622</b>. Ниже <b>пять разборов</b>: две формы <code>yield</code>, типы возврата, <code>return</code> vs <code>yield return</code> (и обход), где <code>yield</code> запрещён, и <b>машинная панель</b> — реальные диагностики Roslyn (CS1622 / CS1623), которые прибивают нарушения контракта.',
      source: "ms-iterators",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "2 3 4 5", caption: 'Вывод: <b>2 3 4 5</b> (реальный прогон). «Iteration also finishes when control reaches the end of an iterator» <span class="ru-tr">«Итерация также завершается, когда управление достигает конца итератора»</span> — но <code>yield break</code> обрывает <span class="hl">раньше</span>.', nodes: [{ id: "res", kind: "gate", at: { zone: "ret", row: 0 }, state: "ok", label: "результат", detail: "2 3 4 5", accent: true }, { id: "b", kind: "gate", at: { zone: "brk", row: 0 }, state: "fail", label: "хвост отброшен", detail: "3, 4 — нет" }], edges: [] },
      ],
      explain: 'Оператор <code>yield</code> имеет две формы: «Use the <code>yield</code> statement in an iterator to provide the next value or signal the end of an iteration». <span class="ru-tr">«Используйте оператор <code>yield</code> в итераторе, чтобы предоставить следующее значение или сигнализировать о конце итерации».</span> <code>yield return</code> — «to provide the next value in iteration» <span class="ru-tr">«чтобы предоставить следующее значение в итерации»</span>; <code>yield break</code> — «to explicitly signal the end of iteration» <span class="ru-tr">«чтобы явно сигнализировать о конце итерации»</span>. Плюс: «Iteration also finishes when control reaches the end of an iterator» <span class="ru-tr">«Итерация также завершается, когда управление достигает конца итератора»</span> — то есть дойти до конца тела тоже завершает обход. В примере из доки <code>TakeWhilePositive({2,3,4,5,-1,3,4})</code> отдаёт <b>2 3 4 5</b> и на <code>-1</code> делает <code>yield break</code> — хвост <code>3, 4</code> не появляется. <code>yield break</code> — способ закончить последовательность посреди тела, не доводя до его конца.',
      sources: ["ms-yield"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 3, caption: '<span class="hl"><code>IAsyncEnumerable&lt;T&gt;</code></span> делает итератор <b>async</b>: «That makes an iterator async» <span class="ru-tr">«Это делает итератор асинхронным»</span> (внутренности — S18.4).', nodes: [{ id: "sync", kind: "chip", at: { zone: "types", row: 0, col: 0 }, value: "4 синхронных типа" }, { id: "async", kind: "gate", at: { zone: "types", row: 1 }, state: "ok", label: "IAsyncEnumerable<T>", detail: "async-итератор", accent: true }], edges: [] },
      ],
      explain: 'Тип возврата итератора строго ограничен: «The return type of an iterator method or <code>get</code> accessor can be <code>IEnumerable</code>, <code>IEnumerable&lt;T&gt;</code>, <code>IEnumerator</code>, or <code>IEnumerator&lt;T&gt;</code>». <span class="ru-tr">«Типом возврата метода-итератора или <code>get</code>-аксессора может быть <code>IEnumerable</code>, <code>IEnumerable&lt;T&gt;</code>, <code>IEnumerator</code> или <code>IEnumerator&lt;T&gt;</code>».</span> Плюс async-вариант: «You can also use <code>IAsyncEnumerable&lt;T&gt;</code> as the return type of an iterator. That makes an iterator async». <span class="ru-tr">«Можно также использовать <code>IAsyncEnumerable&lt;T&gt;</code> в качестве типа возврата итератора. Это делает итератор асинхронным».</span> То есть один и тот же <code>yield return</code> строит и «коллекцию для <code>foreach</code>» (<code>IEnumerable&lt;T&gt;</code>), и «энумератор» (<code>IEnumerator&lt;T&gt;</code>, удобно при написании своего <code>GetEnumerator</code>), и async-поток (<code>IAsyncEnumerable&lt;T&gt;</code>). Компилятор по типу возврата решает, какую стейт-машину сгенерировать (S18.3/S18.4).',
    ...
    },
    {
      ...
      explain: 'Жёсткое ограничение: «There\'s one important restriction on iterator methods: you can\'t have both a <code>return</code> statement and a <code>yield return</code> statement in the same method». <span class="ru-tr">«Есть одно важное ограничение для методов-итераторов: нельзя иметь одновременно оператор <code>return</code> и оператор <code>yield return</code> в одном и том же методе».</span> Нарушение — реальная ошибка компилятора <b>CS1622</b> (машинная панель, разбор 05). Штатный обход — разбить на два метода: внешний использует обычный <code>return</code> (может отдать пустую коллекцию <b>или</b> итератор), а внутренний — только <code>yield return</code>. Реальный прогон: <code>Get(true)</code> → пусто, <code>Get(false)</code> → <code>[1, 2]</code>. Это частый паттерн «eager-проверка аргументов + ленивое тело»: аргументы валидируются в <code>return</code>-методе <b>сразу</b>, а не отложенно на первом <code>MoveNext</code>.',
    ...
    },
    {
      ...
      explain: 'Контракт запретов дословно: «You can\'t use the <code>yield</code> statements in: methods with <code>in</code>, <code>ref</code>, or <code>out</code> parameters. lambda expressions and anonymous methods. unsafe blocks. Before C# 13, <code>yield</code> was invalid in any method with an <code>unsafe</code> block. Beginning with C# 13, you can use <code>yield</code> in methods with <code>unsafe</code> blocks, but not in the <code>unsafe</code> block». <span class="ru-tr">«Нельзя использовать операторы <code>yield</code> в: методах с параметрами <code>in</code>, <code>ref</code> или <code>out</code>. лямбда-выражениях и анонимных методах. блоках <code>unsafe</code>. До C# 13 <code>yield</code> был недопустим в любом методе с блоком <code>unsafe</code>. Начиная с C# 13, можно использовать <code>yield</code> в методах с блоками <code>unsafe</code>, но не внутри самого блока <code>unsafe</code>».</span> И про исключения: «<code>yield return</code> and <code>yield break</code> can\'t be used in <code>catch</code> and <code>finally</code> blocks, or in <code>try</code> blocks with a corresponding <code>catch</code> block. The <code>yield return</code> and <code>yield break</code> statements can be used in a <code>try</code> block with no <code>catch</code> blocks, only a <code>finally</code> block». <span class="ru-tr">«<code>yield return</code> и <code>yield break</code> нельзя использовать в блоках <code>catch</code> и <code>finally</code>, а также в блоках <code>try</code>, у которых есть соответствующий блок <code>catch</code>. Операторы <code>yield return</code> и <code>yield break</code> можно использовать в блоке <code>try</code> без блоков <code>catch</code>, только с блоком <code>finally</code>».</span> Отсюда важное следствие: «Since <code>using</code> statements compile into <code>try</code> blocks with <code>finally</code> clauses (and no <code>catch</code> blocks), they work correctly with iterators» <span class="ru-tr">«Поскольку операторы <code>using</code> компилируются в блоки <code>try</code> с секциями <code>finally</code> (и без блоков <code>catch</code>), они корректно работают с итераторами»</span> — ресурс корректно освобождается даже при раннем <code>break</code>.',
      sources: ["ms-yield"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: '«Cannot return a value from an iterator. Use the <code>yield return</code> statement…» <span class="ru-tr">«Нельзя вернуть значение из итератора. Используйте оператор <code>yield return</code>…»</span> — это <span class="hl">текст самой диагностики</span> Roslyn.', nodes: [{ id: "c1", kind: "gate", at: { zone: "cs1622", row: 0 }, state: "fail", label: "CS1622", detail: "cannot return a value" }, { id: "c1t", kind: "chip", at: { zone: "cs1622", row: 1 }, value: "use yield return", accent: true }], edges: [] },
        { codeLine: 3, caption: '<code>ref</code>-параметр у итератора — <span class="hl">отдельная</span> диагностика <b>CS1623</b>: «Iterators cannot have ref, in or out parameters». <span class="ru-tr">«Итераторы не могут иметь параметры ref, in или out».</span>', nodes: [{ id: "c1", kind: "gate", at: { zone: "cs1622", row: 0 }, state: "fail", label: "CS1622", detail: "return+yield" }, { id: "c2", kind: "gate", at: { zone: "cs1623", row: 0 }, state: "fail", label: "CS1623", detail: "ref/in/out", accent: true }], edges: [] },
      ],
  ...
  takeaways: [
    { icon: "why", k: "две формы", v: '<code>yield return</code> — отдать следующее значение; <code>yield break</code> — явно завершить (итерация также кончается на конце тела). «Use the <code>yield</code> statement… to provide the next value or signal the end». <span class="ru-tr">«Используйте оператор <code>yield</code>… чтобы предоставить следующее значение или сигнализировать о конце».</span>' },
