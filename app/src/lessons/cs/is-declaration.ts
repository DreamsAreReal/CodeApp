...
export const isDeclaration: LessonData = {
  ...
  spec: [
    { text: "«The is operator checks if the result of an expression is compatible with a given type.»", source: "ms-is" },
    { text: "«When a declaration pattern matches an expression, it assigns the variable to the converted expression result, as the following example shows:»", source: "ms-patterns" },
  ],
  edgeCases: [
    ...
    { text: "<code>is</code> с <code>null</code>: «the compiler guarantees that no user-overloaded <code>==</code> or <code>!=</code> operator is invoked».", source: "ms-is" },
  ],
  ...
  segments: [
    ...
    },
    {
      ...
      explain: 'Ключевое свойство: «A <b>declaration pattern</b> with type <code>T</code> matches an expression when an expression result is <span class="hl">non-null</span>» — то есть <code>null</code> отсекается всегда. Обзор это подчёркивает: «The declaration pattern <b>doesn\'t match a null value</b>, regardless of the compile-time type of the variable». Прогон подтверждает: <code>o is string s</code> при <code>o == null</code> → <code>False</code>. Практический вывод: <code>if (x is T v)</code> одновременно проверяет тип И не-null; попал в тело — <code>v</code> гарантированно не null.',
    ...
    },
    {
      ...
      explain: 'Это машинная панель урока — один declaration pattern распаковывает две формы хранения, снято прогоном. Правила матча <code>is T</code> покрывают обе: nullable — «The run-time type of an expression result is a <b>nullable value type</b> with the underlying type <code>T</code> and the <code>Nullable&lt;T&gt;.HasValue</code> is <code>true</code>»; boxed — «A <b>boxing or unboxing conversion</b> exists from the run-time type of an expression result to type <code>T</code>». Поэтому <code>iBoxed is int a</code> (unbox) и <code>jNullable is int b</code> (nullable→underlying) оба истинны, а <code>a + b = 76</code>. Один паттерн, две дороги к <code>int</code> — обе встроенные, без user-defined конверсий.',
      sources: ["ms-patterns", "ms-is"],
    },
  ],
  ...
  cards: [
    ...
    },
    {
      ...
      noText: '«The <code>is</code> operator checks if the result of an expression is compatible with a given type» и возвращает <code>bool</code>. Здесь совместимо ⇒ <b>True</b>.',
