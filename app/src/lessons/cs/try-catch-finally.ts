...
export const tryCatchFinally: LessonData = {
  ...
  spec: [
    { text: "«You can use the try statement in any of the following forms: try-catch… try-finally… and try-catch-finally - as a combination of the preceding two forms.» <span class=\"ru-tr\">«Оператор try можно использовать в любой из следующих форм: try-catch… try-finally… и try-catch-finally — как комбинацию двух предыдущих форм.»</span>", source: "ms-stmts" },
    { text: "«In a try-finally statement, the finally block runs when control leaves the try block.» <span class=\"ru-tr\">«В операторе try-finally блок finally выполняется, когда управление покидает блок try.»</span> — по нормальному выходу, по jump-оператору (return/break/continue/goto) или по распространению исключения.", source: "ms-stmts" },
  ],
  edgeCases: [
    { text: "Порядок и единственность: «the runtime checks catch clauses in the specified order, from top to bottom. <b>At most, only one catch block runs for any thrown exception</b>». <span class=\"ru-tr\">«среда выполнения проверяет предложения catch в указанном порядке, сверху вниз. <b>Максимум один блок catch выполняется для любого брошенного исключения</b>».</span>", source: "ms-stmts" },
    { text: "Catch без типа ловит всё и должен быть последним: «A catch clause without any specified exception type matches any exception and, if present, must be the last catch clause». <span class=\"ru-tr\">«Предложение catch без указанного типа исключения совпадает с любым исключением и, если присутствует, должно быть последним предложением catch».</span>", source: "ms-stmts" },
    { text: "finally после catch: «When a catch block handles an exception, the finally block runs <b>after the catch block finishes</b> (even if another exception occurs during execution of the catch block)». <span class=\"ru-tr\">«Когда блок catch обрабатывает исключение, блок finally выполняется <b>после того, как блок catch завершится</b> (даже если во время выполнения блока catch возникает другое исключение)».</span>", source: "ms-stmts" },
    { text: "Компилятор превращает <code>using</code> в try-finally: «The compiler transforms a <code>using</code> statement into a try-finally statement». <span class=\"ru-tr\">«Компилятор преобразует оператор <code>using</code> в оператор try-finally».</span>", source: "ms-stmts" },
  ],

  misconceptions: [
    {
      wrong: "finally выполняется только если было исключение; при обычном return он пропускается",
      hook: 'Наоборот: <code>finally</code> — про «контроль <b>покидает</b> <code>try</code>», а не «случилось исключение». «<span class="hl">In a try-finally statement, the finally block runs when control leaves the try block</span>». <span class="ru-tr">«В операторе try-finally блок finally выполняется, когда управление покидает блок try».</span> Способов выхода <b>три</b>: «normal execution», <span class="ru-tr">«обычное выполнение»,</span> «execution of a jump statement (that is, <code>return</code>, <code>break</code>, <code>continue</code>, or <code>goto</code>)», <span class="ru-tr">«выполнение оператора перехода (то есть <code>return</code>, <code>break</code>, <code>continue</code> или <code>goto</code>)»,</span> или «propagation of an exception out of the <code>try</code> block». <span class="ru-tr">«распространение исключения из блока <code>try</code>».</span> А <code>catch</code>-ветки проверяются «<span class="hl">in the specified order, from top to bottom</span>» <span class="ru-tr">«в указанном порядке, сверху вниз»</span> и «<span class="hl">At most, only one catch block runs for any thrown exception</span>». <span class="ru-tr">«Максимум один блок catch выполняется для любого брошенного исключения».</span> Дальше <b>пять разборов</b>: три формы <code>try</code>, <code>catch</code> по базовому типу, порядок нескольких <code>catch</code>, три способа выхода из <code>try</code>, и <b>машинная панель</b> — <code>finally</code> срабатывает даже при раннем <code>return</code>, до отдачи значения (реальный прогон).',
      source: "ms-stmts",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>try-catch</code> — «to handle exceptions that might occur during execution of the code inside a <code>try</code> block». <span class="ru-tr">«чтобы обрабатывать исключения, которые могут возникнуть во время выполнения кода внутри блока <code>try</code>».</span> <span class="hl">Ловим</span> и обрабатываем.', nodes: [{ id: "tc", kind: "gate", at: { zone: "forms", row: 0 }, state: "ok", label: "try-catch", detail: "обработка", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: '<code>try-finally</code> — «to specify the code that runs when <span class="hl">control leaves the try block</span>». <span class="ru-tr">«чтобы указать код, который выполняется, когда управление покидает блок try».</span> Гарантированная очистка <b>без</b> обработки.', nodes: [{ id: "tc", kind: "gate", at: { zone: "forms", row: 0 }, state: "ok", label: "try-catch", detail: "обработка" }, { id: "tf", kind: "gate", at: { zone: "forms", row: 1 }, state: "ok", label: "try-finally", detail: "очистка", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: '<code>try-catch-finally</code> — «a <span class="hl">combination of the preceding two forms</span>» <span class="ru-tr">«комбинация двух предыдущих форм»</span>: и поймать, и гарантированно прибраться.', nodes: [{ id: "tc", kind: "gate", at: { zone: "forms", row: 0 }, state: "ok", label: "try-catch", detail: "обработка" }, { id: "tf", kind: "gate", at: { zone: "forms", row: 1 }, state: "ok", label: "try-finally", detail: "очистка" }, { id: "tcf", kind: "gate", at: { zone: "forms", row: 2 }, state: "ok", label: "try-catch-finally", detail: "оба", accent: true }], edges: [] },
      ],
      explain: 'Один оператор <code>try</code>, три формы под три задачи. «You can use the <code>try</code> statement in any of the following forms: <span class="hl">try-catch</span> - to handle exceptions that might occur during execution of the code inside a <code>try</code> block, <span class="hl">try-finally</span> - to specify the code that runs when control leaves the <code>try</code> block, and <span class="hl">try-catch-finally</span> - as a combination of the preceding two forms». <span class="ru-tr">«Оператор <code>try</code> можно использовать в любой из следующих форм: try-catch — чтобы обрабатывать исключения, которые могут возникнуть во время выполнения кода внутри блока <code>try</code>, try-finally — чтобы указать код, который выполняется, когда управление покидает блок <code>try</code>, и try-catch-finally — как комбинацию двух предыдущих форм».</span> Важное различие: <code>catch</code> — про <b>обработку</b> ошибки (решаешь, что делать), <code>finally</code> — про <b>гарантию</b> выполнения кода на выходе (очистка ресурсов). Форма <code>try-finally</code> без <code>catch</code> законна: ты не обрабатываешь исключение (оно летит дальше вверх), но гарантированно закрываешь ресурс. <code>throw</code> и <code>try</code> — базовые операторы модели: «Use the <code>throw</code> statement to throw an exception. Use the <code>try</code> statement to catch and handle exceptions». <span class="ru-tr">«Используйте оператор <code>throw</code>, чтобы бросить исключение. Используйте оператор <code>try</code>, чтобы перехватывать и обрабатывать исключения».</span>',
      sources: ["ms-stmts"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'Код, который может бросить, — <span class="hl">внутри</span> <code>try</code>: «Place the code where an exception might occur inside a <code>try</code> block». <span class="ru-tr">«Помещайте код, где может возникнуть исключение, внутрь блока <code>try</code>».</span>', nodes: [{ id: "t", kind: "gate", at: { zone: "try", row: 0 }, state: "fail", label: "Process(-3,4)", detail: "может бросить", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: '<code>catch (ArgumentException)</code> называет <span class="hl">базовый тип</span>: «Use a <i>catch clause</i> to specify the base type of exceptions you want to handle». <span class="ru-tr">«Используйте <i>предложение catch</i>, чтобы указать базовый тип исключений, которые вы хотите обрабатывать».</span>', nodes: [{ id: "t", kind: "gate", at: { zone: "try", row: 0 }, state: "fail", label: "Process", detail: "бросает" }, { id: "c", kind: "gate", at: { zone: "catch", row: 0 }, state: "ok", label: "catch (ArgumentException)", detail: "базовый тип", accent: true }], edges: [{ id: "e1", from: "t", to: "c", accent: true }] },
      ...
      explain: 'Форма <code>try-catch</code> и правило матча по типу. «Use the <code>try-catch</code> statement to handle exceptions that might occur during execution of a code block. <span class="hl">Place the code where an exception might occur inside a try block</span>. Use a <i>catch clause</i> to <span class="hl">specify the base type of exceptions you want to handle</span> in the corresponding <code>catch</code> block». <span class="ru-tr">«Используйте оператор <code>try-catch</code>, чтобы обрабатывать исключения, которые могут возникнуть во время выполнения блока кода. Помещайте код, где может возникнуть исключение, внутрь блока try. Используйте <i>предложение catch</i>, чтобы указать базовый тип исключений, которые вы хотите обрабатывать, в соответствующем блоке <code>catch</code>».</span> Поскольку <code>catch</code> берёт <b>базовый</b> тип, он ловит и все производные — <code>catch (ArgumentException)</code> поймает <code>ArgumentNullException</code>. «As the preceding example also shows, you can <span class="hl">omit declaration of an exception variable</span> and specify only the exception type in a catch clause» <span class="ru-tr">«Как также показывает предыдущий пример, можно опустить объявление переменной исключения и указать в предложении catch только тип исключения»</span> — если сам объект не нужен. Практика: лови самый <b>конкретный</b> тип, который умеешь обработать, чтобы не проглотить чужие ошибки.',
      sources: ["ms-stmts"],
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: 'Проверка идёт <span class="hl">сверху вниз</span>: «the runtime checks catch clauses in the specified order, from top to bottom». <span class="ru-tr">«среда выполнения проверяет предложения catch в указанном порядке, сверху вниз».</span> Первый — <code>ArgumentNullException</code> — совпал.', nodes: [{ id: "c1", kind: "gate", at: { zone: "order", row: 0 }, state: "ok", label: "catch (ArgumentNullException)", detail: "СОВПАЛ", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: 'Остальные <code>catch</code> <b>не выполняются</b>: «<span class="hl">At most, only one catch block runs for any thrown exception</span>» <span class="ru-tr">«Максимум один блок catch выполняется для любого брошенного исключения»</span> — хотя <code>ArgumentException</code> и <code>Exception</code> тоже подошли бы.', nodes: [{ id: "c1", kind: "gate", at: { zone: "order", row: 0 }, state: "ok", label: "ArgumentNullException", detail: "выполнен" }, { id: "c2", kind: "gate", at: { zone: "order", row: 1 }, state: "fail", label: "ArgumentException", detail: "пропущен", accent: true }, { id: "c3", kind: "gate", at: { zone: "order", row: 2 }, state: "fail", label: "Exception", detail: "пропущен" }], edges: [] },
      ...
      explain: 'Несколько <code>catch</code> — упорядоченный поиск с ровно одним победителем. «When an exception occurs, the runtime <span class="hl">checks catch clauses in the specified order, from top to bottom</span>. <span class="hl">At most, only one catch block runs for any thrown exception</span>». <span class="ru-tr">«Когда возникает исключение, среда выполнения проверяет предложения catch в указанном порядке, сверху вниз. Максимум один блок catch выполняется для любого брошенного исключения».</span> Поэтому порядок веток — часть логики: узкий <code>ArgumentNullException</code> должен стоять <b>выше</b> широкого <code>ArgumentException</code>, иначе широкий перехватит первым и узкая ветка станет мёртвой. Ветка без типа — универсальная и обязана быть последней: «A catch clause without any specified exception type <span class="hl">matches any exception and, if present, must be the last catch clause</span>». <span class="ru-tr">«Предложение catch без указанного типа исключения совпадает с любым исключением и, если присутствует, должно быть последним предложением catch».</span> Прогон печатает <code>argnull</code>: совпал первый, остальные пропущены даже будучи совместимыми.',
      sources: ["ms-stmts"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, out: "", caption: 'Путь 2 — <span class="hl">jump</span>: «execution of a jump statement (that is, <code>return</code>, <code>break</code>, <code>continue</code>, or <code>goto</code>)». <span class="ru-tr">«выполнение оператора перехода (то есть <code>return</code>, <code>break</code>, <code>continue</code> или <code>goto</code>)».</span> Даже ранний <code>return</code> сперва прогонит <code>finally</code>.', nodes: [{ id: "n", kind: "gate", at: { zone: "leave", row: 0 }, state: "ok", label: "normal", detail: "выход" }, { id: "j", kind: "gate", at: { zone: "leave", row: 1 }, state: "ok", label: "return/break/…", detail: "jump", accent: true }, { id: "f", kind: "gate", at: { zone: "fin", row: 0 }, state: "ok", label: "finally", detail: "бежит" }], edges: [{ id: "e2", from: "j", to: "f", accent: true }] },
        { codeLine: 2, out: "", caption: 'Путь 3 — <span class="hl">исключение</span>: «propagation of an exception out of the <code>try</code> block». <span class="ru-tr">«распространение исключения из блока <code>try</code>».</span> Отсюда идиома — «to clean up allocated resources used in the <code>try</code> block». <span class="ru-tr">«чтобы освободить выделенные ресурсы, использованные в блоке <code>try</code>».</span>', nodes: [{ id: "j", kind: "gate", at: { zone: "leave", row: 0 }, state: "ok", label: "jump", detail: "выход" }, { id: "ex", kind: "gate", at: { zone: "leave", row: 1 }, state: "fail", label: "exception", detail: "propagation", accent: true }, { id: "f", kind: "gate", at: { zone: "fin", row: 0 }, state: "ok", label: "finally", detail: "release", accent: true }], edges: [{ id: "e3", from: "ex", to: "f", accent: true }] },
      ],
      explain: '<code>finally</code> — гарантия про <b>выход из <code>try</code></b>, а не про исключение. «In a <code>try-finally</code> statement, the <span class="hl">finally block runs when control leaves the try block</span>. Control might leave the <code>try</code> block as a result of: <span class="hl">normal execution</span>, execution of a <span class="hl">jump statement (that is, return, break, continue, or goto)</span>, or <span class="hl">propagation of an exception</span> out of the <code>try</code> block». <span class="ru-tr">«В операторе <code>try-finally</code> блок finally выполняется, когда управление покидает блок try. Управление может покинуть блок <code>try</code> в результате: обычного выполнения, выполнения оператора перехода (то есть return, break, continue или goto) или распространения исключения из блока <code>try</code>».</span> Все три пути ведут через <code>finally</code>, поэтому это правильное место для освобождения ресурсов. Ровно эту гарантию компилятор использует для <code>using</code>: «<span class="hl">The compiler transforms a using statement into a try-finally statement</span>». <span class="ru-tr">«Компилятор преобразует оператор using в оператор try-finally».</span> Единственное исключение — немедленное завершение процесса: «The only cases where <code>finally</code> blocks don\'t execute involve <span class="hl">immediate termination of a program</span>» <span class="ru-tr">«Единственные случаи, когда блоки <code>finally</code> не выполняются, связаны с немедленным завершением программы»</span> (например, <code>Environment.FailFast</code>).',
      sources: ["ms-stmts"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 0, out: "", caption: '<code>finally</code> <span class="hl">перехватывает</span> выход: «the finally block runs when control leaves the try block». <span class="ru-tr">«блок finally выполняется, когда управление покидает блок try».</span> Логирует «finally» — <b>до</b> фактического возврата.', nodes: [{ id: "t", kind: "gate", at: { zone: "ret", row: 0 }, state: "ok", label: "return 1", detail: "отложен" }, { id: "f", kind: "gate", at: { zone: "before", row: 0 }, state: "ok", label: "finally: log", detail: "ПЕРЕД возвратом", accent: true }], edges: [{ id: "e1", from: "t", to: "f", accent: true }] },
      ...
      explain: 'Машинная панель — <code>finally</code> против раннего <code>return</code>. <code>return</code> входит в список jump-операторов, при которых «the <span class="hl">finally block runs when control leaves the try block</span>» <span class="ru-tr">«блок finally выполняется, когда управление покидает блок try»</span>: контроль уже покидает <code>try</code>, но <code>finally</code> выполняется <b>первым</b>, и только потом метод фактически возвращает значение. Реальный прогон печатает <code>try;finally;r=1</code>: сначала тело <code>try</code>, затем <code>finally</code>, и значение <code>1</code> корректно доходит до вызывающего. Это гарантия очистки, которую нельзя обойти обычным <code>return</code> — ресурс закроется даже на «счастливом» пути с ранним выходом. Именно поэтому «You can also use the <code>finally</code> block to <span class="hl">clean up allocated resources</span> used in the <code>try</code> block» <span class="ru-tr">«Также можно использовать блок <code>finally</code>, чтобы освободить выделенные ресурсы, использованные в блоке <code>try</code>»</span> и почему <code>using</code> лоуэрится в <code>try-finally</code>.',
      sources: ["ms-stmts"],
    },
  ],
  ...
  cards: [
    {
      ...
      okText: '<code>try</code> добавляет T и бросает → <code>catch</code> добавляет C → <code>finally</code> «runs when control leaves the try block» <span class="ru-tr">«выполняется, когда управление покидает блок try»</span> добавляет F. Порядок: <b>TCF</b>.',
    ...
    },
    {
      ...
      okText: '<code>return</code> — jump-оператор: контроль покидает <code>try</code>, но <code>finally</code> «runs when control leaves the try block» <span class="ru-tr">«выполняется, когда управление покидает блок try»</span> отрабатывает <b>до</b> отдачи значения. Затем возвращается 1. Печать: <b>try;finally;r=1</b>.',
    ...
    },
    {
      ...
      okText: 'Проверка <code>catch</code> «in the specified order, from top to bottom» <span class="ru-tr">«в указанном порядке, сверху вниз»</span> — первым совпал <code>ArgumentNullException</code>; «<span class="hl">At most, only one catch block runs</span>», <span class="ru-tr">«Максимум один блок catch выполняется»,</span> остальные пропущены. Печать: <b>argnull</b>.',
  ...
  takeaways: [
    { icon: "why", k: "Три формы", v: '«try-catch - to handle exceptions… try-finally - to specify the code that runs when <span class="hl">control leaves the try block</span>… try-catch-finally - as a combination». <span class="ru-tr">«try-catch — чтобы обрабатывать исключения… try-finally — чтобы указать код, который выполняется, когда управление покидает блок try… try-catch-finally — как комбинацию».</span> <code>catch</code> называет <b>базовый</b> тип (ловит наследников).' },
    { icon: "cost", k: "Порядок catch", v: '«runtime checks catch clauses <span class="hl">in the specified order, from top to bottom</span>… <span class="hl">At most, only one catch block runs</span>». <span class="ru-tr">«среда выполнения проверяет предложения catch в указанном порядке, сверху вниз… Максимум один блок catch выполняется».</span> Узкие типы — выше; <code>catch</code> без типа — последним (замер: argnull).' },
