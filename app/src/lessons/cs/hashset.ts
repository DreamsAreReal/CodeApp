  ...
  misconceptions: [
    {
      ...
      hook: 'Интуиция «List проще, значит быстрее» здесь <span class="wrong">обратна истине</span>. Для проверки принадлежности <code>List&lt;T&gt;.Contains</code> — линейный обход O(n), а <code>HashSet&lt;T&gt;.Contains</code> — «This method is an <span class="hl">O(1) operation</span>»: хеш ведёт в бакет за константу, независимо от размера. HashSet — «In simple terms, the <code>HashSet&lt;T&gt;</code> class can be thought of as a <code>Dictionary&lt;TKey,TValue&gt;</code> collection <b>without values</b>»: ключи хеш-таблицы без нагрузки. Дальше <b>четыре разбора</b> — от модели «словарь без значений» до <b>машинной панели</b>: реальный счётчик сравнений на 100 000 элементов — <b>50001</b> у <code>List</code> против <b>1</b> у <code>HashSet</code>.',
      source: "ms-hashset",
    },
  ],
