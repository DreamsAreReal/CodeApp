...
export const hashSet: LessonData = {
  ...
  spec: [
    { text: "«The HashSet&lt;T&gt; class provides high-performance set operations. A set is a collection that contains no duplicate elements, and whose elements are in no particular order.»", source: "ms-hashset" },
    { text: "«This method is an O(1) operation.»", source: "ms-hashset-contains" },
  ],
  edgeCases: [
    { text: "«In simple terms, the <code>HashSet&lt;T&gt;</code> class can be thought of as a <code>Dictionary&lt;TKey,TValue&gt;</code> collection <b>without values</b>» — те же бакеты, но хранятся только ключи.", source: "ms-hashset" },
    { text: "«A <code>HashSet&lt;T&gt;</code> collection is <b>not sorted</b> and cannot contain duplicate elements» — порядок не определён, дубликаты отбрасываются.", source: "ms-hashset" },
  ...
  misconceptions: [
    {
      ...
      hook: 'Интуиция «List проще, значит быстрее» здесь <span class="wrong">обратна истине</span>. Для проверки принадлежности <code>List&lt;T&gt;.Contains</code> — линейный обход O(n), а <code>HashSet&lt;T&gt;.Contains</code> — «This method is an <span class="hl">O(1) operation</span>»: хеш ведёт в бакет за константу, независимо от размера. HashSet — «a <code>Dictionary&lt;TKey,TValue&gt;</code> collection <b>without values</b>»: ключи хеш-таблицы без нагрузки. Дальше <b>четыре разбора</b> — от модели «словарь без значений» до <b>машинной панели</b>: реальный счётчик сравнений на 100 000 элементов — <b>50001</b> у <code>List</code> против <b>1</b> у <code>HashSet</code>.',
      source: "ms-hashset-contains",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: 'Отсюда всё: уникальность, O(1)-членство и порядок «no particular order» — наследуются от механики бакетов Dictionary.', nodes: [{ id: "dk", kind: "obj", at: { zone: "dict", row: 0 }, typeTag: "key+value", value: "пара" }, { id: "sk", kind: "obj", at: { zone: "set", row: 0 }, typeTag: "только key", value: "хеш → бакет", accent: true }], edges: [] },
      ],
      explain: 'HashSet устроен как хеш-таблица без нагрузки: «The <code>HashSet&lt;T&gt;</code> class is based on the model of mathematical sets and provides high-performance set operations similar to accessing the <b>keys</b> of the <code>Dictionary&lt;TKey,TValue&gt;</code> or <code>Hashtable</code> collections. In simple terms, the <code>HashSet&lt;T&gt;</code> class can be thought of as a <code>Dictionary&lt;TKey,TValue&gt;</code> collection <b>without values</b>». Всё поведение множества — следствие этой механики: элемент кладётся в бакет по своему хешу (уникальность через <code>Equals</code> в бакете), поиск идёт в один бакет (O(1)), а порядок обхода — «in no particular order», как и у ключей словаря.',
    ...
    },
    {
      ...
      explain: 'Уникальность — определяющее свойство: «A set is a collection that contains <b>no duplicate elements</b>», и «A <code>HashSet&lt;T&gt;</code> collection is not sorted and <b>cannot contain duplicate elements</b>». Механически <code>Add</code> хеширует элемент, идёт в бакет и через <code>Equals</code> проверяет, нет ли уже такого; если есть — не добавляет и возвращает <code>false</code>, иначе кладёт и возвращает <code>true</code>. Реальный прогон: <code>Add(5)</code> → <code>true</code>, повторный <code>Add(5)</code> → <code>false</code>, <code>Count</code> = 1. Возврат <code>bool</code> — практичный сигнал: можно детектить «первое появление» без отдельного <code>Contains</code>. Ёмкость при этом «automatically increases as elements are added».',
      sources: ["ms-hashset"],
    },
    {
    ...
    },
    {
      ...
      explain: 'Машинная панель — реально снятый счётчик сравнений. Ключ <code>Probe</code> считает вызовы <code>Equals</code>; поиск элемента @50000 среди 100 000 даёт <b>List: 50001</b> против <b>HashSet: 1</b> (собственный прогон). Разница в природе алгоритма: <code>List</code> идёт линейно (число сравнений ≈ позиция), а <code>HashSet</code> хеширует в бакет и делает одно подтверждающее сравнение — «This method is an <b>O(1) operation</b>». Главное свойство O(1) — <b>независимость от N</b>: то же одно сравнение и на множестве из 100 элементов, и из 100 000. Поэтому для частой проверки принадлежности <code>HashSet</code> бьёт <code>List</code> тем сильнее, чем больше данных — ровно там, где интуиция «список проще» подводит.',
  ...
  cards: [
    {
      ...
      noText: 'Множество «cannot contain duplicate elements». <code>Add</code> возвращает <code>bool</code>: true для нового, false для повтора. Второй 5 отброшен, Count=1. Итого <b>True False 1</b>.',
    ...
    },
    {
      ...
      noText: '«This method is an O(1) operation»: размер множества не влияет. Хеш ведёт в бакет, один <code>Equals</code> подтверждает совпадение → <b>1</b>. Это и есть смысл O(1).',
      verify: { kind: "exec", run: "dotnet run", expect: "Contains(found)=True, Equals-вызовов=1" }, sourceRefs: ["ms-hashset-contains"],
    },
  ],

  takeaways: [
    { icon: "why", k: "Словарь без значений", v: '<code>HashSet&lt;T&gt;</code> — «a <code>Dictionary&lt;TKey,TValue&gt;</code> collection <b>without values</b>»: ключи хеш-таблицы. Уникальность, «no particular order» и O(1)-членство наследуются от механики бакетов.' },
    { icon: "cost", k: "O(1) Contains буквально", v: '<code>Contains</code> — «This method is an <b>O(1) operation</b>»: реальный замер на 100 000 элементов — <b>1</b> сравнение против <b>50001</b> у <code>List</code>. Число сравнений <b>не зависит от N</b>.' },
