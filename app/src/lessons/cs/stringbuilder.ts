...
export const stringbuilder: LessonData = {
  ...
  spec: [
    { text: "«The StringBuilder class represents a string-like object whose value is a mutable sequence of characters.»", source: "ms-sb" },
  ],
  edgeCases: [
    { text: "Мутабельность = правки <b>в одном объекте</b>: «Mutability means that once an instance of the class has been created, it can be modified by <span class=\"hl\">appending, removing, replacing, or inserting characters</span>». <code>Append</code> возвращает <b>тот же</b> экземпляр (собственный прогон: <code>ReferenceEquals</code> = <code>True</code>).", source: "ms-sb" },
    { text: "Ёмкость <b>удваивается</b> при переполнении: «the value of the <code>Capacity</code> property is <span class=\"hl\">doubled</span>, new characters are added… and its <code>Length</code> property is adjusted». Default ctor: «The default capacity of this object is <b>16</b> characters» → после Append 19 символов Capacity = <code>32</code> (собственный прогон).", source: "ms-sb" },
    { text: "StringBuilder — <b>не</b> замена String по умолчанию: «you should not automatically replace <code>String</code> with <code>StringBuilder</code> whenever you want to manipulate strings». Для «a fixed number of concatenation operations, particularly with string literals» компилятор сам объединит их — бери <code>String</code>.", source: "ms-sb" },
    { text: "Достигнув <code>MaxCapacity</code>, рост <b>останавливается</b>: «trying to add characters or expand it beyond its maximum capacity throws either an <code>ArgumentOutOfRangeException</code> or an <code>OutOfMemoryException</code> exception».", source: "ms-sb" },
  ],

  misconceptions: [
    {
      wrong: "StringBuilder — это «быстрая строка»: всегда меняй String на StringBuilder, и любая конкатенация станет быстрее",
      hook: 'Нет — StringBuilder выигрывает не «всегда», а на <b>повторяющихся</b> изменениях, и его нельзя ставить рефлекторно. Суть в мутабельности: «The <b>StringBuilder</b> class represents a string-like object whose value is a <span class="hl">mutable sequence of characters</span>» — «Mutability means that once an instance… has been created, it can be modified by appending, removing, replacing, or inserting characters» <b>в одном</b> объекте, без аллокации нового <code>String</code> на каждый шаг. Но: «you should <b>not automatically replace</b> <code>String</code> with <code>StringBuilder</code> whenever you want to manipulate strings». <span class="ru-tr">(не заменяйте String на StringBuilder автоматически при любой работе со строками).</span> Для малого числа правок или «a fixed number of concatenation operations, particularly with string literals» компилятор сам всё склеит — <code>StringBuilder</code> даст «negligible or no performance improvement». <span class="ru-tr">(ничтожный прирост или его отсутствие).</span> Дальше <b>пять разборов</b>: mutable vs immutable, Append в единый буфер, Length/Capacity и удвоение, когда брать String vs StringBuilder, и <b>машинная панель</b> — default Capacity 16 удваивается до 32 при Append 19 символов (реальный прогон).',
      source: "ms-sb",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'У <code>String</code> «each operation that appears to modify… <span class="hl">actually creates a new string</span>». Три <code>+=</code> — три новых объекта.', nodes: [{ id: "s1", kind: "obj", at: { zone: "imm", row: 0 }, typeTag: "String", value: '"a"' }, { id: "s2", kind: "obj", at: { zone: "imm", row: 1 }, typeTag: "String", value: '"abc"', accent: true }], edges: [] },
        ...
        { codeLine: 1, out: "", caption: 'Разница фундаментальна: String — «immutable type», StringBuilder — «a <b>mutable</b> string class».', nodes: [{ id: "im", kind: "gate", at: { zone: "imm", row: 0 }, state: "fail", label: "immutable", detail: "новый объект на шаг" }, { id: "mu", kind: "gate", at: { zone: "mut", row: 0 }, state: "ok", label: "mutable", detail: "один объект", accent: true }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: 'При переполнении «the value of the <code>Capacity</code> property is <span class="hl">doubled</span>»: 16 → <b>32</b>, Length = 19. Рост амортизирован.', nodes: [{ id: "l", kind: "gate", at: { zone: "len", row: 0 }, state: "ok", label: "Length", detail: "19" }, { id: "c", kind: "gate", at: { zone: "cap", row: 0 }, state: "ok", label: "Capacity", detail: "32 (16×2)", accent: true }], edges: [] },
      ],
      explain: 'Как растёт буфер: «If the number of added characters causes the length of the <code>StringBuilder</code> object to exceed its current capacity, new memory is allocated, the value of the <code>Capacity</code> property is <span class="hl">doubled</span>, new characters are added… and its <code>Length</code> property is adjusted». <span class="ru-tr">(если добавленные символы превышают текущую ёмкость, выделяется новая память, значение Capacity удваивается, символы добавляются, а Length корректируется).</span> Числа из документации и прогона совпадают: «The default capacity of this object is <b>16</b> characters» — Append строки в 19 символов «results in a new memory allocation… The capacity of the object <b>doubles to 32</b> characters». Как и у <code>List&lt;T&gt;</code> (S17), это геометрический рост: суммарная стоимость реаллокаций амортизируется в <code>O(n)</code>. Если финальный размер известен — задай <code>new StringBuilder(capacity)</code> и избеги промежуточных удвоений.',
      sources: ["ms-sb"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'Бери <b>String</b>, когда правок мало, или это «a fixed number of concatenation operations, particularly with <span class="hl">string literals</span>» — компилятор сам склеит.', nodes: [{ id: "s", kind: "gate", at: { zone: "strcase", row: 0 }, state: "ok", label: "String", detail: "мало правок / литералы", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Бери <b>StringBuilder</b>, когда «an <span class="hl">unknown number</span> of changes» или «a significant number of changes» — напр. цикл над вводом.', nodes: [{ id: "s", kind: "gate", at: { zone: "strcase", row: 0 }, state: "ok", label: "String", detail: "мало правок / литералы" }, { id: "sb", kind: "gate", at: { zone: "sbcase", row: 0 }, state: "ok", label: "StringBuilder", detail: "много / неизвестно правок", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Правило: «you should <b>not automatically replace</b> String with StringBuilder». Мерь: для малого числа правок прирост «negligible or no».', nodes: [{ id: "warn", kind: "gate", at: { zone: "sbcase", row: 0 }, state: "fail", label: "не подменяй рефлекторно", detail: "test your code", accent: true }], edges: [] },
      ],
      explain: 'StringBuilder — инструмент под конкретный профиль, а не «быстрая строка на всё». Документация прямо предупреждает: «you should <b>not automatically replace</b> <code>String</code> with <code>StringBuilder</code> whenever you want to manipulate strings… You should be prepared to <b>test your code</b>». <span class="ru-tr">(не заменяйте String на StringBuilder автоматически; будьте готовы протестировать код).</span> Бери <code>String</code>: «When the number of changes… is small» и «When you perform a fixed number of concatenation operations, particularly with string literals. In this case, the <b>compiler might combine</b> the concatenation operations into a single operation». <span class="ru-tr">(компилятор может объединить конкатенации в одну операцию).</span> Бери <code>StringBuilder</code>: «When you expect your code to make an <b>unknown number of changes</b>… (for example, when you use a loop…)» и «a <b>significant number</b> of changes». Ещё нюанс: у StringBuilder нет <code>IndexOf</code>/<code>StartsWith</code> — под поиск конвертируй в <code>String</code>.',
      sources: ["ms-sb"],
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: 'Default ctor: «The default capacity of this object is <b>16</b> characters». Length 0.', nodes: [{ id: "c", kind: "gate", at: { zone: "start", row: 0 }, state: "ok", label: "Capacity", detail: "16", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: 'Append строки в <b>19</b> символов «exceeds the default capacity» → нужна новая память.', nodes: [{ id: "c", kind: "gate", at: { zone: "start", row: 0 }, state: "ok", label: "Capacity", detail: "16" }, { id: "over", kind: "gate", at: { zone: "grown", row: 0 }, state: "ok", label: "19 > 16", detail: "переполнение", accent: true }], edges: [] },
        { codeLine: 3, out: "16\n19 32", caption: 'Панель: <span class="hl">16 · 19 32</span> (реальный прогон) — Capacity удвоился «doubles to <b>32</b>», Length = 19.', nodes: [{ id: "l", kind: "gate", at: { zone: "grown", row: 0, col: 0 }, state: "ok", label: "Length", detail: "19" }, { id: "c2", kind: "gate", at: { zone: "grown", row: 0, col: 1 }, state: "ok", label: "Capacity", detail: "32", accent: true }], edges: [] },
      ],
      explain: 'Это машинная панель урока — рост буфера, снятый прогоном. Default <code>StringBuilder()</code> стартует с Capacity 16 («The default capacity of this object is <b>16</b> characters»). Appending «"This is a sentence." results in a new memory allocation because the string length (<b>19</b> characters) exceeds the default capacity… The capacity of the object <b>doubles to 32</b> characters, the new string is added, and the length of the object now equals <b>19</b> characters». <span class="ru-tr">(добавление приводит к новой аллокации, т.к. длина строки 19 превышает ёмкость по умолчанию; ёмкость удваивается до 32, строка добавляется, длина становится 19).</span> Реальный вывод сниппета: <code>16</code>, затем <code>19 32</code> — ровно как в доке. Вывод для сеньора: удвоение амортизирует стоимость, но каждое удвоение — это аллокация + копирование буфера; знаешь итоговый размер — задай Capacity в конструкторе и убери промежуточные удвоения.',
      sources: ["ms-sb"],
    },
  ],
  ...
  cards: [
    {
      ...
      okText: 'Default Capacity = <b>16</b>. 19 символов > 16 → «capacity… doubles to 32». Length = 19, Capacity = <span class="hl">32</span>.',
      noText: 'Переполнение удваивает Capacity: «the value of the Capacity property is doubled». 16 → 32, Length 19. Реальный вывод: <b>16</b>, затем <b>19 32</b>.',
    ...
    },
    {
      ...
      okText: '<code>Append</code> пишет «on a single string» и возвращает <b>тот же</b> экземпляр (fluent): <code>same</code> == <code>sb</code> (<b>True</b>), значение <b>abc</b>.',
  ...
  takeaways: [
    ...
    { icon: "cost", k: "Capacity удваивается", v: 'Default Capacity <b>16</b>; при переполнении «the value of the Capacity property is <b>doubled</b>» → 32 (замер: Append 19 символов → 19 32). Геометрический рост амортизирован; знаешь размер — задай Capacity в ctor.' },
    { icon: "avoid", k: "Не подменяй рефлекторно", v: '«you should <b>not automatically replace</b> String with StringBuilder». Мало правок / фикс. конкатенация литералов → String (компилятор склеит). Много/неизвестно правок → StringBuilder. Нет <code>IndexOf</code>/<code>StartsWith</code>.' },
  ],
