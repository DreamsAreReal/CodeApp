...
export const immutableCollections: LessonData = {
  ...
  spec: [
    { text: "«Note that these methods return a new object. When you add or remove items from an immutable list, a copy of the original list is made with the items added or removed, and the original list is unchanged.» <span class=\"ru-tr\">«Обратите внимание, что эти методы возвращают новый объект. Когда вы добавляете или удаляете элементы из неизменяемого списка, создаётся копия исходного списка с добавленными или удалёнными элементами, а исходный список остаётся без изменений.»</span>", source: "ms-immutablelist" },
    { text: "«This type is immutable, so it is always thread-safe.» <span class=\"ru-tr\">«Этот тип неизменяем, поэтому он всегда потокобезопасен.»</span>", source: "ms-immutablelist" },
  ],
  edgeCases: [
    { text: "«Provide <b>implicit thread safety</b> in multi-threaded applications (<b>no locks</b> required to access collections)» <span class=\"ru-tr\">«Обеспечивают <b>неявную потокобезопасность</b> в многопоточных приложениях (для доступа к коллекциям <b>не нужны блокировки</b>)»</span> — неизменяемость даёт потокобезопасность бесплатно.", source: "ms-immutable" },
    { text: "«Modify a collection during enumeration, while ensuring that the original collection does not change» <span class=\"ru-tr\">«Изменять коллекцию во время перебора, при этом гарантируя, что исходная коллекция не меняется»</span> — можно «менять» во время обхода, исходник цел.", source: "ms-immutable" },
  ...
  misconceptions: [
    {
      ...
      hook: 'Логика «неизменяемая → каждое <code>Add</code> копирует всё → медленно» верна лишь для <code>ImmutableArray</code>. У <code>ImmutableList</code> под капотом <b>дерево</b>: новая версия <span class="hl">переиспользует</span> почти все узлы старой (структурное разделение), поэтому <code>Add</code> — O(log n), не O(n). Мутирующий метод «return a <b>new object</b>… the original list is <b>unchanged</b>» <span class="ru-tr">«возвращает <b>новый объект</b>… исходный список остаётся <b>без изменений</b>»</span>, а массовые правки идут через <code>Builder</code> «with little or no memory allocations» <span class="ru-tr">«с малым числом выделений памяти или вовсе без них»</span>. Дальше <b>четыре разбора</b> — от «новый объект на мутацию» до <b>машинной панели</b>: реальный замер <code>b = a.Add(4)</code> — <code>a.Count=3</code>, <code>b.Count=4</code>, <code>ReferenceEquals(a,b)=False</code>.',
  ...
  segments: [
    {
      ...
      explain: 'Неизменяемость означает, что «мутация» — это порождение нового значения: «Note that these methods return a <b>new object</b>. When you add or remove items from an immutable list, a copy of the original list is made… and the <b>original list is unchanged</b>» <span class="ru-tr">«Обратите внимание, что эти методы возвращают <b>новый объект</b>. Когда вы добавляете или удаляете элементы из неизменяемого списка, создаётся копия исходного списка… и <b>исходный список остаётся без изменений</b>»</span>. Отсюда два свойства из namespace: «Share a collection in a way that its consumer can be assured that the collection <b>never changes</b>» <span class="ru-tr">«Делиться коллекцией так, чтобы её потребитель мог быть уверен, что коллекция <b>никогда не меняется</b>»</span> и «Modify a collection during enumeration, while ensuring that the original collection does not change» <span class="ru-tr">«Изменять коллекцию во время перебора, при этом гарантируя, что исходная коллекция не меняется»</span>. Ссылку на <code>a</code> можно свободно раздавать между потоками и методами — никто её не испортит. Цена — новый объект на изменение; насколько дорогой, зависит от типа (следующие разборы).',
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 0, caption: 'Но <code>Add</code> у массива — <span class="wrong">O(n)</span>: «Requires creating a new array» <span class="ru-tr">«Требует создания нового массива»</span> — копируется весь массив целиком, разделения нет.', nodes: [{ id: "ai", kind: "gate", at: { zone: "arr", row: 0 }, state: "ok", label: "Item", detail: "O(1)" }, { id: "aa", kind: "gate", at: { zone: "arr", row: 1 }, state: "fail", label: "Add", detail: "O(n) копия", accent: true }], edges: [] },
      ...
      explain: '«Immutable = дорого» — это про <b>тип</b>, а не про идею. <code>ImmutableArray&lt;T&gt;</code> — это массив: чтение по индексу O(1), но любое изменение «Requires creating a new array» <span class="ru-tr">«Требует создания нового массива»</span> — полная копия O(n), структурного разделения нет (таблица сложности на странице <code>ImmutableArray&lt;T&gt;</code>: <code>Item</code> O(1), <code>Add()</code> O(n)). <code>ImmutableList&lt;T&gt;</code> — дерево: и <code>Item</code>, и <code>Add</code> — O(log n). Отсюда выбор по доминирующей операции: <code>ImmutableArray</code> — когда «Updating the data is rare or the number of elements is quite small…» <span class="ru-tr">«Обновление данных происходит редко или число элементов совсем невелико…»</span> и нужны быстрые итерации/чтения; <code>ImmutableList</code> — когда «Updating the data is common…» <span class="ru-tr">«Обновление данных происходит часто…»</span>. Один — за чтение платит копией на запись, другой — за дешёвую запись платит логарифмом на чтение.',
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 3, out: "thread-safe без локов", caption: 'Раз <code>a</code> никогда не меняется, его можно шарить между потоками <span class="hl">без единого лока</span> — «This type is immutable, so it is always thread-safe» <span class="ru-tr">«Этот тип неизменяем, поэтому он всегда потокобезопасен»</span>.', nodes: [{ id: "ts", kind: "gate", at: { zone: "panel", row: 0, col: 0 }, state: "ok", label: "immutable", detail: "thread-safe", accent: true }, { id: "nl", kind: "chip", at: { zone: "panel", row: 0, col: 1 }, value: "no locks" }], edges: [] },
      ],
      explain: 'Машинная панель — реально снятое поведение (замер через exec-бэкенд, .NET 10). <code>var a = ImmutableList.Create(1,2,3); var b = a.Add(4);</code> даёт <b>a.Count=3</b>, <b>b.Count=4</b>, <b>ReferenceEquals(a,b)=False</b>: <code>Add</code> вернул новый объект, а <code>a</code> остался прежним — ровно как обещает дока: «these methods return a new object… the original list is unchanged» <span class="ru-tr">«эти методы возвращают новый объект… исходный список остаётся без изменений»</span>. Это и есть основа бесплатной потокобезопасности: «This type is immutable, so it is <b>always thread-safe</b>» <span class="ru-tr">«Этот тип неизменяем, поэтому он <b>всегда потокобезопасен</b>»</span>, «no locks required» <span class="ru-tr">«блокировки не нужны»</span>. Массовые изменения без промежуточных объектов делает <code>Builder</code> — «a list that mutates with <b>little or no memory allocations</b>» <span class="ru-tr">«список, который меняется <b>с малым числом выделений памяти или вовсе без них</b>»</span>: набираешь через <code>builder.Add(...)</code>, затем <code>ToImmutable()</code> фиксирует результат одним снимком.',
      sources: ["ms-immutablelist", "ms-immutable"],
    },
  ],
  ...
  cards: [
    {
      ...
      noText: '«these methods return a new object… the original list is unchanged» <span class="ru-tr">«эти методы возвращают новый объект… исходный список остаётся без изменений»</span>. <code>a.Add(4)</code> не трогает <code>a</code> (Count остаётся 3), <code>b</code> — новый (Count 4, другая ссылка). Вывод: <b>a.Count=3 b.Count=4 same=False</b>.',
    ...
    },
    {
      ...
      okText: '<code>Builder</code> мутирует «with little or no memory allocations» <span class="ru-tr">«с малым числом выделений памяти или вовсе без них»</span>: 5 <code>Add</code> дают квадраты 0,1,4,9,16. <code>ToImmutable()</code> фиксирует снимок. Вывод: <b>builder.Count=5 result=[0,1,4,9,16]</b>.',
  ...
  takeaways: [
    { icon: "why", k: "Новый объект на мутацию", v: '«these methods return a <b>new object</b>… the original list is <b>unchanged</b>» <span class="ru-tr">«эти методы возвращают <b>новый объект</b>… исходный список остаётся <b>без изменений</b>»</span>. <code>b=a.Add(4)</code>: <code>a</code> цел (Count 3), <code>b</code> новый (Count 4), <code>ReferenceEquals=False</code>. Отсюда «implicit thread safety… no locks required» <span class="ru-tr">«неявная потокобезопасность… блокировки не нужны»</span>.' },
    ...
    { icon: "avoid", k: "Builder для пакета", v: 'Много правок подряд — через <code>Builder</code>: «mutates with <b>little or no memory allocations</b>» <span class="ru-tr">«меняется <b>с малым числом выделений памяти или вовсе без них</b>»</span>, затем <code>ToImmutable()</code>. Выбор типа: частые чтения+редкие правки → <code>Array</code>; частые правки → <code>List</code>.' },
  ],
