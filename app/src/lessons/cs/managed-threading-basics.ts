...
export const managedThreadingBasics: LessonData = {
  ...
  spec: [
    { text: "«A thread is the basic unit to which an operating system allocates processor time.» <span class=\"ru-tr\">«Поток — это базовая единица, которой операционная система выделяет процессорное время.»</span> Каждый поток имеет приоритет планирования и хранит контекст (регистры CPU + стек) для возобновления.", source: "ms-threads" },
    { text: "«All threads of a process share its virtual address space.» <span class=\"ru-tr\">«Все потоки процесса делят его виртуальное адресное пространство.»</span> — потому общее изменяемое состояние надо синхронизировать: «To keep the resource in an uncorrupted state and avoid race conditions, you must synchronize the thread access to it». <span class=\"ru-tr\">«Чтобы сохранить ресурс в неповреждённом состоянии и избежать состояний гонки, нужно синхронизировать доступ потоков к нему».</span>", source: "ms-threads" },
  ],
  edgeCases: [
    { text: "«All threads generated by creating and starting a new Thread object are by default foreground threads» <span class=\"ru-tr\">«Все потоки, порождённые созданием и запуском нового объекта Thread, по умолчанию являются приоритетными (foreground) потоками»</span> — а поток пула фоновый: «Threads that belong to the managed thread pool … are background threads». <span class=\"ru-tr\">«Потоки, принадлежащие управляемому пулу потоков … являются фоновыми (background) потоками».</span>", source: "ms-fgbg" },
    { text: "Гонка — баг порядка: «A race condition is a bug that occurs when the outcome of a program depends on which of two or more threads reaches a particular block of code first». <span class=\"ru-tr\">«Состояние гонки — это баг, который возникает, когда исход программы зависит от того, какой из двух или более потоков первым достигает определённого блока кода».</span>", source: "ms-best" },
    { text: "Дедлок — взаимная блокировка: «A deadlock occurs when each of two threads tries to lock a resource the other has already locked. Neither thread can make any further progress». <span class=\"ru-tr\">«Взаимоблокировка возникает, когда каждый из двух потоков пытается заблокировать ресурс, уже заблокированный другим. Ни один из потоков не может продвинуться дальше».</span>", source: "ms-best" },
  ],

  misconceptions: [
    {
      wrong: "поток — это «магия параллелизма» от CLR, а общее состояние само собой безопасно",
      hook: 'Нет: поток — это <span class="hl">единица планирования ОС</span>, которую CLR лишь оборачивает. «<b>A thread is the basic unit to which an operating system allocates processor time</b>». <span class="ru-tr">«<b>Поток — это базовая единица, которой операционная система выделяет процессорное время</b>».</span> И вот ловушка: «<b>All threads of a process share its virtual address space</b>» <span class="ru-tr">«<b>Все потоки процесса делят его виртуальное адресное пространство</b>»</span> — общая память общая для всех, поэтому «<span class="hl">To keep the resource in an uncorrupted state and avoid race conditions, you must synchronize the thread access to it</span>». <span class="ru-tr">«Чтобы сохранить ресурс в неповреждённом состоянии и избежать состояний гонки, нужно синхронизировать доступ потоков к нему».</span> CLR даёт три вещи: класс <code>Thread</code>, управляемый пул (<code>ThreadPool</code>) и семейство примитивов синхронизации — и два новых риска: «<b>deadlocks and race conditions</b>». <span class="ru-tr">«<b>взаимоблокировки и состояния гонки</b>».</span> Дальше <b>пять разборов</b>: абстракция над ОС, общее адресное пространство, гонка на поле, синхронизация до детерминизма, и <b>машинная панель</b> — dedicated Thread (foreground, не пул) vs ThreadPool-поток (background, пул) реальным прогоном.',
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: 'При <code>Start()</code> поток отдаётся планировщику ОС: «the basic unit to which an operating system <span class="hl">allocates processor time</span>». <span class="ru-tr">«базовая единица, которой операционная система выделяет процессорное время».</span>', nodes: [{ id: "th", kind: "obj", at: { zone: "clr", row: 0 }, typeTag: "Thread", value: "managed" }, { id: "os", kind: "obj", at: { zone: "os", row: 0 }, typeTag: "OS thread", value: "планируется", accent: true }], edges: [{ id: "e1", from: "th", to: "os", accent: true }] },
      ...
      explain: 'Managed threading — это <b>абстракция CLR над потоком ОС</b>, не отдельная сущность. Дословно: «<span class="hl">A thread is the basic unit to which an operating system allocates processor time</span>. Each thread has a scheduling priority and maintains a set of structures the system uses to save the thread context when the thread\'s execution is paused. The thread context includes all the information the thread needs to seamlessly resume execution, including the thread\'s set of CPU registers and stack». <span class="ru-tr">«Поток — это базовая единица, которой операционная система выделяет процессорное время. Каждый поток имеет приоритет планирования и хранит набор структур, которые система использует для сохранения контекста потока, когда выполнение потока приостановлено. Контекст потока включает всю информацию, необходимую потоку для беспрепятственного возобновления выполнения, включая набор регистров CPU и стек потока».</span> По умолчанию «a .NET program is started with a <span class="hl">single thread, often called the primary thread</span>», <span class="ru-tr">«программа .NET запускается с единственным потоком, который часто называют первичным (primary) потоком»,</span> но может создавать worker-потоки. Класс <code>System.Threading.Thread</code> «represents a managed thread» <span class="ru-tr">«представляет управляемый поток»</span> — тонкая типизированная обёртка над этой ОС-единицей.',
      sources: ["ms-threads"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'Процесс изолирует приложения друг от друга, но <b>внутри</b> него память общая: «All threads of a process <span class="hl">share its virtual address space</span>». <span class="ru-tr">«Все потоки процесса делят его виртуальное адресное пространство».</span>', nodes: [{ id: "sh", kind: "obj", at: { zone: "proc", row: 0 }, typeTag: "shared", value: "int (общее)", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Поток A видит и меняет то же поле. «A thread can execute <span class="hl">any part of the program code</span>, including parts currently being executed by another thread». <span class="ru-tr">«Поток может выполнять любую часть кода программы, включая части, которые в данный момент выполняются другим потоком».</span>', nodes: [{ id: "sh", kind: "obj", at: { zone: "proc", row: 0 }, typeTag: "shared", value: "int" }, { id: "a", kind: "chip", at: { zone: "proc", row: 1, col: 0 }, value: "Thread A", accent: true }], edges: [{ id: "e1", from: "a", to: "sh", accent: true }] },
      ...
      explain: 'Ключ к пониманию всех threading-багов — общая память. «A process is an executing program. An operating system uses processes to separate the applications that are being executed. … <span class="hl">Multiple threads can run in the context of a process. All threads of a process share its virtual address space</span>. A thread can execute any part of the program code, including parts currently being executed by another thread». <span class="ru-tr">«Процесс — это выполняющаяся программа. Операционная система использует процессы, чтобы разделять выполняемые приложения. … В контексте процесса может работать несколько потоков. Все потоки процесса делят его виртуальное адресное пространство. Поток может выполнять любую часть кода программы, включая части, которые в данный момент выполняются другим потоком».</span> Отсюда прямое следствие: «Multiple threads might need to access a shared resource. <span class="hl">To keep the resource in an uncorrupted state and avoid race conditions, you must synchronize the thread access to it</span>». <span class="ru-tr">«Нескольким потокам может понадобиться доступ к общему ресурсу. Чтобы сохранить ресурс в неповреждённом состоянии и избежать состояний гонки, нужно синхронизировать доступ потоков к нему».</span> Локальные переменные и стек у каждого потока свои; поля/куча — общие.',
      sources: ["ms-threads"],
    },
    {
      ...
      console: true,
      scenes: [
        { codeLine: 0, out: "", caption: '<code>objCt++</code> — не одна операция: «loading the value from <code>objCt</code> into a register, <span class="hl">incrementing</span> the value, and storing it in <code>objCt</code>». <span class="ru-tr">«загрузка значения из <code>objCt</code> в регистр, инкрементирование значения и запись его в <code>objCt</code>».</span>', nodes: [{ id: "f", kind: "obj", at: { zone: "shared", row: 0 }, typeTag: "objCt", value: "load→inc→store", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Поток может быть <b>вытеснен</b> после первых двух шагов — «a thread … might be preempted by another thread which performs all three steps». <span class="ru-tr">«поток … может быть вытеснен другим потоком, который выполняет все три шага».</span>', nodes: [{ id: "f", kind: "obj", at: { zone: "shared", row: 0 }, typeTag: "objCt", value: "load→inc" }, { id: "p", kind: "gate", at: { zone: "race", row: 0 }, state: "fail", label: "preempt", detail: "между шагами", accent: true }], edges: [] },
      ...
      explain: 'Гонка — самый частый threading-баг: «<span class="hl">A race condition is a bug that occurs when the outcome of a program depends on which of two or more threads reaches a particular block of code first</span>. Running the program many times produces different results, and the result of any given run cannot be predicted». <span class="ru-tr">«Состояние гонки — это баг, который возникает, когда исход программы зависит от того, какой из двух или более потоков первым достигает определённого блока кода. Многократный запуск программы даёт разные результаты, и результат любого отдельного запуска нельзя предсказать».</span> Простейший пример — инкремент поля: «This operation requires loading the value from <code>objCt</code> into a register, incrementing the value, and storing it in <code>objCt</code>». <span class="ru-tr">«Эта операция требует загрузки значения из <code>objCt</code> в регистр, инкрементирования значения и записи его в <code>objCt</code>».</span> Если поток вытеснен между шагами, «when the first thread resumes execution and stores its value, it <span class="hl">overwrites <code>objCt</code></span> without taking into account the fact that the value has changed in the interim» <span class="ru-tr">«когда первый поток возобновляет выполнение и записывает своё значение, он перезаписывает <code>objCt</code>, не учитывая того, что значение тем временем изменилось»</span> — lost update. Лечится <code>Interlocked.Increment</code> или <code>lock</code> (разбор 04).',
      sources: ["ms-best"],
    },
    {
      ...
      explain: 'Синхронизация превращает «непредсказуемо» в «детерминированно». «To keep the resource in an uncorrupted state and avoid race conditions, you must <span class="hl">synchronize the thread access to it</span>». <span class="ru-tr">«Чтобы сохранить ресурс в неповреждённом состоянии и избежать состояний гонки, нужно синхронизировать доступ потоков к нему».</span> <code>lock</code> — общий инструмент: пока поток держит блокировку, остальные ждут; тело исполняется по одному. Реальный прогон: 8 потоков × 1000 инкрементов под <code>lock</code>, все <code>Join</code>-нуты до печати → ровно <b>8000</b> каждый раз. Именно джойн-до-печати даёт детерминизм: мы не читаем состояние, пока все потоки не завершились. Для простого инкремента дешевле <code>Interlocked.Increment</code> — «the <code>Interlocked</code> class provides better performance for updates that must be atomic» <span class="ru-tr">«класс <code>Interlocked</code> обеспечивает более высокую производительность для обновлений, которые должны быть атомарными»</span> (раздел про Interlocked дальше в S8).',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '<code>new Thread(...)</code> — <b>выделенный</b> поток: «All threads generated by creating and starting a new <code>Thread</code> object are by default <span class="hl">foreground threads</span>». <span class="ru-tr">«Все потоки, порождённые созданием и запуском нового объекта <code>Thread</code>, по умолчанию являются приоритетными (foreground) потоками».</span>', nodes: [{ id: "d", kind: "obj", at: { zone: "ded", row: 0 }, typeTag: "new Thread", value: "foreground", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Поток пула — <b>фоновый</b>: «Threads that belong to the managed thread pool … are <span class="hl">background threads</span>». <span class="ru-tr">«Потоки, принадлежащие управляемому пулу потоков … являются фоновыми (background) потоками».</span> Пул переиспользует потоки.', nodes: [{ id: "d", kind: "obj", at: { zone: "ded", row: 0 }, typeTag: "new Thread", value: "foreground" }, { id: "p", kind: "obj", at: { zone: "pool", row: 0 }, typeTag: "ThreadPool", value: "background", accent: true }], edges: [] },
      ...
      explain: 'Машинная панель урока — реально снятый род потока. Прогон: внутри <code>new Thread</code> → <code>IsThreadPoolThread == False</code>, внутри <code>ThreadPool.QueueUserWorkItem</code> → <code>True</code> (печать <b>dedicated_pool=False poolthread_pool=True</b>). Смысл: «<span class="hl">Threads that belong to the managed thread pool</span> … <span class="hl">are background threads</span>. … All threads generated by creating and starting a new <code>Thread</code> object are by default foreground threads». <span class="ru-tr">«Потоки, принадлежащие управляемому пулу потоков … являются фоновыми (background) потоками. … Все потоки, порождённые созданием и запуском нового объекта <code>Thread</code>, по умолчанию являются приоритетными (foreground) потоками».</span> Разница foreground/background практична: «<span class="hl">a background thread does not keep the managed execution environment running</span>. Once all foreground threads have been stopped in a managed process … the system stops all background threads and shuts down». <span class="ru-tr">«фоновый поток не удерживает управляемую среду выполнения работающей. Как только все приоритетные потоки в управляемом процессе остановлены … система останавливает все фоновые потоки и завершает работу».</span> Про пул: «Thread pool threads are <span class="hl">background threads</span>» <span class="ru-tr">«Потоки пула потоков являются фоновыми (background) потоками»</span> и «There is only one thread pool per process» <span class="ru-tr">«На каждый процесс приходится только один пул потоков»</span> — потому пул удобен для коротких задач, но не для того, что должно держать процесс живым.',
  ...
  cards: [
    {
      ...
      okText: 'После <code>Join</code> рабочий поток завершён — <code>result=42</code> виден в общей памяти. А <code>new Thread</code> по умолчанию <b>foreground</b> (<code>IsBackground == False</code>): «All threads generated by … a new <code>Thread</code> object are by default foreground threads». <span class="ru-tr">«Все потоки, порождённые … новым объектом <code>Thread</code>, по умолчанию являются приоритетными (foreground) потоками».</span>',
    ...
    },
    {
      ...
      okText: 'Выделенный <code>new Thread</code> — <span class="hl">не из пула</span> (False); callback <code>ThreadPool.QueueUserWorkItem</code> бежит на пуловом (True). «Threads that belong to the managed thread pool … are background threads». <span class="ru-tr">«Потоки, принадлежащие управляемому пулу потоков … являются фоновыми (background) потоками».</span>',
    ...
    },
    {
      ...
      noText: 'Без <code>lock</code> сумма была бы меньше 8000 и непредсказуема; с <code>lock</code> и <code>Join</code> — ровно <b>8000</b> каждый раз. «you must synchronize the thread access to it». <span class="ru-tr">«нужно синхронизировать доступ потоков к нему».</span>',
      verify: { kind: "exec", run: "dotnet run", expect: "total=8000" }, sourceRefs: ["ms-threads", "ms-best"],
    },
  ],

  takeaways: [
    { icon: "why", k: "Что даёт CLR", v: 'Managed threading = обёртка над <span class="hl">ОС-потоком</span> («the basic unit to which an operating system allocates processor time» <span class="ru-tr">«базовая единица, которой операционная система выделяет процессорное время»</span>) + класс <code>Thread</code>, управляемый пул и примитивы синхронизации.' },
    { icon: "cost", k: "Общая память", v: '«All threads of a process <span class="hl">share its virtual address space</span>» <span class="ru-tr">«Все потоки процесса делят его виртуальное адресное пространство»</span> — поля/куча общие. Отсюда гонки: <code>objCt++</code> = load→inc→store, вытеснение между шагами теряет обновление.' },
