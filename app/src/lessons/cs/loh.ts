...
export const loh: LessonData = {
  ...
  spec: [
    { text: "«If an object is greater than or equal to 85,000 bytes in size, it\'s considered a large object. This number was determined by performance tuning. When an object allocation request is for 85,000 or more bytes, the runtime allocates it on the large object heap.» <span class=\"ru-tr\">«Если объект имеет размер больше либо равный 85 000 байт, он считается большим объектом. Это число было подобрано настройкой производительности. Когда запрос на выделение объекта составляет 85 000 или более байт, среда выполнения размещает его в куче больших объектов.»</span>", source: "ms-loh" },
  ],
  edgeCases: [
    { text: "LOH = «generation 3» <span class=\"ru-tr\">«поколение 3»</span> логически с gen2 (verbatim): «if they are large objects, they go on the large object heap (LOH), which is <span class=\"hl\">sometimes referred to as generation 3</span>. Generation 3 is a physical generation that\'s <b>logically collected as part of generation 2</b>». <span class=\"ru-tr\">«если это большие объекты, они попадают в кучу больших объектов (LOH), которую иногда называют поколением 3. Поколение 3 — это физическое поколение, которое <b>логически собирается как часть поколения 2</b>».</span>", source: "ms-loh" },
    { text: "Собирается только в gen2 (verbatim): «<span class=\"hl\">Large objects belong to generation 2 because they are collected only during a generation 2 collection</span>. … a generation 2 GC is also called a <i>full GC</i>». <span class=\"ru-tr\">«Большие объекты принадлежат поколению 2, потому что собираются только во время сборки поколения 2. … сборку GC поколения 2 также называют <i>полной сборкой (full GC)</i>».</span>", source: "ms-loh" },
    { text: "Runtime сам кладёт малые объекты на LOH (verbatim): «Sometimes, the debugger shows that the total size of the LOH is less than 85,000 bytes. <span class=\"hl\">This happens because the runtime itself uses the LOH to allocate some objects that are smaller than a large object</span>». <span class=\"ru-tr\">«Иногда отладчик показывает, что общий размер LOH меньше 85 000 байт. Это происходит потому, что среда выполнения сама использует LOH для размещения некоторых объектов, которые меньше большого объекта».</span>", source: "ms-loh" },
  ],

  misconceptions: [
    {
      wrong: "LOH компактится как обычная куча / LOH — независимое поколение gen3, собирается само",
      hook: 'Два мифа. «<span class="wrong">LOH компактится как обычная куча</span>» — нет: «because compaction is expensive, the GC <span class="hl"><i>sweeps</i> the LOH</span>; it makes a free list out of dead objects». <span class="ru-tr">«поскольку уплотнение обходится дорого, GC делает <i>sweep</i> для LOH; он строит free list из мёртвых объектов».</span> По умолчанию <code>LargeObjectHeapCompactionMode == Default</code> (не компактится). «<span class="wrong">LOH — независимое gen3</span>» — нет: «Generation 3 is a physical generation that\'s <span class="hl">logically collected as part of generation 2</span>» <span class="ru-tr">«Поколение 3 — это физическое поколение, которое логически собирается как часть поколения 2»</span> → большой объект освобождается только на gen2/full GC. Ниже <b>пять разборов</b>: порог 85000, LOH как gen3/gen2, sweep вместо compact, запрос компакции + пиннинг, и <b>машинная панель</b> — реальный замер порога и «freed только gen2».',
      source: "ms-loh",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: 'Порог <b>85000</b> «was determined by performance tuning» <span class="ru-tr">«был подобран настройкой производительности»</span>; его можно <span class="hl">конфигурировать</span> (large-object-heap-threshold).', nodes: [{ id: "l", kind: "obj", at: { zone: "large", row: 0 }, typeTag: "порог", value: "85000 байт", accent: true }], edges: [] },
      ],
      explain: 'Дословно: «<span class="hl">If an object is greater than or equal to 85,000 bytes in size, it\'s considered a large object</span>. This number was determined by performance tuning. When an object allocation request is for 85,000 or more bytes, the runtime allocates it on the large object heap». <span class="ru-tr">«Если объект имеет размер больше либо равный 85 000 байт, он считается большим объектом. Это число было подобрано настройкой производительности. Когда запрос на выделение объекта составляет 85 000 или более байт, среда выполнения размещает его в куче больших объектов».</span> Почему отдельная куча: «For instance, compacting it—that is, copying it in memory elsewhere on the heap—<b>can be expensive</b>. Because of this, the garbage collector places large objects on the large object heap». <span class="ru-tr">«Например, уплотнять его — то есть копировать его в памяти в другое место кучи — <b>может быть дорого</b>. Из-за этого сборщик мусора размещает большие объекты в куче больших объектов».</span> То есть большие объекты (обычно массивы) выделяются на LOH, а не на SOH, ровно с порога <b>85000</b> байт. Замером (панель) подтвердим: <code>byte[1000]</code> — gen0, <code>byte[100000]</code> — gen2/LOH. Важная деталь: порог считается по <b>полному размеру объекта</b> (payload + заголовок), а не только по payload.',
      sources: ["ms-loh"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'LOH иногда называют <span class="hl">«generation 3»</span> <span class="ru-tr">«поколение 3»</span> — но это отдельная физическая область для больших объектов.', nodes: [{ id: "g", kind: "gate", at: { zone: "gen3", row: 0 }, state: "ok", label: "LOH", detail: "«generation 3»", accent: true }], edges: [] },
  ...
  cards: [
    {
      ...
      okText: 'Порог <b>85000</b>: <code>byte[1000]</code> — SOH, <b>gen0</b>; <code>byte[100000]</code> (≥85000) — <span class="hl">LOH, gen2</span> (LOH иногда зовут «generation 3» <span class="ru-tr">«поколение 3»</span>, но логически это gen2). Печать: <b>small=0 large=2</b>.',
      noText: '«greater than or equal to 85,000 bytes… large object… on the large object heap» <span class="ru-tr">«больше либо равно 85 000 байт… большой объект… в куче больших объектов»</span>; LOH = gen2. Реальный вывод: <b>small=0 large=2</b>.',
    ...
    },
    {
      ...
      okText: 'LOH-объект «<span class="hl">collected only during a generation 2 collection</span>» <span class="ru-tr">«собирается только во время сборки поколения 2»</span>: <code>GC.Collect(0)</code> его НЕ трогает (жив), <code>GC.Collect(2)</code> — собирает. Печать: <b>aliveAfterGen0=True aliveAfterGen2=False</b>.',
  ...
  takeaways: [
    { icon: "why", k: "порог 85000", v: '«greater than or equal to <b>85,000 bytes</b>… large object… on the large object heap» <span class="ru-tr">«больше либо равно <b>85 000 байт</b>… большой объект… в куче больших объектов»</span> (обычно массивы). Замер: <code>byte[1000]</code>=gen0, <code>byte[100000]</code>=gen2. Порог — по полному размеру объекта, конфигурируем.' },
    { icon: "cost", k: "LOH = gen2, не gen3-сам", v: 'LOH — «generation 3» <span class="ru-tr">«поколение 3»</span> физически, но «<span class="hl">logically collected as part of generation 2</span>» <span class="ru-tr">«логически собирается как часть поколения 2»</span> → freed только на gen2/full GC (замер: жив после gen0, собран gen2). Частые временные большие объекты → частые дорогие full-GC.' },
    { icon: "avoid", k: "sweep, не compact", v: 'По умолчанию LOH «<span class="hl">the GC <i>sweeps</i> the LOH</span>» <span class="ru-tr">«GC делает <i>sweep</i> для LOH»</span> (free list), НЕ компактит (<code>Default</code>) → фрагментация. Компакция — по требованию <code>CompactOnce</code> (замер), одноразово. Стоимость аллокации «dominated by memory clearing» <span class="ru-tr">«в основном определяется очисткой памяти»</span> → пул больших объектов.' },
  ],
