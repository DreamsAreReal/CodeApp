...
export const patternVocabulary: LessonData = {
  ...
  spec: [
    { text: "«Use the is expression, the switch statement, and the switch expression to match an input expression against any number of characteristics.»", source: "ms-patterns" },
    { text: "«Logical, property, positional, and list patterns are recursive patterns. That is, they can contain nested patterns.»", source: "ms-patterns" },
  ],
  edgeCases: [
    ...
    { text: "Constant pattern — «an alternative syntax for <code>==</code> when the right operand is a constant»; при <code>x is null</code> компилятор гарантирует, что <b>не вызовет</b> перегруженный <code>==</code>.", source: "ms-patterns" },
    { text: "«Declaration patterns don't consider user-defined conversions or implicit span conversions» — только встроенные преобразования ссылок/boxing.", source: "ms-patterns" },
  ],

  misconceptions: [
    {
      wrong: "«паттерн» — это просто проверка типа через is; остальное — необязательный синтаксис",
      hook: 'Паттернов <span class="hl">десять видов</span>, и это фиксированный словарь: «C# supports multiple patterns, including declaration, type, constant, relational, property, list, var, and discard». Четыре из них <b>рекурсивны</b> — вкладывают другие паттерны: «Logical, property, positional, and list patterns are <span class="hl">recursive patterns</span>». А ниже абстракции — маршрутизация: declaration/type pattern выбирает рукав по <b>run-time типу</b> через встроенные преобразования ссылок и <span class="hl">никогда не матчит null</span>. Дальше <b>пять разборов</b> — карта словаря, declaration/type, constant, relational, и <b>машинная панель</b> диспетчеризации по типу.',
      source: "ms-patterns",
    },
  ],
  ...
  segments: [
    {
      ...
      explain: 'Словарь паттернов — закрытый список, общий для трёх конструкций: «Use the <code>is</code> expression, the <code>switch</code> statement, and the <code>switch</code> expression to match an input expression against <b>any number of characteristics</b>. C# supports multiple patterns, including declaration, type, constant, relational, property, list, var, and discard». Ключевое свойство четырёх из них — вложенность: «Logical, property, positional, and list patterns are <span class="hl">recursive patterns</span>. That is, they can contain nested patterns». Отсюда выразительность: <code>{ Start: { Y: 0 } }</code> — паттерн внутри паттерна внутри паттерна.',
      sources: ["ms-patterns"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, caption: '<code>message</code> — уже <code>string</code>: «assigns the variable to the <span class="hl">converted expression result</span>». <code>ToLower()</code> зовётся без каста.', nodes: [{ id: "g", kind: "obj", at: { zone: "inp", row: 0 }, typeTag: "greeting", value: "object→str" }, { id: "gt", kind: "gate", at: { zone: "arm", row: 0 }, state: "ok", label: "is string", detail: "матч" }, { id: "m", kind: "slot", at: { zone: "arm", row: 1 }, name: "message", value: "string", accent: true }], edges: [{ id: "e", from: "g", to: "gt" }] },
      ],
      explain: 'Declaration и type pattern проверяют run-time тип: «Use declaration and type patterns to check if the <b>run-time type</b> of an expression is compatible with a given type. By using a declaration pattern, you can also declare a new local variable. When a declaration pattern matches an expression, it assigns the variable to the <span class="hl">converted expression result</span>». Матч случается, когда результат «is <b>non-null</b>» и выполнено одно из: identity-конверсия к <code>T</code>, наследование/реализация интерфейса (implicit reference conversion), boxing/unboxing. Важное ограничение: «Declaration patterns don\'t consider <b>user-defined conversions</b> or implicit span conversions».',
      sources: ["ms-patterns"],
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'Вход <code>visitorCount = 3</code>. Constant pattern — «an alternative syntax for <code>==</code> when the right operand is a constant».', nodes: [{ id: "in", kind: "slot", at: { zone: "inp", row: 0 }, name: "count", value: "3", accent: true }], edges: [] },
