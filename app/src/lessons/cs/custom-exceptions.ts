...
export const customExceptions: LessonData = {
  ...
  spec: [
    { text: "«if none of the predefined exceptions meet your needs, you can create your own exception class by deriving from the Exception class».", source: "ms-userdef" },
    { text: "«When creating your own exceptions, end the class name of the user-defined exception with the word \"Exception\", and implement the three common constructors».", source: "ms-userdef" },
  ],
  edgeCases: [
    { text: "Три конструктора: «The derived classes should define at least three constructors: one parameterless constructor, one that sets the message property, and one that sets both the <code>Message</code> and <code>InnerException</code> properties».", source: "ms-create" },
    { text: "Свойства — только полезные: «Add new properties to the exception class when the data they provide is useful to resolving the exception. If new properties are added to the derived exception class, <code>ToString()</code> should be overridden to return the added information».", source: "ms-create" },
    { text: "Не бросай базовые типы: «Don't throw <code>System.Exception</code>, <code>System.SystemException</code>, <code>System.NullReferenceException</code>, or <code>System.IndexOutOfRangeException</code> intentionally from your own source code».", source: "ms-create" },
  ],

  misconceptions: [
    {
      wrong: "свой exception — это отдельная «магическая» сущность; достаточно class MyError без наследования от Exception",
      hook: 'Пользовательское исключение — обычный класс, <b>наследник</b> <code>Exception</code>. «if none of the predefined exceptions meet your needs, you can <span class="hl">create your own exception class by deriving from the Exception class</span>». Есть конвенции: «When creating your own exceptions, <span class="hl">end the class name of the user-defined exception with the word "Exception"</span>, and implement the <span class="hl">three common constructors</span>» — «<span class="hl">one parameterless constructor, one that sets the message property, and one that sets both the Message and InnerException properties</span>». И он живёт в общей иерархии: базовый твой тип ловит производные твои типы. Дальше <b>пять разборов</b>: наследование <code>: Exception</code>, три конструктора, когда бросать, обёртка через <code>innerException</code>, и <b>машинная панель</b> — свой тип реально в иерархии: <code>catch</code> по своей базе ловит наследника (реальный прогон: NotFoundException).',
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '.NET уже даёт иерархию: «.NET provides a hierarchy of exception classes <span class="hl">ultimately derived from the Exception base class</span>». Сначала ищи подходящий готовый.', nodes: [{ id: "p", kind: "obj", at: { zone: "pre", row: 0 }, typeTag: "Exception hierarchy", value: "готовые", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Если ни один не подходит — «<span class="hl">create your own exception class by deriving from the Exception class</span>». Наследуешься от <code>Exception</code>.', nodes: [{ id: "p", kind: "obj", at: { zone: "pre", row: 0 }, typeTag: "готовые", value: "не подошли" }, { id: "o", kind: "obj", at: { zone: "own", row: 0 }, typeTag: "MyException : Exception", value: "свой", accent: true }], edges: [{ id: "e1", from: "o", to: "p", accent: true }] },
        { codeLine: 2, out: "", caption: 'Конвенция имени: «<span class="hl">end the class name of the user-defined exception with the word "Exception"</span>» — <code>EmployeeListNotFoundException</code>, не <code>EmployeeListError</code>.', nodes: [{ id: "o", kind: "obj", at: { zone: "own", row: 0 }, typeTag: "…Exception", value: "имя по конвенции", accent: true }], edges: [] },
      ],
      explain: 'Своё исключение — не отдельная сущность, а <b>класс в общей иерархии</b>. «.NET provides a hierarchy of exception classes ultimately derived from the <code>Exception</code> base class. However, <span class="hl">if none of the predefined exceptions meet your needs, you can create your own exception class by deriving from the Exception class</span>». Две конвенции сразу: имя «<span class="hl">end the class name of the user-defined exception with the word "Exception"</span>» (чтобы тип читался как исключение) и набор конструкторов (следующий разбор). Заметь: свой тип обязан наследовать <code>Exception</code> (или его потомка) — иначе его нельзя <code>throw</code> и он не участвует в <code>catch</code>-иерархии. Практика: не плоди свои типы там, где точно подходит <code>ArgumentException</code>/<code>InvalidOperationException</code>; свой — когда вызывающему полезно ловить <b>именно</b> твою доменную ошибку.',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '1 — <b>без параметров</b>: «one parameterless constructor». Базовый случай, когда сообщение по умолчанию.', nodes: [{ id: "c0", kind: "gate", at: { zone: "ctors", row: 0 }, state: "ok", label: "()", detail: "parameterless", accent: true }], edges: [] },
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'Условие 1: «The method <span class="hl">can\'t complete its defined functionality</span>» — например, параметру передали недопустимое значение.', nodes: [{ id: "c1", kind: "gate", at: { zone: "when", row: 0 }, state: "fail", label: "не может выполнить", detail: "invalid value", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Условие 2: «An <span class="hl">inappropriate call to an object is made, based on the object state</span>» → бросай <code>InvalidOperationException</code> (напр. запись в read-only).', nodes: [{ id: "c1", kind: "gate", at: { zone: "when", row: 0 }, state: "fail", label: "не может выполнить", detail: "invalid value" }, { id: "c2", kind: "gate", at: { zone: "when", row: 1 }, state: "fail", label: "невалидное состояние", detail: "InvalidOperation", accent: true }], edges: [] },
        { codeLine: 2, out: "", caption: 'Условие 3: «When an <span class="hl">argument to a method causes an exception</span>» → поймать исходное и «passed to the constructor… as the <code>InnerException</code> parameter».', nodes: [{ id: "c1", kind: "gate", at: { zone: "when", row: 0 }, state: "fail", label: "не выполнить", detail: "1" }, { id: "c2", kind: "gate", at: { zone: "when", row: 1 }, state: "fail", label: "состояние", detail: "2" }, { id: "c3", kind: "gate", at: { zone: "when", row: 2 }, state: "fail", label: "аргумент → wrap", detail: "3 · inner", accent: true }], edges: [] },
      ],
      explain: 'Доки задают конкретные поводы для <code>throw</code>: «Programmers should throw exceptions when one or more of the following conditions are true: The method <span class="hl">can\'t complete its defined functionality</span>… An <span class="hl">inappropriate call to an object is made, based on the object state</span>… When an <span class="hl">argument to a method causes an exception</span>». Третий случай прямо про обёртку: «the original exception should be caught and an <code>ArgumentException</code> instance should be created. The original exception should be <span class="hl">passed to the constructor of the ArgumentException as the InnerException parameter</span>». И анти-паттерны: «<span class="hl">Don\'t use exceptions to change the flow of a program as part of ordinary execution</span>. Use exceptions to report and handle error conditions»; «<span class="hl">Don\'t throw System.Exception, System.SystemException, System.NullReferenceException, or System.IndexOutOfRangeException intentionally</span>» — бросай самый <b>конкретный</b> подходящий тип.',
      sources: ["ms-create"],
    },
    {
  ...
  cards: [
    {
      ...
      okText: 'Свой тип «derived from the <code>Exception</code> base class», ловится как <code>Exception</code> (<code>is Exception</code> → True), <code>base(message)</code> кладёт текст в <code>Message</code>, <code>GetType()</code> — реальный тип. Печать: <b>EmployeeListNotFoundException|no list|True</b>.',
    ...
    },
    {
      ...
      okText: 'Конструктор <code>(message, inner)</code> «sets both the <span class="hl">Message and InnerException properties</span>»: свой <code>Message</code> = «load failed», обёрнутая причина в <code>InnerException.Message</code> = «disk». Печать: <b>load failed|disk</b>.',
  ...
  takeaways: [
    { icon: "why", k: "Свой = : Exception", v: '«<span class="hl">create your own exception class by deriving from the Exception class</span>», имя «<span class="hl">end… with the word "Exception"</span>». Не отдельная сущность — класс в общей иерархии (замер: is Exception = True).' },
    { icon: "cost", k: "Три конструктора", v: '«<span class="hl">at least three constructors: one parameterless constructor, one that sets the message property, and one that sets both the Message and InnerException properties</span>». Делегируют в <code>base(...)</code> (замер обёртки: load failed|disk).' },
    { icon: "avoid", k: "Когда/что бросать", v: 'Бросай при «can\'t complete… functionality», невалидном состоянии, ошибке аргумента (→ wrap в inner). НЕ бросай <code>System.Exception/NullReferenceException/IndexOutOfRangeException</code> сам. База ловит наследника (замер: NotFoundException).' },
  ],
