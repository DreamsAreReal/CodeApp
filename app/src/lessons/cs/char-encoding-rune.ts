...
export const charEncodingRune: LessonData = {
  ...
  spec: [
    { text: "«A string is logically a sequence of 16-bit values, each of which is an instance of the char struct. The string.Length property returns the number of char instances in the string instance.» <span class=\"ru-tr\">«Строка логически — это последовательность 16-битных значений, каждое из которых является экземпляром структуры char. Свойство string.Length возвращает число экземпляров char в этой строке.»</span>", source: "ms-enc" },
  ],
  edgeCases: [
    { text: "<code>char</code> — это <b>code unit</b>, не символ: «.NET uses UTF-16 to encode the text in a string. A <span class=\"hl\">char instance represents a 16-bit code unit</span>». <span class=\"ru-tr\">«.NET использует UTF-16 для кодирования текста в string. Экземпляр char представляет 16-битную единицу кода».</span> Для supplementary code point нужны <b>два</b> char: «for a code point in the supplementary range, <span class=\"hl\">two char instances are needed</span>». <span class=\"ru-tr\">«для код-пойнта из дополнительного диапазона нужны два экземпляра char».</span>", source: "ms-enc" },
    { text: "Пара char, кодирующая один символ, — <b>surrogate pair</b>: «The char pairs that map to a single character are called <span class=\"hl\">surrogate pairs</span>». <span class=\"ru-tr\">«Пары char, отображающиеся на один символ, называются суррогатными парами».</span> Собственный прогон: <code>\"🐂\".Length</code> = 2, но это <b>один</b> Rune.", source: "ms-enc" },
    { text: "<code>Rune</code> — это <b>scalar value</b>, целый code point: «In .NET, the <span class=\"hl\">System.Text.Rune type represents a Unicode scalar value</span>». <span class=\"ru-tr\">«В .NET тип System.Text.Rune представляет скалярное значение Unicode».</span> Собственный прогон: <code>\"a🐂b\"</code> — 4 char, но <b>3</b> Rune (a, 🐂, b).", source: "ms-enc" },
  ...
  misconceptions: [
    {
      ...
      hook: 'Нет — <code>char</code> в C# это <b>16-битный UTF-16 code unit</b>, а не «символ», и <code>string.Length</code> считает именно code units. «.NET uses UTF-16 to encode the text in a <b>string</b>. A <span class="hl">char instance represents a 16-bit code unit</span>» <span class="ru-tr">«.NET использует UTF-16 для кодирования текста в <b>string</b>. Экземпляр char представляет 16-битную единицу кода».</span> — и «A <b>string</b> is logically a sequence of 16-bit values… The <b>string.Length</b> property returns the number of <b>char</b> instances». <span class="ru-tr">«<b>string</b> логически — это последовательность 16-битных значений… Свойство <b>string.Length</b> возвращает число экземпляров <b>char</b>».</span> Проблема: символ из supplementary-диапазона (почти все эмодзи) не влезает в 16 бит — «for a code point in the supplementary range, <span class="hl">two char instances are needed</span>», <span class="ru-tr">«для код-пойнта из дополнительного диапазона нужны два экземпляра char»,</span> и эта пара называется surrogate pair. Поэтому <code>"🐂".Length == 2</code>, хотя это один визуальный символ. Правильная единица для перебора реальных code point — <code>System.Text.Rune</code> («In .NET, the <b>System.Text.Rune type represents a Unicode scalar value</b>» <span class="ru-tr">«В .NET тип <b>System.Text.Rune</b> представляет скалярное значение Unicode»</span>): один эмодзи = 2 char, но ровно <b>1</b> Rune. А то, что человек видит как символ (grapheme cluster / text element), может быть ещё крупнее. Дальше <b>пять разборов</b>: char = code unit, BMP vs supplementary, механика surrogate pair, Rune как scalar value, и <b>машинная панель</b> — 🐂 это 2 char и 1 Rune, ConvertToUtf32 собирает 128002 (реальный прогон).',
      source: "ms-enc",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: '.NET кодирует текст в <b>UTF-16</b>: <code>string</code> — «a sequence of 16-bit values» <span class="ru-tr">«последовательность 16-битных значений»</span>, каждое — <code>char</code>.', nodes: [{ id: "s", kind: "obj", at: { zone: "strz", row: 0 }, typeTag: "string (UTF-16)", value: '"Hello"', accent: true }], edges: [] },
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, out: "", caption: 'Перебор по <code>Rune</code> — <b>3</b> шага: <code>a</code>, <b>целый</b> 🐂, <code>b</code>. Rune это «Unicode <span class="hl">scalar value</span>» <span class="ru-tr">«скалярное значение Unicode»</span> — валидный целый code point.', nodes: [{ id: "c", kind: "gate", at: { zone: "charz", row: 0 }, state: "fail", label: "по char", detail: "4" }, { id: "r", kind: "gate", at: { zone: "rune", row: 0 }, state: "ok", label: "по Rune", detail: "3 (a, 🐂, b)", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Rune-конструктор <b>валидирует</b> scalar value: «validate that the resulting instance is a <span class="hl">valid Unicode scalar value</span>, otherwise they throw». <span class="ru-tr">«проверяют, что полученный экземпляр — это валидное скалярное значение Unicode, иначе бросают исключение».</span> Половинку суррогата в Rune не засунуть.', nodes: [{ id: "v", kind: "gate", at: { zone: "rune", row: 0 }, state: "ok", label: "new Rune(...)", detail: "валидирует scalar value", accent: true }], edges: [] },
      ],
  ...
  cards: [
    {
      ...
      okText: '🐂 — supplementary code point, «two char instances are needed» <span class="ru-tr">«нужны два экземпляра char»</span> → <code>Length</code> = <b>2</b>. Но это один scalar value → <b>1</b> Rune.',
    ...
    },
    {
      ...
      noText: '«high surrogate… immediately followed by a low surrogate… interpreted as a supplementary code point». <span class="ru-tr">«старший суррогат… за которым сразу следует младший суррогат… интерпретируется как дополнительный код-пойнт».</span> Пара → 0x1F402 = 128002. Реальный вывод: <b>True / True / 128002</b>.',
      verify: { kind: "exec", run: "dotnet run", expect: "True\nTrue\n128002" }, sourceRefs: ["ms-enc"],
    },
  ],

  takeaways: [
    { icon: "why", k: "char = code unit", v: '«A <b>char instance represents a 16-bit code unit</b>» <span class="ru-tr">«Экземпляр <b>char</b> представляет 16-битную единицу кода»</span>; <code>string.Length</code> — число char, не символов. Для латиницы/BMP совпадает, что и создаёт иллюзию «char = символ».' },
    { icon: "cost", k: "Surrogate pair", v: 'Supplementary code point (эмодзи) → «<b>two char instances are needed</b>» <span class="ru-tr">«нужны два экземпляра <b>char</b>»</span> (surrogate pair). <code>"🐂".Length</code> = 2 (замер). Резать строку по индексу char вслепую = ill-formed UTF-16.' },
    { icon: "avoid", k: "Rune = scalar value", v: '«In .NET, the <b>System.Text.Rune type represents a Unicode scalar value</b>» <span class="ru-tr">«В .NET тип <b>System.Text.Rune</b> представляет скалярное значение Unicode»</span> — правильная единица перебора code point. <code>"a🐂b"</code>: 4 char, но 3 Rune (замер). Для видимых символов — <code>StringInfo</code> (grapheme cluster).' },
  ],
