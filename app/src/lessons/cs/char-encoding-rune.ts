...
export const charEncodingRune: LessonData = {
  ...
  misconceptions: [
    {
      ...
      hook: 'Нет — <code>char</code> в C# это <b>16-битный UTF-16 code unit</b>, а не «символ», и <code>string.Length</code> считает именно code units. «.NET uses UTF-16 to encode the text in a <b>string</b>. A <span class="hl">char instance represents a 16-bit code unit</span>» — и «A <b>string</b> is logically a sequence of 16-bit values… The <b>string.Length</b> property returns the number of <b>char</b> instances». Проблема: символ из supplementary-диапазона (почти все эмодзи) не влезает в 16 бит — «for a code point in the supplementary range, <span class="hl">two char instances are needed</span>», и эта пара называется surrogate pair. Поэтому <code>"🐂".Length == 2</code>, хотя это один визуальный символ. Правильная единица для перебора реальных code point — <code>System.Text.Rune</code> («In .NET, the <b>System.Text.Rune type represents a Unicode scalar value</b>»): один эмодзи = 2 char, но ровно <b>1</b> Rune. А то, что человек видит как символ (grapheme cluster / text element), может быть ещё крупнее. Дальше <b>пять разборов</b>: char = code unit, BMP vs supplementary, механика surrogate pair, Rune как scalar value, и <b>машинная панель</b> — 🐂 это 2 char и 1 Rune, ConvertToUtf32 собирает 128002 (реальный прогон).',
      source: "ms-enc",
    },
  ],
  ...
  takeaways: [
    ...
    { icon: "avoid", k: "Rune = scalar value", v: '«In .NET, the <b>System.Text.Rune type represents a Unicode scalar value</b>» — правильная единица перебора code point. <code>"a🐂b"</code>: 4 char, но 3 Rune (замер). Для видимых символов — <code>StringInfo</code> (grapheme cluster).' },
  ],
