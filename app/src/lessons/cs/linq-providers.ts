...
export const linqProviders: LessonData = {
  ...
  spec: [
    { text: "«You use the same basic coding patterns to query and transform data in XML documents, SQL databases, .NET collections, and any other format when a LINQ provider is available.» <span class=\"ru-tr\">«Вы используете одни и те же базовые шаблоны кода, чтобы запрашивать и преобразовывать данные в XML-документах, базах данных SQL, коллекциях .NET и любом другом формате, когда доступен LINQ-провайдер.»</span>", source: "ms-intro" },
  ],
  edgeCases: [
    { text: "In-memory: «If the data is of a type that implements <code>IEnumerable&lt;T&gt;</code>, <span class=\"hl\">query the data by using LINQ to Objects</span>». <span class=\"ru-tr\">«Если данные имеют тип, реализующий <code>IEnumerable&lt;T&gt;</code>, запрашивайте их с помощью LINQ to Objects».</span>", source: "ms-linq" },
    { text: "Удалённый источник: «The best option for enabling LINQ querying of a <b>remote data source</b> is to implement the <span class=\"hl\">IQueryable&lt;T&gt;</span> interface». <span class=\"ru-tr\">«Лучший вариант, чтобы включить LINQ-запросы к <b>удалённому источнику данных</b>, — реализовать интерфейс <code>IQueryable&lt;T&gt;</code>».</span>", source: "ms-linq" },
    { text: "Кастомные операторы обязаны быть отложенными: «Custom implementations of the standard query operators should use <span class=\"hl\">deferred execution</span> to return the results». <span class=\"ru-tr\">«Пользовательские реализации стандартных операторов запроса должны использовать отложенное выполнение, чтобы возвращать результаты».</span>", source: "ms-linq" },
  ],

  misconceptions: [
    {
      wrong: "LINQ — это про коллекции в памяти; SQL/XML/EF — отдельные технологии, не связанные с LINQ",
      hook: 'Узкое представление: <span class="wrong">LINQ работает только с коллекциями в памяти</span>. На деле LINQ — <b>провайдерная</b> модель: «You use the <span class="hl">same basic coding patterns</span> to query and transform data in XML documents, SQL databases, .NET collections, and any other format <b>when a LINQ provider is available</b>». <span class="ru-tr">«Вы используете одни и те же базовые шаблоны кода, чтобы запрашивать и преобразовывать данные в XML-документах, базах данных SQL, коллекциях .NET и любом другом формате, <b>когда доступен LINQ-провайдер</b>».</span> Один синтаксис <code>from…where…select</code> — разные бэкенды: LINQ to Objects (память), LINQ to XML, EF Core (→ SQL). Ниже <b>пять разборов</b>: провайдерная модель, in-memory провайдер, спектр IQueryable-провайдеров (простой→EF Core), как включить LINQ на своём типе, и <b>машинная панель</b> — in-memory провайдер <span class="hl">потоково</span> тянет <b>бесконечный</b> источник и берёт ровно 4 элемента (реальный прогон).',
      source: "ms-intro",
    },
  ],
  ...
  segments: [
    ...
    },
    {
      ...
      explain: 'In-memory провайдер — это LINQ to Objects. Дословно: «"LINQ to Objects" refers to the use of LINQ queries with <span class="hl">any <code>IEnumerable</code> or <code>IEnumerable&lt;T&gt;</code> collection directly</span>. You can use LINQ to query any enumerable collections, such as <code>List&lt;T&gt;</code>, <code>Array</code>, or <code>Dictionary&lt;TKey,TValue&gt;</code>. The collection can be user-defined or a type returned by a .NET API». <span class="ru-tr">«"LINQ to Objects" — это использование LINQ-запросов напрямую с любой коллекцией <code>IEnumerable</code> или <code>IEnumerable&lt;T&gt;</code>. С помощью LINQ можно запрашивать любые перечислимые коллекции, такие как <code>List&lt;T&gt;</code>, <code>Array</code> или <code>Dictionary&lt;TKey,TValue&gt;</code>. Коллекция может быть определена пользователем или быть типом, возвращаемым API .NET».</span> И способ включить его на своём типе: «If the data is of a type that implements <code>IEnumerable&lt;T&gt;</code>, <b>query the data by using LINQ to Objects</b>». <span class="ru-tr">«Если данные имеют тип, реализующий <code>IEnumerable&lt;T&gt;</code>, <b>запрашивайте их с помощью LINQ to Objects</b>».</span> Реализовал <code>IEnumerable&lt;T&gt;</code> (хоть через <code>yield return</code>) — получил весь набор операторов, исполняемых делегатами в процессе. Это докажет карточка с кастомным генератором.',
      sources: ["ms-intro", "ms-linq"],
    },
    {
      ...
      scenes: [
        { codeLine: 1, caption: '<b>Простой</b> провайдер: обращается к одному методу веб-сервиса, «closed type system» <span class="ru-tr">«закрытая система типов»</span>, часто исполняет запрос локально через <code>Enumerable</code>.', nodes: [{ id: "s", kind: "gate", at: { zone: "spectrum", row: 0 }, state: "ok", label: "простой", detail: "1 метод · closed types", accent: true }], edges: [] },
        ...
        { codeLine: 3, caption: '<b>Сложный</b> (EF Core): транслирует <span class="hl">полные LINQ-запросы в SQL</span>, открытая тип-система, обширный маппинг. «significant amount of effort». <span class="ru-tr">«значительный объём усилий».</span>', nodes: [{ id: "m", kind: "gate", at: { zone: "spectrum", row: 0 }, state: "ok", label: "средний", detail: "фиксир." }, { id: "c", kind: "gate", at: { zone: "spectrum", row: 1 }, state: "ok", label: "сложный · EF Core", detail: "весь LINQ → SQL", accent: true }], edges: [] },
      ],
      explain: 'Удалённые провайдеры реализуют <code>IQueryable&lt;T&gt;</code> и «can vary widely in their complexity». <span class="ru-tr">«могут сильно различаться по сложности».</span> Дословно: простой — «A less complex <code>IQueryable</code> provider might access a single method from a Web service… It has a <span class="hl">closed type system</span>… Most of the execution of the query occurs locally, for example by using the <code>Enumerable</code> implementations»; <span class="ru-tr">«Менее сложный провайдер <code>IQueryable</code> может обращаться к одному методу веб-сервиса… У него закрытая система типов… Большая часть выполнения запроса происходит локально, например с помощью реализаций <code>Enumerable</code>»;</span> средний — «has a richer type system than a simple provider, but it\'s still a <b>fixed type system</b>»; <span class="ru-tr">«имеет более богатую систему типов, чем простой провайдер, но это всё ещё <b>фиксированная система типов</b>»;</span> сложный — «A complex <code>IQueryable</code> provider, such as the <b>Entity Framework Core</b> provider, might <span class="hl">translate complete LINQ queries to an expressive query language, such as SQL</span>… It also has an <b>open type system</b>… Developing a complex provider requires a significant amount of effort». <span class="ru-tr">«Сложный провайдер <code>IQueryable</code>, такой как провайдер <b>Entity Framework Core</b>, может транслировать целые LINQ-запросы в выразительный язык запросов, например SQL… У него также есть <b>открытая система типов</b>… Разработка сложного провайдера требует значительного объёма усилий».</span> Отсюда практический вывод: возможности <code>IQueryable</code>-запроса ограничены тем, что провайдер умеет перевести.',
      sources: ["ms-linq"],
    },
    {
      ...
      explain: 'Как сделать свой источник LINQ-запрашиваемым — дословно: «You can enable LINQ querying of in-memory data in two ways. If the data is of a type that implements <code>IEnumerable&lt;T&gt;</code>, <span class="hl">query the data by using LINQ to Objects</span>. If it doesn\'t make sense to enable enumeration… define LINQ standard query operator methods either in that type or as <b>extension members</b> for that type. Custom implementations of the standard query operators should use <b>deferred execution</b> to return the results». <span class="ru-tr">«Включить LINQ-запросы к данным в памяти можно двумя способами. Если данные имеют тип, реализующий <code>IEnumerable&lt;T&gt;</code>, запрашивайте их с помощью LINQ to Objects. Если включать перечисление не имеет смысла… определите методы стандартных операторов запроса LINQ либо в самом типе, либо как <b>члены-расширения</b> для этого типа. Пользовательские реализации стандартных операторов запроса должны использовать <b>отложенное выполнение</b>, чтобы возвращать результаты».</span> А для удалённого: «The best option for enabling LINQ querying of a <b>remote data source</b> is to implement the <code>IQueryable&lt;T&gt;</code> interface». <span class="ru-tr">«Лучший вариант, чтобы включить LINQ-запросы к <b>удалённому источнику данных</b>, — реализовать интерфейс <code>IQueryable&lt;T&gt;</code>».</span> Итог: in-memory → <code>IEnumerable&lt;T&gt;</code> (или свои deferred-операторы); remote → <code>IQueryable&lt;T&gt;</code> (свой provider). Кастомные операторы — тема урока S3.8.',
      sources: ["ms-linq"],
    },
    {
  ...
  takeaways: [
    { icon: "why", k: "LINQ — провайдерная модель", v: '«same basic coding patterns to query… XML documents, SQL databases, .NET collections… when a LINQ provider is available». <span class="ru-tr">«одни и те же базовые шаблоны кода, чтобы запрашивать… XML-документы, базы данных SQL, коллекции .NET… когда доступен LINQ-провайдер».</span> Один <code>from…select</code>, разные бэкенды: Objects / XML / EF Core.' },
    ...
    { icon: "avoid", k: "включить на своём типе", v: 'In-memory: реализуй <code>IEnumerable&lt;T&gt;</code> (замер: кастомный/бесконечный итератор → весь LINQ работает, 3,6,9,12). Свои операторы «should use deferred execution». <span class="ru-tr">«должны использовать отложенное выполнение».</span> Remote → реализуй <code>IQueryable&lt;T&gt;</code>.' },
  ],
