...
export const listInternals: LessonData = {
  ...
  spec: [
    { text: "«Represents a strongly typed list of objects that can be accessed by index.» <span class=\"ru-tr\">«Представляет строго типизированный список объектов, к которым можно обращаться по индексу.»</span>", source: "ms-list" },
    { text: "«Capacity is the number of elements that the List&lt;T&gt; can store before resizing is required, whereas Count is the number of elements that are actually in the List&lt;T&gt;.» <span class=\"ru-tr\">«Capacity — это количество элементов, которые <code>List&lt;T&gt;</code> может хранить до того, как потребуется изменение размера, тогда как Count — это количество элементов, которые фактически находятся в <code>List&lt;T&gt;</code>.»</span>", source: "ms-list-capacity" },
  ],
  edgeCases: [
    { text: "«The List&lt;T&gt; class is the generic equivalent of the <code>ArrayList</code> class» <span class=\"ru-tr\">«Класс <code>List&lt;T&gt;</code> — это универсальный (generic) эквивалент класса <code>ArrayList</code>»</span> — та же идея (массив по индексу), но type-safe и без боксинга.", source: "ms-list" },
    { text: "«<code>List&lt;T&gt;</code> accepts <code>null</code> as a valid value for reference types and allows <b>duplicate</b> elements» <span class=\"ru-tr\">«<code>List&lt;T&gt;</code> принимает <code>null</code> как допустимое значение для ссылочных типов и разрешает <b>повторяющиеся</b> элементы»</span> — не множество, дубликаты и <code>null</code> разрешены.", source: "ms-list" },
  ...
  misconceptions: [
    {
      ...
      hook: 'Два разных мифа с одним источником — незнанием, что <code>List&lt;T&gt;</code> внутри. Первый: «связный список» — нет, «Represents a strongly typed list of objects that can be <span class="hl">accessed by index</span>» <span class="ru-tr">«Представляет строго типизированный список объектов, к которым можно обращаться по индексу»</span>, это <b>массив</b> (связный — отдельный <code>LinkedList&lt;T&gt;</code>). Второй: «ёмкость +1 на каждый Add» — нет, массив растёт <span class="wrong">скачками</span>: «If <code>Count</code> exceeds <code>Capacity</code> while adding elements, the capacity is increased by <b>automatically reallocating the internal array</b> before copying the old elements and adding the new elements» <span class="ru-tr">«Если <code>Count</code> превышает <code>Capacity</code> при добавлении элементов, ёмкость увеличивается путём <b>автоматической реаллокации внутреннего массива</b> перед копированием старых элементов и добавлением новых»</span> (страница <code>List&lt;T&gt;.Capacity</code>). Дальше <b>пять разборов</b> — от массива под капотом до <b>машинной панели</b>: реально снятый рост Capacity <b>0 → 4 → 8 → 16 → 32</b>, где 1000 <code>Add</code> дают всего <b>9 реаллокаций</b> (amortized O(1)).',
  ...
  segments: [
    {
      ...
      explain: 'За «списком» стоит массив: «Represents a strongly typed list of objects that can be <b>accessed by index</b>» <span class="ru-tr">«Представляет строго типизированный список объектов, к которым можно <b>обращаться по индексу</b>»</span>, и «The <code>List&lt;T&gt;</code> class is the <b>generic equivalent</b> of the <code>ArrayList</code> class» <span class="ru-tr">«Класс <code>List&lt;T&gt;</code> — это <b>универсальный (generic) эквивалент</b> класса <code>ArrayList</code>»</span>. Внутри — поле-массив <code>_items</code> и счётчик <code>_size</code> (= <code>Count</code>). Элементы лежат непрерывно, поэтому <code>list[i]</code> — это адресная арифметика <code>base + i·size</code>, а не обход узлов: индекс O(1) и amortized, и worst-case. Из массива же растут остальные свойства: дубликаты и <code>null</code> разрешены («accepts <code>null</code>… and allows duplicate elements» <span class="ru-tr">«принимает <code>null</code>… и разрешает повторяющиеся элементы»</span>), но порядок не гарантирован отсортированным («not guaranteed to be sorted» <span class="ru-tr">«не гарантированно отсортирован»</span>).',
    ...
    },
    {
      ...
      explain: 'Два числа: «<code>Capacity</code> is the number of elements that the <code>List&lt;T&gt;</code> can store <b>before resizing is required</b>, whereas <code>Count</code> is the number of elements that are actually in the <code>List&lt;T&gt;</code>» <span class="ru-tr">«<code>Capacity</code> — это количество элементов, которые <code>List&lt;T&gt;</code> может хранить <b>до того, как потребуется изменение размера</b>, тогда как <code>Count</code> — это количество элементов, которые фактически находятся в <code>List&lt;T&gt;</code>»</span>. Инвариант — «<code>Capacity</code> is always <b>greater than or equal to</b> <code>Count</code>» <span class="ru-tr">«<code>Capacity</code> всегда <b>больше или равна</b> <code>Count</code>»</span>. Само свойство определено как «the total number of elements the internal data structure can hold <b>without resizing</b>» <span class="ru-tr">«общее число элементов, которое внутренняя структура данных может вместить <b>без изменения размера</b>»</span>. Запас (Capacity − Count) — это свободные слоты в массиве; пока они есть, <code>Add</code> пишет в них за O(1), не трогая GC. Именно этот запас превращает O(n)-реаллокации в amortized O(1) на большинстве вставок.',
    ...
    },
    {
      ...
      explain: 'Рост — это не «+1», а реаллокация: «If <code>Count</code> exceeds <code>Capacity</code> while adding elements, the capacity is increased by <b>automatically reallocating the internal array</b> before copying the old elements and adding the new elements» <span class="ru-tr">«Если <code>Count</code> превышает <code>Capacity</code> при добавлении элементов, ёмкость увеличивается путём <b>автоматической реаллокации внутреннего массива</b> перед копированием старых элементов и добавлением новых»</span>. Дока гарантирует только сам факт реаллокации+копирования; <b>множитель ×2</b> — деталь реализации <code>List.cs</code> (dotnet/runtime), а не контракт документации (не приписывать «doubles» <span class="ru-tr">«удваивается»</span> как цитату Learn). Одна реаллокация — O(n) (копирование всех элементов), но происходит она всё реже по мере роста, поэтому суммарно <code>Add</code> — amortized O(1). Установка <code>Capacity</code> вручную тоже O(n): «setting the property is an O(<i>n</i>) operation» <span class="ru-tr">«установка этого свойства — операция сложности O(<i>n</i>)»</span>.',
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 3, out: "prealloc 1000: 0 реаллокаций", caption: '<code>new List&lt;int&gt;(1000)</code> — <span class="hl">0 реаллокаций</span>: место выделено сразу. Дока и советует «set the initial capacity to be the estimated size» <span class="ru-tr">«задайте начальную ёмкость равной предполагаемому размеру»</span>.', nodes: [{ id: "r", kind: "gate", at: { zone: "grow", row: 0, col: 0 }, state: "ok", label: "1000 Add", detail: "9 реаллок." }, { id: "p", kind: "gate", at: { zone: "grow", row: 0, col: 1 }, state: "ok", label: "prealloc", detail: "0 реаллок.", accent: true }], edges: [] },
      ],
      explain: 'Машинная панель — реально снятые числа. Рост <code>Capacity</code>: <b>0 → 4 → 8 → 16 → 32</b> (собственный прогон) — после первой аллокации на 4 массив <b>удваивается</b> (множитель ×2 — деталь реализации <code>List.cs</code>, не цитата доков, GT M8). Ключевой замер: <b>1000 Add = 9 реаллокаций</b> (Capacity 1024), а не 1000 — потому что каждое удвоение вдвое отдаляет следующее. Отсюда amortized O(1) на <code>Add</code>, хотя каждая отдельная реаллокация — O(n). Пред-выделение (<code>new List&lt;int&gt;(1000)</code>) даёт <b>0 реаллокаций</b>; дока: «The best way to avoid poor performance caused by multiple reallocations is to set the <b>initial capacity</b> to be the estimated size of the collection» <span class="ru-tr">«Лучший способ избежать низкой производительности из-за множественных реаллокаций — задать <b>начальную ёмкость</b> равной предполагаемому размеру коллекции»</span>. Совпадает с примером Learn: 5 <code>Add</code> → Capacity 8, <code>TrimExcess()</code> → 5.',
  ...
  cards: [
    ...
    },
    {
      ...
      noText: 'Начальная Capacity=1000 покрывает все 1000 <code>Add</code>: <code>Count</code> не превышает <code>Capacity</code>, реаллокаций <b>0</b>. Пред-выделение убирает копирования — «set the initial capacity» <span class="ru-tr">«задайте начальную ёмкость»</span>.',
      verify: { kind: "exec", run: "dotnet run", expect: "prealloc 1000: 0 реаллокаций" }, sourceRefs: ["ms-list-capacity", "ms-collections"],
    },
  ],

  takeaways: [
    { icon: "why", k: "Массив, не список", v: '<code>List&lt;T&gt;</code> — «accessed by index» <span class="ru-tr">«доступен по индексу»</span>, generic-эквивалент <code>ArrayList</code>: данные в непрерывном <code>int[]</code>. Индекс <code>list[i]</code> — O(1) (адресная арифметика). Связный список — отдельный <code>LinkedList&lt;T&gt;</code>.' },
    ...
    { icon: "avoid", k: "Пред-выделяй и не ищи линейно", v: '<code>new List&lt;T&gt;(n)</code> → <b>0</b> реаллокаций (дока: «set the initial capacity» <span class="ru-tr">«задайте начальную ёмкость»</span>). <code>Contains</code>/<code>IndexOf</code> — O(n): для частого членства бери <code>HashSet</code>. Дубликаты и <code>null</code> разрешены.' },
  ],
