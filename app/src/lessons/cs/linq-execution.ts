...
export const linqExecution: LessonData = {
  ...
  spec: [
    { text: "«All LINQ query operations consist of three distinct actions: Obtain the data source. Create the query. Execute the query.» <span class=\"ru-tr\">«Все операции LINQ-запроса состоят из трёх отдельных действий: получить источник данных. Создать запрос. Выполнить запрос.»</span>", source: "ms-intro" },
  ],
  edgeCases: [
    { text: "Query-переменная <b>ничего не делает и не возвращает данные</b>: «the query variable itself takes no action and returns no data. It just stores the information that is required to produce the results when the query is executed at some later point». <span class=\"ru-tr\">«сама query-переменная не выполняет никаких действий и не возвращает данных. Она лишь хранит информацию, необходимую для получения результатов, когда запрос будет выполнен в некоторый более поздний момент».</span>", source: "ms-intro" },
    { text: "Исполнение отделено от объявления: «In LINQ, the execution of the query is <span class=\"hl\">distinct from the query itself</span>. In other words, you don't retrieve any data by creating a query variable». <span class=\"ru-tr\">«В LINQ выполнение запроса отделено от самого запроса. Иными словами, создавая query-переменную, вы не извлекаете никаких данных».</span>", source: "ms-intro" },
    { text: "Результат зависит от <b>момента</b> исполнения: «The results of executing the query <span class=\"hl\">depend on the contents of the data source when the query is executed</span> rather than when the query is defined». <span class=\"ru-tr\">«Результаты выполнения запроса зависят от содержимого источника данных в момент выполнения запроса, а не в момент его определения».</span>", source: "ms-intro" },
  ],

  misconceptions: [
    {
      wrong: "создание query-переменной уже выполняет запрос и «забирает» данные из источника",
      hook: 'Частая ошибка: <span class="wrong">строка <code>var q = from x in src …</code> уже выполнила запрос</span> и <code>q</code> держит результат. Нет: создание запроса <b>не забирает</b> никаких данных. Дословно: «In LINQ, the <span class="hl">execution of the query is distinct from the query itself</span>. In other words, you don\'t retrieve any data by creating a query variable». <span class="ru-tr">«В LINQ выполнение запроса отделено от самого запроса. Иными словами, создавая query-переменную, вы не извлекаете никаких данных».</span> <code>q</code> хранит только <b>рецепт</b>. Ниже <b>пять разборов</b>: три действия запроса, query-переменная как рецепт, где данные на самом деле, момент исполнения (foreach), и <b>машинная панель</b> — один и тот же <code>q</code>, исполненный дважды вокруг мутации источника, даёт <b>2, потом 4</b> (реальный замер).',
      source: "ms-intro",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'Шаг 1 — <b>получить источник</b>: массив, коллекция, БД. Он должен быть «queryable» <span class="ru-tr">«поддающимся запросам»</span> (<code>IEnumerable&lt;T&gt;</code> или производный).', nodes: [{ id: "d", kind: "chip", at: { zone: "parts", row: 0 }, value: "1 · источник данных", accent: true }], edges: [] },
      ...
      explain: 'Каркас любой LINQ-операции — дословно: «All LINQ query operations consist of <span class="hl">three distinct actions: Obtain the data source. Create the query. Execute the query</span>». <span class="ru-tr">«Все операции LINQ-запроса состоят из трёх отдельных действий: получить источник данных. Создать запрос. Выполнить запрос».</span> Первый шаг — источник, поддерживающий <code>IEnumerable&lt;T&gt;</code>: «a LINQ data source is any object that supports the generic <code>IEnumerable&lt;T&gt;</code> interface, or an interface that inherits from it». <span class="ru-tr">«источник данных LINQ — это любой объект, поддерживающий обобщённый интерфейс <code>IEnumerable&lt;T&gt;</code> или интерфейс, унаследованный от него».</span> Второй — описание запроса. Третий — исполнение. Критично: «the <b>execution of the query is distinct from the query itself</b>. In other words, you don\'t retrieve any data by creating a query variable». <span class="ru-tr">«<b>выполнение запроса отделено от самого запроса</b>. Иными словами, создавая query-переменную, вы не извлекаете никаких данных».</span> Разделение «объявить/исполнить» — фундамент всего раздела.',
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 3, caption: 'Итог: «<b>the query variable itself takes no action and returns no data</b>» <span class="ru-tr">«<b>сама query-переменная не выполняет никаких действий и не возвращает данных</b>»</span> — рецепт хранит инфо, но не запускает чтение.', nodes: [{ id: "q", kind: "obj", at: { zone: "qvar", row: 0 }, typeTag: "q", value: "takes no action", accent: true }, { id: "s", kind: "obj", at: { zone: "src", row: 0 }, typeTag: "numbers", value: "[0..6]" }], edges: [{ id: "e", from: "q", to: "s" }] },
      ],
      explain: 'Что физически держит query-переменная — дословно: «A query is stored in a query variable and initialized with a query expression… the <span class="hl">query variable itself takes no action and returns no data</span>. It just stores the information that is required to produce the results when the query is executed at some later point». <span class="ru-tr">«Запрос хранится в query-переменной и инициализируется query-выражением… сама query-переменная не выполняет никаких действий и не возвращает данных. Она лишь хранит информацию, необходимую для получения результатов, когда запрос будет выполнен в некоторый более поздний момент».</span> То есть <code>q</code> — это упакованный рецепт: ссылка на источник + цепочка операций (фильтр, проекция). Никакого буфера с результатом внутри нет. Данные остаются в источнике до момента исполнения — что делает возможным трюк из машинной панели (мутируешь источник — меняется результат).',
      sources: ["ms-intro"],
    },
    {
      ...
      explain: 'Range-переменная — это НЕ переменная цикла в привычном смысле. Дословно (standard query operators overview): «The <b>range variable</b> is like the iteration variable in a <code>foreach</code> loop except that <span class="hl">no actual iteration occurs in a query expression</span>. When the query is executed, the range variable serves as a reference to each successive element». <span class="ru-tr">«<b>range-переменная</b> похожа на переменную итерации в цикле <code>foreach</code>, за исключением того, что в query-выражении не происходит фактической итерации. Когда запрос выполняется, range-переменная служит ссылкой на каждый последующий элемент».</span> И тип выводится: «Because the compiler can infer the type of <code>student</code>, you don\'t have to specify it explicitly». <span class="ru-tr">«Поскольку компилятор может вывести тип <code>student</code>, указывать его явно не требуется».</span> Смысл: <code>from x in src</code> объявляет «имя для каждого элемента», а перебор элементов случится позже, при исполнении — снова граница «объявить ≠ исполнить».',
      sources: ["ms-sqo"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 3, caption: 'Именно <code>foreach</code> «is also where the query results are retrieved» <span class="ru-tr">«также и место, где извлекаются результаты запроса»</span>: <code>x</code> держит каждое значение по одному за раз.', nodes: [{ id: "f", kind: "gate", at: { zone: "tl", row: 0 }, state: "ok", label: "foreach", detail: "retrieves results" }, { id: "x", kind: "gate", at: { zone: "tl", row: 1 }, state: "ok", label: "x", detail: "по одному элементу", accent: true }], edges: [] },
      ],
      explain: 'Где именно исполняется — дословно: «A query isn\'t executed until you <span class="hl">iterate over the query variable</span>, for example in a <code>foreach</code> statement». <span class="ru-tr">«Запрос не выполняется, пока вы не начнёте перебирать query-переменную, например в операторе <code>foreach</code>».</span> И далее: «The <code>foreach</code> statement is also where the query results are retrieved. For example, in the previous query, the iteration variable <code>num</code> holds each value (one at a time) in the returned sequence». <span class="ru-tr">«Оператор <code>foreach</code> — также и место, где извлекаются результаты запроса. Например, в предыдущем запросе переменная итерации <code>num</code> держит каждое значение (по одному за раз) из возвращённой последовательности».</span> То есть строка объявления запроса не трогает источник вовсе; чтение начинается на <code>foreach</code> (или на <code>ToList/Count</code> — они тоже итерируют внутри). Практическое следствие: тяжёлый запрос, объявленный, но не итерированный, ничего не стоит — цена появляется на перечислении.',
    ...
    },
    {
      ...
      explain: 'Машинная панель — прямое доказательство «query-переменная не хранит данные». Один и тот же <code>q</code> исполнен дважды: между вызовами мутируем <b>источник</b> (не запрос). Реальный вывод — <code>2 4</code>: второй <code>Count()</code> увидел добавленные элементы. Дословно: «The results of executing the query <span class="hl">depend on the contents of the data source when the query is executed rather than when the query is defined</span>. If the query variable is enumerated multiple times, the results might differ every time». <span class="ru-tr">«Результаты выполнения запроса зависят от содержимого источника данных в момент выполнения запроса, а не в момент его определения. Если query-переменную перечислять несколько раз, результаты каждый раз могут отличаться».</span> И: «Because the <b>query variable itself never holds the query results</b>, you can execute it repeatedly to retrieve updated data». <span class="ru-tr">«Поскольку <b>сама query-переменная никогда не хранит результаты запроса</b>, вы можете выполнять её многократно, чтобы получать обновлённые данные».</span> Если бы <code>q</code> держал снимок результата — было бы <code>2 2</code>. Он держит рецепт — поэтому <code>2 4</code>.',
      sources: ["ms-intro"],
    },
  ],
  ...
  cards: [
    ...
    },
    {
      ...
      noText: '«you don\'t retrieve any data by creating a query variable». <span class="ru-tr">«создавая query-переменную, вы не извлекаете никаких данных».</span> К моменту итерации <code>list = {1,2,5,9}</code>; фильтр &gt;1 → <b>2,5,9</b>. Снимка на строке объявления нет.',
    ...
    },
    {
      ...
      noText: '«the query variable itself never holds the query results, you can execute it repeatedly to retrieve updated data». <span class="ru-tr">«сама query-переменная никогда не хранит результаты запроса, вы можете выполнять её многократно, чтобы получать обновлённые данные».</span> #1 → 2, источник += {6,8}, #2 → 4. Реальный вывод: <b>2 4</b> (не 2 2).',
      verify: { kind: "exec", run: "dotnet run", expect: "2 4" }, sourceRefs: ["ms-intro"],
    },
  ],

  takeaways: [
    { icon: "why", k: "три отдельных действия", v: '«three distinct actions: Obtain the data source. Create the query. Execute the query». <span class="ru-tr">«три отдельных действия: получить источник данных. Создать запрос. Выполнить запрос».</span> Создание запроса — только описание; данные извлекаются на исполнении.' },
    { icon: "cost", k: "переменная = рецепт", v: '«the query variable itself takes no action and returns no data» <span class="ru-tr">«сама query-переменная не выполняет никаких действий и не возвращает данных»</span> — <code>q</code> хранит ссылку на источник + операции, не буфер результата. Объявить тяжёлый запрос без итерации — бесплатно.' },
    { icon: "avoid", k: "результат — от момента исполнения", v: 'Замер: тот же <code>q</code>, мутируем источник между исполнениями → <b>2, потом 4</b>. «results … depend on the contents of the data source when the query is executed». <span class="ru-tr">«результаты … зависят от содержимого источника данных в момент выполнения запроса».</span> Держишь свежесть — не кешируй.' },
  ],
