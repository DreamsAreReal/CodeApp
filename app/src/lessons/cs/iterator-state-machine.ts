...
export const iteratorStateMachine: LessonData = {
  ...
  spec: [
    { text: "«Although you write an iterator as a method, the compiler translates it into a nested class that is, in effect, a state machine.» <span class=\"ru-tr\">«Хотя итератор ты пишешь как метод, компилятор превращает его во вложенный класс, который, по сути, является стейт-машиной.»</span>", source: "ms-iter-pg" },
  ],
  edgeCases: [
    { text: 'Компилятор сам генерирует члены: «When the compiler detects the iterator, it automatically generates the <code>Current</code>, <code>MoveNext</code>, and <code>Dispose</code> methods». <span class="ru-tr">«Когда компилятор обнаруживает итератор, он автоматически генерирует методы <code>Current</code>, <code>MoveNext</code> и <code>Dispose</code>».</span> Вручную <code>IEnumerator</code> не пишешь.', source: "ms-iter-pg" },
    { text: 'Контракт <code>IEnumerator&lt;T&gt;</code>: «Initially, the enumerator is positioned before the first element… <code>Current</code> is undefined. Therefore, you must call <code>MoveNext</code>…» <span class="ru-tr">«Изначально энумератор расположен перед первым элементом… <code>Current</code> не определён. Поэтому нужно вызвать <code>MoveNext</code>…»</span>; за концом «<code>MoveNext</code> returns <code>false</code>». <span class="ru-tr">«<code>MoveNext</code> возвращает <code>false</code>».</span>', source: "ms-ienumerator" },
  ...
  misconceptions: [
    {
      ...
      hook: 'Два мифа о механике итераторов. «<span class="wrong">надо вручную реализовать IEnumerator</span>» — нет: «you don\'t have to implement the whole <code>IEnumerator</code> interface… it automatically generates the <code>Current</code>, <code>MoveNext</code>, and <code>Dispose</code> methods». <span class="ru-tr">«тебе не нужно реализовывать весь интерфейс <code>IEnumerator</code>… он автоматически генерирует методы <code>Current</code>, <code>MoveNext</code> и <code>Dispose</code>».</span> Компилятор превращает твой метод в <b>nested-класс = стейт-машину</b>. «<span class="wrong">Reset() перезапускает</span>» — нет: «Iterators don\'t support the <code>IEnumerator.Reset</code> method… Calling <code>Reset</code>… throws a <code>NotSupportedException</code>». <span class="ru-tr">«Итераторы не поддерживают метод <code>IEnumerator.Reset</code>… Вызов <code>Reset</code>… бросает <code>NotSupportedException</code>».</span>; чтобы начать сначала — <b>новый</b> итератор. Ниже <b>пять разборов</b>: метод → nested-класс, сгенерированные члены, контракт <code>IEnumerator&lt;T&gt;</code> (перед первым → MoveNext → false), <code>Reset</code> бросает, и <b>машинная панель</b> — реальная рефлексия: тип <code>&lt;G&gt;d__0</code>, поле <code>&lt;&gt;1__state</code> = <b>-2</b> до обхода (Roslyn .NET 10).',
      source: "ms-iter-pg",
    },
  ],
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: 'Компилятор генерирует <span class="hl">nested-класс</span> <code>&lt;G&gt;d__0</code> — «in effect, a state machine» <span class="ru-tr">«по сути, стейт-машина»</span>, которая отслеживает позицию обхода.', nodes: [{ id: "m", kind: "gate", at: { zone: "src", row: 0 }, state: "ok", label: "G()", detail: "метод" }, { id: "sm", kind: "obj", at: { zone: "gen", row: 0 }, typeTag: "<G>d__0", value: "стейт-машина", accent: true }], edges: [{ id: "e1", from: "m", to: "sm" }] },
      ...
      explain: 'Ключевой факт S18.3: «Although you write an iterator as a method, the compiler translates it into a nested class that is, in effect, a state machine. This class keeps track of the position of the iterator as long the <code>foreach</code> loop in the client code continues». <span class="ru-tr">«Хотя итератор ты пишешь как метод, компилятор превращает его во вложенный класс, который, по сути, является стейт-машиной. Этот класс отслеживает позицию итератора, пока продолжается цикл <code>foreach</code> в клиентском коде».</span> То есть <code>yield return</code> — синтаксис, за которым компилятор строит объект-автомат: <code>&lt;G&gt;d__0</code> (имя — Roslyn-generated, .NET 10; провенанс — рефлексия, не Learn). А <code>foreach</code> — не магия и не копия: он разворачивается в <code>GetEnumerator()</code> + <code>while(MoveNext()){ Current }</code> + <code>Dispose()</code> в <code>finally</code> (обход через энумератор, без снимка коллекции — поэтому мутация коллекции во время обхода делает поведение undefined).',
      sources: ["ms-iter-pg"],
    },
    {
      ...
      scenes: [
        { codeLine: 1, caption: '<span class="hl"><code>MoveNext()</code></span> — «resumes after the previous <code>yield return</code>» <span class="ru-tr">«возобновляет после предыдущего <code>yield return</code>»</span> и идёт до следующего. Возвращает <code>true</code>/<code>false</code>.', nodes: [{ id: "mn", kind: "gate", at: { zone: "members", row: 0, col: 0 }, state: "ok", label: "MoveNext()", detail: "bool", accent: true }], edges: [] },
        { codeLine: 2, caption: '<span class="hl"><code>Current</code></span> — текущий элемент. Оба сгенерированы: «you don\'t have to implement the whole <code>IEnumerator</code> interface». <span class="ru-tr">«тебе не нужно реализовывать весь интерфейс <code>IEnumerator</code>».</span>', nodes: [{ id: "mn", kind: "gate", at: { zone: "members", row: 0, col: 0 }, state: "ok", label: "MoveNext()", detail: "bool" }, { id: "cur", kind: "gate", at: { zone: "members", row: 0, col: 1 }, state: "ok", label: "Current", detail: "T", accent: true }], edges: [] },
        { codeLine: 3, caption: 'И <span class="hl"><code>Dispose()</code></span> — «it automatically generates the <code>Current</code>, <code>MoveNext</code>, and <code>Dispose</code> methods». <span class="ru-tr">«он автоматически генерирует методы <code>Current</code>, <code>MoveNext</code> и <code>Dispose</code>».</span> Сюда попадает <code>finally</code> от <code>using</code>.', nodes: [{ id: "mn", kind: "gate", at: { zone: "members", row: 0, col: 0 }, state: "ok", label: "MoveNext", detail: "bool" }, { id: "cur", kind: "gate", at: { zone: "members", row: 0, col: 1 }, state: "ok", label: "Current", detail: "T" }, { id: "dis", kind: "gate", at: { zone: "members", row: 1 }, state: "ok", label: "Dispose()", detail: "очистка / finally", accent: true }], edges: [] },
      ],
      explain: 'Ты не реализуешь энумератор руками: «When you create an iterator for a class or struct, you don\'t have to implement the whole <code>IEnumerator</code> interface. When the compiler detects the iterator, it automatically generates the <code>Current</code>, <code>MoveNext</code>, and <code>Dispose</code> methods of the <code>IEnumerator</code> or <code>IEnumerator&lt;T&gt;</code> interface». <span class="ru-tr">«Когда ты создаёшь итератор для класса или структуры, тебе не нужно реализовывать весь интерфейс <code>IEnumerator</code>. Когда компилятор обнаруживает итератор, он автоматически генерирует методы <code>Current</code>, <code>MoveNext</code> и <code>Dispose</code> интерфейса <code>IEnumerator</code> или <code>IEnumerator&lt;T&gt;</code>».</span> А как именно <code>MoveNext</code> двигает тело: «On each successive iteration of the <code>foreach</code> loop (or the direct call to <code>IEnumerator.MoveNext</code>), the next iterator code body resumes after the previous <code>yield return</code> statement. It then continues to the next <code>yield return</code> statement until the end of the iterator body is reached, or until a <code>yield break</code> statement is encountered». <span class="ru-tr">«На каждой очередной итерации цикла <code>foreach</code> (или при прямом вызове <code>IEnumerator.MoveNext</code>) следующая часть кода итератора возобновляется после предыдущего оператора <code>yield return</code>. Затем она продолжается до следующего оператора <code>yield return</code>, пока не будет достигнут конец тела итератора или не встретится оператор <code>yield break</code>».</span> В сгенерированном <code>&lt;G&gt;d__0</code> реальная рефлексия видит <code>MoveNext</code>, <code>get_Current</code>, <code>Dispose</code>, а также поля состояния (машинная панель, разбор 05).',
      sources: ["ms-iter-pg"],
    },
    {
      ...
      console: true,
      scenes: [
        { codeLine: 0, out: "", caption: 'Свежий энумератор стоит <span class="hl">ПЕРЕД</span> первым элементом; <code>Current</code> ещё не определён. «Initially, the enumerator is positioned before the first element». <span class="ru-tr">«Изначально энумератор расположен перед первым элементом».</span>', nodes: [{ id: "e", kind: "gate", at: { zone: "contract", row: 0, col: 0 }, state: "ok", label: "перед первым", detail: "Current undefined", accent: true }], edges: [] },
        { codeLine: 1, out: "m1=True:10", caption: 'Первый <code>MoveNext()</code> → <code>true</code>, ставит <span class="hl"><code>Current = 10</code></span>. «you must call <code>MoveNext</code>… before reading… <code>Current</code>». <span class="ru-tr">«нужно вызвать <code>MoveNext</code>… перед чтением… <code>Current</code>».</span>', nodes: [{ id: "e", kind: "gate", at: { zone: "contract", row: 0, col: 0 }, state: "ok", label: "MoveNext #1", detail: "true" }, { id: "c1", kind: "gate", at: { zone: "contract", row: 0, col: 1 }, state: "ok", label: "Current", detail: "10", accent: true }], edges: [] },
        { codeLine: 3, out: "m1=True:10 m2=True:20 m3=False", caption: 'Второй → <code>20</code>; третий — <span class="hl"><code>false</code></span> (за концом). «If <code>MoveNext</code> passes the end… <code>MoveNext</code> returns <code>false</code>». <span class="ru-tr">«Если <code>MoveNext</code> проходит за конец… <code>MoveNext</code> возвращает <code>false</code>».</span> Вывод: <b>m1=True:10 m2=True:20 m3=False</b>.', nodes: [{ id: "c2", kind: "gate", at: { zone: "contract", row: 0, col: 0 }, state: "ok", label: "MoveNext #2", detail: "20" }, { id: "end", kind: "gate", at: { zone: "contract", row: 0, col: 1 }, state: "fail", label: "MoveNext #3", detail: "false", accent: true }], edges: [] },
      ],
      explain: 'Панель, куда «садится» стейт-машина, — контракт <code>IEnumerator&lt;T&gt;</code> (S4): «Initially, the enumerator is positioned before the first element in the collection. At this position, <code>Current</code> is undefined. Therefore, you must call <code>MoveNext</code> to advance the enumerator to the first element of the collection before reading the value of <code>Current</code>». <span class="ru-tr">«Изначально энумератор расположен перед первым элементом коллекции. В этой позиции <code>Current</code> не определён. Поэтому нужно вызвать <code>MoveNext</code>, чтобы продвинуть энумератор к первому элементу коллекции, прежде чем читать значение <code>Current</code>».</span> Далее: «<code>Current</code> returns the same object until <code>MoveNext</code> is called. <code>MoveNext</code> sets <code>Current</code> to the next element». <span class="ru-tr">«<code>Current</code> возвращает один и тот же объект, пока не вызван <code>MoveNext</code>. <code>MoveNext</code> устанавливает <code>Current</code> на следующий элемент».</span> За концом: «If <code>MoveNext</code> passes the end of the collection, the enumerator is positioned after the last element in the collection and <code>MoveNext</code> returns <code>false</code>… If the last call to <code>MoveNext</code> returned <code>false</code>, <code>Current</code> is undefined». <span class="ru-tr">«Если <code>MoveNext</code> проходит за конец коллекции, энумератор располагается после последнего элемента коллекции, и <code>MoveNext</code> возвращает <code>false</code>… Если последний вызов <code>MoveNext</code> вернул <code>false</code>, <code>Current</code> не определён».</span> Реальный прогон подтверждает: <code>m1=True:10 m2=True:20 m3=False</code>. Именно этот контракт стейт-машина реализует полями состояния.',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, caption: '<code>Reset()</code> на итераторе <span class="hl">бросает</span>: «Iterators don\'t support the <code>IEnumerator.Reset</code> method… throws a <code>NotSupportedException</code>». <span class="ru-tr">«Итераторы не поддерживают метод <code>IEnumerator.Reset</code>… бросает <code>NotSupportedException</code>».</span>', nodes: [{ id: "r", kind: "gate", at: { zone: "reset", row: 0 }, state: "fail", label: "e.Reset()", detail: "NotSupportedException", accent: true }], edges: [] },
        ...
        { codeLine: 2, caption: 'Чтобы пройти заново — <span class="hl">новый</span> итератор: «To reiterate from the start, you must obtain a new iterator». <span class="ru-tr">«Чтобы пройти заново с начала, нужно получить новый итератор».</span> Свежая машина со свежим состоянием.', nodes: [{ id: "r", kind: "gate", at: { zone: "reset", row: 0 }, state: "fail", label: "Reset", detail: "нет" }, { id: "nw", kind: "gate", at: { zone: "newit", row: 0 }, state: "ok", label: "Nums().GetEnumerator()", detail: "новый обход", accent: true }], edges: [{ id: "e1", from: "r", to: "nw" }] },
      ],
      explain: 'Итераторы <b>однопроходны</b>: «Iterators don\'t support the <code>IEnumerator.Reset</code> method. To reiterate from the start, you must obtain a new iterator. Calling <code>Reset</code> on the iterator returned by an iterator method throws a <code>NotSupportedException</code>». <span class="ru-tr">«Итераторы не поддерживают метод <code>IEnumerator.Reset</code>. Чтобы пройти заново с начала, нужно получить новый итератор. Вызов <code>Reset</code> на итераторе, возвращённом методом-итератором, бросает <code>NotSupportedException</code>».</span> Причина — стейт-машина хранит позицию в поле состояния и не реализует откат; сам <code>Reset</code> в API «is provided for COM interoperability… the implementer can simply throw a <code>NotSupportedException</code>» <span class="ru-tr">«предоставляется для совместимости с COM… реализующий может просто бросить <code>NotSupportedException</code>»</span> — что компилятор и делает. Практический вывод: если нужно пройти последовательность дважды, вызови метод-итератор ещё раз (получишь новую машину), а не пытайся «перемотать» существующий энумератор. Реальный прогон в машинной панели (05) ловит этот бросок: <code>resetThrows=True</code>.',
  ...
  takeaways: [
    { icon: "why", k: "метод → автомат", v: 'Компилятор превращает итератор в <span class="hl">nested-класс = стейт-машину</span> (<code>&lt;G&gt;d__0</code>, Roslyn .NET 10). «the compiler translates it into a nested class that is, in effect, a state machine». <span class="ru-tr">«компилятор превращает его во вложенный класс, который, по сути, является стейт-машиной».</span> <code>foreach</code> обходит через энумератор, не копирует коллекцию.' },
