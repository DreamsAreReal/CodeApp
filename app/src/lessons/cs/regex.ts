...
export const regex: LessonData = {
  ...
  spec: [
    { text: "«The centerpiece of text processing with regular expressions is the regular expression engine, which is represented by the System.Text.RegularExpressions.Regex object in .NET.» <span class=\"ru-tr\">«Центральный элемент обработки текста регулярными выражениями — движок регулярных выражений, представленный объектом System.Text.RegularExpressions.Regex в .NET.»</span>", source: "ms-regex" },
  ],
  edgeCases: [
    { text: "У <code>Regex</code> один pattern — несколько операций: «Determine whether the regular expression pattern occurs in the input text by calling the <span class=\"hl\">System.Text.RegularExpressions.Regex.IsMatch</span> method». <span class=\"ru-tr\">«Определите, встречается ли шаблон регулярного выражения во входном тексте, вызвав метод <code>System.Text.RegularExpressions.Regex.IsMatch</code>».</span> <code>Matches</code> возвращает коллекцию, что «contains one System.Text.RegularExpressions.Match object for each match found in the parsed text». <span class=\"ru-tr\">«Содержит один объект System.Text.RegularExpressions.Match на каждое совпадение, найденное в разобранном тексте».</span> Собственный прогон: <code>IsMatch</code>=True, <code>Match.Value</code>=555-1234.", source: "ms-regex" },
    { text: "<code>new Regex(...)</code> по умолчанию идёт на <b>интерпретатор</b>: «When instantiating a new <code>Regex</code> instance or calling one of the static methods on <code>Regex</code>, the <span class=\"hl\">interpreter is the default engine</span> employed». <span class=\"ru-tr\">«При создании нового экземпляра <code>Regex</code> или вызове одного из статических методов у <code>Regex</code> по умолчанию используется движок-интерпретатор».</span> Собственный прогон подтверждает поведение движка (детерминированные совпадения).", source: "ms-srcgen" },
    { text: "<code>RegexOptions.Compiled</code> эмитит IL: «The resulting instructions are transformed further by the <span class=\"hl\">reflection-emit-based compiler into IL instructions</span> that are written to a few System.Reflection.Emit.DynamicMethod objects». <span class=\"ru-tr\">«Полученные инструкции далее преобразуются компилятором на базе reflection-emit в IL-инструкции, которые записываются в несколько объектов System.Reflection.Emit.DynamicMethod».</span> Это «a fundamental tradeoff between overheads on the first use and overheads on every subsequent use». <span class=\"ru-tr\">«Фундаментальный компромисс между накладными расходами на первое использование и накладными расходами на каждое последующее использование».</span> Собственный прогон: Compiled даёт тот же результат (Count=3).", source: "ms-srcgen" },
    { text: "<code>[GeneratedRegex]</code> — <b>compile-time</b>, не рантайм. Рекомендация доков — предпочесть source generation режиму <code>Compiled</code>, потому что «Source generation can help your app <span class=\"hl\">start faster, run more quickly, and be more trimmable</span>». <span class=\"ru-tr\">«Генерация исходного кода помогает вашему приложению быстрее стартовать, быстрее работать и лучше поддаваться триммингу».</span> Источник-генератор не исполняется в песочнице — доказан concept, поведение снято на <code>Compiled</code>.", source: "ms-srcgen" },
  ],
  ...
  misconceptions: [
    {
      ...
      hook: 'Нет — за одним <code>pattern</code> стоит <b>движок</b> с несколькими реализациями, и выбор между ними — это трейдоф, а не «всегда быстрее». «The centerpiece of text processing with regular expressions is the <b>regular expression engine</b>, which is represented by the <b>System.Text.RegularExpressions.Regex</b> object in .NET». <span class="ru-tr">«Центральный элемент обработки текста регулярными выражениями — это <b>движок регулярных выражений</b>, представленный объектом <b>System.Text.RegularExpressions.Regex</b> в .NET».</span> Когда ты пишешь <code>new Regex(...)</code>, «the <span class="hl">interpreter is the default engine</span> employed» <span class="ru-tr">«по умолчанию используется движок-интерпретатор»</span> — он просто «walks through those instructions» <span class="ru-tr">«просто идёт по этим инструкциям»</span> (опкоды). <code>RegexOptions.Compiled</code> добавляет «reflection-emit-based compiler into IL instructions… written to a few System.Reflection.Emit.DynamicMethod objects» <span class="ru-tr">«компилятор на базе reflection-emit преобразует их в IL-инструкции… записанные в несколько объектов System.Reflection.Emit.DynamicMethod»</span> — быстрее в устоявшемся режиме, но это «a <b>fundamental tradeoff</b> between overheads on the first use and overheads on every subsequent use» <span class="ru-tr">«<b>фундаментальный компромисс</b> между накладными расходами на первое использование и на каждое последующее»</span> (дорогой старт). А <code>[GeneratedRegex]</code> переносит всю работу на <b>компиляцию</b>: «use source-generated regular expressions instead of compiling regular expressions» <span class="ru-tr">«используйте source-generated регулярные выражения вместо компиляции регулярных выражений»</span> — что помогает «<span class="hl">start faster, run more quickly, and be more trimmable</span>» <span class="ru-tr">«быстрее стартовать, быстрее работать и лучше поддаваться триммингу»</span>. Три разных engine — три разных профиля. Дальше <b>пять разборов</b>: движок и два входа, методы Regex, interpreter vs Compiled, GeneratedRegex как compile-time, и <b>машинная панель</b> — Compiled даёт тот же результат, что интерпретатор (реальный прогон: Count 3, [2]=three). <i>Примечание: <code>[GeneratedRegex]</code>-генератор не исполняется в песочнице — он показан как концепт из доков, а рантайм-поведение доказано на <code>Compiled</code>/<code>new Regex</code>.</i>',
  ...
  segments: [
    {
      ...
      scenes: [
        ...
        { codeLine: 0, out: "", caption: 'Вход 1 — <b>pattern</b>: «The regular expression pattern to identify in the text». <span class="ru-tr">«Шаблон регулярного выражения для поиска в тексте».</span> Синтаксис «compatible with Perl 5». <span class="ru-tr">«совместим с Perl 5».</span>', nodes: [{ id: "e", kind: "obj", at: { zone: "engine", row: 0 }, typeTag: "Regex", value: "движок" }, { id: "p", kind: "chip", at: { zone: "inputs", row: 0 }, value: '@"\\d{3}-\\d{4}"', accent: true }], edges: [{ id: "ep", from: "p", to: "e", accent: true }] },
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'По умолчанию pattern превращается в <b>опкоды</b>, и «the <span class="hl">interpreter</span> simply walks through those instructions». <span class="ru-tr">«интерпретатор просто идёт по этим инструкциям».</span> Дёшево создать, медленнее матчить.', nodes: [{ id: "i", kind: "gate", at: { zone: "interp", row: 0 }, state: "ok", label: "interpreter", detail: "walks opcodes · default", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: '<code>RegexOptions.Compiled</code>: «reflection-emit-based compiler into <span class="hl">IL instructions</span>… written to a few System.Reflection.Emit.DynamicMethod objects». <span class="ru-tr">«компилятор на базе reflection-emit — в IL-инструкции… записанные в несколько объектов System.Reflection.Emit.DynamicMethod».</span> Быстрее матчить.', nodes: [{ id: "i", kind: "gate", at: { zone: "interp", row: 0 }, state: "ok", label: "interpreter", detail: "opcodes" }, { id: "c", kind: "gate", at: { zone: "compiled", row: 0 }, state: "ok", label: "Compiled", detail: "IL → DynamicMethod", accent: true }], edges: [] },
        { codeLine: 1, out: "", caption: 'Но это трейдоф: «a <b>fundamental tradeoff</b> between overheads on the <span class="hl">first use</span> and overheads on every <span class="hl">subsequent use</span>». <span class="ru-tr">«<b>фундаментальный компромисс</b> между накладными расходами на первое использование и на каждое последующее использование».</span> Дорогой старт (эмит + JIT) ради быстрого стедистейта.', nodes: [{ id: "t", kind: "gate", at: { zone: "compiled", row: 0 }, state: "fail", label: "tradeoff", detail: "дорогой первый запуск", accent: true }], edges: [] },
      ],
      explain: 'За одним pattern скрыты два движка. По умолчанию — <b>интерпретатор</b>: «When you write <code>new Regex("somepattern")</code>… The tree is written into a form that can be interpreted as a series of opcodes and operands… the interpreter simply walks through those instructions». <span class="ru-tr">(дерево записывается в набор опкодов и операндов; интерпретатор просто идёт по этим инструкциям).</span> И «When instantiating a new <code>Regex</code> instance or calling one of the static methods on <code>Regex</code>, the <span class="hl">interpreter is the default engine</span> employed». <span class="ru-tr">(при создании нового Regex или вызове статических методов по умолчанию используется интерпретатор).</span> С <code>RegexOptions.Compiled</code>: «The resulting instructions are transformed further by the <b>reflection-emit-based compiler into IL instructions</b> that are written to a few System.Reflection.Emit.DynamicMethod objects» <span class="ru-tr">«полученные инструкции далее преобразуются <b>компилятором на базе reflection-emit в IL-инструкции</b>, которые записываются в несколько объектов System.Reflection.Emit.DynamicMethod»</span> — это «some of the main reasons that specifying <b>RegexOptions.Compiled</b> yields <b>much faster-matching throughput</b> than the interpreter». <span class="ru-tr">«одна из главных причин, почему указание <b>RegexOptions.Compiled</b> даёт <b>заметно бо́льшую пропускную способность матчинга</b>, чем интерпретатор».</span> Цена: «<span class="hl">a fundamental tradeoff between overheads on the first use and overheads on every subsequent use</span>» <span class="ru-tr">«фундаментальный компромисс между накладными расходами на первое использование и на каждое последующее использование»</span> + инхибирует trimming/AOT (эмит запрещён в некоторых средах, там <code>Compiled</code> «becomes a no-op» <span class="ru-tr">«становится пустой операцией»</span>). Собственный прогон подтвердит: результат матча <b>тот же</b>, что у интерпретатора.',
      sources: ["ms-srcgen"],
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 2, out: "", caption: '<code>[GeneratedRegex]</code> на <code>partial</code>-методе → «A source generator… <span class="hl">plugs into the compiler</span> and augments the compilation unit». <span class="ru-tr">«source generator… встраивается в компилятор и дополняет единицу компиляции».</span> Код генерится на <b>билде</b>.', nodes: [{ id: "r", kind: "gate", at: { zone: "runtime", row: 0 }, state: "fail", label: "Compiled", detail: "рантайм" }, { id: "g", kind: "obj", at: { zone: "buildtime", row: 0 }, typeTag: "[GeneratedRegex]", value: "код на компиляции", accent: true }], edges: [] },
        { codeLine: 3, out: "", caption: 'Выигрыш: «start faster, run more quickly, and be more <span class="hl">trimmable</span>». <span class="ru-tr">«быстрее стартовать, быстрее работать и лучше поддаваться триммингу».</span> Рекомендация доков — предпочесть его <code>Compiled</code>, где возможно.', nodes: [{ id: "g", kind: "gate", at: { zone: "buildtime", row: 0 }, state: "ok", label: "source-generated", detail: "startup · trimmable", accent: true }], edges: [] },
      ],
  ...
  cards: [
    {
      ...
      noText: '<code>IsMatch</code> = «occurs in the input text» <span class="ru-tr">«встречается во входном тексте»</span> → True. <code>Match.Value</code> — только совпавшая подстрока (555-1234), не весь input. Реальный вывод: <b>True</b>, <b>555-1234</b>.',
    ...
    },
    {
      ...
      okText: '<code>Matches</code> = «one System.Text.RegularExpressions.Match object for each match». <span class="ru-tr">«один объект System.Text.RegularExpressions.Match на каждое совпадение».</span> <code>\\w+</code> находит <b>3</b> слова; <code>ms[2]</code> — третье: <b>three</b>. <code>Compiled</code> (IL) даёт <span class="hl">тот же</span> результат, что интерпретатор.',
  ...
  takeaways: [
    { icon: "why", k: "Движок + pattern + text", v: '«the regular expression engine… represented by the <b>System.Text.RegularExpressions.Regex</b> object». <span class="ru-tr">«движок регулярных выражений… представленный объектом <b>System.Text.RegularExpressions.Regex</b>».</span> Один Regex → <code>IsMatch</code>/<code>Match</code>/<code>Matches</code>/<code>Replace</code>. <code>Match</code> несёт <code>Value</code>/<code>Index</code>/<code>Groups</code> (замер: IsMatch=True, Value=555-1234).' },
    { icon: "cost", k: "Interpreter vs Compiled", v: '<code>new Regex(...)</code> → «the <b>interpreter is the default engine</b>» <span class="ru-tr">«<b>по умолчанию используется движок-интерпретатор</b>»</span> (opcodes). <code>RegexOptions.Compiled</code> → «reflection-emit… into <b>IL</b>… DynamicMethod» <span class="ru-tr">«reflection-emit… в <b>IL</b>… DynamicMethod»</span> — «fundamental tradeoff» <span class="ru-tr">«фундаментальный компромисс»</span> (дорогой старт, быстрый матч). Результат тот же (замер: Count 3).' },
    { icon: "avoid", k: "GeneratedRegex = compile-time", v: '«use <b>source-generated</b>… instead of… <code>RegexOptions.Compiled</code>… start faster, run more quickly, and be more <b>trimmable</b>». <span class="ru-tr">«используйте <b>source-generated</b>… вместо… <code>RegexOptions.Compiled</code>… быстрее стартовать, быстрее работать и лучше поддаваться <b>триммингу</b>».</span> <code>[GeneratedRegex]</code> генерит код на билде. Source-gen не бежит в песочнице — доказан концептуально, поведение снято на Compiled.' },
  ],
