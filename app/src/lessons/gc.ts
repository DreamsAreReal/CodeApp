  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'Все reference-типы живут на <b>управляемой куче</b>. Куча держит <span class="hl">указатель</span> на адрес следующего объекта.', nodes: [{ id: "a", kind: "obj", x: 90, y: 96, w: 96, h: 46, typeTag: "object", value: "" , accent: true }, { id: "ptr", kind: "chip", x: 158, y: 150, w: 44, h: 26, value: "next→" }], edges: [] },
        { codeLine: 1, caption: 'Следующий объект аллоцируется <span class="hl">сразу за</span> предыдущим — «adding a value to a pointer», почти как стек по скорости.', nodes: [{ id: "a", kind: "obj", x: 90, y: 96, w: 96, h: 46, typeTag: "object", value: "" }, { id: "b", kind: "obj", x: 210, y: 96, w: 96, h: 46, typeTag: "byte[16]", value: "", accent: true }, { id: "ptr", kind: "chip", x: 278, y: 150, w: 44, h: 26, value: "next→" }], edges: [] },
        { codeLine: 2, caption: 'Куча — <span class="hl">общая</span>: «There\'s a managed heap for each managed process. All threads… allocate on the same heap».', nodes: [{ id: "a", kind: "obj", x: 90, y: 96, w: 96, h: 46, typeTag: "object", value: "" }, { id: "b", kind: "obj", x: 210, y: 96, w: 96, h: 46, typeTag: "byte[16]", value: "" }, { id: "ptr", kind: "chip", x: 278, y: 150, w: 44, h: 26, value: "next→", accent: true }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 1, caption: 'Gen 0 <span class="hl">заполнилась</span> → «If an application attempts to create a new object when generation 0 is full, the garbage collector performs a collection».', nodes: [{ id: "gate", kind: "gate", x: 64, y: 118, w: 88, h: 44, state: "ok", label: "gen 0 полна", detail: "→ collect gen 0" }], edges: [] },
        { codeLine: 1, caption: 'Сборщик смотрит <span class="hl">только gen 0</span>, не всю кучу: «starts by examining the objects in generation 0 rather than all objects in the managed heap».', nodes: [{ id: "gate", kind: "gate", x: 64, y: 118, w: 88, h: 44, state: "ok", label: "GC · gen 0", detail: "не вся куча" }, { id: "g2", kind: "chip", x: 276, y: 118, w: 44, h: 26, value: "нетронут" }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        ...
        { codeLine: 0, caption: '<b>Relocate</b>: корни <span class="hl">перенастраиваются</span> на новые адреса — «makes the necessary pointer corrections so that the… roots point to the objects in their new locations».', nodes: [{ id: "A", kind: "obj", x: 60, y: 108, w: 50, h: 44, typeTag: "A", value: "✓" }, { id: "C", kind: "obj", x: 130, y: 108, w: 50, h: 44, typeTag: "C", value: "✓" }, { id: "root", kind: "ref", x: 130, y: 170, w: 84, h: 28, name: "root→C", accent: true }], edges: [{ id: "r", from: "root", to: "C", accent: true }] },
      ],
