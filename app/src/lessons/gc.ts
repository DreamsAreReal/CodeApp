  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'Все reference-типы живут на <b>управляемой куче</b>. Куча держит <span class="hl">указатель</span> на адрес следующего объекта.', nodes: [{ id: "a", kind: "obj", x: 96, y: 108, typeTag: "object", value: "", accent: true }, { id: "ptr", kind: "chip", x: 96, y: 168, value: "next→" }], edges: [] },
        { codeLine: 1, caption: 'Следующий объект аллоцируется <span class="hl">сразу за</span> предыдущим — «adding a value to a pointer», почти как стек по скорости.', nodes: [{ id: "a", kind: "obj", x: 96, y: 108, typeTag: "object", value: "" }, { id: "b", kind: "obj", x: 240, y: 108, typeTag: "byte[16]", value: "", accent: true }, { id: "ptr", kind: "chip", x: 240, y: 168, value: "next→" }], edges: [] },
        { codeLine: 2, caption: 'Куча — <span class="hl">общая</span>: «There\'s a managed heap for each managed process. All threads… allocate on the same heap».', nodes: [{ id: "a", kind: "obj", x: 96, y: 108, typeTag: "object", value: "" }, { id: "b", kind: "obj", x: 240, y: 108, typeTag: "byte[16]", value: "" }, { id: "ptr", kind: "chip", x: 240, y: 168, value: "next→", accent: true }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'Куча делится на <b>три поколения</b>. Идея: «Newer objects have shorter lifetimes, and older objects have longer lifetimes».', nodes: [{ id: "g0", kind: "chip", x: 64, y: 120, value: "0" }, { id: "g1", kind: "chip", x: 170, y: 120, value: "1" }, { id: "g2", kind: "chip", x: 276, y: 120, value: "2" }], edges: [] },
        { codeLine: 1, caption: 'Новый объект попадает в <span class="hl">gen 0</span> — «the garbage collector stores new objects in generation 0». Gen 0 — самое молодое и короткоживущее.', nodes: [{ id: "o", kind: "obj", x: 64, y: 118, typeTag: "object", value: "new", accent: true }, { id: "g1", kind: "chip", x: 170, y: 150, value: "gen 1" }, { id: "g2", kind: "chip", x: 276, y: 150, value: "gen 2" }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, caption: 'Аллокации <b>копятся</b> в gen 0. Большинство объектов тут и умирает — «Most objects are reclaimed… in generation 0».', nodes: [{ id: "a", kind: "obj", x: 64, y: 96, typeTag: "obj", value: "" }, { id: "b", kind: "obj", x: 64, y: 158, typeTag: "obj", value: "", accent: true }], edges: [] },
        { codeLine: 1, caption: 'Gen 0 <span class="hl">заполнилась</span> → «If an application attempts to create a new object when generation 0 is full, the garbage collector performs a collection».', nodes: [{ id: "gate", kind: "gate", x: 96, y: 118, state: "ok", label: "gen 0 полна", detail: "→ collect gen 0" }], edges: [] },
        { codeLine: 1, caption: 'Сборщик смотрит <span class="hl">только gen 0</span>, не всю кучу: «starts by examining the objects in generation 0 rather than all objects in the managed heap».', nodes: [{ id: "gate", kind: "gate", x: 88, y: 118, state: "ok", label: "GC · gen 0", detail: "не вся куча" }, { id: "g2", kind: "chip", x: 276, y: 150, value: "нетронут" }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'Живой объект <code>keep</code> в <b>gen 0</b>: на него ссылается root, значит он <span class="hl">достижим</span> и не мусор.', nodes: [{ id: "keep", kind: "obj", x: 64, y: 100, typeTag: "object", value: "keep", accent: true }, { id: "root", kind: "ref", x: 64, y: 172, name: "root" }], edges: [{ id: "r", from: "root", to: "keep" }] },
        { codeLine: 1, out: "gen 1", caption: 'Сборка gen 0: выжившие <span class="hl">продвигаются в gen 1</span> — «Objects that survive a generation 0 garbage collection are promoted to generation 1».', nodes: [{ id: "keep", kind: "obj", x: 170, y: 100, typeTag: "object", value: "keep", accent: true }, { id: "root", kind: "ref", x: 170, y: 172, name: "root" }], edges: [{ id: "r", from: "root", to: "keep", accent: true }] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, caption: '<b>Mark</b>: сборщик строит список <span class="hl">живых</span> объектов из корней; A и C достижимы, B и D — нет.', nodes: [{ id: "A", kind: "obj", x: 60, y: 108, typeTag: "A", value: "✓", accent: true }, { id: "B", kind: "obj", x: 132, y: 108, typeTag: "B", value: "×", good: false }, { id: "C", kind: "obj", x: 204, y: 108, typeTag: "C", value: "✓", accent: true }, { id: "D", kind: "obj", x: 276, y: 108, typeTag: "D", value: "×" }], edges: [] },
        { codeLine: 0, caption: '<b>Compact</b>: живые A и C <span class="hl">сдвигаются вместе</span>, мёртвое место освобождается — «uses a memory-copying function to compact the reachable objects».', nodes: [{ id: "A", kind: "obj", x: 60, y: 108, typeTag: "A", value: "✓" }, { id: "C", kind: "obj", x: 132, y: 108, typeTag: "C", value: "✓", accent: true }, { id: "free", kind: "chip", x: 264, y: 150, value: "свободно" }], edges: [] },
        { codeLine: 0, caption: '<b>Relocate</b>: корни <span class="hl">перенастраиваются</span> на новые адреса — «makes the necessary pointer corrections so that the… roots point to the objects in their new locations».', nodes: [{ id: "A", kind: "obj", x: 60, y: 100, typeTag: "A", value: "✓" }, { id: "C", kind: "obj", x: 132, y: 100, typeTag: "C", value: "✓" }, { id: "root", kind: "chip", x: 132, y: 172, value: "root→C", accent: true }], edges: [{ id: "r", from: "root", to: "C", accent: true }] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'Малый объект (< 85 000 байт) идёт на <b>small object heap</b>, стартует в gen 0.', nodes: [{ id: "s", kind: "obj", x: 88, y: 118, typeTag: "byte[1000]", value: "gen 0", accent: true }], edges: [] },
        { codeLine: 1, caption: 'Объект <span class="hl">≥ 85 000</span> байт идёт на <b>LOH</b>. Реальный прогон: <code>GC.GetGeneration(big)</code> = <b>2</b> — LOH собирается с gen 2.', nodes: [{ id: "s", kind: "obj", x: 88, y: 118, typeTag: "byte[1000]", value: "gen 0" }, { id: "big", kind: "obj", x: 252, y: 118, typeTag: "byte[100000]", value: "gen 2", accent: true }], edges: [] },
        { codeLine: 2, caption: '<b>GC ≠ Dispose</b>: сборщик <span class="hl">не</span> зовёт <code>Dispose</code>. Unmanaged-ресурс освобождает <code>using</code> — он гарантирует <code>Dispose</code> даже при исключении.', nodes: [{ id: "gate", kind: "gate", x: 170, y: 118, state: "fail", label: "GC не зовёт Dispose", detail: "→ using / try-finally" }], edges: [] },
      ],
