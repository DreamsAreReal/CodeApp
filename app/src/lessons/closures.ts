  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: 'Лямбда ссылается на <b>внешнюю переменную</b> <code>n</code> — «Lambdas can refer to <span class="hl">outer variables</span>». Это захват переменной, а не копия значения.', nodes: [{ id: "n", kind: "slot", x: 83, y: 88, name: "n", value: "1" }, { id: "add", kind: "ref", x: 83, y: 152, name: "add", accent: true }, { id: "cap", kind: "chip", x: 257, y: 112, value: "→ n" }], edges: [{ id: "e", from: "add", to: "cap", accent: true }] },
        { codeLine: 2, out: "", caption: '<code>n = 100</code> меняет <span class="hl">ту же ячейку</span>, на которую смотрит лямбда — захвачена переменная, не снимок.', nodes: [{ id: "n", kind: "slot", x: 83, y: 88, name: "n", value: "100", accent: true }, { id: "add", kind: "ref", x: 83, y: 152, name: "add" }, { id: "cap", kind: "chip", x: 257, y: 112, value: "→ n" }], edges: [{ id: "e", from: "add", to: "cap", accent: true }] },
        { codeLine: 3, out: "105", caption: 'При вызове лямбда читает <span class="hl">текущее</span> <code>n = 100</code> → <code>5 + 100 = 105</code>. Значение берётся в момент <b>вызова</b>, не создания.', nodes: [{ id: "n", kind: "slot", x: 83, y: 88, name: "n", value: "100" }, { id: "add", kind: "ref", x: 83, y: 152, name: "add", accent: true }, { id: "cap", kind: "chip", x: 257, y: 112, value: "→ 100", accent: true }], edges: [{ id: "e", from: "add", to: "cap", accent: true }] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, ilLine: 0, caption: 'Есть захват → компилятор генерит <b>display-класс</b> (<code>&lt;&gt;c__DisplayClass</code>) и создаёт его экземпляр на <span class="hl">куче</span>.', nodes: [{ id: "dc", kind: "obj", x: 257, y: 112, h: 60, typeTag: "DisplayClass", value: "", accent: true }], edges: [] },
        { codeLine: 1, ilLine: 1, caption: 'Локальная <code>n</code> <span class="hl">поднимается</span> (hoisting) и становится <b>полем</b> этого объекта — она больше не живёт просто на стеке.', nodes: [{ id: "dc", kind: "obj", x: 257, y: 112, h: 60, typeTag: "DisplayClass", value: "", accent: true }, { id: "nf", kind: "slot", x: 257, y: 116, name: "n", value: "1" }], edges: [] },
        { codeLine: 1, ilLine: 2, caption: 'Тело лямбды — <b>метод</b> display-класса (<code>&lt;&gt;b__0</code>); делегат <code>add</code> держит ссылку на объект и на этот метод.', nodes: [{ id: "add", kind: "ref", x: 83, y: 152, name: "add", accent: true }, { id: "dc", kind: "obj", x: 257, y: 112, h: 60, typeTag: "DisplayClass", value: "b__0()" }, { id: "nf", kind: "slot", x: 257, y: 116, name: "n", value: "1" }], edges: [{ id: "e", from: "add", to: "dc", accent: true }] },
      ],
    ...
    },
    {
      ...
      console: true,
      scenes: [
        { codeLine: 1, out: "", caption: 'Обе лямбды захватывают <b>ту же</b> <code>x</code> — компилятор кладёт их в <span class="hl">один</span> display-класс, <code>x</code> = общее поле.', nodes: [{ id: "inc", kind: "ref", x: 83, y: 88, name: "inc" }, { id: "get", kind: "ref", x: 83, y: 160, name: "get" }, { id: "dc", kind: "obj", x: 257, y: 112, typeTag: "shared x", value: "1", accent: true }], edges: [{ id: "e1", from: "inc", to: "dc" }, { id: "e2", from: "get", to: "dc" }] },
        { codeLine: 3, out: "", caption: '<code>inc()</code> дважды меняет <span class="hl">общую ячейку</span> <code>x</code> → 3. Изменение видно всем, кто захватил <code>x</code>.', nodes: [{ id: "inc", kind: "ref", x: 83, y: 88, name: "inc", accent: true }, { id: "get", kind: "ref", x: 83, y: 160, name: "get" }, { id: "dc", kind: "obj", x: 257, y: 112, typeTag: "shared x", value: "3", accent: true }], edges: [{ id: "e1", from: "inc", to: "dc", accent: true }, { id: "e2", from: "get", to: "dc" }] },
        { codeLine: 3, out: "3", caption: '<code>get()</code> читает ту же ячейку → печатает <b>3</b>. Разные лямбды — <span class="hl">одно</span> состояние.', nodes: [{ id: "inc", kind: "ref", x: 83, y: 88, name: "inc" }, { id: "get", kind: "ref", x: 83, y: 160, name: "get", accent: true }, { id: "dc", kind: "obj", x: 257, y: 112, typeTag: "shared x", value: "3", accent: true }], edges: [{ id: "e1", from: "inc", to: "dc" }, { id: "e2", from: "get", to: "dc", accent: true }] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, caption: '<code>seed</code> — локальная в <code>Make</code>. Обычно после выхода из метода её слот на стеке исчезает.', nodes: [{ id: "seed", kind: "slot", x: 83, y: 100, name: "seed", value: "42", accent: true }], edges: [] },
        { codeLine: 2, caption: 'Лямбда <span class="hl">захватывает</span> <code>seed</code> → она уже поле display-класса на <b>куче</b>, а не стековый слот.', nodes: [{ id: "seed", kind: "slot", x: 83, y: 100, name: "seed", value: "42", state: "" }, { id: "dc", kind: "obj", x: 257, y: 112, typeTag: "DisplayClass", value: "seed=42", accent: true }], edges: [] },
        { codeLine: 3, caption: '<code>Make</code> вернулся, стек-кадр ушёл — но объект <span class="hl">жив</span>, пока жив делегат: «isn\'t garbage collected until the delegate… becomes eligible».', nodes: [{ id: "ret", kind: "chip", x: 83, y: 100, value: "делегат", accent: true }, { id: "dc", kind: "obj", x: 257, y: 112, typeTag: "DisplayClass", value: "seed=42", accent: true }], edges: [{ id: "e", from: "ret", to: "dc", accent: true }] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, out: "", caption: '<b>for</b>: единственная переменная <code>i</code> на весь цикл. Все три лямбды захватывают <span class="hl">одну и ту же</span> ячейку.', nodes: [{ id: "l0", kind: "ref", x: 89, y: 76, name: "λ0" }, { id: "l1", kind: "ref", x: 89, y: 116, name: "λ1" }, { id: "l2", kind: "ref", x: 89, y: 156, name: "λ2" }, { id: "i", kind: "chip", x: 251, y: 116, value: "i", accent: true }], edges: [{ id: "e0", from: "l0", to: "i" }, { id: "e1", from: "l1", to: "i" }, { id: "e2", from: "l2", to: "i", accent: true }] },
        { codeLine: 2, out: "333", caption: 'После цикла <code>i == 3</code>. Каждая лямбда читает <span class="hl">общую</span> ячейку → печатает <b>3</b> трижды: <code>333</code>. (Реальный прогон.)', nodes: [{ id: "l0", kind: "ref", x: 89, y: 76, name: "λ0" }, { id: "l1", kind: "ref", x: 89, y: 116, name: "λ1" }, { id: "l2", kind: "ref", x: 89, y: 156, name: "λ2" }, { id: "i", kind: "chip", x: 251, y: 116, value: "3", accent: true }], edges: [{ id: "e0", from: "l0", to: "i" }, { id: "e1", from: "l1", to: "i" }, { id: "e2", from: "l2", to: "i" }] },
        { codeLine: 2, out: "333", caption: '<b>foreach</b> (C# 5+) даёт <span class="hl">свежую</span> переменную на итерацию → <code>012</code>. Спека: «because each iteration has its own variable v… earlier versions of C# declared v outside».', nodes: [{ id: "n0", kind: "chip", x: 251, y: 72, value: "n=0", good: true }, { id: "n1", kind: "chip", x: 251, y: 108, value: "n=1", good: true }, { id: "n2", kind: "chip", x: 251, y: 144, value: "n=2", good: true, accent: true }, { id: "gate", kind: "gate", x: 89, y: 118, state: "ok", label: "foreach → 012", detail: "своя ячейка/итер." }], edges: [] },
      ],
