...
// Auto-layout v2: each track is a zone with an `id`; a track's nodes declare
// `at:{zone,row,col}` where `col` is the TIME PHASE (0 = before await, 1 = after) so
// the engine keeps same-phase nodes on a stable X across the timeline.
// Track heights hold a state-machine `obj` (h60) with PAD≥8, and the bottom track's
// label sits BELOW the top track so a top-track box never overlaps a caption.
const Z_TRACKS: Zone[] = [
  { id: "caller", x: 14, y: 30, w: 312, h: 76, cls: "vz-zone", label: "ВЫЗЫВАЮЩИЙ ПОТОК", labelCls: "vz-zlabel sm", lx: 170, ly: 22 },
  { id: "io", x: 14, y: 128, w: 312, h: 76, cls: "vz-zone heap", label: "I/O · ДРАЙВЕР / IOCP", labelCls: "vz-zlabel heap sm", lx: 170, ly: 120 },
];
// Context-contrast backdrop: captured UI context vs the thread pool.
const Z_CTX: Zone[] = [
  { id: "ui", x: 14, y: 30, w: 312, h: 76, cls: "vz-zone", label: "UI-ПОТОК · захваченный контекст", labelCls: "vz-zlabel sm", lx: 170, ly: 22 },
  { id: "pool", x: 14, y: 128, w: 312, h: 76, cls: "vz-zone good", label: "ПУЛ ПОТОКОВ", labelCls: "vz-zlabel good sm", lx: 170, ly: 120 },
];
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'Компилятор переписывает <code>async</code>-метод в <b>стейт-машину</b>. Пока это обычный код на <span class="hl">вызывающем потоке</span>.', nodes: [{ id: "sm", kind: "obj", at: { zone: "caller", row: 0, col: 0 }, typeTag: "state machine", value: "state -1" }, { id: "ctrl", kind: "chip", at: { zone: "io", row: 0, col: 0 }, value: "поток → метод", accent: true }], edges: [] },
        { codeLine: 1, caption: '«An async method runs <span class="hl">synchronously</span> until it reaches its first <code>await</code> expression» — до этой точки всё как в обычном методе.', nodes: [{ id: "sm", kind: "obj", at: { zone: "caller", row: 0, col: 0 }, typeTag: "state machine", value: "state 0", accent: true }, { id: "ctrl", kind: "chip", at: { zone: "io", row: 0, col: 0 }, value: "поток → метод" }, { id: "m", kind: "chip", at: { zone: "io", row: 0, col: 1 }, value: "await", accent: true }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'На <code>await</code> метод <span class="hl">приостановлен</span>; I/O стартует на драйвере, а метод <b>возвращает незавершённый Task</b>.', nodes: [{ id: "sm", kind: "obj", at: { zone: "caller", row: 0, col: 0 }, typeTag: "state machine", value: "⏸ await" }, { id: "task", kind: "chip", at: { zone: "caller", row: 0, col: 1 }, value: "Task<int> ⏳", accent: true }, { id: "io", kind: "chip", at: { zone: "io", row: 0, col: 0 }, value: "I/O запущен" }], edges: [{ id: "r", from: "sm", to: "task", accent: true }] },
        { codeLine: 1, out: "не завершён", caption: '<b>await не блокирует</b>: «signs up the rest of the method as a <span class="hl">continuation</span> and returns control to the caller». Поток свободен для другой работы.', nodes: [{ id: "sm", kind: "obj", at: { zone: "caller", row: 0, col: 0 }, typeTag: "state machine", value: "⏸ await" }, { id: "ctrl", kind: "chip", at: { zone: "caller", row: 0, col: 1 }, value: "поток свободен", accent: true }, { id: "io", kind: "chip", at: { zone: "io", row: 0, col: 0 }, value: "I/O летит", accent: true }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'I/O «в полёте», но <span class="hl">нить не занята</span>: ждёт драйвер/IOCP. «The <code>async</code> and <code>await</code> keywords <b>don\'t cause extra threads to be created</b>».', nodes: [{ id: "ctrl", kind: "chip", at: { zone: "caller", row: 0, col: 0 }, value: "поток свободен" }, { id: "io", kind: "obj", at: { zone: "io", row: 0, col: 0 }, typeTag: "IOCP · драйвер", value: "нет потока", accent: true }], edges: [] },
        { codeLine: 0, caption: '<b>Task ≠ Thread</b>: «an async method <span class="hl">doesn\'t run on its own thread</span>»; «a background thread doesn\'t help with a process that\'s just waiting for results».', nodes: [{ id: "ctrl", kind: "chip", at: { zone: "caller", row: 0, col: 0 }, value: "поток свободен" }, { id: "io", kind: "obj", at: { zone: "io", row: 0, col: 0 }, typeTag: "IOCP · драйвер", value: "нет потока", accent: true }, { id: "note", kind: "chip", at: { zone: "caller", row: 0, col: 1 }, value: "no thread", accent: true }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, caption: 'По умолчанию <code>await</code> <span class="hl">захватывает</span> текущий контекст и постит продолжение <b>обратно в UI-поток</b> через <code>Post</code>.', nodes: [{ id: "done", kind: "chip", at: { zone: "pool", row: 0 }, value: "I/O готово" }, { id: "cont", kind: "chip", at: { zone: "ui", row: 0 }, value: "→ продолжение · UI", accent: true }], edges: [{ id: "post", from: "done", to: "cont", accent: true }] },
        { codeLine: 2, caption: '<code>ConfigureAwait(false)</code>: продолжение остаётся на <span class="hl">пуле</span> — «not to capture and resume on the context, but to continue… wherever the asynchronous operation completed».', nodes: [{ id: "done", kind: "chip", at: { zone: "ui", row: 0 }, value: "I/O готово" }, { id: "cont", kind: "chip", at: { zone: "pool", row: 0 }, value: "→ продолжение · пул", good: true, accent: true }], edges: [{ id: "stay", from: "done", to: "cont" }] },
      ],
      explain: 'Когда I/O завершается, срабатывает <b>continuation</b>. По умолчанию он идёт на <span class="hl">захваченный контекст</span>: «If a synchronization context… is associated with the thread that was executing the asynchronous method at the time of suspension… the asynchronous method resumes on that same synchronization context by using the context\'s <code>Post</code> method. Otherwise, it relies on the task scheduler». В UI это <b>тот же UI-поток</b> (можно трогать элементы). <code>ConfigureAwait(false)</code> отключает захват: «use the <code>Task.ConfigureAwait</code> method to inform the await operation <b>not to capture and resume on the context</b>, but to continue execution wherever the asynchronous operation… completed». В библиотеках это предпочтительно: меньше лишних переключений контекста и ниже риск дедлока для блокирующих вызывающих.',
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 2, caption: '<code>t.Wait()</code> <span class="hl">блокирует</span> UI-поток, ожидая задачу. Но continuation по умолчанию хочет вернуться <b>на этот же</b> контекст.', nodes: [{ id: "ui", kind: "chip", at: { zone: "ui", row: 0 }, value: "UI: .Wait() ⏳", accent: true }, { id: "cont", kind: "chip", at: { zone: "pool", row: 0 }, value: "continuation → UI?" }], edges: [] },
        { codeLine: 2, caption: '<b>Дедлок</b>: «that context already has a thread in it, which is (synchronously) waiting… They\'re each waiting for the other».', nodes: [{ id: "gate", kind: "gate", at: { zone: "ui", row: 0 }, state: "fail", label: "UI занят ↔ continuation ждёт UI", detail: "deadlock" }], edges: [] },
        { codeLine: 2, caption: 'Выход: <b>await</b> вместо <code>.Result</code>/<code>.Wait()</code> («async all the way»), в библиотеках — <span class="hl">ConfigureAwait(false)</span> (continuation на пул, дедлока нет).', nodes: [{ id: "free", kind: "chip", at: { zone: "ui", row: 0 }, value: "UI: свободен", good: true, accent: true }, { id: "good", kind: "gate", at: { zone: "pool", row: 0 }, state: "ok", label: "await / CA(false)", detail: "continuation → пул · нет дедлока" }], edges: [] },
      ],
