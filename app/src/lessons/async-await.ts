...
const Z_TRACKS: Zone[] = [
  { x: 14, y: 34, w: 312, h: 78, cls: "vz-zone", label: "ВЫЗЫВАЮЩИЙ ПОТОК", labelCls: "vz-zlabel sm", lx: 170, ly: 24, sub: "не блокируется", subCls: "vz-zsub", subY: 47 },
...
const Z_CTX: Zone[] = [
  { x: 14, y: 34, w: 312, h: 78, cls: "vz-zone", label: "UI-ПОТОК · захваченный контекст", labelCls: "vz-zlabel sm", lx: 170, ly: 24, sub: "по одному куску", subCls: "vz-zsub", subY: 47 },
  ...
  segments: [
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'Компилятор переписывает <code>async</code>-метод в <b>стейт-машину</b>. Пока это обычный код на <span class="hl">вызывающем потоке</span>.', nodes: [{ id: "sm", kind: "obj", x: 100, y: 56, w: 150, h: 40, typeTag: "state machine", value: "state -1" }, { id: "ctrl", kind: "ref", x: 100, y: 150, w: 130, h: 34, name: "поток → метод", accent: true }], edges: [] },
        { codeLine: 1, caption: '«An async method runs <span class="hl">synchronously</span> until it reaches its first <code>await</code> expression» — до этой точки всё как в обычном методе.', nodes: [{ id: "sm", kind: "obj", x: 100, y: 56, w: 150, h: 40, typeTag: "state machine", value: "state 0", accent: true }, { id: "ctrl", kind: "ref", x: 100, y: 150, w: 130, h: 34, name: "поток → метод" }, { id: "m", kind: "chip", x: 250, y: 62, w: 70, h: 28, value: "await", accent: true }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, out: "", caption: 'На <code>await</code> метод <span class="hl">приостановлен</span>; I/O стартует на драйвере, а метод <b>возвращает незавершённый Task</b>.', nodes: [{ id: "sm", kind: "obj", x: 60, y: 50, w: 130, h: 40, typeTag: "state machine", value: "⏸ await" }, { id: "task", kind: "chip", x: 210, y: 58, w: 96, h: 26, value: "Task<int> ⏳", accent: true }, { id: "io", kind: "chip", x: 120, y: 150, w: 120, h: 28, value: "I/O запущен" }], edges: [{ id: "r", from: "sm", to: "task", accent: true }] },
        { codeLine: 1, out: "не завершён", caption: '<b>await не блокирует</b>: «signs up the rest of the method as a <span class="hl">continuation</span> and returns control to the caller». Поток свободен для другой работы.', nodes: [{ id: "sm", kind: "obj", x: 60, y: 50, w: 130, h: 40, typeTag: "state machine", value: "⏸ await" }, { id: "ctrl", kind: "ref", x: 210, y: 58, w: 116, h: 34, name: "поток свободен", accent: true }, { id: "io", kind: "chip", x: 120, y: 150, w: 120, h: 28, value: "I/O летит", accent: true }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 0, caption: 'I/O «в полёте», но <span class="hl">нить не занята</span>: ждёт драйвер/IOCP. «The <code>async</code> and <code>await</code> keywords <b>don\'t cause extra threads to be created</b>».', nodes: [{ id: "ctrl", kind: "ref", x: 100, y: 60, w: 140, h: 34, name: "поток свободен" }, { id: "io", kind: "obj", x: 100, y: 138, w: 140, h: 46, typeTag: "IOCP · драйвер", value: "нет потока", accent: true }], edges: [] },
        { codeLine: 0, caption: '<b>Task ≠ Thread</b>: «an async method <span class="hl">doesn\'t run on its own thread</span>»; «a background thread doesn\'t help with a process that\'s just waiting for results».', nodes: [{ id: "ctrl", kind: "ref", x: 100, y: 60, w: 140, h: 34, name: "поток свободен" }, { id: "io", kind: "obj", x: 100, y: 138, w: 140, h: 46, typeTag: "IOCP · драйвер", value: "нет потока", accent: true }, { id: "note", kind: "chip", x: 250, y: 66, w: 70, h: 26, value: "no thread", accent: true }], edges: [] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 1, caption: 'По умолчанию <code>await</code> <span class="hl">захватывает</span> текущий контекст и постит продолжение <b>обратно в UI-поток</b> через <code>Post</code>.', nodes: [{ id: "done", kind: "chip", x: 80, y: 150, w: 110, h: 28, value: "I/O готово" }, { id: "cont", kind: "ref", x: 90, y: 60, w: 150, h: 34, name: "→ продолжение · UI", accent: true }], edges: [{ id: "post", from: "done", to: "cont", accent: true }] },
        { codeLine: 2, caption: '<code>ConfigureAwait(false)</code>: продолжение остаётся на <span class="hl">пуле</span> — «not to capture and resume on the context, but to continue… wherever the asynchronous operation completed».', nodes: [{ id: "done", kind: "chip", x: 80, y: 60, w: 110, h: 28, value: "I/O готово" }, { id: "cont", kind: "ref", x: 90, y: 150, w: 160, h: 34, name: "→ продолжение · пул", good: true, accent: true }], edges: [{ id: "stay", from: "done", to: "cont" }] },
      ],
    ...
    },
    {
      ...
      scenes: [
        { codeLine: 2, caption: '<code>t.Wait()</code> <span class="hl">блокирует</span> UI-поток, ожидая задачу. Но continuation по умолчанию хочет вернуться <b>на этот же</b> контекст.', nodes: [{ id: "ui", kind: "ref", x: 90, y: 60, w: 150, h: 34, name: "UI: .Wait() ⏳", accent: true }, { id: "cont", kind: "chip", x: 100, y: 150, w: 150, h: 28, value: "continuation → UI?" }], edges: [] },
        { codeLine: 2, caption: '<b>Дедлок</b>: «that context already has a thread in it, which is (synchronously) waiting… They\'re each waiting for the other».', nodes: [{ id: "gate", kind: "gate", x: 170, y: 96, w: 200, h: 46, state: "fail", label: "UI занят ↔ continuation ждёт UI", detail: "deadlock" }], edges: [] },
        { codeLine: 2, caption: 'Выход: <b>await</b> вместо <code>.Result</code>/<code>.Wait()</code> («async all the way»), в библиотеках — <span class="hl">ConfigureAwait(false)</span> (continuation на пул, дедлока нет).', nodes: [{ id: "good", kind: "gate", x: 170, y: 96, w: 200, h: 46, state: "ok", label: "await / CA(false)", detail: "continuation → пул · нет дедлока" }], edges: [] },
      ],
